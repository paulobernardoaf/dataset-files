...
#if !defined(__al_included_allegro5_debug_h)
...
#define ALLEGRO_DEBUG_CHANNEL(x) static char const *__al_debug_channel __attribute__((unused)) = x;

#else
#define ALLEGRO_DEBUG_CHANNEL(x) static char const *__al_debug_channel = x;

#endif
#define ALLEGRO_TRACE_CHANNEL_LEVEL(channel, level) !_al_trace_prefix(channel, level, __FILE__, __LINE__, __func__) ? (void)0 : _al_trace_suffix
...
#else
#define ALLEGRO_ASSERT(e) ((e) ? (void) 0 : (_al_user_assert_handler) ? _al_user_assert_handler(#e, __FILE__, __LINE__, __func__) : assert(e))
