#if !defined(__al_included_allegro5_aintern_blend_h)
...
#define _AL_DEST_IS_ZERO (dst_mode == ALLEGRO_ZERO && dst_alpha == ALLEGRO_ZERO && op != ALLEGRO_DEST_MINUS_SRC && op_alpha != ALLEGRO_DEST_MINUS_SRC)



#define _AL_SRC_NOT_MODIFIED (src_mode == ALLEGRO_ONE && src_alpha == ALLEGRO_ONE)


#define _AL_SRC_NOT_MODIFIED_TINT_WHITE (_AL_SRC_NOT_MODIFIED && tint.r == 1.0f && tint.g == 1.0f && tint.b == 1.0f && tint.a == 1.0f)




#if !defined(_AL_NO_BLEND_INLINE_FUNC)
...
#define BLEND(c, src, dst) result->c = OP(result->c * src, dcol->c * dst);
...
#define BLEND(c, src, dst) result->c = OP(result->c * src.c, dcol->c * dst.c);
