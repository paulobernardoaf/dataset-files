...
#define SHADE_COLORS(A, B) A.r = B.r * A.r; A.g = B.g * A.g; A.b = B.b * A.b; A.a = B.a * A.a;
...
#define FIRST first(state, x, y, vtx1, vtx2); if((x2 - x1) * ((float)x - x1) + (y2 - y1) * ((float)y - y1) >= 0) draw(state, x, y); (void)minor;





#define STEP step(state, minor); draw(state, x, y);



#define LAST step(state, minor); if((x1 - x2) * ((float)x - x2) + (y1 - y2) * ((float)y - y2) > 0) draw(state, x, y);





#define WORKER(var1, var2, comp, dvar1, dvar2, derr1, derr2, func) { int minor = 1; if(err comp) { var1 += dvar1; err += derr1; minor = 0; } func var2 += dvar2; err += derr2; }
