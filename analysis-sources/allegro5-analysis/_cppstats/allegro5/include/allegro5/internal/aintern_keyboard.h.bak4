#if !defined(__al_included_allegro5_aintern_keyboard_h)
...
#define _AL_KEYBOARD_STATE_KEY_DOWN(STATE, KEYCODE) (((STATE).__key_down__internal__[(KEYCODE) / 32] & (1 << ((KEYCODE) % 32)))? true : false)



#define _AL_KEYBOARD_STATE_SET_KEY_DOWN(STATE, KEYCODE) do { int kc = (KEYCODE); (STATE).__key_down__internal__[kc / 32] |= (1 << (kc % 32)); } while (0)





#define _AL_KEYBOARD_STATE_CLEAR_KEY_DOWN(STATE, KEYCODE) do { int kc = (KEYCODE); (STATE).__key_down__internal__[kc / 32] &= ~(1 << (kc % 32)); } while (0)
