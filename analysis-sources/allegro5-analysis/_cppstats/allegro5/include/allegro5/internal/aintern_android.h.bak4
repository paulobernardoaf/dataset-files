#if !defined(ALLEGRO_AINTERN_ANDROID_H)
...
#define _jni_call(env, rett, method, args...) ({ rett ret = (*env)->method(env, ##args); _jni_checkException(env); ret; })
...
#define _jni_callv(env, method, args...) ({ (*env)->method(env, ##args); _jni_checkException(env); })





#define _jni_callBooleanMethodV(env, obj, name, sig, args...) ({ jclass class_id = _jni_call(env, jclass, GetObjectClass, obj); jmethodID method_id = _jni_call(env, jmethodID, GetMethodID, class_id, name, sig); bool ret = false; if (method_id == NULL) { ALLEGRO_DEBUG("couldn't find method %s", name); } else { ret = _jni_call(env, bool, CallBooleanMethod, obj, method_id, ##args); } _jni_callv(env, DeleteLocalRef, class_id); ret; })
...
#define _jni_callIntMethodV(env, obj, name, sig, args...) ({ jclass class_id = _jni_call(env, jclass, GetObjectClass, obj); jmethodID method_id = _jni_call(env, jmethodID, GetMethodID, class_id, name, sig); int ret = -1; if (method_id == NULL) { ALLEGRO_DEBUG("couldn't find method %s", #name); } else { ret = _jni_call(env, int, CallIntMethod, obj, method_id, ##args); } _jni_callv(env, DeleteLocalRef, class_id); ret; })
...
#define _jni_callLongMethodV(env, obj, name, sig, args...) ({ jclass class_id = _jni_call(env, jclass, GetObjectClass, obj); jmethodID method_id = _jni_call(env, jmethodID, GetMethodID, class_id, name, sig); long ret = -1; if (method_id == NULL) { ALLEGRO_DEBUG("couldn't find method %s", name); } else { ret = _jni_call(env, long, CallLongMethod, obj, method_id, ##args); } _jni_callv(env, DeleteLocalRef, class_id); ret; })
...
#define _jni_callVoidMethodV(env, obj, name, sig, args...) ({ jclass class_id = _jni_call(env, jclass, GetObjectClass, obj); jmethodID method_id = _jni_call(env, jmethodID, GetMethodID, class_id, name, sig); if (method_id == NULL) { ALLEGRO_ERROR("couldn't find method %s", name); } else { _jni_callv(env, CallVoidMethod, obj, method_id, ##args); } _jni_callv(env, DeleteLocalRef, class_id); })
...
#define JNI_FUNC_PASTER(ret, cls, name, params, x) JNIEXPORT ret JNICALL Java_ ##x ##_ ##cls ##_ ##name params

#define JNI_FUNC_EVALUATOR(ret, cls, name, params, x) JNI_FUNC_PASTER(ret, cls, name, params, x)

#define JNI_FUNC(ret, cls, name, params) JNI_FUNC_EVALUATOR(ret, cls, name, params, ALLEGRO_ANDROID_PACKAGE_NAME)
