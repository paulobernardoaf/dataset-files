...
#define DRAW if(ii != 0) { vtx[idx++] = outer_vtx; vtx[idx++] = inner_vtx; al_draw_prim(vtx, 0, 0, 0, idx, ALLEGRO_PRIM_TRIANGLE_FAN); idx = 0; }
for(ii = 0; ii < 3; ii++)
{
...
float sine_half_delta = fabsf(side_dir_x * midpoint_dir_y - side_dir_y * midpoint_dir_x); 
