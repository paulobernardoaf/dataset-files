...
struct wgl_vt vt;
#define LOAD_EXT(name, type) do { vt.name = (type) vlc_gl_GetProcAddress(interop->gl, "wgl" #name); if (!vt.name) { msg_Warn(obj, "'wgl " #name "' could not be loaded"); return VLC_EGENERIC; } } while(0)
