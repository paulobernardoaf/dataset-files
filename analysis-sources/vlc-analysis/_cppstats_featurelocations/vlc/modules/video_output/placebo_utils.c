...
#define PLANE(n, bits, map, wd, hd, pad) { .components = n, .pixel_size = SIZE(n, bits, pad), .comp_bits = {bits, bits, bits, bits}, .comp_map = map, .w_denom = wd, .h_denom = hd, }
...
#define _PLANAR(n, bits, wd, hd) .type = PL_FMT_UNORM, .num_planes = n, .planes = { PLANE(1, bits, {0}, 1, 1, 0), PLANE(1, bits, {1}, wd, hd, 0), PLANE(1, bits, {2}, wd, hd, 0), PLANE(1, bits, {3}, 1, 1, 0), }
...
#define _SEMIPLANAR(n, bits, wd, hd) .type = PL_FMT_UNORM, .num_planes = n, .planes = { PLANE(1, bits, {0}, 1, 1, 0), PLANE(2, bits, COMPS(1, 2), wd, hd, 0), PLANE(1, bits, {3}, 1, 1, 0), }
...
#define PACKED(n, bits, pad) .type = PL_FMT_UNORM, .num_planes = 1, .planes = { PLANE(n, bits, COMPS(0, 1, 2, 3), 1, 1, pad), }
...
#define SINGLE(t, bits) .type = PL_FMT_##t, .num_planes = 1, .planes = { PLANE(1, bits, {0}, 1, 1, 0), }
...
[COLOR_SPACE_UNDEF] = PL_COLOR_SYSTEM_BT_709, 
...
const struct fmt_desc *desc = FindDesc(fmt->i_chroma);
int sample_depth = desc->planes[0].comp_bits[0]; 
