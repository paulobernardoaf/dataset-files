...
#define WIDTH_LONGTEXT N_( "You can enforce the image width. By default " "(-1) VLC will adapt to the video " "characteristics.")



#define HEIGHT_TEXT N_( "Image height" )
#define HEIGHT_LONGTEXT N_( "You can enforce the image height. By default " "(-1) VLC will adapt to the video " "characteristics.")



#define RATIO_TEXT N_( "Recording ratio" )
#define RATIO_LONGTEXT N_( "Ratio of images to record. ""3 means that one image out of three is recorded." )


#define PREFIX_TEXT N_( "Filename prefix" )
#define PREFIX_LONGTEXT N_( "Prefix of the output images filenames. Output " "filenames will have the \"prefixNUMBER.format\" ""form if replace is not true." )



#define PATH_TEXT N_( "Directory path prefix" )
#define PATH_LONGTEXT N_( "Directory path where images files should be saved. " "If not set, then images will be automatically saved in " "users homedir." )



#define REPLACE_TEXT N_( "Always write to the same file" )
#define REPLACE_LONGTEXT N_( "Always write to the same file instead of " "creating one file per image. In this case, " "the number is not appended to the filename." )
