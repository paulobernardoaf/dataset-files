...
#define HELP_TEXT N_("This filter enhances the stereo effect by ""suppressing mono (signal common to both channels) ""and by delaying the signal of left into right and vice versa, ""thereby widening the stereo effect.")



#define DELAY_TEXT N_("Delay time")
#define DELAY_LONGTEXT N_("Time in ms of the delay of left signal into right ""and vice versa.")

#define FEEDBACK_TEXT N_("Feedback gain")
#define FEEDBACK_LONGTEXT N_("Amount of gain in delayed left signal into ""right and vice versa. Gives a delay effect of left signal in ""right output and vice versa which gives widening effect.")


#define CROSSFEED_TEXT N_("Crossfeed")
#define CROSSFEED_LONGTEXT N_("Cross feed of left into right with inverted ""phase. This helps in suppressing the mono. If the value is 1 it ""will cancel all the signal common to both channels.")
...
#define CREATE_VAR( stor, var ) p_sys->stor = var_CreateGetFloat( p_aout, var ); var_AddCallback( p_aout, var, paramCallback, p_sys );
...
#define DEL_VAR(var) var_DelCallback( p_aout, var, paramCallback, p_sys ); var_Destroy( p_aout, var );
