...
#define IFSET_NAL(type, var) if( i_nal_type == type && pp_##var && *pp_##var == NULL ){ *pp_##var = p_nal; *p_##var##_size = i_nal; }
...
#define IMPL_h264_generic_decode( name, h264type, decode, release ) h264type * name( const uint8_t *p_buf, size_t i_buf, bool b_escaped ) { h264type *p_h264type = calloc(1, sizeof(h264type)); if(likely(p_h264type)) { bs_t bs; struct hxxx_bsfw_ep3b_ctx_s bsctx; if( b_escaped ) { hxxx_bsfw_ep3b_ctx_init( &bsctx ); bs_init_custom( &bs, p_buf, i_buf, &hxxx_bsfw_ep3b_callbacks, &bsctx );} else bs_init( &bs, p_buf, i_buf ); bs_skip( &bs, 8 ); if( !decode( &bs, p_h264type ) ) { release( p_h264type ); p_h264type = NULL; } } return p_h264type; }
