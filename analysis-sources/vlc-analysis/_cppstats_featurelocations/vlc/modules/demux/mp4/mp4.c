...
#define HEIF_DURATION_LONGTEXT N_( "Duration in seconds before simulating an end of file. " "A negative value means an unlimited play time.")
...
#define DEMUX_INCREMENT VLC_TICK_FROM_MS(250) 
#define DEMUX_TRACK_MAX_PRELOAD VLC_TICK_FROM_SEC(15) 
...
static int xTTS_CountEntries( demux_t *p_demux, uint32_t *pi_entry ,
...
{
MP4_ReadBoxContainerChildren( p_demux->s, p_vroot, NULL ); 
...
if( p_track->context.b_resync_time_offset ) 
{
...
{
i_traf_base_data_offset = p_moof->i_pos ;
}
else
{
if ( i_traf == 0 )
...
else if( MP4_Seek( p_demux->s, i_pos + i_size ) != VLC_SUCCESS ) 
{
i_status = VLC_DEMUXER_EOF;
goto end;
}
