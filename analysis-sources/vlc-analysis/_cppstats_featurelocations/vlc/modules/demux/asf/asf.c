...
uint8_t i_access_selected_track[ES_CATEGORY_COUNT]; 
...
#define GET_CHECKED( target, getter, maxtarget, temp ) {temp i_temp = getter;if ( i_temp > maxtarget ) {msg_Warn( p_demux, "rejecting stream %u : " #target " overflow", i_stream );es_format_Clean( &fmt );goto error;} else {target = i_temp;}}
...
#define set_meta( name, vlc_type ) if( p_ecd->ppsz_name[i] && !strncmp( p_ecd->ppsz_name[i], name, strlen(name) ) ) vlc_meta_Set( p_sys->meta, vlc_type, p_ecd->ppsz_value[i] );
