...
#if !defined(SI_DEBUG_EIT)
#define SI_DEBUG_TIMESHIFT(t)
#else
static time_t i_eit_debug_offset = 0;
#define SI_DEBUG_TIMESHIFT(t) do {if( i_eit_debug_offset == 0 )i_eit_debug_offset = time(NULL) - t;t = t + i_eit_debug_offset;} while(0);
...
#define attach_SI_decoders(i_pid, name, member) do {ts_pid_t *pid = GetPID(p_sys, i_pid);if ( PIDSetup( p_demux, TYPE_SI, pid, NULL ) ){if( !ts_attach_SI_Tables_Decoders( pid ) ){msg_Err( p_demux, "Can't attach SI table decoders for pid %d", i_pid );PIDRelease( p_demux, pid );}else{sdt->u.p_si->member = pid;SetPIDFilter( p_demux->p_sys, pid, true );msg_Dbg( p_demux, " * pid=%d listening for "name, i_pid );}}} while(0)
...
( i_table_id == 0x4e || 
(i_table_id >= 0x50 && i_table_id <= 0x5f) ) ) 
{
