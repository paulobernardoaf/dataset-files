...
#define FPS_LONGTEXT N_("This is the desired frame rate when " "playing raw video streams. In the form 30000/1001 or 29.97")


#define WIDTH_TEXT N_("Width")
#define WIDTH_LONGTEXT N_("This specifies the width in pixels of the raw " "video stream.")


#define HEIGHT_TEXT N_("Height")
#define HEIGHT_LONGTEXT N_("This specifies the height in pixels of the raw " "video stream.")
...
#define ASPECT_RATIO_LONGTEXT N_( "Aspect ratio (4:3, 16:9). Default assumes square pixels." )
...
#define READ_FRAC( key, num, den ) do { psz_buf = strstr( psz+9, key );if( psz_buf ){char *end = strchr( psz_buf+1, ' ' );char *sep;if( end ) *end = '\0';sep = strchr( psz_buf+1, ':' );if( sep ){*sep = '\0';den = atoi( sep+1 );}else{den = 1;}num = atoi( psz_buf+2 );if( sep ) *sep = ':';if( end ) *end = ' ';} } while(0)
