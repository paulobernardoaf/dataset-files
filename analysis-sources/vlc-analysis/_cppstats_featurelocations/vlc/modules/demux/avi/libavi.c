...
p_father->common.p_father->common.p_father != NULL ) 
return VLC_EGENERIC;
}
...
#define AVI_READCHUNK_ENTER int64_t i_read = __EVEN(p_chk->common.i_chunk_size ) + 8; if( i_read > 100000000 ) { msg_Err( s, "Big chunk ignored" ); return VLC_EGENERIC; } uint8_t *p_read, *p_buff; if( !( p_read = p_buff = malloc(i_read ) ) ) { return VLC_EGENERIC; } i_read = vlc_stream_Read( s, p_read, i_read ); if( i_read < (int64_t)__EVEN(p_chk->common.i_chunk_size ) + 8 ) { free( p_buff ); return VLC_EGENERIC; }p_read += 8; i_read -= 8
...
#define AVI_READ( res, func, size ) if( i_read < size ) { free( p_buff); return VLC_EGENERIC; } i_read -= size; res = func( p_read ); p_read += size 
...
#define AVI_READCHUNK_EXIT( code ) do { free( p_buff ); return code; } while(0)
