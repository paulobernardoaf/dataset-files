...
#define PROPERTY_SET_FUNC( prop ) if( !strcmp( psz_property_name, #prop ) ) return prop##Set( p_conn, p_from, p_this ); else
...
#define ADD_PROPERTY( prop, signature ) if( VLC_SUCCESS != AddProperty( (intf_thread_t*) p_this, &dict, #prop, signature, Marshal##prop ) ) { dbus_message_iter_abandon_container( &args, &dict ); return VLC_ENOMEM; }
...
#define METHOD_FUNC( interface, method, function ) else if( dbus_message_is_method_call( p_from, interface, method ) )return function( p_conn, p_from, p_this )
...
#define PROPERTY_ENTRY( prop, signature ) else if( !strcmp( ppsz_properties[i], #prop ) ) { if( VLC_SUCCESS != AddProperty( (intf_thread_t*) p_intf, &changed_properties, #prop, signature, Marshal##prop ) ) { for( ; ppsz_properties[i]; ++i ) free( ppsz_properties[i] ); free( ppsz_properties ); dbus_message_iter_abandon_container( &args, &changed_properties ); return DBUS_HANDLER_RESULT_NEED_MEMORY; } }
...
#define PROPERTY_MAPPING_END else { for( ; ppsz_properties[i]; ++i ) free( ppsz_properties[i] ); free( ppsz_properties ); dbus_message_iter_abandon_container( &args, &changed_properties ); return DBUS_HANDLER_RESULT_NOT_YET_HANDLED; }
