...
#define RELEASE_NALS(list, max, release) for (size_t i = 0; i <= max; ++i) { hnal = &list[i]; if (hnal->b) { block_Release(hnal->b); release; } }
...
#define HELPER_FOREACH_NAL(it, p_nal_list, i_nal_count, i_nal_max) for (size_t ii = 0, i_nal_found = 0; ii < i_nal_max && i_nal_count > i_nal_found; ++ii) if (p_nal_list[ii].b != NULL && (it = &p_nal_list[ii]) && ++i_nal_found)
...
#define LOAD_xPS(list, count, id, max, xpstype, xpsdecode, xpsrelease) if (helper_search_nal(list, count, max+1, p_nal, i_nal) != NULL)continue;xpstype *p_xps = xpsdecode(p_nal, i_nal, true);if (!p_xps)return VLC_EGENERIC;struct hxxx_helper_nal *hnal = &list[id];if (helper_dup_buf(hnal, p_nal, i_nal)){xpsrelease(p_xps);return VLC_EGENERIC;}if (hnal->xps)xpsrelease(hnal->xps);elsecount++;hnal->xps = p_xps;*p_config_changed = true
