...
#define EXPECT_AT(index, id) assert(vlc_playlist_Get(playlist, index)->media == media[id])
...
#define CALLBACK_CTX_INITIALIZER { VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, VLC_VECTOR_INITIALIZER, }
...
assert(ctx.vec_items_removed.data[0].state.current == 3); 
assert(ctx.vec_items_removed.data[0].state.has_prev);
assert(ctx.vec_items_removed.data[0].state.has_next);
...
vlc_playlist_item_t *item = vlc_playlist_Get(playlist, 4);
ret = vlc_playlist_RequestGoTo(playlist, item, -1); 
assert(ret == VLC_SUCCESS);
...
vlc_playlist_item_t *item = vlc_playlist_Get(playlist, 4);
ret = vlc_playlist_RequestGoTo(playlist, item, 7); 
assert(ret == VLC_SUCCESS);
