...
#define READ_MPI(d, bits) do { size_t n = read_mpi(d, p_buf, i_packet_len - i_read, bits); if (!n) goto error; p_buf += n; i_read += n; } while(0)
...
const char *psz_url )
{
char *psz_sig = (char*) malloc( strlen( psz_url ) + 4 + 1 ); 
if( !psz_sig )
return VLC_ENOMEM;
