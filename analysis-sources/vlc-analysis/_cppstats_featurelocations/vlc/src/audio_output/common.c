...
#define REORDER_TYPE(type) do { const size_t frames = (bytes / sizeof (type)) / channels; type *buf = ptr; for( size_t i = 0; i < frames; i++ ) { type tmp[AOUT_CHAN_MAX]; for( size_t j = 0; j < channels; j++ ) tmp[chans_table[j]] = buf[j]; memcpy( buf, tmp, sizeof (type) * channels ); buf += channels; } } while(0)
...
{
#define INTERLEAVE_TYPE(type) do { type *d = dst; for( size_t i = 0; i < chans; i++ ) { const type *s = srcv[i]; for( size_t j = 0, k = 0; j < samples; j++, k += chans ) d[k] = *(s++); d++; } } while(0)
...
unsigned samples, unsigned chans, vlc_fourcc_t fourcc )
{
#define DEINTERLEAVE_TYPE(type) do { type *d = dst; const type *s = src; for( size_t i = 0; i < chans; i++ ) { for( size_t j = 0, k = 0; j < samples; j++, k += chans ) *(d++) = s[k]; s++; } } while(0)
