...
#define LOAD_IMMEDIATE(a) if (vlc_cache_load_immediate(&(a), file, sizeof (a))) goto error
#define LOAD_FLAG(a) do { bool b; if (vlc_cache_load_bool(&b, file)) goto error; (a) = b; } while (0)
#define LOAD_ARRAY(a,n) do { const void *base; if (vlc_cache_load_array(&base, sizeof (*(a)), (n), file)) goto error; (a) = base; } while (0)
#define LOAD_STRING(a) if (vlc_cache_load_string(&(a), file)) goto error
#define LOAD_ALIGNOF(t) if (vlc_cache_load_align(alignof(t), file)) goto error
...
#define SAVE_IMMEDIATE( a ) if (fwrite (&(a), sizeof(a), 1, file) != 1) goto error
#define SAVE_FLAG(a) do { char b = (a); SAVE_IMMEDIATE(b); } while (0)
...
#define SAVE_STRING( a ) if (CacheSaveString (file, (a))) goto error
...
#define SAVE_ALIGNOF(t) if (CacheSaveAlign(file, alignof (t))) goto error
