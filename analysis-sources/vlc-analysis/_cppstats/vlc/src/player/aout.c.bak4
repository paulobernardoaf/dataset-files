...
#define vlc_player_aout_SendEvent(player, event, ...) do { vlc_mutex_lock(&player->aout_listeners_lock); vlc_player_aout_listener_id *listener; vlc_list_foreach(listener, &player->aout_listeners, node) { if (listener->cbs->event) listener->cbs->event(__VA_ARGS__, listener->cbs_data); } vlc_mutex_unlock(&player->aout_listeners_lock); } while(0)
