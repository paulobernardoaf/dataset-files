...
#if !defined(VLC_PLAYER_INTERNAL_H)
...
#define vlc_player_SendEvent(player, event, ...) do { vlc_player_listener_id *listener; vlc_list_foreach(listener, &player->listeners, node) { if (listener->cbs->event) listener->cbs->event(player, ##__VA_ARGS__, listener->cbs_data); } } while(0)
