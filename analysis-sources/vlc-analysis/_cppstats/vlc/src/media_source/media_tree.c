...
#define vlc_media_tree_NotifyListener(tree, listener, event, ...) do { if (listener->cbs->event) listener->cbs->event(tree, ##__VA_ARGS__, listener->userdata); } while(0)
#define vlc_media_tree_Notify(tree, event, ...) do { vlc_media_tree_AssertLocked(tree); vlc_media_tree_listener_id *listener; vlc_media_tree_listener_foreach(listener, tree) vlc_media_tree_NotifyListener(tree, listener, event, ##__VA_ARGS__); } while (0)
static bool
