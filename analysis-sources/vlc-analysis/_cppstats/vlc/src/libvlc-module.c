...
{ "png", "jpg", "tiff" };
#define INTF_CAT_LONGTEXT N_( "These options allow you to configure the interfaces used by VLC. " "You can select the main interface, additional " "interface modules, and define various related options." )
#define INTF_TEXT N_("Interface module")
#define INTF_LONGTEXT N_( "This is the main interface used by VLC. " "The default behavior is to automatically select the best module " "available.")
#define EXTRAINTF_TEXT N_("Extra interface modules")
#define EXTRAINTF_LONGTEXT N_( "You can select \"additional interfaces\" for VLC. " "They will be launched in the background in addition to the default " "interface. Use a colon separated list of interface modules. (common " "values are \"rc\" (remote control), \"http\", \"gestures\" ...)")
#define CONTROL_TEXT N_("Control interfaces")
#define CONTROL_LONGTEXT N_( "You can select control interfaces for VLC.")
#define VERBOSE_TEXT N_("Verbosity (0,1,2)")
#define VERBOSE_LONGTEXT N_( "This is the verbosity level (0=only errors and " "standard messages, 1=warnings, 2=debug).")
#define OPEN_TEXT N_("Default stream")
#define OPEN_LONGTEXT N_( "This stream will always be opened at VLC startup." )
#define COLOR_TEXT N_("Color messages")
#define COLOR_LONGTEXT N_( "This enables colorization of the messages sent to the console. " "Your terminal needs Linux color support for this to work.")
#define INTERACTION_TEXT N_("Interface interaction")
#define INTERACTION_LONGTEXT N_( "When this is enabled, the interface will show a dialog box each time " "some user input is required." )
#define AOUT_CAT_LONGTEXT N_( "These options allow you to modify the behavior of the audio " "subsystem, and to add audio filters which can be used for " "post processing or visual effects (spectrum analyzer, etc.). " "Enable these filters here, and configure them in the \"audio filters\" " "modules section.")
#define AOUT_TEXT N_("Audio output module")
#define AOUT_LONGTEXT N_( "This is the audio output method used by VLC. " "The default behavior is to automatically select the best method " "available.")
...
#define AUDIO_BITEXACT_LONGTEXT N_( "This will disable all audio filters, even audio converters. " "This may result on audio not working if the output can't adapt to the " "input format.")
#define AUDIO_TEXT N_("Enable audio")
#define AUDIO_LONGTEXT N_( "You can completely disable the audio output. The audio " "decoding stage will not take place, thus saving some processing power.")
...
#define GAIN_LONGTEXT N_( "This linear gain will be applied to outputted audio.")
#define VOLUME_STEP_TEXT N_("Audio output volume step")
#define VOLUME_STEP_LONGTEXT N_( "The step size of the volume is adjustable using this option.")
#define AOUT_VOLUME_STEP 12.8
#define VOLUME_SAVE_TEXT N_( "Remember the audio volume" )
#define VOLUME_SAVE_LONGTEXT N_( "The volume can be recorded and automatically restored next time " "VLC is used." )
#define DESYNC_TEXT N_("Audio desynchronization compensation")
#define DESYNC_LONGTEXT N_( "This delays the audio output. The delay must be given in milliseconds. " "This can be handy if you notice a lag between the video and the audio.")
#define AUDIO_RESAMPLER_TEXT N_("Audio resampler")
#define AUDIO_RESAMPLER_LONGTEXT N_( "This selects which plugin to use for audio resampling." )
#define MULTICHA_LONGTEXT N_( "Sets the audio output channels mode that will be used by default " "if your hardware and the audio stream are compatible.")
#if defined(__ANDROID__) || defined(__APPLE__) || defined(_WIN32)
#define SPDIF_TEXT N_("Force S/PDIF support")
#define SPDIF_LONGTEXT N_( "This option should be used when the audio output can't negotiate S/PDIF support.")
#endif
#define FORCE_DOLBY_TEXT N_("Force detection of Dolby Surround")
#define FORCE_DOLBY_LONGTEXT N_( "Use this when you know your stream is (or is not) encoded with Dolby ""Surround but fails to be detected as such. Even if the stream is ""not actually encoded with Dolby Surround, turning on this option might ""enhance your experience, especially when combined with the Headphone ""Channel Mixer." )
...
#define AUDIO_FILTER_LONGTEXT N_( "This adds audio post processing filters, to modify " "the sound rendering." )
#define AUDIO_VISUAL_TEXT N_("Audio visualizations")
#define AUDIO_VISUAL_LONGTEXT N_( "This adds visualization modules (spectrum analyzer, etc.).")
#define AUDIO_REPLAY_GAIN_MODE_TEXT N_( "Replay gain mode" )
#define AUDIO_REPLAY_GAIN_MODE_LONGTEXT N_( "Select the replay gain mode" )
#define AUDIO_REPLAY_GAIN_PREAMP_TEXT N_( "Replay preamp" )
#define AUDIO_REPLAY_GAIN_PREAMP_LONGTEXT N_( "This allows you to change the default target level (89 dB) " "for stream with replay gain information" )
#define AUDIO_REPLAY_GAIN_DEFAULT_TEXT N_( "Default replay gain" )
#define AUDIO_REPLAY_GAIN_DEFAULT_LONGTEXT N_( "This is the gain used for stream without replay gain information" )
#define AUDIO_REPLAY_GAIN_PEAK_PROTECTION_TEXT N_( "Peak protection" )
#define AUDIO_REPLAY_GAIN_PEAK_PROTECTION_LONGTEXT N_( "Protect against sound clipping" )
#define AUDIO_TIME_STRETCH_TEXT N_( "Enable time stretching audio" )
#define AUDIO_TIME_STRETCH_LONGTEXT N_( "This allows playing audio at lower or higher speed without " "affecting the audio pitch" )
static const char *const ppsz_replay_gain_mode[] = {
"none", "track", "album" };
static const char *const ppsz_replay_gain_mode_text[] = {
N_("None"), N_("Track"), N_("Album") };
#define VOUT_CAT_LONGTEXT N_( "These options allow you to modify the behavior of the video output " "subsystem. You can for example enable video filters (deinterlacing, " "image adjusting, etc.). Enable these filters here and configure " "them in the \"video filters\" modules section. You can also set many " "miscellaneous video options." )
#define VOUT_TEXT N_("Video output module")
#define VOUT_LONGTEXT N_( "This is the the video output method used by VLC. " "The default behavior is to automatically select the best method available.")
#define VIDEO_TEXT N_("Enable video")
#define VIDEO_LONGTEXT N_( "You can completely disable the video output. The video " "decoding stage will not take place, thus saving some processing power.")
#define WIDTH_TEXT N_("Video width")
#define WIDTH_LONGTEXT N_( "You can enforce the video width. By default (-1) VLC will " "adapt to the video characteristics.")
#define HEIGHT_TEXT N_("Video height")
#define HEIGHT_LONGTEXT N_( "You can enforce the video height. By default (-1) VLC will " "adapt to the video characteristics.")
#define VIDEOX_TEXT N_("Video X coordinate")
#define VIDEOX_LONGTEXT N_( "You can enforce the position of the top left corner of the video window ""(X coordinate).")
#define VIDEOY_TEXT N_("Video Y coordinate")
#define VIDEOY_LONGTEXT N_( "You can enforce the position of the top left corner of the video window ""(Y coordinate).")
#define VIDEO_TITLE_TEXT N_("Video title")
#define VIDEO_TITLE_LONGTEXT N_( "Custom title for the video window (in case the video is not embedded in ""the interface).")
#define ALIGN_TEXT N_("Video alignment")
#define ALIGN_LONGTEXT N_( "Enforce the alignment of the video in its window. By default (0) it " "will be centered (0=center, 1=left, 2=right, 4=top, 8=bottom, you can " "also use combinations of these values, like 6=4+2 meaning top-right).")
...
#define ZOOM_LONGTEXT N_( "You can zoom the video by the specified factor.")
#define GRAYSCALE_TEXT N_("Grayscale video output")
#define GRAYSCALE_LONGTEXT N_( "Output video in grayscale. As the color information aren't decoded, " "this can save some processing power." )
#define EMBEDDED_TEXT N_("Embedded video")
#define EMBEDDED_LONGTEXT N_( "Embed the video output in the main interface." )
#define FULLSCREEN_TEXT N_("Fullscreen video output")
#define FULLSCREEN_LONGTEXT N_( "Start video in fullscreen mode" )
#define VIDEO_ON_TOP_TEXT N_("Always on top")
#define VIDEO_ON_TOP_LONGTEXT N_( "Always place the video window on top of other windows." )
#define WALLPAPER_TEXT N_("Enable wallpaper mode")
#define WALLPAPER_LONGTEXT N_( "The wallpaper mode allows you to display the video as the desktop " "background." )
#define VIDEO_TITLE_SHOW_TEXT N_("Show media title on video")
#define VIDEO_TITLE_SHOW_LONGTEXT N_( "Display the title of the video on top of the movie.")
#define VIDEO_TITLE_TIMEOUT_TEXT N_("Show video title for x milliseconds")
#define VIDEO_TITLE_TIMEOUT_LONGTEXT N_( "Show the video title for n milliseconds, default is 5000 ms (5 sec.)")
#define VIDEO_TITLE_POSITION_TEXT N_("Position of video title")
#define VIDEO_TITLE_POSITION_LONGTEXT N_( "Place on video where to display the title (default bottom center).")
#define MOUSE_HIDE_TIMEOUT_TEXT N_("Hide cursor and fullscreen " "controller after x milliseconds")
#define MOUSE_HIDE_TIMEOUT_LONGTEXT N_( "Hide mouse cursor and fullscreen controller after " "n milliseconds.")
#define DEINTERLACE_TEXT N_("Deinterlace")
#define DEINTERLACE_LONGTEXT N_("Deinterlace")
...
#define DEINTERLACE_MODE_LONGTEXT N_( "Deinterlace method to use for video processing.")
...
#define VIDEO_DECO_LONGTEXT N_( "VLC can avoid creating window caption, frames, etc... around the video" ", giving a \"minimal\" window.")
#define VIDEO_FILTER_TEXT N_("Video filter module")
#define VIDEO_FILTER_LONGTEXT N_( "This adds post-processing filters to enhance the " "picture quality, for instance deinterlacing, or distort " "the video.")
#define SNAP_PATH_TEXT N_("Video snapshot directory (or filename)")
#define SNAP_PATH_LONGTEXT N_( "Directory where the video snapshots will be stored.")
#define SNAP_PREFIX_TEXT N_("Video snapshot file prefix")
#define SNAP_PREFIX_LONGTEXT N_( "Video snapshot file prefix" )
#define SNAP_FORMAT_TEXT N_("Video snapshot format")
#define SNAP_FORMAT_LONGTEXT N_( "Image format which will be used to store the video snapshots" )
...
#define SNAP_SEQUENTIAL_LONGTEXT N_( "Use sequential numbers instead of timestamps for snapshot numbering")
#define SNAP_WIDTH_TEXT N_("Video snapshot width")
#define SNAP_WIDTH_LONGTEXT N_( "You can enforce the width of the video snapshot. By default " "it will keep the original width (-1). Using 0 will scale the width " "to keep the aspect ratio." )
#define SNAP_HEIGHT_TEXT N_("Video snapshot height")
#define SNAP_HEIGHT_LONGTEXT N_( "You can enforce the height of the video snapshot. By default " "it will keep the original height (-1). Using 0 will scale the height " "to keep the aspect ratio." )
#define CROP_TEXT N_("Video cropping")
#define CROP_LONGTEXT N_( "This forces the cropping of the source video. " "Accepted formats are x:y (4:3, 16:9, etc.) expressing the global image " "aspect.")
#define ASPECT_RATIO_TEXT N_("Source aspect ratio")
#define ASPECT_RATIO_LONGTEXT N_( "This forces the source aspect ratio. For instance, some DVDs claim " "to be 16:9 while they are actually 4:3. This can also be used as a " "hint for VLC when a movie does not have aspect ratio information. " "Accepted formats are x:y (4:3, 16:9, etc.) expressing the global image " "aspect, or a float value (1.25, 1.3333, etc.) expressing pixel " "squareness.")
#define AUTOSCALE_TEXT N_("Video Auto Scaling")
#define AUTOSCALE_LONGTEXT N_( "Let the video scale to fit a given window or fullscreen.")
#define SCALEFACTOR_TEXT N_("Video scaling factor")
#define SCALEFACTOR_LONGTEXT N_( "Scaling factor used when Auto Scaling is disabled.\n" "Default value is 1.0 (original video size).")
#define CUSTOM_CROP_RATIOS_TEXT N_("Custom crop ratios list")
#define CUSTOM_CROP_RATIOS_LONGTEXT N_( "Comma separated list of crop ratios which will be added in the " "interface's crop ratios list.")
#define CUSTOM_ASPECT_RATIOS_TEXT N_("Custom aspect ratios list")
#define CUSTOM_ASPECT_RATIOS_LONGTEXT N_( "Comma separated list of aspect ratios which will be added in the " "interface's aspect ratio list.")
#define HDTV_FIX_TEXT N_("Fix HDTV height")
#define HDTV_FIX_LONGTEXT N_( "This allows proper handling of HDTV-1080 video format " "even if broken encoder incorrectly sets height to 1088 lines. " "You should only disable this option if your video has a " "non-standard format requiring all 1088 lines.")
#define MASPECT_RATIO_TEXT N_("Monitor pixel aspect ratio")
#define MASPECT_RATIO_LONGTEXT N_( "This forces the monitor aspect ratio. Most monitors have square " "pixels (1:1). If you have a 16:9 screen, you might need to change this " "to 4:3 in order to keep proportions.")
#define SKIP_FRAMES_TEXT N_("Skip frames")
#define SKIP_FRAMES_LONGTEXT N_( "Enables framedropping on MPEG2 stream. Framedropping " "occurs when your computer is not powerful enough" )
#define DROP_LATE_FRAMES_TEXT N_("Drop late frames")
#define DROP_LATE_FRAMES_LONGTEXT N_( "This drops frames that are late (arrive to the video output after " "their intended display date)." )
#define QUIET_SYNCHRO_TEXT N_("Quiet synchro")
#define QUIET_SYNCHRO_LONGTEXT N_( "This avoids flooding the message log with debug output from the " "video output synchronization mechanism.")
#define KEYBOARD_EVENTS_TEXT N_("Key press events")
#define KEYBOARD_EVENTS_LONGTEXT N_( "This enables VLC hotkeys from the (non-embedded) video window." )
#define MOUSE_EVENTS_TEXT N_("Mouse events")
#define MOUSE_EVENTS_LONGTEXT N_( "This enables handling of mouse clicks on the video." )
#define INPUT_CAT_LONGTEXT N_( "These options allow you to modify the behavior of the input " "subsystem, such as the DVD or VCD device, the network interface " "settings or the subtitle channel.")
#define CACHING_TEXT N_("File caching (ms)")
#define CACHING_LONGTEXT N_( "Caching value for local files, in milliseconds." )
#define CAPTURE_CACHING_TEXT N_("Live capture caching (ms)")
#define CAPTURE_CACHING_LONGTEXT N_( "Caching value for cameras and microphones, in milliseconds." )
#define DISC_CACHING_TEXT N_("Disc caching (ms)")
#define DISC_CACHING_LONGTEXT N_( "Caching value for optical media, in milliseconds." )
#define NETWORK_CACHING_TEXT N_("Network caching (ms)")
#define NETWORK_CACHING_LONGTEXT N_( "Caching value for network resources, in milliseconds." )
#define CR_AVERAGE_TEXT N_("Clock reference average counter")
#define CR_AVERAGE_LONGTEXT N_( "When using the PVR input (or a very irregular source), you should " "set this to 10000.")
#define CLOCK_SYNCHRO_TEXT N_("Clock synchronisation")
#define CLOCK_SYNCHRO_LONGTEXT N_( "It is possible to disable the input clock synchronisation for " "real-time sources. Use this if you experience jerky playback of " "network streams.")
#define CLOCK_JITTER_TEXT N_("Clock jitter")
#define CLOCK_JITTER_LONGTEXT N_( "This defines the maximum input delay jitter that the synchronization " "algorithms should try to compensate (in milliseconds)." )
...
#define NETSYNC_LONGTEXT N_( "This allows you to remotely " "synchronise clocks for server and client. The detailed settings " "are available in Advanced / Network Sync." )
...
#define MTU_LONGTEXT N_( "This is the maximum application-layer packet size that can be " "transmitted over the network (in bytes).")
#define MTU_DEFAULT 1400
#define TTL_TEXT N_("Hop limit (TTL)")
#define TTL_LONGTEXT N_( "This is the hop limit (also known as \"Time-To-Live\" or TTL) of " "the multicast packets sent by the stream output (-1 = use operating " "system built-in default).")
#define MIFACE_TEXT N_("Multicast output interface")
#define MIFACE_LONGTEXT N_( "Default multicast interface. This overrides the routing table.")
#define DSCP_TEXT N_("DiffServ Code Point")
#define DSCP_LONGTEXT N_("Differentiated Services Code Point " "for outgoing UDP streams (or IPv4 Type Of Service, " "or IPv6 Traffic Class). This is used for network Quality of Service.")
#define INPUT_PROGRAM_TEXT N_("Program")
#define INPUT_PROGRAM_LONGTEXT N_( "Choose the program to select by giving its Service ID. " "Only use this option if you want to read a multi-program stream " "(like DVB streams for example)." )
#define INPUT_PROGRAMS_TEXT N_("Programs")
#define INPUT_PROGRAMS_LONGTEXT N_( "Choose the programs to select by giving a comma-separated list of " "Service IDs (SIDs). " "Only use this option if you want to read a multi-program stream " "(like DVB streams for example)." )
#define INPUT_VIDEOTRACK_TEXT N_("Video track")
#define INPUT_VIDEOTRACK_LONGTEXT N_( "Stream number of the video track to use " "(from 0 to n).")
#define INPUT_AUDIOTRACK_TEXT N_("Audio track")
#define INPUT_AUDIOTRACK_LONGTEXT N_( "Stream number of the audio track to use " "(from 0 to n).")
#define INPUT_SUBTRACK_TEXT N_("Subtitle track")
#define INPUT_SUBTRACK_LONGTEXT N_( "Stream number of the subtitle track to use " "(from 0 to n).")
#define INPUT_AUDIOTRACK_LANG_TEXT N_("Audio language")
#define INPUT_AUDIOTRACK_LANG_LONGTEXT N_( "Language of the audio track you want to use " "(comma separated, two or three letter country code, you may use 'none' to avoid a fallback to another language).")
#define INPUT_SUBTRACK_LANG_TEXT N_("Subtitle language")
#define INPUT_SUBTRACK_LANG_LONGTEXT N_( "Language of the subtitle track you want to use " "(comma separated, two or three letters country code, you may use 'any' as a fallback).")
#define INPUT_MENUTRACK_LANG_TEXT N_("Menu language")
#define INPUT_MENUTRACK_LANG_LONGTEXT N_( "Language of the menus you want to use with DVD/BluRay " "(comma separated, two or three letters country code, you may use 'any' as a fallback).")
#define INPUT_VIDEOTRACK_ID_TEXT N_("Video track ID")
#define INPUT_VIDEOTRACK_ID_LONGTEXT N_( "Stream ID of the video track to use.")
#define INPUT_AUDIOTRACK_ID_TEXT N_("Audio track ID")
#define INPUT_AUDIOTRACK_ID_LONGTEXT N_( "Stream ID of the audio track to use.")
#define INPUT_SUBTRACK_ID_TEXT N_("Subtitle track ID")
#define INPUT_SUBTRACK_ID_LONGTEXT N_( "Stream ID of the subtitle track to use.")
...
#define INPUT_PREFERREDRESOLUTION_LONGTEXT N_( "When several video formats are available, select one whose " "resolution is closest to (but not higher than) this setting, " "in number of lines. Use this option if you don't have enough CPU " "power or network bandwidth to play higher resolutions.")
...
#define INPUT_LOWDELAY_LONGTEXT N_("Try to minimize delay along decoding chain.""Might break with non compliant streams.")
...
#define START_TIME_LONGTEXT N_( "The stream will start at this position (in seconds)." )
#define STOP_TIME_TEXT N_("Stop time")
#define STOP_TIME_LONGTEXT N_( "The stream will stop at this position (in seconds)." )
#define RUN_TIME_TEXT N_("Run time")
#define RUN_TIME_LONGTEXT N_( "The stream will run this duration (in seconds)." )
#define INPUT_FAST_SEEK_TEXT N_("Fast seek")
#define INPUT_FAST_SEEK_LONGTEXT N_( "Favor speed over precision while seeking" )
#define INPUT_RATE_TEXT N_("Playback speed")
#define INPUT_RATE_LONGTEXT N_( "This defines the playback speed (nominal speed is 1.0)." )
#define INPUT_LIST_TEXT N_("Input list")
#define INPUT_LIST_LONGTEXT N_( "You can give a comma-separated list " "of inputs that will be concatenated together after the normal one.")
#define INPUT_SLAVE_TEXT N_("Input slave (experimental)")
#define INPUT_SLAVE_LONGTEXT N_( "This allows you to play from several inputs at " "the same time. This feature is experimental, not all formats " "are supported. Use a '#' separated list of inputs.")
#define BOOKMARKS_TEXT N_("Bookmarks list for a stream")
#define BOOKMARKS_LONGTEXT N_( "You can manually give a list of bookmarks for a stream in " "the form \"{name=bookmark-name,time=optional-time-offset," "bytes=optional-byte-offset},{...}\"")
#define RESTORE_PLAYBACK_POS_TEXT N_("Continue playback?")
#define RESTORE_PLAYBACK_STATE_TEXT N_("Resume last playback states")
#define RESTORE_PLAYBACK_STATE_LONGTEXT N_("This will resume the last playback " "state, such as the selected tracks, rate, aspect-ratio, ..." )
#define INPUT_RECORD_PATH_TEXT N_("Record directory")
#define INPUT_RECORD_PATH_LONGTEXT N_( "Directory where the records will be stored" )
#define INPUT_RECORD_NATIVE_TEXT N_("Prefer native stream recording")
#define INPUT_RECORD_NATIVE_LONGTEXT N_( "When possible, the input stream will be recorded instead of using " "the stream output module" )
#define INPUT_TIMESHIFT_PATH_TEXT N_("Timeshift directory")
#define INPUT_TIMESHIFT_PATH_LONGTEXT N_( "Directory used to store the timeshift temporary files." )
#define INPUT_TIMESHIFT_GRANULARITY_TEXT N_("Timeshift granularity")
#define INPUT_TIMESHIFT_GRANULARITY_LONGTEXT N_( "This is the maximum size in bytes of the temporary files " "that will be used to store the timeshifted streams." )
#define INPUT_TITLE_FORMAT_TEXT N_( "Change title according to current media" )
#define INPUT_TITLE_FORMAT_LONGTEXT N_( "This option allows you to set the title according to what's being played<br>" "$a: Artist<br>$b: Album<br>$c: Copyright<br>$t: Title<br>$g: Genre<br>" "$n: Track num<br>$p: Now playing<br>$A: Date<br>$D: Duration<br>" "$Z: \"Now playing\" (Fall back on Title - Artist)" )
#define INPUT_LUA_TEXT N_( "Disable all lua plugins" )
#define SUB_CAT_LONGTEXT N_( "These options allow you to modify the behavior of the subpictures " "subsystem. You can for example enable subpictures sources (logo, etc.). " "Enable these filters here and configure them in the " "\"subsources filters\" modules section. You can also set many " "miscellaneous subpictures options." )
#define SUB_MARGIN_TEXT N_("Force subtitle position")
#define SUB_MARGIN_LONGTEXT N_( "You can use this option to place the subtitles under the movie, " "instead of over the movie. Try several positions.")
...
#define SPU_LONGTEXT N_( "You can completely disable the sub-picture processing.")
#define SECONDARY_SUB_POSITION_TEXT N_("Position of secondary subtitles")
#define SECONDARY_SUB_POSITION_LONGTEXT N_( "Place on video where to display secondary subtitles (default bottom center).")
#define SECONDARY_SUB_MARGIN_TEXT N_("Force secondary subtitle position")
#define SECONDARY_SUB_MARGIN_LONGTEXT N_( "You can use this option to vertically adjust the position secondary " "subtitles are displayed.")
#define OSD_TEXT N_("On Screen Display")
#define OSD_LONGTEXT N_( "VLC can display messages on the video. This is called OSD (On Screen " "Display).")
#define TEXTRENDERER_TEXT N_("Text rendering module")
#define TEXTRENDERER_LONGTEXT N_( "VLC normally uses Freetype for rendering, but this allows you to use svg for instance.")
#define SUB_SOURCE_TEXT N_("Subpictures source module")
#define SUB_SOURCE_LONGTEXT N_( "This adds so-called \"subpicture sources\". These filters overlay " "some images or text over the video (like a logo, arbitrary text, ...)." )
#define SUB_FILTER_TEXT N_("Subpictures filter module")
#define SUB_FILTER_LONGTEXT N_( "This adds so-called \"subpicture filters\". These filter subpictures " "created by subtitle decoders or other subpictures sources." )
#define SUB_AUTO_TEXT N_("Autodetect subtitle files")
#define SUB_AUTO_LONGTEXT N_( "Automatically detect a subtitle file, if no subtitle filename is " "specified (based on the filename of the movie).")
#define SUB_FUZZY_TEXT N_("Subtitle autodetection fuzziness")
#define SUB_FUZZY_LONGTEXT N_( "This determines how fuzzy subtitle and movie filename matching " "will be. Options are:\n" "0 = no subtitles autodetected\n" "1 = any subtitle file\n" "2 = any subtitle file containing the movie name\n" "3 = subtitle file matching the movie name with additional chars\n" "4 = subtitle file matching the movie name exactly")
#define SUB_PATH_TEXT N_("Subtitle autodetection paths")
#define SUB_PATH_LONGTEXT N_( "Look for a subtitle file in those paths too, if your subtitle " "file was not found in the current directory.")
...
#define SUB_FILE_LONGTEXT N_( "Load this subtitle file. To be used when autodetect cannot detect " "your subtitle file.")
#define DVD_DEV_TEXT N_("DVD device")
#define VCD_DEV_TEXT N_("VCD device")
#define CDAUDIO_DEV_TEXT N_("Audio CD device")
#if defined( _WIN32 ) || defined( __OS2__ )
#define DVD_DEV_LONGTEXT N_( "This is the default DVD drive (or file) to use. Don't forget the colon " "after the drive letter (e.g. D:)")
#define VCD_DEV_LONGTEXT N_( "This is the default VCD drive (or file) to use. Don't forget the colon " "after the drive letter (e.g. D:)")
...
#define VCD_DEVICE "D:"
#else
#define DVD_DEV_LONGTEXT N_( "This is the default DVD device to use.")
#define VCD_DEV_LONGTEXT N_( "This is the default VCD device to use." )
...
#define TIMEOUT_LONGTEXT N_( "Default TCP connection timeout (in milliseconds)." )
#define HTTP_HOST_TEXT N_( "HTTP server address" )
#define HOST_LONGTEXT N_( "By default, the server will listen on any local IP address. " "Specify an IP address (e.g. ::1 or 127.0.0.1) or a host name " "(e.g. localhost) to restrict them to a specific network interface." )
...
#define HTTP_PORT_LONGTEXT N_( "The HTTP server will listen on this TCP port. " "The standard HTTP port number is 80. " "However allocation of port numbers below 1025 is usually restricted " "by the operating system." )
#define HTTPS_PORT_TEXT N_( "HTTPS server port" )
#define HTTPS_PORT_LONGTEXT N_( "The HTTPS server will listen on this TCP port. " "The standard HTTPS port number is 443. " "However allocation of port numbers below 1025 is usually restricted " "by the operating system." )
#define RTSP_PORT_TEXT N_( "RTSP server port" )
#define RTSP_PORT_LONGTEXT N_( "The RTSP server will listen on this TCP port. " "The standard RTSP port number is 554. " "However allocation of port numbers below 1025 is usually restricted " "by the operating system." )
#define HTTP_CERT_TEXT N_("HTTP/TLS server certificate")
#define CERT_LONGTEXT N_( "This X.509 certicate file (PEM format) is used for server-side TLS. " "On OS X, the string is used as a label to search the certificate in the keychain." )
#define HTTP_KEY_TEXT N_("HTTP/TLS server private key")
#define KEY_LONGTEXT N_( "This private key file (PEM format) is used for server-side TLS.")
#define PROXY_TEXT N_("HTTP proxy")
#define PROXY_LONGTEXT N_( "HTTP proxy to be used It must be of the form " "http://[user@]myproxy.mydomain:myport/ ; " "if empty, the http_proxy environment variable will be tried." )
#define PROXY_PASS_TEXT N_("HTTP proxy password")
#define PROXY_PASS_LONGTEXT N_( "If your HTTP proxy requires a password, set it here." )
#define SOCKS_SERVER_TEXT N_("SOCKS server")
#define SOCKS_SERVER_LONGTEXT N_( "SOCKS proxy server to use. This must be of the form " "address:port. It will be used for all TCP connections" )
#define SOCKS_USER_TEXT N_("SOCKS user name")
#define SOCKS_USER_LONGTEXT N_( "User name to be used for connection to the SOCKS proxy." )
#define SOCKS_PASS_TEXT N_("SOCKS password")
#define SOCKS_PASS_LONGTEXT N_( "Password to be used for connection to the SOCKS proxy." )
#define META_TITLE_TEXT N_("Title metadata")
#define META_TITLE_LONGTEXT N_( "Allows you to specify a \"title\" metadata for an input.")
#define META_AUTHOR_TEXT N_("Author metadata")
#define META_AUTHOR_LONGTEXT N_( "Allows you to specify an \"author\" metadata for an input.")
#define META_ARTIST_TEXT N_("Artist metadata")
#define META_ARTIST_LONGTEXT N_( "Allows you to specify an \"artist\" metadata for an input.")
#define META_GENRE_TEXT N_("Genre metadata")
#define META_GENRE_LONGTEXT N_( "Allows you to specify a \"genre\" metadata for an input.")
#define META_CPYR_TEXT N_("Copyright metadata")
#define META_CPYR_LONGTEXT N_( "Allows you to specify a \"copyright\" metadata for an input.")
#define META_DESCR_TEXT N_("Description metadata")
#define META_DESCR_LONGTEXT N_( "Allows you to specify a \"description\" metadata for an input.")
#define META_DATE_TEXT N_("Date metadata")
#define META_DATE_LONGTEXT N_( "Allows you to specify a \"date\" metadata for an input.")
#define META_URL_TEXT N_("URL metadata")
#define META_URL_LONGTEXT N_( "Allows you to specify a \"url\" metadata for an input.")
#define CODEC_CAT_LONGTEXT N_( "This option can be used to alter the way VLC selects " "its codecs (decompression methods). Only advanced users should " "alter this option as it can break playback of all your streams." )
#define CODEC_TEXT N_("Preferred decoders list")
#define CODEC_LONGTEXT N_( "List of codecs that VLC will use in " "priority. For instance, 'dummy,a52' will try the dummy and a52 codecs " "before trying the other ones. Only advanced users should " "alter this option as it can break playback of all your streams." )
#define HW_DEC_TEXT N_("Enable hardware decoders")
#define HW_DEC_LONGTEXT N_( "VLC will fallback automatically to software decoders in case of " "hardware decoder failure." )
#define ENCODER_TEXT N_("Preferred encoders list")
#define ENCODER_LONGTEXT N_( "This allows you to select a list of encoders that VLC will use in " "priority.")
...
#define SOUT_CAT_LONGTEXT N_( "These options allow you to set default global options for the " "stream output subsystem." )
#define SOUT_TEXT N_("Default stream output chain")
#define SOUT_LONGTEXT N_( "You can enter here a default stream output chain. Refer to ""the documentation to learn how to build such chains. " "Warning: this chain will be enabled for all streams." )
#define SOUT_ALL_TEXT N_("Enable streaming of all ES")
#define SOUT_ALL_LONGTEXT N_( "Stream all elementary streams (video, audio and subtitles)")
#define SOUT_DISPLAY_TEXT N_("Display while streaming")
#define SOUT_DISPLAY_LONGTEXT N_( "Play locally the stream while streaming it.")
#define SOUT_VIDEO_TEXT N_("Enable video stream output")
#define SOUT_VIDEO_LONGTEXT N_( "Choose whether the video stream should be redirected to " "the stream output facility when this last one is enabled.")
#define SOUT_AUDIO_TEXT N_("Enable audio stream output")
#define SOUT_AUDIO_LONGTEXT N_( "Choose whether the audio stream should be redirected to " "the stream output facility when this last one is enabled.")
#define SOUT_SPU_TEXT N_("Enable SPU stream output")
#define SOUT_SPU_LONGTEXT N_( "Choose whether the SPU streams should be redirected to " "the stream output facility when this last one is enabled.")
#define SOUT_KEEP_TEXT N_("Keep stream output open" )
#define SOUT_KEEP_LONGTEXT N_( "This allows you to keep an unique stream output instance across " "multiple playlist item (automatically insert the gather stream output " "if not specified)" )
#define SOUT_MUX_CACHING_TEXT N_("Stream output muxer caching (ms)")
#define SOUT_MUX_CACHING_LONGTEXT N_( "This allow you to configure the initial caching amount for stream output " "muxer. This value should be set in milliseconds." )
#define PACKETIZER_TEXT N_("Preferred packetizer list")
#define PACKETIZER_LONGTEXT N_( "This allows you to select the order in which VLC will choose its " "packetizers." )
#define MUX_TEXT N_("Mux module")
#define MUX_LONGTEXT N_( "This is a legacy entry to let you configure mux modules")
#define ACCESS_OUTPUT_TEXT N_("Access output module")
#define ACCESS_OUTPUT_LONGTEXT N_( "This is a legacy entry to let you configure access output modules")
#define ANN_SAPCTRL_LONGTEXT N_( "If this option is enabled, the flow on " "the SAP multicast address will be controlled. This is needed if you " "want to make announcements on the MBone." )
#define ANN_SAPINTV_TEXT N_("SAP announcement interval")
#define ANN_SAPINTV_LONGTEXT N_( "When the SAP flow control is disabled, " "this lets you set the fixed interval between SAP announcements." )
#define MISC_CAT_LONGTEXT N_( "These options allow you to select default modules. Leave these " "alone unless you really know what you are doing." )
#define ACCESS_TEXT N_("Access module")
#define ACCESS_LONGTEXT N_( "This allows you to force an access module. You can use it if " "the correct access is not automatically detected. You should not ""set this as a global option unless you really know what you are doing." )
#define STREAM_FILTER_TEXT N_("Stream filter module")
#define STREAM_FILTER_LONGTEXT N_( "Stream filters are used to modify the stream that is being read." )
#define DEMUX_FILTER_TEXT N_("Demux filter module")
#define DEMUX_FILTER_LONGTEXT N_( "Demux filters are used to modify/control the stream that is being read." )
#define DEMUX_TEXT N_("Demux module")
#define DEMUX_LONGTEXT N_( "Demultiplexers are used to separate the \"elementary\" streams " "(like audio and video streams). You can use it if " "the correct demuxer is not automatically detected. You should not ""set this as a global option unless you really know what you are doing." )
#define VOD_SERVER_TEXT N_("VoD server module")
#define VOD_SERVER_LONGTEXT N_( "You can select which VoD server module you want to use. Set this " "to 'vod_rtsp' to switch back to the old, legacy module." )
#define USE_STREAM_IMMEDIATE_LONGTEXT N_( "This option is useful if you want to lower the latency when " "reading a stream")
#define VLM_CONF_TEXT N_("VLM configuration file")
#define VLM_CONF_LONGTEXT N_( "Read a VLM configuration file as soon as VLM is started." )
#define PLUGINS_CACHE_TEXT N_("Use a plugins cache")
#define PLUGINS_CACHE_LONGTEXT N_( "Use a plugins cache which will greatly improve the startup time of VLC.")
#define PLUGINS_SCAN_TEXT N_("Scan for new plugins")
#define PLUGINS_SCAN_LONGTEXT N_( "Scan plugin directories for new plugins at startup. " "This increases the startup time of VLC.")
#define KEYSTORE_TEXT N_("Preferred keystore list")
#define KEYSTORE_LONGTEXT N_( "List of keystores that VLC will use in priority." )
#define STATS_TEXT N_("Locally collect statistics")
#define STATS_LONGTEXT N_( "Collect miscellaneous local statistics about the playing media.")
...
#define STARTEDFROMFILE_LONGTEXT N_( "Tell VLC that it is being launched due to a file association in the OS" )
#define ONEINSTANCEWHENSTARTEDFROMFILE_TEXT N_( "Use only one instance when started from file manager")
#define HPRIORITY_TEXT N_("Increase the priority of the process")
#define HPRIORITY_LONGTEXT N_( "Increasing the priority of the process will very likely improve your " "playing experience as it allows VLC not to be disturbed by other " "applications that could otherwise take too much processor time. " "However be advised that in certain circumstances (bugs) VLC could take " "all the processor time and render the whole system unresponsive which " "might require a reboot of your machine.")
...
#endif
#define PLAYLISTENQUEUE_TEXT N_( "Enqueue items into playlist in one instance mode")
#define PLAYLISTENQUEUE_LONGTEXT N_( "When using the one instance only option, enqueue items to playlist " "and keep playing current item.")
#define DBUS_TEXT N_("Expose media player via D-Bus")
#define DBUS_LONGTEXT N_("Allow other applications to control VLC " "using the D-Bus MPRIS protocol.")
#define PLAYLIST_CAT_LONGTEXT N_( "These options define the behavior of the playlist. Some " "of them can be overridden in the playlist dialog box." )
#define PREPARSE_TEXT N_( "Automatically preparse items")
#define PREPARSE_LONGTEXT N_( "Automatically preparse items added to the playlist " "(to retrieve some metadata)." )
#define PREPARSE_TIMEOUT_TEXT N_( "Preparsing timeout" )
#define PREPARSE_TIMEOUT_LONGTEXT N_( "Maximum time allowed to preparse an item, in milliseconds" )
#define PREPARSE_THREADS_TEXT N_( "Preparsing threads" )
#define PREPARSE_THREADS_LONGTEXT N_( "Maximum number of threads used to preparse items" )
#define FETCH_ART_THREADS_TEXT N_( "Fetch-art threads" )
#define FETCH_ART_THREADS_LONGTEXT N_( "Maximum number of threads used to fetch art" )
...
#define IGNORE_LONGTEXT N_( "Files with these extensions will not be added to playlist when " "opening a directory.\n" "This is useful if you add directories that contain playlist files " "for instance. Use a comma-separated list of extensions." )
#define SHOW_HIDDENFILES_TEXT N_("Show hidden files")
#define SHOW_HIDDENFILES_LONGTEXT N_( "Ignore files starting with '.'" )
#define SD_TEXT N_( "Services discovery modules")
#define SD_LONGTEXT N_( "Specifies the services discovery modules to preload, separated by " "colons. Typical value is \"sap\"." )
#define RANDOM_TEXT N_("Play files randomly forever")
#define RANDOM_LONGTEXT N_( "VLC will randomly play files in the playlist until interrupted.")
#define LOOP_TEXT N_("Repeat all")
#define LOOP_LONGTEXT N_( "VLC will keep playing the playlist indefinitely." )
#define REPEAT_TEXT N_("Repeat current item")
#define REPEAT_LONGTEXT N_( "VLC will keep playing the current playlist item." )
#define PAS_TEXT N_("Play and stop")
#define PAS_LONGTEXT N_( "Stop the playlist after each played playlist item." )
#define PAE_TEXT N_("Play and exit")
#define PAE_LONGTEXT N_( "Exit if there are no more items in the playlist." )
#define PAP_TEXT N_("Play and pause")
#define PAP_LONGTEXT N_( "Pause each item in the playlist on the last frame." )
#define SP_TEXT N_("Start paused")
#define SP_LONGTEXT N_( "Pause each item in the playlist on the first frame." )
...
#define CORK_LONGTEXT N_( "If pending audio communication is detected, playback will be paused " "automatically." )
#define ML_TEXT N_("Use media library")
#define ML_LONGTEXT N_( "The media library is automatically saved and reloaded each time you " "start VLC." )
#define PLTREE_TEXT N_("Display playlist tree")
#define PLTREE_LONGTEXT N_( "The playlist can use a tree to categorize some items, like the " "contents of a directory." )
#define HOTKEY_CAT_LONGTEXT N_( "These settings are the global VLC key " "bindings, known as \"hotkeys\"." )
...
#define MOUSE_Y_WHEEL_MODE_LONGTEXT N_( "The mouse wheel vertical (up/down) axis can control volume, " "position or be ignored.")
#define MOUSE_X_WHEEL_MODE_TEXT N_("Mouse wheel horizontal axis control")
#define MOUSE_X_WHEEL_MODE_LONGTEXT N_( "The mouse wheel horizontal (left/right) axis can control volume, " "position or be ignored.")
...
#define WALLPAPER_KEY_LONGTEXT N_( "Toggle wallpaper mode in video output." )
...
add_obsolete_integer( "aout-rate" ) 
add_obsolete_bool( "hq-resampling" ) 
#if defined(__ANDROID__) || defined(__APPLE__) || defined(_WIN32)
add_bool( "spdif", false, SPDIF_TEXT, SPDIF_LONGTEXT, true )
#else
add_obsolete_bool("spdif") 
#endif
...
add_obsolete_integer( "vout-event" ) 
add_obsolete_integer( "x11-event" ) 
...
add_obsolete_bool( "ipv6" ) 
add_obsolete_bool( "ipv4" ) 
...
add_obsolete_string( "sout-http-cert" ) 
add_loadfile("http-key", NULL, HTTP_KEY_TEXT, KEY_LONGTEXT)
add_obsolete_string( "sout-http-key" ) 
add_obsolete_string( "http-ca" ) 
add_obsolete_string( "sout-http-ca" ) 
add_obsolete_string( "http-crl" ) 
add_obsolete_string( "sout-http-crl" ) 
#if defined(_WIN32)
...
change_safe()
add_obsolete_integer( "vdr-caching" ) 
add_integer( "live-caching", MS_FROM_VLC_TICK(DEFAULT_PTS_DELAY),
CAPTURE_CACHING_TEXT, CAPTURE_CACHING_LONGTEXT, true )
change_integer_range( 0, 60000 )
change_safe()
add_obsolete_integer( "alsa-caching" ) 
add_obsolete_integer( "dshow-caching" ) 
add_obsolete_integer( "dv-caching" ) 
add_obsolete_integer( "dvb-caching" ) 
add_obsolete_integer( "eyetv-caching" ) 
add_obsolete_integer( "jack-input-caching" ) 
add_obsolete_integer( "linsys-hdsdi-caching" ) 
add_obsolete_integer( "linsys-sdi-caching" ) 
add_obsolete_integer( "oss-caching" ) 
add_obsolete_integer( "screen-caching" ) 
add_obsolete_integer( "v4l2-caching" ) 
add_integer( "disc-caching", MS_FROM_VLC_TICK(DEFAULT_PTS_DELAY),
DISC_CACHING_TEXT, DISC_CACHING_LONGTEXT, true )
change_integer_range( 0, 60000 )
change_safe()
add_obsolete_integer( "bd-caching" ) 
add_obsolete_integer( "bluray-caching" ) 
add_obsolete_integer( "cdda-caching" ) 
add_obsolete_integer( "dvdnav-caching" ) 
add_obsolete_integer( "dvdread-caching" ) 
add_obsolete_integer( "vcd-caching" ) 
add_integer( "network-caching", 1000,
NETWORK_CACHING_TEXT, NETWORK_CACHING_LONGTEXT, true )
change_integer_range( 0, 60000 )
change_safe()
add_obsolete_integer( "ftp-caching" ) 
add_obsolete_integer( "http-caching" ) 
add_obsolete_integer( "mms-caching" ) 
add_obsolete_integer( "realrtsp-caching" ) 
add_obsolete_integer( "rtp-caching" ) 
add_obsolete_integer( "rtsp-caching" ) 
add_obsolete_integer( "sftp-caching" ) 
add_obsolete_integer( "smb-caching" ) 
add_obsolete_integer( "tcp-caching" ) 
add_obsolete_integer( "udp-caching" ) 
...
add_obsolete_bool( "mmx" ) 
add_obsolete_bool( "3dn" ) 
add_obsolete_bool( "mmxext" ) 
add_obsolete_bool( "sse" ) 
add_obsolete_bool( "sse2" ) 
add_obsolete_bool( "sse3" ) 
add_obsolete_bool( "ssse3" ) 
add_obsolete_bool( "sse41" ) 
add_obsolete_bool( "sse42" ) 
#endif
#if defined( __powerpc__ ) || defined( __ppc__ ) || defined( __ppc64__ )
add_obsolete_bool( "altivec" ) 
#endif
...
#endif
add_obsolete_string( "data-path" ) 
add_string( "keystore", NULL, KEYSTORE_TEXT,
KEYSTORE_LONGTEXT, true )
set_section( N_("Performance options"), NULL )
#if defined (LIBVLC_USE_PTHREAD)
add_obsolete_bool( "rt-priority" ) 
add_obsolete_integer( "rt-offset" ) 
#endif
#if defined(HAVE_DBUS)
add_obsolete_bool( "inhibit" ) 
#endif
#if defined(_WIN32) || defined(__OS2__)
...
change_short('v')
change_volatile ()
add_obsolete_string( "verbose-objects" ) 
#if !defined(_WIN32) && !defined(__OS2__)
...
add_obsolete_string( "language" ) 
#endif
add_bool( "color", true, COLOR_TEXT, COLOR_LONGTEXT, true )
change_volatile ()
add_obsolete_bool( "advanced" ) 
...
change_integer_list( mouse_wheel_values, mouse_wheel_texts )
add_obsolete_integer( "hotkeys-mousewheel-mode" ) 
...
#define FULL_HELP_TEXT N_("Exhaustive help for VLC and its modules")
#define LONGHELP_TEXT N_("print help for VLC and all its modules (can be combined with " "--help-verbose)")
...
#define MODULE_TEXT N_("print help on a specific module (can be combined with " "--help-verbose). Prefix the module name with = for strict " "matches.")
