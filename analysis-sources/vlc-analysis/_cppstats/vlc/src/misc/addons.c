...
#define INIT_QUEUE( name ) p_manager->p_priv->name.b_live = false;vlc_mutex_init( &p_manager->p_priv->name.lock );vlc_cond_init( &p_manager->p_priv->name.waitcond );ARRAY_INIT( p_manager->p_priv->name.entries );
INIT_QUEUE( finder )
...
#define FREE_QUEUE( name ) ARRAY_FOREACH( p_entry, p_manager->p_priv->name.entries )addon_entry_Release( p_entry );ARRAY_RESET( p_manager->p_priv->name.entries );vlc_interrupt_destroy( p_manager->p_priv->name.p_interrupt );
FREE_QUEUE( finder )
