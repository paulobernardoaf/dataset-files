...
#define MERGE(var, fflag) if( (p_src->i_features & fflag) && (b_override || !(p_dst->i_features & fflag)) )p_dst->var = p_src->var
#define MERGE_SIZE(var) if( p_src->var > 0 && (b_override || p_dst->var <= 0) )p_dst->var = p_src->var
