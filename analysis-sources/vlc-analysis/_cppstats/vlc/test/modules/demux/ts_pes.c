...
#define RESET do {block_ChainRelease(output);output = NULL;block_ChainRelease(pes.gather.p_data);memset(&pes, 0, sizeof(pes));pes.transport = TS_TRANSPORT_PES;pes.gather.pp_last = &pes.gather.p_data;} while(0)
#define ASSERT(a) do {if(!(a)) { RESET; fprintf(stderr, "failed line %d\n", __LINE__); return 1; } } while(0)
#define PKT_FROMSZ(a, b) do {pkt = block_Alloc(sizeof(a) + b);ASSERT(pkt);memcpy(pkt->p_buffer, a, sizeof(a));for(size_t i=1; i<1+b;i++)pkt->p_buffer[sizeof(a) + i] = i % 0xFF;} while(0)
