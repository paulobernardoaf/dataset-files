...
#define DEFAULT_MEDIA_PARAMS(param_length) { .length = param_length, .audio_sample_length = VLC_TICK_FROM_MS(100), .track_count = { [VIDEO_ES] = 1, [AUDIO_ES] = 1, [SPU_ES] = 1, }, .program_count = 0, .video_packetized = true, .audio_packetized = true, .sub_packetized = true,.video_frame_rate = 25, .video_frame_rate_base = 1, .title_count = 0, .chapter_count = 0, .can_seek = true, .can_pause = true, .error = false, .null_names = false, }
struct ctx
{
...
#define VEC_PUSH(vec, item) do { bool success = vlc_vector_push(&ctx->report.vec, item); assert(success); vlc_cond_signal(&ctx->wait); } while(0)
static void
...
#define assert_position(ctx, report) do { assert(fabs((report)->pos - (report)->time / (float) ctx->params.length) < 0.001); } while (0)
static inline void
...
#define assert_state(ctx, state) do { vec_on_state_changed *vec = &ctx->report.on_state_changed; assert(VEC_LAST(vec) == state); } while(0)
#define assert_normal_state(ctx) do { vec_on_state_changed *vec = &ctx->report.on_state_changed; assert(vec->size >= 4); assert(vec->data[vec->size - 4] == VLC_PLAYER_STATE_STARTED); assert(vec->data[vec->size - 3] == VLC_PLAYER_STATE_PLAYING); assert(vec->data[vec->size - 2] == VLC_PLAYER_STATE_STOPPING); assert(vec->data[vec->size - 1] == VLC_PLAYER_STATE_STOPPED); } while(0)
static void
ctx_reset(struct ctx *ctx)
{
#define FOREACH_VEC(item, vec) vlc_vector_foreach(item, &ctx->report.vec)
#define CLEAN_MEDIA_VEC(vec) do { input_item_t *media; FOREACH_VEC(media, vec) { if (media) input_item_Release(media); } } while(0)
...
#define assert_media_name(media, name) do { assert(media); char *media_name = input_item_GetName(media); assert(media_name && strcmp(media_name, name) == 0); free(media_name); } while(0)
static void
