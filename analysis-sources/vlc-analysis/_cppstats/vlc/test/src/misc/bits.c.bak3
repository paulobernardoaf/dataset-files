...
#define test_assert(foo, bar) do {ssize_t a = (foo); ssize_t b = (bar); if(a != b) { printf("Failed (%s) line %d : %ld != %ld\n", psz_tag, __LINE__, a, b ); return 1; } } while( 0 )
...
#define bs_init(a,b,c) bs_init( a, b, c); if( callbacks ) { (a)->cb = *callbacks; if( cb_priv ) { (a)->p_priv = cb_priv; priv_init( cb_priv ); }; }
