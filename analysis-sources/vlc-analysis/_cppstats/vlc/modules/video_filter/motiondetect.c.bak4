...
#define CHECK( A ) if( p_smooth[A] && p_smooth[A] != p_smooth[i*i_pitch+j] ) { if( p_smooth[A] < p_smooth[i*i_pitch+j] ) colors[p_smooth[i*i_pitch+j]] = p_smooth[A]; else colors[p_smooth[A]] = p_smooth[i*i_pitch+j]; }
