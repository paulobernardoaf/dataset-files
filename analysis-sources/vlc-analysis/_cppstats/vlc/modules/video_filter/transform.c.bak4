...
#define PLANE(f,bits) static void Plane##bits##_##f(plane_t *restrict dst, const plane_t *restrict src) { const uint##bits##_t *src_pixels = (const void *)src->p_pixels; uint##bits##_t *restrict dst_pixels = (void *)dst->p_pixels; const unsigned src_width = src->i_pitch / sizeof (*src_pixels); const unsigned dst_width = dst->i_pitch / sizeof (*dst_pixels); const unsigned dst_visible_width = dst->i_visible_pitch / sizeof (*dst_pixels); for (int y = 0; y < dst->i_visible_lines; y++) { for (unsigned x = 0; x < dst_visible_width; x++) { int sx, sy; (f)(&sx, &sy, dst_visible_width, dst->i_visible_lines, x, y); dst_pixels[y * dst_width + x] = src_pixels[sy * src_width + sx]; } } }
...
#define I422(f) static void Plane422_##f(plane_t *restrict dst, const plane_t *restrict src) { for (int y = 0; y < dst->i_visible_lines; y += 2) { for (int x = 0; x < dst->i_visible_pitch; x++) { int sx, sy, uv; (f)(&sx, &sy, dst->i_visible_pitch, dst->i_visible_lines / 2, x, y / 2); uv = (1 + src->p_pixels[2 * sy * src->i_pitch + sx] + src->p_pixels[(2 * sy + 1) * src->i_pitch + sx]) / 2; dst->p_pixels[y * dst->i_pitch + x] = uv; dst->p_pixels[(y + 1) * dst->i_pitch + x] = uv; } } }
...
#define YUY2(f) static void PlaneYUY2_##f(plane_t *restrict dst, const plane_t *restrict src) { unsigned dst_visible_width = dst->i_visible_pitch / 2; for (int y = 0; y < dst->i_visible_lines; y += 2) { for (unsigned x = 0; x < dst_visible_width; x+= 2) { int sx0, sy0, sx1, sy1; (f)(&sx0, &sy0, dst_visible_width, dst->i_visible_lines, x, y); (f)(&sx1, &sy1, dst_visible_width, dst->i_visible_lines, x + 1, y + 1); dst->p_pixels[(y + 0) * dst->i_pitch + 2 * (x + 0)] = src->p_pixels[sy0 * src->i_pitch + 2 * sx0]; dst->p_pixels[(y + 0) * dst->i_pitch + 2 * (x + 1)] = src->p_pixels[sy1 * src->i_pitch + 2 * sx0]; dst->p_pixels[(y + 1) * dst->i_pitch + 2 * (x + 0)] = src->p_pixels[sy0 * src->i_pitch + 2 * sx1]; dst->p_pixels[(y + 1) * dst->i_pitch + 2 * (x + 1)] = src->p_pixels[sy1 * src->i_pitch + 2 * sx1]; int sx, sy, u, v; (f)(&sx, &sy, dst_visible_width / 2, dst->i_visible_lines / 2, x / 2, y / 2); u = (1 + src->p_pixels[2 * sy * src->i_pitch + 4 * sx + 1] + src->p_pixels[(2 * sy + 1) * src->i_pitch + 4 * sx + 1]) / 2; v = (1 + src->p_pixels[2 * sy * src->i_pitch + 4 * sx + 3] + src->p_pixels[(2 * sy + 1) * src->i_pitch + 4 * sx + 3]) / 2; dst->p_pixels[(y + 0) * dst->i_pitch + 2 * x + 1] = u; dst->p_pixels[(y + 0) * dst->i_pitch + 2 * x + 3] = v; dst->p_pixels[(y + 1) * dst->i_pitch + 2 * x + 1] = u; dst->p_pixels[(y + 1) * dst->i_pitch + 2 * x + 3] = v; } } }
...
#undef PLANES 
#define PLANES(f) PLANE(f,8) PLANE(f,16) PLANE(f,32)
...
#define DESC(str, f, invf, op) { str, f, invf, op, Plane8_##f, Plane16_##f, Plane32_##f, Plane422_##f, PlaneYUY2_##f }
