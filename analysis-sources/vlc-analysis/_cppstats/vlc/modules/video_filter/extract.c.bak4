...
#define GET_CUSTOM_PIX() do { val = (*y_in[0] * m[3] + (**u_in - u) * m[4] + (**v_in - v) * m[5]) / 65536 + u; *(*u_out)++ = VLC_CLIP( val, 0, maxval ); val = (*y_in[0] * m[6] + (**u_in - u) * m[7] + (**v_in - v) * m[8]) / 65536 + v; *(*v_out)++ = VLC_CLIP( val, 0, maxval ); val = (*y_in[0]++ * m[0] + (**u_in - u) * m[1] + (**v_in - v) * m[2]) / 65536; *y_out[0]++ = VLC_CLIP( val, 0, maxval ); val = (*y_in[0]++ * m[0] + (**u_in - u) * m[1] + (**v_in - v) * m[2]) / 65536; *y_out[0]++ = VLC_CLIP( val, 0, maxval ); val = (*y_in[1]++ * m[0] + (**u_in - u) * m[1] + (**v_in - v) * m[2]) / 65536; *y_out[1]++ = VLC_CLIP( val, 0, maxval ); val = (*y_in[1]++ * m[0] + (*(*u_in)++ - u) * m[1] + (*(*v_in)++ - v) * m[2]) / 65536; *y_out[1]++ = VLC_CLIP( val, 0, maxval ); } while (0);
