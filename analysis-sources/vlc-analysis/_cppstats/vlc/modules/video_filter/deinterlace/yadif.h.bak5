...
#define CHECK(j)score = FFABS(cur[mrefs-1+(j)] - cur[prefs-1-(j)])+ FFABS(cur[mrefs +(j)] - cur[prefs -(j)])+ FFABS(cur[mrefs+1+(j)] - cur[prefs+1-(j)]);if (score < spatial_score) {spatial_score= score;spatial_pred= (cur[mrefs +(j)] + cur[prefs -(j)])>>1;
