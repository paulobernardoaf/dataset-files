...
#define BLUESCREEN_HELP N_( "This effect, also known as \"greenscreen\" or \"chroma key\" blends " "the \"blue parts\" of the foreground image of the mosaic on the " "background (like weather forecasts). You can choose the \"key\" " "color for blending (blue by default)." )
#define BLUESCREENU_TEXT N_("Bluescreen U value")
#define BLUESCREENU_LONGTEXT N_( "\"U\" value for the bluescreen key color " "(in YUV values). From 0 to 255. Defaults to 120 for blue." )
#define BLUESCREENV_TEXT N_("Bluescreen V value")
#define BLUESCREENV_LONGTEXT N_( "\"V\" value for the bluescreen key color " "(in YUV values). From 0 to 255. Defaults to 90 for blue." )
#define BLUESCREENUTOL_TEXT N_("Bluescreen U tolerance")
#define BLUESCREENUTOL_LONGTEXT N_( "Tolerance of the bluescreen blender " "on color variations for the U plane. A value between 10 and 20 " "seems sensible." )
#define BLUESCREENVTOL_TEXT N_("Bluescreen V tolerance")
#define BLUESCREENVTOL_LONGTEXT N_( "Tolerance of the bluescreen blender " "on color variations for the V plane. A value between 10 and 20 " "seems sensible." )
...
#define GET_VAR( name, min, max ) val = var_CreateGetIntegerCommand( p_filter, CFG_PREFIX #name ); p_sys->i_##name = VLC_CLIP( val, min, max ); var_AddCallback( p_filter, CFG_PREFIX #name, BluescreenCallback, p_sys );
GET_VAR( u, 0x00, 0xff );
GET_VAR( v, 0x00, 0xff );
