...
#define FASTSTART_LONGTEXT N_("Create \"Fast Start\" files. " "\"Fast Start\" files are optimized for downloads and allow the user " "to start previewing the file while it is downloading.")
...
#define ENQUEUE_ENTRY(object, entry) do {if (object.p_last)object.p_last->p_next = entry;object.p_last = entry;if (!object.p_first)object.p_first = entry;} while(0)
#define DEQUEUE_ENTRY(object, entry) do {entry = object.p_first;if (object.p_last == entry)object.p_last = NULL;object.p_first = object.p_first->p_next;entry->p_next = NULL;} while(0)
...
assert(moof->b->i_flags & BLOCK_FLAG_TYPE_I); 
box_send(p_mux, moof);
