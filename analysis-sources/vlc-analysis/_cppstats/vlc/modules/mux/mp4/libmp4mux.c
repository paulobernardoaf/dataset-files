...
#define ATAN(a, b) do { mvhd_matrix[1] = ((uint32_t)(a)) << 16; mvhd_matrix[0] = ((uint32_t)(b)) << 16; } while(0)
switch (orientation) {
...
break;
case VLC_CODEC_SPU:
i_object_profile_indication = 0xe0; 
break;
default:
...
if (p_meta) {
#define ADD_META_BOX(type, box_string) { bo_t *box = NULL; if (vlc_meta_Get(p_meta, vlc_meta_##type)) box = box_new("\251" box_string); if (box) { bo_add_16be(box, strlen(vlc_meta_Get(p_meta, vlc_meta_##type))); bo_add_16be(box, 0); bo_add_mem(box, strlen(vlc_meta_Get(p_meta, vlc_meta_##type)), (uint8_t*)(vlc_meta_Get(p_meta, vlc_meta_##type))); box_gather(udta, box); } }
...
bo_add_32be(mvhd, i_timestamp); 
bo_add_32be(mvhd, i_timestamp); 
bo_add_32be(mvhd, i_movie_timescale); 
bo_add_32be(mvhd, i_movie_duration); 
} else {
...
bo_add_64be(mvhd, i_timestamp); 
bo_add_64be(mvhd, i_timestamp); 
bo_add_32be(mvhd, i_movie_timescale); 
bo_add_64be(mvhd, i_movie_duration); 
}
bo_add_32be(mvhd, 0x10000); 
bo_add_16be(mvhd, 0x100); 
bo_add_16be(mvhd, 0); 
for (int i = 0; i < 2; i++)
bo_add_32be(mvhd, 0); 
uint32_t mvhd_matrix[9] = { 0x10000, 0, 0, 0, 0x10000, 0, 0, 0, 0x40000000 };
for (int i = 0; i < 9; i++)
bo_add_32be(mvhd, mvhd_matrix[i]);
for (int i = 0; i < 6; i++)
bo_add_32be(mvhd, 0); 
...
bo_add_32be(mvhd, i_next_track_id); 
...
bo_add_32be(tkhd, i_timestamp); 
bo_add_32be(tkhd, i_timestamp); 
bo_add_32be(tkhd, p_stream->i_track_id);
bo_add_32be(tkhd, 0); 
bo_add_32be(tkhd, i_stream_duration); 
} else {
if (h->options & QUICKTIME)
...
bo_add_64be(tkhd, i_timestamp); 
bo_add_64be(tkhd, i_timestamp); 
bo_add_32be(tkhd, p_stream->i_track_id);
bo_add_32be(tkhd, 0); 
bo_add_64be(tkhd, i_stream_duration); 
}
for (int i = 0; i < 2; i++)
bo_add_32be(tkhd, 0); 
bo_add_16be(tkhd, 0); 
...
for (int i = 0; i < 9; i++)
bo_add_32be(tkhd, mvhd_matrix[i]); 
...
bo_add_32be(tkhd, i_width << 16); 
bo_add_32be(tkhd, i_height << 16); 
}
...
bo_add_32be(mdhd, i_timestamp); 
bo_add_32be(mdhd, i_timestamp); 
bo_add_32be(mdhd, p_stream->i_timescale); 
bo_add_32be(mdhd, i_stream_duration * p_stream->i_timescale / i_movie_timescale); 
} else {
...
bo_add_64be(mdhd, i_timestamp); 
bo_add_64be(mdhd, i_timestamp); 
bo_add_32be(mdhd, p_stream->i_timescale); 
bo_add_64be(mdhd, i_stream_duration * p_stream->i_timescale / i_movie_timescale); 
}
...
bo_add_32be(trex, 1); 
bo_add_32be(trex, samples_from_vlc_tick(p_stream->i_trex_default_length, p_stream->i_timescale)); 
bo_add_32be(trex, p_stream->i_trex_default_size); 
bo_add_32be(trex, 0); 
