...
#define AVI_BOX_ENTER( fcc ) int i_datasize_offset; bo_add_fourcc( p_bo, fcc ); i_datasize_offset = p_bo->b->i_buffer; bo_add_32le( p_bo, 0 )
#define AVI_BOX_ENTER_LIST( fcc ) AVI_BOX_ENTER( "LIST" ); bo_add_fourcc( p_bo, fcc )
#define AVI_BOX_EXIT( i_err ) if( p_bo->b->i_buffer&0x01 ) bo_add_8( p_bo, 0 ); bo_set_32le( p_bo, i_datasize_offset, p_bo->b->i_buffer - i_datasize_offset - 4 ); return( i_err );
...
{
char *psz;
#define APPLY_META(var, fourcc) psz = var_InheritString( p_mux, SOUT_CFG_PREFIX var );if ( psz ){avi_HeaderAdd_meta( p_bo, fourcc, psz );free( psz );}
