...
#define INDEXINTVL_LONGTEXT N_("Minimal index interval, in milliseconds. " "Set to 0 to disable index creation.")

#define INDEXRATIO_TEXT N_("Index size ratio")
#define INDEXRATIO_LONGTEXT N_("Set index size ratio. Alters default (60min content) or estimated size." )
...
uint64_t i_index_payload; 
uint64_t i_index_count;

uint64_t i_index_offset; 
int64_t i_index_packetno; 
int i_index_pageno;
...
flac_streaminfo[0] = 0x80; 
flac_streaminfo[1] = 0;
flac_streaminfo[2] = 0;
flac_streaminfo[3] = 34; 
op.packet = flac_streaminfo;
} else {
...
p_data->p_buffer[0] = PACKET_IS_SYNCPOINT; 
}
...
#define FRAME_ROUND(a) if ( ( a + 5000 / CLOCK_FREQ ) > ( a / CLOCK_FREQ ) )a += 5000;a /= CLOCK_FREQ;
