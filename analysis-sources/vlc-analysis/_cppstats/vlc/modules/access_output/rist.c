...
#define CACHING_LONGTEXT N_( "Having this cache will guarantee that the packets going out are " "delivered at a spacing determined by the chain timestamps thus ensuring " "a near jitter free output. Be aware that this setting will also add to " "the overall latency of the stream." )
#define BUFFER_TEXT N_("RIST retry-buffer queue size (ms)")
#define BUFFER_LONGTEXT N_( "This must match the buffer size (latency) configured on the server side. If you " "are not sure, leave the default of 0 which will set it the maximum " "value and will use about 100MB of RAM" )
#define SSRC_TEXT N_("SSRC used in RTP output (default is random, i.e. 0)")
#define SSRC_LONGTEXT N_( "Use this setting to specify a known SSRC for the RTP header. This is only useful " "if your receiver acts on it. When using VLC as receiver, it is not." )
#define NAME_TEXT N_("Stream name")
#define NAME_LONGTEXT N_( "This Stream name will be sent to the receiver using the rist RTCP channel" )
vlc_module_begin()
