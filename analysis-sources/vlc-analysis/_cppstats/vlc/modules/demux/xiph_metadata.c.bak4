...
if( size < 8 )
return NULL;
#define RM(x) do { assert(size >= (x)); size -= (x); p_data += (x); } while (0)
...
#define TRY_EXTRACT_CUEMETA(var, string, quoted) if( !(*pi_flags & XIPHMETA_##var) &&( psz_string = xiph_ExtractCueSheetMeta( psz_line, string, sizeof(string) - 1, quoted ) ) ){vlc_meta_Set( p_meta, vlc_meta_##var, psz_string );free( psz_string );*pi_flags |= XIPHMETA_##var;}
...
#define IF_EXTRACT(txt,var) if( !strncasecmp(psz_comment, txt, strlen(txt)) ) { const char *oldval = vlc_meta_Get( p_meta, vlc_meta_ ##var ); if( oldval && (hasMetaFlags & XIPHMETA_##var)) { char * newval; if( asprintf( &newval, "%s,%s", oldval, &psz_comment[strlen(txt)] ) == -1 ) newval = NULL; vlc_meta_Set( p_meta, vlc_meta_ ##var, newval ); free( newval ); } else vlc_meta_Set( p_meta, vlc_meta_ ##var, &psz_comment[strlen(txt)] ); hasMetaFlags |= XIPHMETA_##var; }
...
#define IF_EXTRACT_ONCE(txt,var) if( !strncasecmp(psz_comment, txt, strlen(txt)) && !(hasMetaFlags & XIPHMETA_##var) ) { vlc_meta_Set( p_meta, vlc_meta_ ##var, &psz_comment[strlen(txt)] ); hasMetaFlags |= XIPHMETA_##var; }
...
#define IF_EXTRACT_FMT(txt,var,fmt,target) if( !strncasecmp(psz_comment, txt, strlen(txt)) ) { IF_EXTRACT(txt,var)if( fmt ){free( fmt->target );fmt->target = strdup(&psz_comment[strlen(txt)]);}}
