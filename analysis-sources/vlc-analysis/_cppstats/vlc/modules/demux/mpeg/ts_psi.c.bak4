...
{
case 0x1f: 
while( i_length >= 2 && !p_es->i_sl_es_id )
{
...
if( p_dvbpsies->i_type == 0x12 ) 
{
if( !p_pes->p_proc )
p_pes->p_proc = SL_stream_processor_New( p_pes );
}
else if( p_dvbpsies->i_type == 0x13 ) 
{
...
int i_extra = 0;
opus_write_header(&p_extra, &i_extra, &h, NULL );
if (p_extra) {
...
SetPIDFilter( p_sys, GetPID(p_sys, p_pmt->i_pid_pcr), true ); 
...
{
assert(ts_Count_es(pespid->u.p_stream->p_es, false, NULL)); 
ts_es_t *p_new = ts_stream_Extract_es( p_pes, p_pmt );
assert( p_new );
ts_stream_Add_es( pespid->u.p_stream, p_new, false );
ts_stream_Del( p_demux, p_pes );
...
if ( PIDSetup( p_demux, TYPE_SI, p_sdt_pid, pmtpid ) ) 
{
