...
atsc_a65_handle_t *p_a65; 
uint16_t i_tabletype; 
DECL_ARRAY(dvbpsi_atsc_ett_t *) etts; 
DECL_ARRAY(dvbpsi_atsc_eit_t *) eits; 
};
...
#define ATSC_ATTACH( handle, type, table, extension, pid ) ( ATSC_Ready_SubDecoders( handle, pid ) &&( dvbpsi_demuxGetSubDec( (dvbpsi_demux_t *) handle->p_decoder, table, extension ) ||dvbpsi_atsc_Attach ##type( handle, table, extension, ATSC_ ##type ##_Callback, pid ) ) )
#define ATSC_ATTACH_WITH_FIXED_DECODER( handle, type, table, extension, pid ) ( ATSC_Ready_SubDecoders( handle, pid ) &&( dvbpsi_demuxGetSubDec( (dvbpsi_demux_t *) handle->p_decoder, table, extension ) ||ts_dvbpsi_AttachRawSubDecoder( handle, table, extension, ATSC_ ##type ##_RawCallback, pid ) ) )
...
#if !defined(ATSC_DEBUG_EIT)
#define EIT_DEBUG_TIMESHIFT(t)
#else
static time_t i_eit_debug_offset = 0;
#define EIT_DEBUG_TIMESHIFT(t) do {if( i_eit_debug_offset == 0 )i_eit_debug_offset = time(NULL) - t;t = t + i_eit_debug_offset;} while(0);
#endif
