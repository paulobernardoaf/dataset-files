...
#endif
#define MP4_GETX_PRIVATE(dst, code, size) do { if( (i_read) >= (size) ) { dst = (code); p_peek += (size); i_read -= (size); } else { dst = 0; i_read = 0; } } while(0)
...
#define MP4_GETVERSIONFLAGS( p_void ) MP4_GET1BYTE( p_void->i_version ); MP4_GET3BYTES( p_void->i_flags )
...
#define MP4_GETSTRINGZ( p_str ) do (p_str) = mp4_getstringz( &p_peek, &i_read ); while(0)
...
#define MP4_READBOX_ENTER_PARTIAL( MP4_Box_data_TYPE_t, maxread, release ) uint64_t i_read = (maxread); uint8_t *p_buff = mp4_readbox_enter_partial( p_stream, p_box, sizeof( MP4_Box_data_TYPE_t ), release, &i_read ); if( unlikely(p_buff == NULL) ) return 0; const size_t header_size = mp4_box_headersize( p_box ); uint8_t *p_peek = p_buff + header_size; i_read -= header_size
#define MP4_READBOX_ENTER( MP4_Box_data_TYPE_t, release ) uint8_t *p_buff = mp4_readbox_enter( p_stream, p_box, sizeof(MP4_Box_data_TYPE_t), release ); if( unlikely(p_buff == NULL) ) return 0; uint64_t i_read = p_box->i_size; const size_t header_size = mp4_box_headersize( p_box ); uint8_t *p_peek = p_buff + header_size; i_read -= header_size
#define MP4_READBOX_EXIT( i_code ) do { free( p_buff ); return( i_code ); } while (0)
...
{
MP4_Seek( p_stream, i_next - 1 ); 
MP4_Seek( p_stream, i_next );
if( vlc_stream_Tell( p_stream ) < i_next - 1 ) 
{
...
if( i_version > 1 )
MP4_READBOX_EXIT( 0 );
#define READ_CSLG(readbytes) {readbytes( p_box->data.p_cslg->ct_to_dts_shift );readbytes( p_box->data.p_cslg->i_least_delta );readbytes( p_box->data.p_cslg->i_max_delta );readbytes( p_box->data.p_cslg->i_composition_starttime );readbytes( p_box->data.p_cslg->i_composition_endtime ); }
if( i_version == 0 )
...
p_box->data.p_sample_soun->i_sampleratehi != 0x1 ||
p_box->data.p_sample_soun->i_sampleratelo != 0x0 ) 
{
msg_Err( p_stream, "invalid stsd V2 box defaults" );
MP4_READBOX_EXIT( 0 );
}
MP4_GET4BYTES( i_extoffset ); 
...
{
#define READ_VARIABLE_LENGTH(lengthvar, p_array) switch (lengthvar){case 0:MP4_GET1BYTE( p_array[i] );break;case 1:MP4_GET2BYTES( *((uint16_t *)&p_array[i*2]) );break;case 2:MP4_GET3BYTES( *((uint32_t *)&p_array[i*4]) );break;case 3:MP4_GET4BYTES( *((uint32_t *)&p_array[i*4]) );break;default:goto error;}
...
{ ATOM_wave, MP4_ReadBoxContainer, ATOM_mp4a }, 
{ ATOM_wave, MP4_ReadBoxContainer, ATOM_WMA2 }, 
...
{ ATOM_wave, MP4_ReadBoxContainer, ATOM_XiFL }, 
{ ATOM_wave, MP4_ReadBoxContainer, ATOM_XiVs }, 
...
{ ATOM_esds, MP4_ReadBox_esds, ATOM_wave }, 
...
{ ATOM_jpeC, MP4_ReadBox_Binary, 0 }, 
{ ATOM_av1C, MP4_ReadBox_av1C, ATOM_ipco }, 
{ ATOM_vpcC, MP4_ReadBox_vpcC, ATOM_vp08 },
{ ATOM_vpcC, MP4_ReadBox_vpcC, ATOM_vp09 },
{ ATOM_vpcC, MP4_ReadBox_vpcC, ATOM_vp10 },
{ ATOM_SmDm, MP4_ReadBox_SmDm, ATOM_vpcC }, 
{ ATOM_mdcv, MP4_ReadBox_SmDm, 0 }, 
{ ATOM_CoLL, MP4_ReadBox_CoLL, ATOM_vpcC }, 
...
{ ATOM_WMA2, MP4_ReadBox_sample_soun, ATOM_stsd }, 
{ ATOM_wma, MP4_ReadBox_sample_soun, ATOM_stsd }, 
{ ATOM_Opus, MP4_ReadBox_sample_soun, ATOM_stsd },
{ ATOM_chan, MP4_ReadBox_stsdext_chan, 0 },
{ ATOM_WMA2, MP4_ReadBox_WMA2, ATOM_wave }, 
{ ATOM_dOps, MP4_ReadBox_Binary, ATOM_Opus },
{ ATOM_wfex, MP4_ReadBox_WMA2, ATOM_wma }, 
...
{ ATOM_mjqt, MP4_ReadBox_default, 0 }, 
...
{ ATOM_strf, MP4_ReadBox_strf, ATOM_WVC1 }, 
{ ATOM_strf, MP4_ReadBox_strf, ATOM_H264 }, 
{ ATOM_strf, MP4_ReadBox_strf, ATOM_WMV3 }, 
{ ATOM_ASF , MP4_ReadBox_ASF, ATOM_WMV3 }, 
{ ATOM_ASF , MP4_ReadBox_ASF, ATOM_wave }, 
...
{ ATOM_frma, MP4_ReadBox_frma, ATOM_sinf }, 
{ ATOM_frma, MP4_ReadBox_frma, ATOM_wave }, 
...
{ ATOM_atID, MP4_ReadBox_Metadata, ATOM_ilst }, 
{ ATOM_cnID, MP4_ReadBox_Metadata, ATOM_ilst }, 
...
{ ATOM_gshh, MP4_ReadBox_Metadata, ATOM_ilst }, 
...
{ ATOM_ITUN, MP4_ReadBox_Metadata, ATOM_ilst }, 
...
{ ATOM_chpl, MP4_ReadBox_chpl, ATOM_udta }, 
...
{ ATOM_HMMT, MP4_ReadBox_HMMT, ATOM_udta }, 
{ ATOM_tsel, MP4_ReadBox_tsel, ATOM_udta },
{ ATOM_meta, MP4_ReadBox_meta, 0 },
{ ATOM_ID32, MP4_ReadBox_Binary, ATOM_meta }, 
