...
set_callback(Open)
vlc_module_end ()
#define DEFINE_PACK(pack, pict) struct yuv_pack pack = { (pict)->Y_PIXELS, (pict)->Y_PITCH }
#define DEFINE_PLANES(planes, pict) struct yuv_planes planes = { (pict)->Y_PIXELS, (pict)->U_PIXELS, (pict)->V_PIXELS, (pict)->Y_PITCH }
#define DEFINE_PLANES_SWAP(planes, pict) struct yuv_planes planes = { (pict)->Y_PIXELS, (pict)->V_PIXELS, (pict)->U_PIXELS, (pict)->Y_PITCH }
#define DEFINE_UV_PLANES(planes, pict) struct uv_planes planes = { (pict)->U_PIXELS, (pict)->V_PIXELS, (pict)->U_PITCH }
#define DEFINE_UV_PLANES_SWAP(planes, pict) struct uv_planes planes = { (pict)->V_PIXELS, (pict)->U_PIXELS, (pict)->U_PITCH }
#define DEFINE_UV_PACK(pack, pict) struct yuv_pack pack = { (pict)->U_PIXELS, (pict)->U_PITCH }
...
#define SEMIPLANAR_FILTERS(name, h_subsamp, v_subsamp) static void name (filter_t *filter, picture_t *src, picture_t *dst) { DEFINE_UV_PLANES(out, dst); DEFINE_UV_PACK(in, src); copy_y_plane (filter, src, dst); deinterleave_chroma_neon (&out, &in, filter->fmt_in.video.i_width / h_subsamp, filter->fmt_in.video.i_height / v_subsamp); } VIDEO_FILTER_WRAPPER (name) 
#define SEMIPLANAR_FILTERS_SWAP(name, h_subsamp, v_subsamp) static void name (filter_t *filter, picture_t *src, picture_t *dst) { DEFINE_UV_PLANES_SWAP(out, dst); DEFINE_UV_PACK(in, src); copy_y_plane (filter, src, dst); deinterleave_chroma_neon (&out, &in, filter->fmt_in.video.i_width / h_subsamp, filter->fmt_in.video.i_height / v_subsamp); } VIDEO_FILTER_WRAPPER (name) 
