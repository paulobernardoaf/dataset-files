...
#define HOST_LONGTEXT N_("Accept commands over a socket rather than stdin. " "You can set the address and port the interface will bind to." )
#if defined(_WIN32)
#define QUIET_TEXT N_("Do not open a DOS command box interface")
#define QUIET_LONGTEXT N_( "By default the rc interface plugin will start a DOS command box. " "Enabling the quiet mode will not bring this command box but can also " "be pretty annoying when you want to stop VLC and no video window is " "open." )
#endif
vlc_module_begin()
set_shortname(N_("RC"))
