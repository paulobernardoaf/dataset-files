...
#define INTF_ACTION_HANDLER(name) static inline void action_handler_Intf##name(intf_thread_t *intf, vlc_action_id_t action_id)
INTF_ACTION_HANDLER()
{
char const *varname;
switch (action_id)
...
#define PLAYLIST_ACTION_HANDLER(name) static inline void action_handler_Playlist##name(intf_thread_t *intf, vlc_playlist_t *playlist, vlc_action_id_t action_id)
...
#define PLAYER_ACTION_HANDLER(name) static inline void action_handler_Player##name(intf_thread_t *intf, vlc_player_t *player, vlc_action_id_t action_id)
...
switch (action_id)
{
#define PLAYER_NAV_FROM_ACTION(navval) case ACTIONID_NAV_##navval: nav = VLC_PLAYER_NAV_##navval; break;
...
#define VOUT_ACTION_HANDLER(name) static inline void action_handler_Vout##name(intf_thread_t *intf, vout_thread_t *vout, vlc_action_id_t action_id)
static inline void
...
#define vout_CycleVariable(vout, varname, vartype, next) do { static_assert(vartype == VLC_VAR_FLOAT || vartype == VLC_VAR_STRING, "vartype must be either VLC_VAR_FLOAT or VLC_VAR_STRING"); vout_CycleVariable(vout, varname, vartype, next); } while (0)
...
switch (action_id)
{
#define CASE_CROP(crop, var) case ACTIONID_CROP_##crop: case ACTIONID_UNCROP_##crop: varname = "crop-"#var; delta = action_id == ACTIONID_CROP_##crop? +1 : -1; break;
CASE_CROP(TOP, top)
...
{
#define VLC_ACTION(typeval, first, last, name, lock) { .type = typeval, .range = { ACTIONID_##first, ACTIONID_##last }, .handler.fptr = action_handler_##name, .pl_need_lock = lock },
#define VLC_ACTION_INTF(first, last, name, lock) VLC_ACTION(INTF_ACTION, first, last, Intf ##name, lock)
#define VLC_ACTION_PLAYLIST(first, last, name) VLC_ACTION(PLAYLIST_ACTION, first, last, Playlist ##name, true)
#define VLC_ACTION_PLAYER(first, last, name, lock) VLC_ACTION(PLAYER_ACTION, first, last, Player ##name, lock)
#define VLC_ACTION_VOUT(first, last, name) VLC_ACTION(VOUT_ACTION, first, last, Vout ##name, false)
