...
#define PROPERTY_GET_FUNC( prop, signature ) else if( !strcmp( psz_property_name, #prop ) ) { if( !dbus_message_iter_open_container( &args, DBUS_TYPE_VARIANT, signature, &v ) ) return DBUS_HANDLER_RESULT_NEED_MEMORY; if( VLC_SUCCESS != Marshal##prop( p_this, &v ) ) { dbus_message_iter_abandon_container( &args, &v ); return DBUS_HANDLER_RESULT_NEED_MEMORY; } if( !dbus_message_iter_close_container( &args, &v ) ) return DBUS_HANDLER_RESULT_NEED_MEMORY; }
#define PROPERTY_SET_FUNC( prop ) else if( !strcmp( psz_property_name, #prop ) ) { return prop##Set( p_conn, p_from, p_this ); }
