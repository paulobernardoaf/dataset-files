...
#define TELNETHOST_LONGTEXT N_( "This is the host on which the " "interface will listen. It defaults to all network interfaces (0.0.0.0)." " If you want this interface to be available only on the local " "machine, enter \"127.0.0.1\"." )
#define TELNETPORT_TEXT N_( "Port" )
#define TELNETPORT_LONGTEXT N_( "This is the TCP port on which this " "interface will listen. It defaults to 4212." )
#define TELNETPWD_TEXT N_( "Password" )
#define TELNETPWD_LONGTEXT N_( "A single password restricts access " "to this interface." )
...
{
#define TRY_META( a, b ) lua_getfield( L, -1, a ); if( lua_isstring( L, -1 ) && strcmp( lua_tostring( L, -1 ), "" ) ) { char *psz_value = strdup( lua_tostring( L, -1 ) ); EnsureUTF8( psz_value ); msg_Dbg( p_this, #b ": %s", psz_value ); input_item_Set ##b ( p_input, psz_value ); free( psz_value ); } lua_pop( L, 1 ); 
