...
free( psz_uri );
#define PUSH_META( n, m ) psz_meta = vlc_meta_Get( p_item->p_meta, vlc_meta_ ##n ); lua_pushstring( L, psz_meta ); lua_setfield( L, -2, m )
...
if( p_stats != NULL )
{
#define STATS_INT( n ) lua_pushinteger( L, p_item->p_stats->i_ ##n ); lua_setfield( L, -2, #n );
#define STATS_FLOAT( n ) lua_pushnumber( L, p_item->p_stats->f_ ##n ); lua_setfield( L, -2, #n );
