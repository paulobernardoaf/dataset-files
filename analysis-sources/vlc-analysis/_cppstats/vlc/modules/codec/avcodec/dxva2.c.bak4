...
#define DXVA2_PICCONTEXT_FROM_PICCTX(pic_ctx) container_of((pic_ctx), struct dxva2_pic_context, ctx.s)
...
return VLC_EGENERIC;
assert(d3d9_decoder->d3ddev.dev != NULL); 
...
preferredOutput = MAKEFOURCC('Y','4','1','0'); 
else if (mode->log2_chroma_w == 1 && mode->log2_chroma_h == 0)
preferredOutput = MAKEFOURCC('Y','2','1','0'); 
else
preferredOutput = MAKEFOURCC('P','0','1','0');
}
else if (mode->log2_chroma_w == 0 && mode->log2_chroma_h == 0)
preferredOutput = MAKEFOURCC('A','Y','U','V'); 
else if (mode->log2_chroma_w == 1 && mode->log2_chroma_h == 0)
preferredOutput = MAKEFOURCC('Y','U','Y','2'); 
