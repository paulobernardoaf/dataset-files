...
#define SW_IMPL_LONGTEXT N_("Allow the use of the Intel Media SDK software " "implementation of the codecs if no QuickSync Video hardware " "acceleration is present on the system.")



#define PROFILE_TEXT N_("Codec Profile")
#define PROFILE_LONGTEXT N_( "Specify the codec profile explicitly. If you don't, the codec will " "determine the correct profile from other sources, such as resolution " "and bitrate. E.g. 'high'")




#define LEVEL_TEXT N_("Codec Level")
#define LEVEL_LONGTEXT N_( "Specify the codec level explicitly. If you don't, the codec will " "determine the correct profile from other sources, such as resolution " "and bitrate. E.g. '4.2' for mpeg4-part10 or 'low' for mpeg2")




#define GOP_SIZE_TEXT N_("Group of Picture size")
#define GOP_SIZE_LONGTEXT N_( "Number of pictures within the current GOP (Group of Pictures); if " "GopPicSize=0, then the GOP size is unspecified. If GopPicSize=1, " "only I-frames are used.")




#define GOP_REF_DIST_TEXT N_("Group of Picture Reference Distance")
#define GOP_REF_DIST_LONGTEXT N_( "Distance between I- or P- key frames; if it is zero, the GOP " "structure is unspecified. Note: If GopRefDist = 1, there are no B-" "frames used.")




#define TARGET_USAGE_TEXT N_("Target Usage")
#define TARGET_USAGE_LONGTEXT N_("The target usage allow to choose between " "different trade-offs between quality and speed. Allowed values are: " "'speed', 'balanced' and 'quality'.")



#define IDR_INTERVAL_TEXT N_("IDR interval")
#define IDR_INTERVAL_LONGTEXT N_( "For H.264, IdrInterval specifies IDR-frame interval in terms of I-" "frames; if IdrInterval=0, then every I-frame is an IDR-frame. If " "IdrInterval=1, then every other I-frame is an IDR-frame, etc. " "For MPEG2, IdrInterval defines sequence header interval in terms " "of I-frames. If IdrInterval=N, SDK inserts the sequence header " "before every Nth I-frame. If IdrInterval=0 (default), SDK inserts " "the sequence header once at the beginning of the stream.")
...
#define RATE_CONTROL_LONGTEXT N_( "The rate control method to use when encoding. Can be one of " "'cbr', 'vbr', 'qp', 'avbr'. 'qp' mode isn't supported for mpeg2.")



#define QP_TEXT N_("Quantization parameter")
#define QP_LONGTEXT N_("Quantization parameter for all types of frames. " "This parameters sets qpi, qpp and qpb. It has less precedence than " "the forementionned parameters. Used only if rc_method is 'qp'.")



#define QPI_TEXT N_("Quantization parameter for I-frames")
#define QPI_LONGTEXT N_("Quantization parameter for I-frames. This parameter " "overrides any qp set globally. Used only if rc_method is 'qp'.")


#define QPP_TEXT N_("Quantization parameter for P-frames")
#define QPP_LONGTEXT N_("Quantization parameter for P-frames. This parameter " "overrides any qp set globally. Used only if rc_method is 'qp'.")


#define QPB_TEXT N_("Quantization parameter for B-frames")
#define QPB_LONGTEXT N_("Quantization parameter for B-frames. This parameter " "overrides any qp set globally. Used only if rc_method is 'qp'.")


#define MAX_BITRATE_TEXT N_("Maximum Bitrate")
#define MAX_BITRATE_LONGTEXT N_("Defines the maximum bitrate in kbps " "(1000 bits/s) for VBR rate control method. If not set, this parameter" " is computed from other sources such as bitrate, profile, level, etc.")



#define ACCURACY_TEXT N_("Accuracy of RateControl")
#define ACCURACY_LONGTEXT N_("Tolerance in percentage of the 'avbr' " " (Average Variable BitRate) method. (e.g. 10 with a bitrate of 800 " " kbps means the encoder tries not to go above 880 kbps and under " " 730 kbps. The targeted accuracy is only reached after a certained " " convergence period. See the convergence parameter")
...
#define CONVERGENCE_LONGTEXT N_("Number of 100 frames before the " "'avbr' rate control method reaches the requested bitrate with " "the requested accuracy. See the accuracy parameter.")



#define NUM_SLICE_TEXT N_("Number of slices per frame")
#define NUM_SLICE_LONGTEXT N_("Number of slices in each video frame; ""each slice contains one or more macro-block rows. If numslice is " "not set, the encoder may choose any slice partitioning allowed " "by the codec standard.")
...
#define ASYNC_DEPTH_LONGTEXT N_("Defines the number of parallel " "encoding operations before we synchronise the result. Higher " "numbers may result on better throughput depending on hardware. " "MPEG2 needs at least 1 here.")
...
uint64_t dts_warn_counter; 
uint64_t busy_warn_counter; 
uint64_t async_depth; 
fifo_t packets; 
vlc_tick_t offset_pts; 
vlc_tick_t last_dts; 

picture_pool_t *input_pool; 
...
sys->params.mfx.FrameInfo.BitDepthChroma = 8; 
sys->params.mfx.FrameInfo.BitDepthLuma = 8; 
...
enc->fmt_in.i_codec = VLC_CODEC_NV12; 
