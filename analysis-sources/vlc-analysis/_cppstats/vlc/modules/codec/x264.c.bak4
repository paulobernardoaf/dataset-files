...
#include <pthread.h>
#endif
#if defined(MODULE_NAME_IS_x262)
#include <x262.h>
#else
#include <x264.h>
#endif
...
#define KEYINT_LONGTEXT N_( "Sets maximum interval between IDR-frames. " "Larger values save bits, thus improving quality for a given bitrate at " "the cost of seeking precision. Use -1 for infinite." )



#define MIN_KEYINT_TEXT N_("Minimum GOP size")
#define MIN_KEYINT_LONGTEXT N_( "Sets minimum interval between IDR-frames. " "In H.264, I-frames do not necessarily bound a closed GOP because it is " "allowable for a P-frame to be predicted from more frames than just the " "one frame before it (also see reference frame option). Therefore, " "I-frames are not necessarily seekable. IDR-frames restrict subsequent " "P-frames from referring to any frame prior to the IDR-frame. \n" "If scenecuts appear within this interval, they are still encoded as " "I-frames, but do not start a new GOP." )
...
#define BFRAMES_LONGTEXT N_( "Number of consecutive B-frames between I and " "P-frames. Range 1 to 16." )


#define B_ADAPT_TEXT N_("Adaptive B-frame decision")
#define B_ADAPT_LONGTEXT N_( "Force the specified number of " "consecutive B-frames to be used, except possibly before an I-frame. " "Range 0 to 2." )



#define B_BIAS_TEXT N_("Influence (bias) B-frames usage")
#define B_BIAS_LONGTEXT N_( "Bias the choice to use B-frames. Positive values " "cause more B-frames, negative values cause less B-frames." )



#define BPYRAMID_TEXT N_("Keep some B-frames as references")
#define BPYRAMID_LONGTEXT N_( "Allows B-frames to be used as references for " "predicting other frames. Keeps the middle of 2+ consecutive B-frames " "as a reference, and reorders frame appropriately.\n" " - none: Disabled\n" " - strict: Strictly hierarchical pyramid\n" " - normal: Non-strict (not Blu-ray compatible)\n")
...
#define FULLRANGE_LONGTEXT N_("TV-range is usually used colorrange, defining this to true " "will enable libx264 to use full colorrange on encoding")


#define CABAC_TEXT N_("CABAC")
#define CABAC_LONGTEXT N_( "CABAC (Context-Adaptive Binary Arithmetic ""Coding). Slightly slows down encoding and decoding, but should save " "10 to 15% bitrate." )



#define REF_TEXT N_("Number of reference frames")
#define REF_LONGTEXT N_( "Number of previous frames used as predictors. " "This is effective in Anime, but seems to make little difference in " "live-action source material. Some decoders are unable to deal with " "large frameref values. Range 1 to 16." )
...
#define FILTER_LONGTEXT N_( "Loop filter AlphaC0 and Beta parameters. " "Range -6 to 6 for both alpha and beta parameters. -6 means light " "filter, 6 means strong.")



#define LEVEL_TEXT N_("H.264 level")
#define LEVEL_LONGTEXT N_( "Specify H.264 level (as defined by Annex A " "of the standard). Levels are not enforced; it's up to the user to select " "a level compatible with the rest of the encoding options. Range 1 to 5.1 " "(10 to 51 is also allowed). Set to 0 for letting x264 set level.")




#define PROFILE_TEXT N_("H.264 profile")
#define PROFILE_LONGTEXT N_("Specify H.264 profile which limits are enforced over " "other settings" )
...
#define QP_LONGTEXT N_( "This selects the quantizer to use. " "Lower values result in better fidelity, but higher bitrates. 26 is a " "good default value. Range 0 (lossless) to 51." )
...
#define QPMIN_LONGTEXT N_( "Minimum quantizer parameter. 15 to 35 seems to " "be a useful range." )
...
#define RATETOL_LONGTEXT N_( "Allowed variance in average " "bitrate (in kbits/s).")
...
#define VBV_BUFSIZE_LONGTEXT N_( "Averaging period for the maximum " "local bitrate (in kbits).")


#define VBV_INIT_TEXT N_("Initial VBV buffer occupancy")
#define VBV_INIT_LONGTEXT N_( "Sets the initial buffer occupancy as a " "fraction of the buffer size. Range 0.0 to 1.0.")
...
#define CPLXBLUR_LONGTEXT N_( "This reduces the fluctuations in QP " "before curve compression. Temporally blurs complexity.")


#define QBLUR_TEXT N_("Reduce fluctuations in QP")
#define QBLUR_LONGTEXT N_( "This reduces the fluctuations in QP " "after curve compression. Temporally blurs quants.")
...
#define MERANGE_LONGTEXT N_( "Maximum distance to search for " "motion estimation, measured from predicted position(s). " "Default of 16 is good for most footage, high motion sequences may " "benefit from settings between 24 and 32. Range 0 to 64." )




#define MVRANGE_TEXT N_("Maximum motion vector length")
#define MVRANGE_LONGTEXT N_( "Maximum motion vector length in pixels. " "-1 is automatic, based on level." )


#define MVRANGE_THREAD_TEXT N_("Minimum buffer space between threads")
#define MVRANGE_THREAD_LONGTEXT N_( "Minimum buffer space between threads. " "-1 is automatic, based on number of threads." )
...
#define SUBME_TEXT N_("Subpixel motion estimation and partition decision " "quality")

#define SUBME_LONGTEXT N_( "This parameter controls quality versus speed " "tradeoffs involved in the motion estimation decision process " "(lower = quicker and higher = better quality). Range 1 to 9." )



#define B_RDO_LONGTEXT N_( "RD based mode decision for B-frames. This " "requires subme 6 (or higher).")


#define MIXED_REFS_TEXT N_("Decide references on a per partition basis")
#define MIXED_REFS_LONGTEXT N_( "Allows each 8x8 or 16x8 partition to " "independently select a reference frame, as opposed to only one ref " "per macroblock." )
...
#define DCT_DECIMATE_LONGTEXT N_( "Coefficient thresholding on P-frames. " "Eliminate dct blocks containing only a small single coefficient.")
...
#define NR_LONGTEXT N_( "Dct-domain noise reduction. Adaptive pseudo-deadzone. " "10 to 1000 seems to be a useful range." )


#define DEADZONE_INTER_TEXT N_("Inter luma quantization deadzone")
#define DEADZONE_INTER_LONGTEXT N_( "Set the size of the inter luma quantization deadzone. " "Range 0 to 32." )


#define DEADZONE_INTRA_TEXT N_("Intra luma quantization deadzone")
#define DEADZONE_INTRA_LONGTEXT N_( "Set the size of the intra luma quantization deadzone. " "Range 0 to 32." )




#define NON_DETERMINISTIC_TEXT N_("Non-deterministic optimizations when threaded")
#define NON_DETERMINISTIC_LONGTEXT N_( "Slightly improve quality of SMP, " "at the cost of repeatability.")
...
#define PSNR_LONGTEXT N_( "Compute and print PSNR stats. This has no effect on " "the actual encoding quality." )


#define SSIM_TEXT N_("SSIM computation")
#define SSIM_LONGTEXT N_( "Compute and print SSIM stats. This has no effect on " "the actual encoding quality." )
...
#define SPS_ID_LONGTEXT N_( "Set SPS and PPS id numbers to allow concatenating " "streams with different settings.")
...
#define LOOKAHEAD_LONGTEXT N_("Framecount to use on frametype lookahead. " "Currently default can cause sync-issues on unmuxable output, like rtsp-output without ts-mux" )
...
p_sys->param.vui.i_colmatrix = 5; 
break;
case COLOR_SPACE_BT709:
p_sys->param.vui.i_colmatrix = 1; 
break;
case COLOR_SPACE_BT2020:
p_sys->param.vui.i_colmatrix = 10; 
break;
default:
break;
}
...
p_sys->param.vui.i_transfer = 7; 
break;
case TRANSFER_FUNC_SRGB:
case TRANSFER_FUNC_BT709:
p_sys->param.vui.i_transfer = 1; 
break;
default:
break;
}
...
p_sys->param.vui.i_colorprim = 5; 
break;
case COLOR_PRIMARIES_BT601_525:
p_sys->param.vui.i_colorprim = 6; 
break;
case COLOR_PRIMARIES_BT709:
p_sys->param.vui.i_colorprim = 1; 
break;
case COLOR_PRIMARIES_BT2020:
p_sys->param.vui.i_colorprim = 9; 
break;
default:
break;
}
...
#if !defined(X264_ANALYSE_BSUB16x16)
