...
#define OMX_VERSION_STEP 0
#endif
#define OMX_INIT_COMMON(a) (a).nSize = sizeof(a); (a).nVersion.s.nVersionMajor = OMX_VERSION_MAJOR; (a).nVersion.s.nVersionMinor = OMX_VERSION_MINOR; (a).nVersion.s.nRevision = OMX_VERSION_REV; (a).nVersion.s.nStep = OMX_VERSION_STEP
#define OMX_INIT_STRUCTURE(a) memset(&(a), 0, sizeof(a)); OMX_INIT_COMMON(a)
#define OMX_ComponentRoleEnum(hComponent, cRole, nIndex) ((OMX_COMPONENTTYPE*)hComponent)->ComponentRoleEnum ? ((OMX_COMPONENTTYPE*)hComponent)->ComponentRoleEnum( hComponent, cRole, nIndex ) : OMX_ErrorNotImplemented
#define CHECK_ERROR(a, ...) if(a != OMX_ErrorNone) {msg_Dbg( p_dec, __VA_ARGS__ ); goto error;}
...
#endif
#define OMX_FIFO_INIT(p_fifo, next) do { vlc_mutex_init( &(p_fifo)->lock ); vlc_cond_init( &(p_fifo)->wait ); (p_fifo)->offset = offsetof(OMX_BUFFERHEADERTYPE, next) / sizeof(void *); (p_fifo)->pp_last = &(p_fifo)->p_first; } while(0)
#define OMX_FIFO_DESTROY(p_fifo) while(0) { }
#define OMX_FIFO_PEEK(p_fifo, p_buffer) p_buffer = (p_fifo)->p_first;
#define OMX_FIFO_GET(p_fifo, p_buffer) do { vlc_mutex_lock( &(p_fifo)->lock ); while( !(p_fifo)->p_first ) vlc_cond_wait( &(p_fifo)->wait, &(p_fifo)->lock ); p_buffer = (p_fifo)->p_first; OMX_BUFFERHEADERTYPE **pp_next = (OMX_BUFFERHEADERTYPE **) ((void **)p_buffer + (p_fifo)->offset); (p_fifo)->p_first = *pp_next; *pp_next = 0; if( !(p_fifo)->p_first ) (p_fifo)->pp_last = &(p_fifo)->p_first; vlc_mutex_unlock( &(p_fifo)->lock ); } while(0)
#define OMX_FIFO_GET_TIMEOUT(p_fifo, p_buffer, timeout) do { vlc_mutex_lock( &(p_fifo)->lock ); vlc_tick_t end = vlc_tick_now() + timeout; if( !(p_fifo)->p_first ) vlc_cond_timedwait( &(p_fifo)->wait, &(p_fifo)->lock, end ); p_buffer = (p_fifo)->p_first; if( p_buffer ) { OMX_BUFFERHEADERTYPE **pp_next = (OMX_BUFFERHEADERTYPE **) ((void **)p_buffer + (p_fifo)->offset); (p_fifo)->p_first = *pp_next; *pp_next = 0; if( !(p_fifo)->p_first ) (p_fifo)->pp_last = &(p_fifo)->p_first; } vlc_mutex_unlock( &(p_fifo)->lock ); } while(0)
#define OMX_FIFO_PUT(p_fifo, p_buffer) do { vlc_mutex_lock (&(p_fifo)->lock); OMX_BUFFERHEADERTYPE **pp_next = (OMX_BUFFERHEADERTYPE **) ((void **)p_buffer + (p_fifo)->offset); *(p_fifo)->pp_last = p_buffer; (p_fifo)->pp_last = pp_next; *pp_next = 0; vlc_cond_signal( &(p_fifo)->wait ); vlc_mutex_unlock( &(p_fifo)->lock ); } while(0)
typedef union {
