...
#define CALL(caller, obj, method, ...) (*p_env)->caller(p_env, obj, fields.method, ##__VA_ARGS__)
#define CALL_VOID(obj, method, ...) CALL(CallVoidMethod, obj, method, ##__VA_ARGS__)
#define CALL_OBJ(obj, method, ...) CALL(CallObjectMethod, obj, method, ##__VA_ARGS__)
#define CALL_STATICOBJ(obj, method, ...) CALL(CallStaticObjectMethod, fields.obj.clazz, method, ##__VA_ARGS__)
#define CALL_INT(obj, method, ...) CALL(CallIntMethod, obj, method, ##__VA_ARGS__)
#define CALL_BOOL(obj, method, ...) CALL(CallBooleanMethod, obj, method, ##__VA_ARGS__)
#define NEW_OBJECT(arg, ...) (*p_env)->NewObject(p_env, fields.arg.clazz, fields.arg.ctor, ##__VA_ARGS__)
#define NEW_GREF(obj) (*p_env)->NewGlobalRef(p_env, obj)
#define DEL_GREF(obj) (*p_env)->DeleteGlobalRef(p_env, obj)
#define DEL_LREF(obj) (*p_env)->DeleteLocalRef(p_env, obj)
#define NEW_STR(str) (*p_env)->NewStringUTF(p_env, str)
static bool
...
#define GET_CLASS(str) do { if (clazz != NULL) DEL_LREF(clazz); clazz = (*p_env)->FindClass(p_env, (str)); if (CHECK_EXCEPTION()) return VLC_EGENERIC; } while (0)
#define GET_GLOBAL_CLASS(id) do { fields.id.clazz = (jclass) NEW_GREF(clazz); } while (0)
#define GET_ID(get, id, str, args) do { fields.id = (*p_env)->get(p_env, clazz, (str), (args)); if (CHECK_EXCEPTION()) return VLC_EGENERIC; } while (0)
#define GET_CONST_INT(id, str) do { jfieldID field = (*p_env)->GetStaticFieldID(p_env, clazz, (str), "I"); if (!CHECK_EXCEPTION()) { fields.id = (*p_env)->GetStaticIntField(p_env, clazz, field); if (CHECK_EXCEPTION()) return VLC_EGENERIC; } else { return VLC_EGENERIC; } } while(0)
#define GET_CONST_OBJ(id, str, type) do { jfieldID field = (*p_env)->GetStaticFieldID(p_env, clazz, (str), type); if (!CHECK_EXCEPTION()) { jobject jobj = (*p_env)->GetStaticObjectField(p_env, clazz, field); if (CHECK_EXCEPTION()) return VLC_EGENERIC; fields.id = NEW_GREF(jobj); DEL_LREF(jobj); } else { return VLC_EGENERIC; } } while(0)
static int
