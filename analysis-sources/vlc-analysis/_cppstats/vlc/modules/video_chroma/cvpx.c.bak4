...
#define DO(x) x(dst, src_planes, src_pitches, height, &p_sys->sw.cache)

#define DO_S(x, shift) x(dst, src_planes, src_pitches, height, shift, &p_sys->sw.cache)

#define DO_P(x) x(dst, src_planes[0], src_pitches[0], height, &p_sys->sw.cache)
...
#define CASE_CVPX_INPUT(x, i420_fcc) case VLC_CODEC_CVPX_##x: sw_fmt = p_filter->fmt_out.video; if (p_filter->fmt_out.video.i_chroma == VLC_CODEC_##x) p_filter->pf_video_filter = CVPX_TO_SW_Filter; else if (i420_fcc != 0 && p_filter->fmt_out.video.i_chroma == i420_fcc) { p_filter->pf_video_filter = CVPX_TO_SW_Filter; sw_fmt.i_chroma = VLC_CODEC_##x; } else return VLC_EGENERIC; 
...
#define CASE_CVPX_OUTPUT(x, i420_fcc) case VLC_CODEC_CVPX_##x: sw_fmt = p_filter->fmt_in.video; if (p_filter->fmt_in.video.i_chroma == VLC_CODEC_##x) { p_filter->pf_video_filter = SW_TO_CVPX_Filter; } else if (i420_fcc != 0 && p_filter->fmt_in.video.i_chroma == i420_fcc) { p_filter->pf_video_filter = SW_TO_CVPX_Filter; sw_fmt.i_chroma = VLC_CODEC_##x; } else return VLC_EGENERIC; b_need_pool = true;
...
unsigned int i;
#define CHECK_CHROMA(fourcc) i = 0; while (i < ARRAY_SIZE(supported_chromas) && fourcc != supported_chromas[i]) ++i; if (i == ARRAY_SIZE(supported_chromas)) return VLC_EGENERIC; 
