...
#define ASSERT_PLANE(i) assert(src[i]); assert(src_pitch[i])


#define ASSERT_2PLANES assert(dst); ASSERT_PLANE(0); ASSERT_PLANE(1); assert(height)





#define ASSERT_3PLANES ASSERT_2PLANES; ASSERT_PLANE(2)
...
#define COPY16_SHIFTR(x) "psrlw "x", %%xmm1\n"
...
#define COPY64_SHIFTR(x) "psrlw "x", %%xmm1\n" "psrlw "x", %%xmm2\n" "psrlw "x", %%xmm3\n" "psrlw "x", %%xmm4\n"
...
#define COPY64(dstp, srcp, load, store) COPY64_S(dstp, srcp, load, store, "")


#if defined(COPY_TEST_NOOPTIM)
...
#define LOAD64 "movdqa 0(%[src]), %%xmm0\n" "movdqa 16(%[src]), %%xmm1\n" "movdqa 32(%[src]), %%xmm2\n" "movdqa 48(%[src]), %%xmm3\n"
...
#define SPLIT_PLANES(type, pitch_den) do { size_t copy_pitch = __MIN(__MIN(src_pitch / pitch_den, dstu_pitch), dstv_pitch); for (unsigned y = 0; y < height; y++) { for (unsigned x = 0; x < copy_pitch; x++) { ((type *) dstu)[x] = ((const type *) src)[2*x+0]; ((type *) dstv)[x] = ((const type *) src)[2*x+1]; } src += src_pitch; dstu += dstu_pitch; dstv += dstv_pitch; } } while(0)
...
#define SPLIT_PLANES_SHIFTR(type, pitch_den, bitshift) do { size_t copy_pitch = __MIN(__MIN(src_pitch / pitch_den, dstu_pitch), dstv_pitch); for (unsigned y = 0; y < height; y++) { for (unsigned x = 0; x < copy_pitch; x++) { ((type *) dstu)[x] = (((const type *) src)[2*x+0]) >> (bitshift); ((type *) dstv)[x] = (((const type *) src)[2*x+1]) >> (bitshift); } src += src_pitch; dstu += dstu_pitch; dstv += dstv_pitch; } } while(0)
...
#define SPLIT_PLANES_SHIFTL(type, pitch_den, bitshift) do { size_t copy_pitch = __MIN(__MIN(src_pitch / pitch_den, dstu_pitch), dstv_pitch); for (unsigned y = 0; y < height; y++) { for (unsigned x = 0; x < copy_pitch; x++) { ((type *) dstu)[x] = (((const type *) src)[2*x+0]) << (bitshift); ((type *) dstv)[x] = (((const type *) src)[2*x+1]) << (bitshift); } src += src_pitch; dstu += dstu_pitch; dstv += dstv_pitch; } } while(0)
...
#define INTERLEAVE_UV() do { for ( unsigned int line = 0; line < copy_lines; line++ ) { for ( unsigned int col = 0; col < copy_pitch; col++ ) { *dstUV++ = *srcU++; *dstUV++ = *srcV++; } dstUV += i_extra_pitch_uv; srcU += i_extra_pitch_u; srcV += i_extra_pitch_v; } }while(0)
...
#define INTERLEAVE_UV_SHIFTR(bitshitf) do { for ( unsigned int line = 0; line < copy_lines; line++ ) { for ( unsigned int col = 0; col < copy_pitch; col++ ) { *dstUV++ = (*srcU++) >> (bitshitf); *dstUV++ = (*srcV++) >> (bitshitf); } dstUV += i_extra_pitch_uv; srcU += i_extra_pitch_u; srcV += i_extra_pitch_v; } }while(0)
...
#define INTERLEAVE_UV_SHIFTL(bitshitf) do { for ( unsigned int line = 0; line < copy_lines; line++ ) { for ( unsigned int col = 0; col < copy_pitch; col++ ) { *dstUV++ = (*srcU++) << (bitshitf); *dstUV++ = (*srcV++) << (bitshitf); } dstUV += i_extra_pitch_uv; srcU += i_extra_pitch_u; srcV += i_extra_pitch_v; } }while(0)
...
bool init)
{
#define ASSERT_COLOR(good) do { fprintf(stderr, "error: pixel doesn't match @ plane: %d: %d x %d: 0x%X vs 0x%X\n", i, x, y, *(--p), good); assert(!"error: pixel doesn't match"); } while(0)




#define PICCHECK(type_u, type_uv, colors_P, color_UV, pitch_den) do { for (int i = 0; i < pic->i_planes; ++i) { const struct plane_t *plane = &pic->p[i]; for (int y = 0; y < plane->i_visible_lines; ++y) { if (pic->i_planes == 2 && i == 1) { type_uv *p = (type_uv *)&plane->p_pixels[y * plane->i_pitch]; for (int x = 0; x < plane->i_visible_pitch / 2 / pitch_den; ++x) if (init) *(p++) = color_UV; else if (*(p++) != color_UV) ASSERT_COLOR(color_UV); } else { type_u *p = (type_u *) &plane->p_pixels[y * plane->i_pitch]; for (int x = 0; x < plane->i_visible_pitch / pitch_den; ++x) if (init) *(p++) = colors_P[i]; else if (*(p++) != colors_P[i]) ASSERT_COLOR(colors_P[i]); } } } } while (0)
