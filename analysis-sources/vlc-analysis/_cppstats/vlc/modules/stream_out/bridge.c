...
#define ID_LONGTEXT N_( "Integer identifier for this elementary stream. This will be used to " "\"find\" this stream later." )
#define DEST_TEXT N_( "Destination bridge-in name" )
#define DEST_LONGTEXT N_( "Name of the destination bridge-in. If you do not need more " "than one bridge-in at a time, you can discard this option." )
#define DELAY_TEXT N_("Delay")
#define DELAY_LONGTEXT N_("Pictures coming from the picture video outputs " "will be delayed according to this value (in milliseconds, should be "">= 100 ms). For high values, you will need to raise caching values." )
#define ID_OFFSET_TEXT N_("ID Offset")
#define ID_OFFSET_LONGTEXT N_("Offset to add to the stream IDs specified in " "bridge_out to obtain the stream IDs bridge_in will register.")
#define NAME_TEXT N_( "Name of current instance" )
#define NAME_LONGTEXT N_( "Name of this bridge-in instance. If you do not need more " "than one bridge-in at a time, you can discard this option." )
#define PLACEHOLDER_TEXT N_( "Fallback to placeholder stream when out of data" )
#define PLACEHOLDER_LONGTEXT N_( "If set to true, the bridge will discard all input elementary streams " "except if it doesn't receive data from another bridge-in. This can " "be used to configure a place holder stream when the real source " "breaks. Source and placeholder streams should have the same format." )
#define PLACEHOLDER_DELAY_TEXT N_( "Placeholder delay" )
#define PLACEHOLDER_DELAY_LONGTEXT N_( "Delay (in ms) before the placeholder kicks in." )
#define PLACEHOLDER_IFRAME_TEXT N_( "Wait for I frame before toggling placeholder" )
#define PLACEHOLDER_IFRAME_LONGTEXT N_( "If enabled, switching between the placeholder and the normal stream " "will only occur on I frames. This will remove artifacts on stream " "switching at the expense of a slightly longer delay, depending on " "the frequence of I frames in the streams." )
