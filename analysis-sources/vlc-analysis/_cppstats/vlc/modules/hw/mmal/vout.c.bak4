...
#define MMAL_DISPLAY_LONGTEXT N_("Output device for Rpi fullscreen. " "Valid values are HDMI-1,HDMI-2. By default HDMI-1.")
...
#define MMAL_NATIVE_INTERLACE_LONGTEXT N_("Force the HDMI output into an " "interlaced video mode for interlaced video content.")
...
set_callback_display(OpenMmalVout, 16) 
vlc_module_end()
...
kill_pool(sys);
vd->fmt = vd->source; 
vd->fmt.i_chroma = req_chroma(&vd->fmt); 
ret = VLC_SUCCESS;
break;
...
place_dest(sys, vd->cfg, &vd->source); 
