...
#define MSG_LONGTEXT N_( "Marquee text to display. " "(Available format strings: " "%Y = year, %m = month, %d = day, %H = hour, " "%M = minute, %S = second, ...)" )
...
#define REFRESH_LONGTEXT N_("Number of milliseconds between string updates. " "This is mainly useful when using meta data " "or time format string sequences.")


#define OPACITY_TEXT N_("Opacity")
#define OPACITY_LONGTEXT N_("Opacity (inverse of transparency) of " "overlayed text. 0 = transparent, 255 = totally opaque." )

#define SIZE_TEXT N_("Font size, pixels")
#define SIZE_LONGTEXT N_("Font size, in pixels. Default is 0 (use default " "font size)." )


#define COLOR_TEXT N_("Color")
#define COLOR_LONGTEXT N_("Color of the text that will be rendered on ""the video. This must be an hexadecimal (like HTML colors). The first two ""chars are for red, then green, then blue. #000000 = black, #FF0000 = red,"" #00FF00 = green, #FFFF00 = yellow (red + green), #FFFFFF = white" )




#define POS_TEXT N_("Marquee position")
#define POS_LONGTEXT N_( "You can enforce the marquee position on the video " "(0=center, 1=left, 2=right, 4=top, 8=bottom, you can " "also use combinations of these values, eg 6 = top-right).")
...
#define CREATE_VAR( stor, type, var ) p_sys->stor = var_CreateGet##type##Command( p_filter, var ); var_AddCallback( p_filter, var, MarqueeCallback, p_sys );
...
#define DEL_VAR(var) var_DelCallback( p_filter, var, MarqueeCallback, p_sys ); var_Destroy( p_filter, var );
