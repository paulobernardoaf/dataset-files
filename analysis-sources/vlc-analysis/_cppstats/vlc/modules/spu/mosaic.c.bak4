...
#define ALPHA_LONGTEXT N_( "Transparency of the mosaic foreground pictures. " "0 means transparent, 255 opaque (default)." )
...
#define ALIGN_LONGTEXT N_( "You can enforce the mosaic alignment on the video " "(0=center, 1=left, 2=right, 4=top, 8=bottom, you can " "also use combinations of these values, eg 6 = top-right).")




#define POS_TEXT N_("Positioning method")
#define POS_LONGTEXT N_( "Positioning method for the mosaic. auto: " "automatically choose the best number of rows and columns. " "fixed: use the user-defined number of rows and columns. " "offsets: use the user-defined offsets for each image." )
...
#define ROWS_LONGTEXT N_( "Number of image rows in the mosaic (only used if " "positioning method is set to \"fixed\")." )



#define COLS_TEXT N_("Number of columns")
#define COLS_LONGTEXT N_( "Number of image columns in the mosaic (only used if " "positioning method is set to \"fixed\")." )



#define AR_TEXT N_("Keep aspect ratio")
#define AR_LONGTEXT N_( "Keep the original aspect ratio when resizing " "mosaic elements." )
...
#define ORDER_LONGTEXT N_( "You can enforce the order of the elements on " "the mosaic. You must give a comma-separated list of picture ID(s). " "These IDs are assigned in the \"mosaic-bridge\" module." )




#define OFFSETS_TEXT N_("Offsets in order" )
#define OFFSETS_LONGTEXT N_( "You can enforce the (x,y) offsets of the elements on the mosaic " "(only used if positioning method is set to \"offsets\"). You " "must give a comma-separated list of coordinates (eg: 10,10,150,10)." )




#define DELAY_TEXT N_("Delay")
#define DELAY_LONGTEXT N_( "Pictures coming from the mosaic elements will be delayed " "according to this value (in milliseconds). For high " "values you will need to raise caching at input.")
...
#define mosaic_ParseSetOffsets( a, b, c ) mosaic_ParseSetOffsets( VLC_OBJECT( a ), b, c )
...
#define GET_VAR( name, min, max ) i_command = var_CreateGetIntegerCommand( p_filter, CFG_PREFIX #name ); p_sys->i_##name = VLC_CLIP( i_command, min, max ); var_AddCallback( p_filter, CFG_PREFIX #name, MosaicCallback, p_sys );
...
{
int i_numpics = p_sys->i_order_length; 
