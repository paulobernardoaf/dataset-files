...
#define APPENDIF(idmax, set, rg, b) for(size_t i=0; i<=idmax; i++){if(((set != rg[i].p_decoded) == !b) && rg[i].p_nal){AppendAsAnnexB(rg[i].p_nal, &p_data, &i_data);break;}}
...
block_Release(p_frag);
return GatherAndValidateChain(OutputQueues(p_sys, false)); 
}
