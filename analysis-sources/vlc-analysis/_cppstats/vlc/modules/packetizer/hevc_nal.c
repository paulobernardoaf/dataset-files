...
uint8_t sublayer_profile_present_flag; 
uint8_t sublayer_level_present_flag; 
...
{
*pi_result = get_hvcC_to_AnnexB_NAL_size( p_buf, i_buf ); 
if( *pi_result == 0 )
return NULL;
...
#define HEVC_DCR_ADD_NALS(type, count, buffers, sizes) for (uint8_t i = 0; i < count; i++) { if( i ==0 ) { *p++ = (type | (b_completeness ? 0x80 : 0)); SetWBE( p, count ); p += 2; } SetWBE( p, sizes[i]); p += 2; memcpy( p, buffers[i], sizes[i] ); p += sizes[i];}
#define HEVC_DCR_ADD_SIZES(count, sizes) if(count > 0) {i_total_size += 3;for(uint8_t i=0; i<count; i++)i_total_size += 2 + sizes[i];}
...
!( ( p_slice->nal_type <= HEVC_NAL_RSV_VCL_N14 && p_slice->nal_type % 2 == 0 ) ||
( p_slice->nal_type >= HEVC_NAL_RADL_N && p_slice->nal_type <= HEVC_NAL_RASL_R ) ) )
{
