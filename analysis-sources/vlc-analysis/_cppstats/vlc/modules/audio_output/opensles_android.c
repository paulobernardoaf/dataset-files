...
#define CHECK_OPENSL_ERROR(msg) if (unlikely(result != SL_RESULT_SUCCESS)) { msg_Err(aout, msg" (%" PRIu32 ")", (uint32_t)result); goto error; }
...
#define OPENSL_DLSYM(dest, name) do { const SLInterfaceID *sym = dlsym(sys->p_so_handle, "SL_IID_"name); if (unlikely(sym == NULL)) { msg_Err(aout, "Failed to load symbol SL_IID_"name); goto error; } sys->dest = *sym; } while(0)
