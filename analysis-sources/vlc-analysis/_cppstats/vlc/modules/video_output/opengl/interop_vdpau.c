...
#include "internal.h"
#include "interop.h"
#define INTEROP_CALL(fct, ...) _##fct(__VA_ARGS__); { GLenum ret = interop->vt->GetError(); if (ret != GL_NO_ERROR) { msg_Err(interop->gl, #fct " failed: 0x%x", ret); return VLC_EGENERIC; } }
...
#define SAFE_GPA(fct) _##fct = vlc_gl_GetProcAddress(interop->gl, #fct); if (!_##fct) { vlc_decoder_device_Release(dec_device); return VLC_EGENERIC; }
