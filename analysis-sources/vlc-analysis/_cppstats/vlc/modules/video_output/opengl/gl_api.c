...
#endif
#define GET_PROC_ADDR_EXT(name, critical) do { api->vt.name = vlc_gl_GetProcAddress(gl, "gl"#name); if (api->vt.name == NULL && critical) { msg_Err(gl, "gl"#name" symbol not found, bailing out"); return VLC_EGENERIC; } } while(0)
...
#endif
#define GET_PROC_ADDR_OPTIONAL(name) GET_PROC_ADDR_EXT(name, false) 
