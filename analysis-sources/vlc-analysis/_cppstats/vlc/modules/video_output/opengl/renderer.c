...
if (!program_id)
return VLC_EGENERIC;
#define GET_LOC(type, x, str) do { x = vt->Get##type##Location(program_id, str); assert(x != -1); if (x == -1) { msg_Err(renderer->gl, "Unable to Get"#type"Location(%s)", str); goto error; } } while (0)
