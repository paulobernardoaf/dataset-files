...
#define GET_LOC(type, x, str) do { x = vt->Get##type##Location(sr->program_id, str); assert(x != -1); if (x == -1) { msg_Err(sr->gl, "Unable to Get"#type"Location(%s)", str); return VLC_EGENERIC; } } while (0)
