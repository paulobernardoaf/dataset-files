...
if (i_init_state != -1)
goto end;
#define CHECK_EXCEPTION(what, critical) do { if( (*env)->ExceptionCheck(env) ) { msg_Err(p_obj, "%s failed", what); (*env)->ExceptionClear(env); if (critical) { i_init_state = 0; goto end; } } } while( 0 )
#define GET_METHOD(id, str, args, critical) do { jfields.id = (*env)->GetMethodID(env, clazz, (str), (args)); CHECK_EXCEPTION("GetMethodID("str")", critical); } while( 0 )
...
#define JNI_CALL(what, obj, method, ...) (*p_env)->what(p_env, obj, jfields.method, ##__VA_ARGS__)
#define JNI_ANWCALL(what, method, ...) (*p_env)->what(p_env, p_awh->jobj, jfields.AndroidNativeWindow.method, ##__VA_ARGS__)
#define JNI_STEXCALL(what, method, ...) (*p_env)->what(p_env, p_awh->jobj, jfields.SurfaceTexture.method, ##__VA_ARGS__)
static JNIEnv*
