...
#define COLS_LONGTEXT N_("Select the number of horizontal video windows in " "which to split the video")
#define ROWS_TEXT N_("Number of rows")
#define ROWS_LONGTEXT N_("Select the number of vertical video windows in " "which to split the video")
#define ACTIVE_TEXT N_("Active windows")
#define ACTIVE_LONGTEXT N_("Comma-separated list of active windows, " "defaults to all")
#define CFG_PREFIX "panoramix-"
#define PANORAMIX_HELP N_("Split the video in multiple windows to " "display on a wall of screens")
...
int lambdav[VOUT_MAX_PLANES][2][ACCURACY/2]; 
int lambdah[VOUT_MAX_PLANES][2][ACCURACY/2];
unsigned int i_overlap_w2; 
unsigned int i_overlap_h2; 
uint8_t p_lut[VOUT_MAX_PLANES][ACCURACY + 1][256];
int i_col;
int i_row;
panoramix_output_t pp_output[COL_MAX][ROW_MAX]; 
