...
#endif
#if defined(DEBUG_BLURAY_EVENTS)
...
#define blurayDebugEvent(e, v) do {if(e < ARRAY_SIZE(bluray_event_debug_strings))msg_Dbg(p_demux, "Event %s %x", bluray_event_debug_strings[e], v);elsemsg_Dbg(p_demux, "Event Unk 0x%x %x", e, v);} while(0)
#else
#define blurayDebugEvent(e, v)
#endif
#if defined(BLURAY_HAS_BDJO_DATA_H)
...
#define BD_MENU_LONGTEXT N_("Use Blu-ray menus. If disabled, ""the movie will start directly")
#define BD_REGION_TEXT N_("Region code")
#define BD_REGION_LONGTEXT N_("Blu-Ray player region code. ""Some discs can be played only with a correct region code.")
...
#define BLURAY_DEMUX
#endif
#if !defined(BD_STREAM_TYPE_VIDEO_HEVC)
...
#define DONOTIFY(memb) notifyStreamsDiscontinuity( p_sys->p_parser, p_clip->memb##_streams, p_clip->memb##_stream_count )
static void notifyDiscontinuityToParser( demux_sys_t *p_sys )
{
...
uint8_t seq_end_pes[] = {
0x00, 0x00, 0x01, 0xe0, 0x00, 0x08, 0x80, 0x00, 0x00, 
