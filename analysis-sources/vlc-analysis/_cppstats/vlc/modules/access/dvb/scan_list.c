...
#define STRING_EQUALS(token, string, stringlen) ((sizeof(token) - 1) == stringlen && !strncasecmp( string, token, stringlen ))
#define KEY_EQUALS(token) ((sizeof(token) - 1) == i_keylen && !strncasecmp( psz_key, token, i_keylen ))
#define VALUE_EQUALS(token) ((sizeof(token) - 1) == i_valuelen && !strncasecmp( psz_value, token, i_valuelen ))
#define READ_STRINGVAL(variable, parser) { char *psz_val = strndup(psz_value, i_valuelen);if( psz_val ){variable = parser( psz_val );free( psz_val );} }
