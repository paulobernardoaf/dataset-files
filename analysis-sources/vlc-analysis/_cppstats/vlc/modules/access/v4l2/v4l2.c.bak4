...
#define VBI_DEVICE_LONGTEXT N_( "The device node where VBI data can be read " "(for closed captions)." )
...
#define CHROMA_LONGTEXT N_( "Force the Video4Linux2 video device to use a specific chroma format " "(eg. I420 or I422 for raw images, MJPG for M-JPEG compressed input). " "Complete list: GREY, I240, RV16, RV15, RV24, RV32, YUY2, YUYV, UYVY, " "I41N, I422, I420, I411, I410, MJPG")




#define INPUT_TEXT N_( "Input" )
#define INPUT_LONGTEXT N_( "Input of the card to use (see debug)." )

#define AUDIO_INPUT_TEXT N_( "Audio input" )
#define AUDIO_INPUT_LONGTEXT N_( "Audio input of the card to use (see debug)." )

#define WIDTH_TEXT N_( "Width" )
#define HEIGHT_TEXT N_( "Height" )
#define SIZE_LONGTEXT N_( "The specified pixel resolution is forced " "(if both width and height are strictly positive)." )
...
#define FREQUENCY_LONGTEXT N_( "Tuner frequency in Hz or kHz (see debug output)." )

#define TUNER_AUDIO_MODE_TEXT N_("Audio mode")
#define TUNER_AUDIO_MODE_LONGTEXT N_( "Tuner audio mono/stereo and track selection." )
...
#define BRIGHTNESS_AUTO_LONGTEXT N_( "Automatically adjust the picture brightness." )
...
#define HUE_AUTO_LONGTEXT N_( "Automatically adjust the picture hue." )

#define WHITE_BALANCE_TEMP_TEXT N_( "White balance temperature (K)" )
#define WHITE_BALANCE_TEMP_LONGTEXT N_( "White balance temperature as a color temperation in Kelvin " "(2800 is minimum incandescence, 6500 is maximum daylight)." )


#define AUTOWHITEBALANCE_TEXT N_( "Automatic white balance" )
#define AUTOWHITEBALANCE_LONGTEXT N_( "Automatically adjust the picture white balance." )

#define REDBALANCE_TEXT N_( "Red balance" )
#define REDBALANCE_LONGTEXT N_( "Red chroma balance." )

#define BLUEBALANCE_TEXT N_( "Blue balance" )
#define BLUEBALANCE_LONGTEXT N_( "Blue chroma balance." )

#define GAMMA_TEXT N_( "Gamma" )
#define GAMMA_LONGTEXT N_( "Gamma adjust." )

#define AUTOGAIN_TEXT N_( "Automatic gain" )
#define AUTOGAIN_LONGTEXT N_( "Automatically set the video gain." )

#define GAIN_TEXT N_( "Gain" )
#define GAIN_LONGTEXT N_( "Picture gain." )
...
#define CHROMA_GAIN_AUTO_LONGTEXT N_( "Automatically control the chroma gain." )

#define POWER_FREQ_TEXT N_( "Power line frequency" )
#define POWER_FREQ_LONGTEXT N_( "Power line frequency anti-flicker filter." )
...
#define BAND_STOP_FILTER_LONGTEXT N_( "Cut a light band induced by fluorescent lighting (unit undocumented)." )

#define HFLIP_TEXT N_( "Horizontal flip" )
#define HFLIP_LONGTEXT N_( "Flip the picture horizontally." )

#define VFLIP_TEXT N_( "Vertical flip" )
#define VFLIP_LONGTEXT N_( "Flip the picture vertically." )
...
#define COLOR_KILLER_LONGTEXT N_( "Enable the color killer, i.e. switch to black & white picture " "whenever the signal is weak." )
...
#define AUDIO_VOLUME_LONGTEXT N_( "Volume of the audio input." )

#define AUDIO_BALANCE_TEXT N_( "Audio balance" )
#define AUDIO_BALANCE_LONGTEXT N_( "Balance of the audio input." )

#define AUDIO_BASS_TEXT N_( "Bass level" )
#define AUDIO_BASS_LONGTEXT N_( "Bass adjustment of the audio input." )

#define AUDIO_TREBLE_TEXT N_( "Treble level" )
#define AUDIO_TREBLE_LONGTEXT N_( "Treble adjustment of the audio input." )

#define AUDIO_MUTE_TEXT N_( "Mute" )
#define AUDIO_MUTE_LONGTEXT N_( "Mute the audio." )

#define AUDIO_LOUDNESS_TEXT N_( "Loudness mode" )
#define AUDIO_LOUDNESS_LONGTEXT N_( "Loudness mode a.k.a. bass boost." )


#define S_CTRLS_TEXT N_("v4l2 driver controls")
#define S_CTRLS_LONGTEXT N_( "Set the v4l2 driver controls to the values specified using a comma " "separated list optionally encapsulated by curly braces " "(e.g.: {video_bitrate=6000000,audio_crc=0,stream_type=3} ). " "To list available controls, increase verbosity (-vv) " "or use the v4l2-ctl application." )
...
change_safe()
add_obsolete_integer( CFG_PREFIX "io" ) 
...
change_integer_list( tristate_vlc, tristate_user )
add_obsolete_integer( CFG_PREFIX "black-level" ) 
...
change_integer_list( tristate_vlc, tristate_user )
add_obsolete_integer( CFG_PREFIX"do-white-balance" ) 
...
add_obsolete_integer( CFG_PREFIX "hcenter" ) 
add_obsolete_integer( CFG_PREFIX "vcenter" ) 
