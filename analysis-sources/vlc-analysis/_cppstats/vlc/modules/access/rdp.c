...
#endif
#if !defined(FREERDP_VERSION_MAJOR) || (defined(FREERDP_VERSION_MAJOR) && !(FREERDP_VERSION_MAJOR > 1 || (FREERDP_VERSION_MAJOR == 1 && FREERDP_VERSION_MINOR >= 1)))
...
#define USER_LONGTEXT N_("Username that will be used for the connection, " "if no username is set in the URL.")
#define PASS_TEXT N_("Password")
#define PASS_LONGTEXT N_("Password that will be used for the connection, " "if no username or password are set in URL.")
...
demux_sys_t *p_sys = p_vlccontext->p_demux->p_sys;
p_instance->settings->SoftwareGdi = true; 
