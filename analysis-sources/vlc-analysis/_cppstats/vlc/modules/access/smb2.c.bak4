...
#define VLC_SMB2_CHECK_STATUS(access, status) smb2_check_status(access, status, __func__)


#define VLC_SMB2_SET_ERROR(access, func, err) smb2_set_error(access, func, err)
...
#define VLC_SMB2_GENERIC_CB() VLC_UNUSED(smb2); stream_t *access = private_data; struct access_sys *sys = access->p_sys; assert(sys->smb2 == smb2); if (VLC_SMB2_CHECK_STATUS(access, status)) return
