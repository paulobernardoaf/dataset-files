...
{
l--; d++; 
#define GET_FIELD( x ) if ( l > 0 ) { p_mmi->u.menu.psz_##x = MMIGetText( p_cam, &d, &l ); msg_Dbg( p_cam->obj, "MMI " STRINGIFY( x ) ": %s", p_mmi->u.menu.psz_##x ); }
GET_FIELD( title );
GET_FIELD( subtitle );
GET_FIELD( bottom );
#undef GET_FIELD
...
#define CHECK_CAPS( x, s ) if ( caps.slot_type & (CA_##x) ) fprintf( p, "<tr><td>%s</td></tr>\n", s )
...
#define CHECK_DESC( x ) if ( caps.descr_type & (CA_##x) ) fprintf( p, "<tr><td>%s</td></tr>", STRINGIFY(x) )
CHECK_DESC( ECD );
CHECK_DESC( NDS );
CHECK_DESC( DSS );
#undef CHECK_DESC
...
#define CHECK_TYPE( x, s ) if ( sinfo.type & (CA_##x) ) fputs( s, p )
