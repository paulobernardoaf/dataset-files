...
#endif
#define DVBv5(minor) (DVB_API_VERSION > 5 || (DVB_API_VERSION == 5 && DVB_API_VERSION_MINOR >= (minor)))
typedef struct
{
int vlc;
int linux_;
...
#define S(f) if (s & FE_ ##f) msg_Dbg(obj, "\t%s", #f);
S(HAS_SIGNAL);
S(HAS_CARRIER);
S(HAS_VITERBI);
S(HAS_SYNC);
...
[SYS_ISDBC] = DTV_DELIVERY_ISDB_C, 
[SYS_ATSC] = DTV_DELIVERY_ATSC,
[SYS_DVBT2] = DTV_DELIVERY_DVB_T2,
[SYS_DVBC_ANNEX_C] = DTV_DELIVERY_ISDB_C, 
};
...
vlc_tick_sleep(VLC_TICK_FROM_MS(125)); 
}
