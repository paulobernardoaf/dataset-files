...
#define TAB_CLEAN( count, tab ) do { free( tab ); (count)= 0; (tab)= NULL; } while(0)
...
#define TAB_APPEND_CAST( cast, count, tab, p ) do { if( (count) > 0 ) (tab) = cast realloc( tab, sizeof( *(tab) ) * ( (count) + 1 ) ); else (tab) = cast malloc( sizeof( *(tab) ) ); if( !(tab) ) abort(); (tab)[count] = (p); (count)++; } while(0)
...
#define TAB_FIND( count, tab, p, idx ) do { for( (idx) = 0; (idx) < (count); (idx)++ ) if( (tab)[(idx)] == (p) ) break; if( (idx) >= (count) ) (idx) = -1; } while(0)
...
#define TAB_ERASE( count, tab, index ) do { if( (count) > 1 ) memmove( (tab) + (index), (tab) + (index) + 1, ((count) - (index) - 1 ) * sizeof( *(tab) ) );(count)--; if( (count) == 0 ) { free( tab ); (tab) = NULL; } } while(0)
...
#define TAB_REMOVE( count, tab, p ) do { int i_index; TAB_FIND( count, tab, p, i_index ); if( i_index >= 0 ) TAB_ERASE( count, tab, i_index ); } while(0)
...
#define TAB_INSERT_CAST( cast, count, tab, p, index ) do { if( (count) > 0 ) (tab) = cast realloc( tab, sizeof( *(tab) ) * ( (count) + 1 ) ); else (tab) = cast malloc( sizeof( *(tab) ) ); if( !(tab) ) abort(); if( (count) - (index) > 0 ) memmove( (tab) + (index) + 1, (tab) + (index), ((count) - (index)) * sizeof( *(tab) ) );(tab)[(index)] = (p); (count)++; } while(0)
...
#define BSEARCH( entries, count, elem, zetype, key, answer ) do { int low = 0, high = count - 1; answer = -1; while( low <= high ) {int mid = ((unsigned int)low + (unsigned int)high) >> 1;zetype mid_val = entries[mid] elem;if( mid_val < key ) low = mid + 1; else if ( mid_val > key ) high = mid -1; else { answer = mid; break; }} } while(0)
...
#define _ARRAY_ALLOC(array, newsize) { (array).i_alloc = newsize; (array).p_elems = realloc( (array).p_elems, (array).i_alloc * sizeof(*(array).p_elems) ); if( !(array).p_elems ) abort(); }
...
#define _ARRAY_GROW1(array) { if( (array).i_alloc < 10 ) _ARRAY_ALLOC(array, 10 ) else if( (array).i_alloc == (array).i_size ) _ARRAY_ALLOC(array, (int)((array).i_alloc * 1.5) ) }
...
#define DECL_ARRAY(type) struct { int i_alloc; int i_size; type *p_elems; }
...
#define ARRAY_INIT(array) do { (array).i_alloc = 0; (array).i_size = 0; (array).p_elems = NULL; } while(0)
...
#define ARRAY_RESET(array) do { (array).i_alloc = 0; (array).i_size = 0; free( (array).p_elems ); (array).p_elems = NULL; } while(0)
...
#define ARRAY_APPEND(array, elem) do { _ARRAY_GROW1(array); (array).p_elems[(array).i_size] = elem; (array).i_size++; } while(0)
...
#define ARRAY_INSERT(array,elem,pos) do { _ARRAY_GROW1(array); if( (array).i_size - (pos) ) { memmove( (array).p_elems + (pos) + 1, (array).p_elems + (pos), ((array).i_size-(pos)) * sizeof(*(array).p_elems) ); } (array).p_elems[pos] = elem; (array).i_size++; } while(0)
...
#define _ARRAY_SHRINK(array) { if( (array).i_size > 10 && (array).i_size < (int)((array).i_alloc / 1.5) ) { _ARRAY_ALLOC(array, (array).i_size + 5); } }
...
#define ARRAY_REMOVE(array,pos) do { if( (array).i_size - (pos) - 1 ) { memmove( (array).p_elems + (pos), (array).p_elems + (pos) + 1, ( (array).i_size - (pos) - 1 ) *sizeof(*(array).p_elems) );} (array).i_size--; _ARRAY_SHRINK(array); } while(0)
...
#define ARRAY_FOREACH(item, array) for (int array_index_##item = 0; array_index_##item < (array).i_size && ((item) = (array).p_elems[array_index_##item], 1); ++array_index_##item)
...
#define vlc_array_item_at_index(ar, idx) _Generic((ar), const vlc_array_t *: ((ar)->pp_elems[idx]), vlc_array_t *: ((ar)->pp_elems[idx]))
