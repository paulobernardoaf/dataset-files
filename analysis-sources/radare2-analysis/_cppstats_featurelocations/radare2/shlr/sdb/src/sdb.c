...
#define BUCKET_FOREACH_SAFE(ht, bt, j, count, kv) if ((bt)->arr) for ((j) = 0, (kv) = (SdbKv *)(bt)->arr, (count) = (ht)->count; (j) < (bt)->count; (j) = (count) == (ht)->count? j + 1: j, (kv) = (count) == (ht)->count? next_kv (ht, kv): kv, (count) = (ht)->count)
...
sdb_ht_insert_kvp (s->ht, kv, true );
free (kv);
sdb_hook_call (s, key, val);
return cas;
}
