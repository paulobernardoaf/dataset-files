...
#define DEFINE_CMD_DESCRIPTOR(core, cmd_) { RCmdDescriptor *d = R_NEW0 (RCmdDescriptor); if (d) { d->cmd = #cmd_; d->help_msg = help_msg_##cmd_; r_list_append ((core)->cmd_descriptors, d); } }
...
#define DEFINE_CMD_DESCRIPTOR_WITH_DETAIL(core, cmd_) { RCmdDescriptor *d = R_NEW0 (RCmdDescriptor); if (d) { d->cmd = #cmd_; d->help_msg = help_msg_##cmd_; d->help_detail = help_detail_##cmd_; r_list_append ((core)->cmd_descriptors, d); } }
...
#define DEFINE_CMD_DESCRIPTOR_WITH_DETAIL2(core, cmd_) { RCmdDescriptor *d = R_NEW0 (RCmdDescriptor); if (d) { d->cmd = #cmd_; d->help_msg = help_msg_##cmd_; d->help_detail = help_detail_##cmd_; d->help_detail2 = help_detail2_##cmd_; r_list_append ((core)->cmd_descriptors, d); } }
...
#define DEFINE_CMD_DESCRIPTOR_SPECIAL(core, cmd_, named_cmd) { RCmdDescriptor *d = R_NEW0 (RCmdDescriptor); if (d) { d->cmd = #cmd_; d->help_msg = help_msg_##named_cmd; r_list_append ((core)->cmd_descriptors, d); } }
...
#if __WINDOWS__
#include <tchar.h>
#define __CLOSE_DUPPED_PIPES() close (1); close (fd_out); fd_out = -1;
...
#define TS_START_END(node, start, end) do { start = ts_node_start_byte (node); end = ts_node_end_byte (node); } while (0)
...
#define DEFINE_IS_TS_FCN(name) TSSymbol ts_##name##_symbol; static inline bool is_ts_##name(TSNode node) { return ts_node_symbol (node) == ts_##name##_symbol; }





#define DEFINE_HANDLE_TS_FCN(name) DEFINE_IS_TS_FCN (name) static bool handle_ts_##name##_internal(struct tsr2cmd_state *state, TSNode node, char *node_string); static bool handle_ts_##name(struct tsr2cmd_state *state, TSNode node) { char *node_string = ts_node_sub_string (node, state->input); R_LOG_DEBUG (#name ": '%s'\n", node_string); bool res = handle_ts_##name##_internal (state, node, node_string); free (node_string); return res; } static bool handle_ts_##name##_internal(struct tsr2cmd_state *state, TSNode node, char *node_string)
