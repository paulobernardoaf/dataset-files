...
#define printline(k, fmt, arg){ if (use_color)r_cons_printf ("%s%s: " Color_RESET, color, k);elser_cons_printf ("%s: ", k);if (fmt) r_cons_printf (fmt, arg);}
...
r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, 16, mode2, use_color); 
if (mode == 'j') {
r_cons_printf (",");
}
}
...
if (pcbits) {
r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, pcbits, mode, use_color); 
}
}
...
break;
case 'd': 
r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, bits, 3, use_color); 
break;
case 'o': 
r_reg_arena_swap (core->dbg->reg, false);
r_debug_reg_list (core->dbg, R_REG_TYPE_GPR, bits, 0, use_color); 
r_reg_arena_swap (core->dbg->reg, false);
break;
case '=': 
{
...
ret = r_anal_op (core->anal, &op, pc, buf + pc - bb->addr, left, R_ANAL_OP_MASK_HINT | R_ANAL_OP_MASK_ESIL); 
opskip = false;
switch (op.type) {
case R_ANAL_OP_TYPE_CALL:
case R_ANAL_OP_TYPE_RET:
