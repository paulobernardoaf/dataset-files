...
#if (__linux__ && __GNU_LIBRARY__) || defined(NETBSD_WITH_BACKTRACE) || defined(FREEBSD_WITH_BACKTRACE) || __DragonFly__
...
#if defined(_MSC_VER)
#define R_SYS_ASM_START_ROP() eprintf ("r_sys_run_rop: Unsupported arch\n");
...
#elif __i386__
#if defined(_MSC_VER)
#define R_SYS_ASM_START_ROP() __asm { __asm lea esp, bufptr__asm ret}
...
#else
#define R_SYS_ASM_START_ROP() __asm__ __volatile__ ("leal %0, %%esp; ret" : : "m" (*bufptr));
...
#if (__linux__ && __GNU_LIBRARY__) || (__APPLE__ && APPLE_WITH_BACKTRACE) || defined(NETBSD_WITH_BACKTRACE) || defined(FREEBSD_WITH_BACKTRACE) || __DragonFly__
