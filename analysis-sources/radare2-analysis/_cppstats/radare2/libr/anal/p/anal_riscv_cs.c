...
#define SET_VAL(op,i) if ((i)<OPCOUNT() && OPERAND(i).type == RISCV_OP_IMM) {(op)->val = OPERAND(i).imm;}
...
#define SET_SRC_DST_3_REG_OR_IMM(op) if (OPERAND(2).type == RISCV_OP_IMM) {SET_SRC_DST_3_IMM (op);} else if (OPERAND(2).type == RISCV_OP_REG) {SET_SRC_DST_3_REGS (op);}
...
#endif
#define ES_SIGN_EXT64(arg) arg",0x80000000,&,0,<,?{,""0xffffffff00000000,"arg",|=,""}"
