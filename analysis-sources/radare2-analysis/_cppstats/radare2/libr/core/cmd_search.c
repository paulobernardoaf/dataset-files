...
const ut8 crop = r_config_get_i (core->config, "rop.conditional"); 
...
RList *end_list = r_list_newf (free);
RList *rx_list = NULL;
...
r_reg_set_value (core->dbg->reg, reg_a0, -2); 
return sysno;
}
#endif
...
#endif
scVector = (aop.val > 0)? aop.val: -1; 
RSyscallItem *item = r_syscall_get (core->anal->syscall, scNumber, scVector);
if (item) {
r_cons_printf ("0x%08"PFMT64x" %s\n", at, item->name);
}
memset (previnstr, 0, (MAXINSTR + 1) * sizeof (*previnstr)); 
if (searchflags) {
