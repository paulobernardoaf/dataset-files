...
SETICB ("anal.depth", 64, &cb_analdepth, "Max depth at code analysis"); 
...
SETCB ("anal.recont", "false", &cb_analrecont, "End block after splitting a basic block instead of error"); 
SETCB ("anal.jmp.indir", "false", &cb_analijmp, "Follow the indirect jumps in function analysis"); 
...
#else
if (r_file_exists ("/usr/bin/openURL")) { 
