...
if (fcn) {
switch (str[2]) {
case 'B': return fcn->addr; 
case 'E': return r_anal_function_max_addr (fcn); 
...
case 'j': return bbJump (fcn, core->offset); 
case 'f': return bbFail (fcn, core->offset); 
}
return fcn->addr;
}
return 0;
...
free (home);
} else if (lpath[0] == '.' || lpath[0] == R_SYS_DIR[0] ) { 
dirname = r_str_newf ("%s%s", lpath, R_SYS_DIR);
} else { 
char *fmt = ".%s%s%s";
#if __WINDOWS__
...
if (type & R_ANAL_ADDR_TYPE_EXEC) {
return core->cons->context->pal.ai_exec; 
}
if (type & R_ANAL_ADDR_TYPE_WRITE) {
return core->cons->context->pal.ai_write; 
}
if (type & R_ANAL_ADDR_TYPE_READ) {
return core->cons->context->pal.ai_read; 
}
if (type & R_ANAL_ADDR_TYPE_SEQUENCE) {
return core->cons->context->pal.ai_seq; 
}
if (type & R_ANAL_ADDR_TYPE_ASCII) {
return core->cons->context->pal.ai_ascii; 
}
return NULL;
}
...
core->sdb = sdb_new (NULL, "r2kv.sdb", 0); 
...
core->assembler->syscall = r_syscall_ref (core->anal->syscall); 
...
switch ((ut8)cmd) {
case RMT_OPEN:
r_socket_read_block (c, &flg, 1); 
eprintf ("open (%d): ", cmd);
r_socket_read_block (c, &cmd, 1); 
pipefd = -1;
ptr = malloc (cmd + 1);
if (!ptr) {
