...
#define DS_COMMENT_FUNC(name, align, nl) static void ds_comment_##name(RDisasmState *ds, const char *format, ...) { va_list ap; va_start (ap, format); ds_comment_ (ds, align, nl, format, ap); va_end (ap); }
...
ds->colorop = r_config_get_i (core->config, "scr.color.ops"); 
...
ds->adistrick = r_config_get_i (core->config, "asm.middle"); 
...
r_cons_printf (FLAG_PREFIX "default:"); 
...
if (ds->cyclespace) {
char spaces [32];
int times = R_MIN (ds->analop.cycles/4, 30); 
memset (spaces, ' ', sizeof (spaces));
spaces[times] = 0;
...
if (!f) {
f = fcnIn (ds, at, R_ANAL_FCN_TYPE_NULL); 
}
if (f) {
delta = at - f->addr;
sfi.name = f->name;
...
pj_end (pj);
i += ds->oplen + asmop.payload + (ds->asmop.payload % ds->core->assembler->dataalign); 
k += ds->oplen + asmop.payload + (ds->asmop.payload % ds->core->assembler->dataalign); 
j++; 
line++;
