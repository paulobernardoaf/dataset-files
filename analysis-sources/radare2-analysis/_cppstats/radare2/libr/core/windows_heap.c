...
#define CHECK_INFO(heapInfo)if (!heapInfo) {eprintf ("It wasn't possible to get the heap information\n");return;}if (!heapInfo->count) {r_cons_print ("No heaps for this process\n");return;}
#define UPDATE_FLAGS(hb, flags)if (((flags) & 0xf1) || ((flags) & 0x0200)) {hb->dwFlags = LF32_FIXED;} else if ((flags) & 0x20) {hb->dwFlags = LF32_MOVEABLE;} else if ((flags) & 0x0100) {hb->dwFlags = LF32_FREE;}hb->dwFlags |= ((flags) >> SHIFT) << SHIFT;
...
#define GROW_BLOCKS()if (allocated <= count * sizeof (HeapBlockBasicInfo)) {SIZE_T old_alloc = allocated;allocated *= 2;PVOID tmp = blocks;blocks = realloc (blocks, allocated);if (!blocks) {blocks = tmp;goto err;}memset ((BYTE *)blocks + old_alloc, 0, old_alloc);}
#define GROW_PBLOCKS()if (*allocated <= *count * sizeof (HeapBlockBasicInfo)) {SIZE_T old_alloc = *allocated;*allocated *= 2;PVOID tmp = *blocks;tmp = realloc (*blocks, *allocated);if (!tmp) {return false;}*blocks = tmp;memset ((BYTE *)(*blocks) + old_alloc, 0, old_alloc);}
...
WPARAM segSignature;
ReadProcessMemory (h_proc, (PVOID)(pgSegOff + sizeof (LIST_ENTRY)), &segSignature, sizeof (WPARAM), NULL); 
