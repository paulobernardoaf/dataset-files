...
#define run_action(n,x,y) {if (action&(x)) {if (isradjson) r_cons_printf ("%s\"%s\":",actions_done?",":"",n);if (!r_core_bin_info (&core, y, rad, va, &filter, chksum)) {if (isradjson) r_cons_print ("false");};actions_done++;}}
core.bin = bin;
