...
#define CLOSE_OVERLAPPED(ovl) do { if (port->ovl.hEvent != INVALID_HANDLE_VALUE && CloseHandle (port->ovl.hEvent) == 0) return -1; } while (0)
...
#define INIT_OVERLAPPED(ovl) do { memset (&port->ovl, 0, sizeof (port->ovl)); port->ovl.hEvent = INVALID_HANDLE_VALUE; if ((port->ovl.hEvent = CreateEvent (NULL, TRUE, TRUE, NULL)) == INVALID_HANDLE_VALUE) { sp_close (port); return -1; } } while (0)
