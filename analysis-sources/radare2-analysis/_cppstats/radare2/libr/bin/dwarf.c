...
hdr->unit_length.part2 = READ32 (buf); 
}
...
#define READ_BUF(x,y) if (idx+sizeof(y)>=len) { return false;} (x)=*(y*)buf; idx+=sizeof(y);buf+=sizeof(y)
#define READ_BUF64(x) if (idx+sizeof(ut64)>=len) { return false;} (x)=r_read_ble64(buf, 0); idx+=sizeof(ut64);buf+=sizeof(ut64)
#define READ_BUF32(x) if (idx+sizeof(ut32)>=len) { return false;} (x)=r_read_ble32(buf, 0); idx+=sizeof(ut32);buf+=sizeof(ut32)
#define READ_BUF16(x) if (idx+sizeof(ut16)>=len) { return false;} (x)=r_read_ble16(buf, 0); idx+=sizeof(ut16);buf+=sizeof(ut16)
...
list = r_list_newf (r_bin_dwarf_row_free); 
if (!list) {
free (buf);
return NULL;
}
