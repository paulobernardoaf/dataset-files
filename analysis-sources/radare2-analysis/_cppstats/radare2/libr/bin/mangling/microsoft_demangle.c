...
#define SET_OPERATOR_CODE(str) { str_info = (SStrInfo *) malloc (sizeof(SStrInfo)); if (!str_info) break; str_info->len = strlen (str); str_info->str_ptr = str; r_list_append (names_l, str_info); }
SStrInfo *str_info;
int read_len = 1;
switch (*++buf) {
...
#define GO_TO_NEXT_STATE(state, new_state) { (state)->amount_of_read_chars++; (state)->buff_for_parsing++; (state)->state = eTCStateEnd; }
#define ONE_LETTER_ACTIION(action, type) static void tc_state_##action(SStateInfo *state, STypeCodeStr *type_code_str) { if (copy_string(type_code_str, type, 0) == 0) { state->err = eTCStateMachineErrAlloc; } state->state = eTCStateEnd; } 
...
DEF_STATE_ACTION(_)
{
#define PROCESS_CASE(letter, type_str) case CHAR_WITH_QUOTES(letter): copy_string(type_code_str, type_str, 0); break;
...
#define GET_USER_DEF_TYPE_NAME(data_struct_str) { copy_string (type_code_str, data_struct_str, 0); check_len = get_namespace_and_name (state->buff_for_parsing, type_code_str, 0); if (check_len) { state->amount_of_read_chars += check_len + 1; state->buff_for_parsing += check_len + 1; } else { state->err = eTCStateMachineErrUncorrectTypeCode; } }
DEF_STATE_ACTION(T)
{
#define PROCESS_CASE(case_string, type_str) { check_len = strlen (case_string); if ((check_len < buff_len) && (strncmp (state->buff_for_parsing, case_string, check_len) == 0)) { copy_string (type_code_str, type_str, 0); state->buff_for_parsing += check_len; state->amount_of_read_chars += check_len; return; } }
...
DEF_STATE_ACTION(U)
{
#define PROCESS_CASE(case_string, type_str) { check_len = strlen (case_string); if ((check_len < buff_len) && (strncmp (state->buff_for_parsing, case_string, check_len) == 0)) { copy_string (type_code_str, type_str, 0); state->amount_of_read_chars += check_len; state->buff_for_parsing += check_len; return; } }
int buff_len = strlen (state->buff_for_parsing);
int check_len = 0;
state->state = eTCStateEnd;
...
#define ONE_LETTER_STATE(letter) case CHAR_WITH_QUOTES(letter): state->state = eTCState##letter; break; 
switch (*(state->buff_for_parsing)) {
ONE_LETTER_STATE (X)
ONE_LETTER_STATE (D)
ONE_LETTER_STATE (C)
...
#define SET_STORAGE_CLASS(letter, storage_class_str) { case letter: storage_class = storage_class_str; break; }
switch (*curr_pos) {
...
err = eDemanglerErrUnsupportedMangling;
break;
#define SET_ACCESS_MODIFIER(letter, flag_set, modifier_str) { case letter: access_modifier = modifier_str; (flag_set) = 1; break; }
