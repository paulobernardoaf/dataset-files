...
if (bin->nsects > 0) {
int last_section = R_MIN (bin->nsects, 128); 
...
addr += ULEB();
break;
#define DO_BIND() do {if ((sym_ord < 0 && !sym_name) || seg_idx < 0 ) break;if (!addr) break;struct reloc_t *reloc = R_NEW0 (struct reloc_t);reloc->addr = addr;reloc->offset = addr - bin->segs[seg_idx].vmaddr + bin->segs[seg_idx].fileoff;if (type == BIND_TYPE_TEXT_PCREL32)reloc->addend = addend - (bin->baddr + addr);elsereloc->addend = addend; reloc->ord = lib_ord;reloc->ord = sym_ord;reloc->type = rel_type;if (sym_name)r_str_ncpy (reloc->name, sym_name, 256);r_skiplist_insert (relocs, reloc);} while (0)
...
if (hdr->cputype == CPU_TYPE_ARM64_32) { 
return 64;
}
...
#define READWORD() if (r_buf_read_at (buf, addr, (ut8*)wordbuf, 4) != 4) { eprintf ("Invalid address in buffer."); break; } addr += 4; pvaddr += 4;word = isBe? r_read_be32 (wordbuf): r_read_le32 (wordbuf);
...
#if R_BIN_MACH064
ut64 ep = r_read_ble64 (&data, false); 
cb_printf ("0x%08"PFMT64x" entry0 0x%" PFMT64x "\n", pvaddr, ep);
ut64 ss = r_read_ble64 (&data[8], false); 
cb_printf ("0x%08"PFMT64x" stacksize 0x%" PFMT64x "\n", pvaddr + 8, ss);
#else
ut32 ep = r_read_ble32 (&data, false); 
cb_printf ("0x%08"PFMT32x" entry0 0x%" PFMT32x "\n", (ut32)pvaddr, ep);
ut32 ss = r_read_ble32 (&data[4], false); 
