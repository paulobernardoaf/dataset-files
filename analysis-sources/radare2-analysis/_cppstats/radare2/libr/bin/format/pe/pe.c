...
if (r_buf_read_at (bin->b, sym_tbl_off + idx + shsz, (ut8*) name, 128)) { 
longname = name;
name[sizeof(name) - 1] = 0;
...
#define EXIT_ON_OVERFLOW(S)if (curAddr > bin->size || curAddr + (S) > bin->size) { goto out_error; }
...
dbg_dir = &bin->nt_headers->optional_header.DataDirectory[6 ];
