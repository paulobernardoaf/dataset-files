...
static bool simplifiedDemangling = false; 
...
char *method_name = dex_method_name (dex, MI);
...
s = getstr (dex, dex->types[handler_type].descriptor_id);
if (dexdump) {
cb_printf (
" %s "
"-> 0x%04"PFMT64x"\n",
...
ret->free = free;
ut64 addr = 0;
#define ROW(nam,siz,val,fmt) r_list_append (ret, r_bin_field_new (addr, addr, siz, nam, sdb_fmt ("0x%08"PFMT64x, (ut64)val), fmt, false)); addr += siz;
