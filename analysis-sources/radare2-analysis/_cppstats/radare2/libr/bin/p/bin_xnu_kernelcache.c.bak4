...
#define KEXT_SHORT_NAME_FROM_SECTION(io_section) ({char *result = NULL;char *clone = strdup (io_section->name);char *cursor = strstr (clone, "__");if (cursor) {cursor--;*cursor = 0;cursor--;cursor = strrchr (cursor, '.');if (cursor) {*cursor = 0;cursor = strrchr (cursor, '.');if (cursor) {result = strdup (cursor + 1);R_FREE (clone);}}}result ? result : clone;})
...
#define r_kext_index_foreach(index, i, item)if (index)for (i = 0; i < index->length && (item = index->entries[i], 1); i++)
