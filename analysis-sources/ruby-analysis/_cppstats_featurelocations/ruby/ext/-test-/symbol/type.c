...
#if defined(HAVE_RB_IS_CONST_NAME)
#define get_symbol_type(type, t, name) do { ID id = rb_check_id(&name); t = (id ? rb_is_##type##_id(id) : rb_is_##type##_name(name)); } while (0)



#else
#define get_symbol_type(type, t, name) do { t = rb_is_##type##_id(rb_to_id(name)); } while (0)


#endif

#define define_symbol_type_p(type) static VALUE bug_sym_##type##_p(VALUE self, VALUE name) { int t; get_symbol_type(type, t, name); return (t ? Qtrue : Qfalse); }
...
#define declare_symbol_type_p(type) rb_define_singleton_method(klass, #type"?", bug_sym_##type##_p, 1);
...
#if defined(HAVE_RB_PIN_DYNAMIC_SYMBOL)
