...
#define Check_Max_Args_(name, len, fmt) if ((size_t)(len) < MAX_ARGS) { } else { rb_raise(rb_eTypeError, name" is so large that it can cause integer overflow (%"fmt"d)", (len)); }
...
#if defined(HAVE_CONST_FFI_STDCALL)
