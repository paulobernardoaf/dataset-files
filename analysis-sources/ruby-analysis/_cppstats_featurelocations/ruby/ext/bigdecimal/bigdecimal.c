...
#if !defined(RRATIONAL_ZERO_P)
#define RRATIONAL_ZERO_P(x) (FIXNUM_P(rb_rational_num(x)) && FIX2LONG(rb_rational_num(x)) == 0)

#endif

#if !defined(RRATIONAL_NEGATIVE_P)
#define RRATIONAL_NEGATIVE_P(x) RTEST(rb_funcall((x), '<', 1, INT2FIX(0)))
#endif

#if !defined(DECIMAL_SIZE_OF_BITS)
...
#if !defined(HAVE_RB_RATIONAL_NUM)
static inline VALUE
rb_rational_num(VALUE rat)
{
#if defined(HAVE_TYPE_STRUCT_RRATIONAL)
...
#if !defined(HAVE_RB_RATIONAL_DEN)
static inline VALUE
...
#if defined(BIGDECIMAL_ENABLE_VPRINT)
...
#if defined(ENABLE_NUMERIC_STRING)
case T_STRING:
StringValueCStr(v);
...
#if defined(HAVE_RB_OPTS_EXCEPTION_P)
...
d = BigDecimal_mult(d, x); 
d = BigDecimal_div2(d, SSIZET2NUM(i), SSIZET2NUM(m)); 
...
#if !defined(RUBY_BIGDECIMAL_VERSION)
...
#define rmpd_set_thread_local_exception_mode(mode) rb_thread_local_aset( rb_thread_current(), id_BigDecimal_exception_mode, INT2FIX((int)(mode)) )
...
#define rmpd_set_thread_local_precision_limit(limit) rb_thread_local_aset( rb_thread_current(), id_BigDecimal_precision_limit, SIZET2NUM(limit) )
...
#define rmpd_set_thread_local_rounding_mode(mode) rb_thread_local_aset( rb_thread_current(), id_BigDecimal_rounding_mode, INT2FIX((int)(mode)) )
...
if (!VpIsDefOP(c, a, b, (operation > 0) ? OP_SW_ADD : OP_SW_SUB)) return 0; 
...
if (!VpIsDefOP(c, a, b, OP_SW_MULT)) return 0; 
