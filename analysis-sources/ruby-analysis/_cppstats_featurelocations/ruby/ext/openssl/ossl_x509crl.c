...
#define SetX509CRL(obj, crl) do { if (!(crl)) { ossl_raise(rb_eRuntimeError, "CRL wasn't initialized!"); } RTYPEDDATA_DATA(obj) = (crl); } while (0)





#define GetX509CRL(obj, crl) do { TypedData_Get_Struct((obj), X509_CRL, &ossl_x509crl_type, (crl)); if (!(crl)) { ossl_raise(rb_eRuntimeError, "CRL wasn't initialized!"); } } while (0)
...
return ossl_x509name_new(X509_CRL_get_issuer(crl)); 
}
...
if (!X509_CRL_set_issuer_name(crl, GetX509NamePtr(issuer))) { 
ossl_raise(eX509CRLError, NULL);
}
return issuer;
}
...
if (!X509_CRL_add0_revoked(crl, rev)) { 
X509_REVOKED_free(rev);
