...
#define SetX509Ext(obj, ext) do { if (!(ext)) { ossl_raise(rb_eRuntimeError, "EXT wasn't initialized!"); } RTYPEDDATA_DATA(obj) = (ext); } while (0)





#define GetX509Ext(obj, ext) do { TypedData_Get_Struct((obj), X509_EXTENSION, &ossl_x509ext_type, (ext)); if (!(ext)) { ossl_raise(rb_eRuntimeError, "EXT wasn't initialized!"); } } while (0)





#define MakeX509ExtFactory(klass, obj, ctx) do { (obj) = TypedData_Wrap_Struct((klass), &ossl_x509extfactory_type, 0); if (!((ctx) = OPENSSL_malloc(sizeof(X509V3_CTX)))) ossl_raise(rb_eRuntimeError, "CTX wasn't allocated!"); X509V3_set_ctx((ctx), NULL, NULL, NULL, NULL, 0); RTYPEDDATA_DATA(obj) = (ctx); } while (0)
...
#define GetX509ExtFactory(obj, ctx) do { TypedData_Get_Struct((obj), X509V3_CTX, &ossl_x509extfactory_type, (ctx)); if (!(ctx)) { ossl_raise(rb_eRuntimeError, "CTX wasn't initialized!"); } } while (0)
...
ctx->issuer_cert = GetX509CertPtr(cert); 
...
ctx->subject_cert = GetX509CertPtr(cert); 
...
ctx->subject_req = GetX509ReqPtr(req); 
