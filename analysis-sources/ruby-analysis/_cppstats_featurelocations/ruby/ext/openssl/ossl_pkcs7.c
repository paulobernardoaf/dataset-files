...
#define SetPKCS7si(obj, p7si) do { if (!(p7si)) { ossl_raise(rb_eRuntimeError, "PKCS7si wasn't initialized."); } RTYPEDDATA_DATA(obj) = (p7si); } while (0)





#define GetPKCS7si(obj, p7si) do { TypedData_Get_Struct((obj), PKCS7_SIGNER_INFO, &ossl_pkcs7_signer_info_type, (p7si)); if (!(p7si)) { ossl_raise(rb_eRuntimeError, "PKCS7si wasn't initialized."); } } while (0)
...
#define SetPKCS7ri(obj, p7ri) do { if (!(p7ri)) { ossl_raise(rb_eRuntimeError, "PKCS7ri wasn't initialized."); } RTYPEDDATA_DATA(obj) = (p7ri); } while (0)





#define GetPKCS7ri(obj, p7ri) do { TypedData_Get_Struct((obj), PKCS7_RECIP_INFO, &ossl_pkcs7_recip_info_type, (p7ri)); if (!(p7ri)) { ossl_raise(rb_eRuntimeError, "PKCS7ri wasn't initialized."); } } while (0)
...
key = GetPrivPKeyPtr(pkey); 
x509 = NIL_P(cert) ? NULL : GetX509CertPtr(cert); 
flg = NIL_P(flags) ? 0 : NUM2INT(flags);
GetPKCS7(self, p7);
if(!(out = BIO_new(BIO_s_mem())))
ossl_raise(ePKCS7Error, NULL);
