...
#define SetX509Name(obj, name) do { if (!(name)) { ossl_raise(rb_eRuntimeError, "Name wasn't initialized."); } RTYPEDDATA_DATA(obj) = (name); } while (0)





#define GetX509Name(obj, name) do { TypedData_Get_Struct((obj), X509_NAME, &ossl_x509name_type, (name)); if (!(name)) { ossl_raise(rb_eRuntimeError, "Name wasn't initialized."); } } while (0)
