...
#define EACH_FEATURES(X, SEP) X(gems) SEP X(did_you_mean) SEP X(rubyopt) SEP X(frozen_string_literal) SEP X(jit) 
...
#define EACH_DEBUG_FEATURES(X, SEP) X(frozen_string_literal) 


#define AMBIGUOUS_FEATURE_NAMES 0 
...
#define EACH_DUMPS(X, SEP) X(version) SEP X(copyright) SEP X(usage) SEP X(help) SEP X(yydebug) SEP X(syntax) SEP X(parsetree) SEP X(parsetree_with_comment) SEP X(insns) 
...
#define M(shortopt, longopt, desc) { shortopt " " longopt " " desc, (unsigned short)sizeof(shortopt), (unsigned short)sizeof(longopt), }
...
#define UNSET_WHEN(name, bit, str, len) if (NAME_MATCH_P((name), (str), (len))) { *(unsigned int *)arg &= ~(bit); return; }





#define SET_WHEN(name, bit, str, len) if (NAME_MATCH_P((name), (str), (len))) { *(unsigned int *)arg |= (bit); return; }
...
#define ADD_FEATURE_NAME(bit) if (FEATURE_BIT(bit) & set) { rb_str_cat_cstr(mesg, #bit); if (--matched) rb_str_cat_cstr(mesg, ", "); }
...
ruby_features_t *argp = arg;
#define SET_WHEN_DEBUG(bit) if (NAME_MATCH_P(#bit, str, len)) { FEATURE_SET(*argp, DEBUG_BIT(bit)); return; }
...
#define check_envopt(name, allow_envopt) (((allow_envopt) || !envopt) ? (void)0 : rb_raise(rb_eRuntimeError, "invalid switch in RUBYOPT: --" name))


#define need_argument(name, s, needs_arg, next_arg) ((*(s) ? !*++(s) : (next_arg) && (!argc || !((s) = argv[1]) || (--argc, ++argv, 0))) && (needs_arg) ? rb_raise(rb_eRuntimeError, "missing argument for --" name) : (void)0)
...
#define is_option_with_optarg(name, allow_hyphen, allow_envopt, needs_arg, next_arg) (strncmp((name), s, n = sizeof(name) - 1) == 0 && is_option_end(s[n], (allow_hyphen)) ? (check_envopt(name, (allow_envopt)), s += n, need_argument(name, s, needs_arg, next_arg), 1) : 0)
...
encoding:
do {
#define set_encoding_part(type) if (!(p = strchr(s, ':'))) { set_##type##_encoding_once(opt, s, 0); break; } else if (p > s) { set_##type##_encoding_once(opt, s, p-s); }
...
#define SET_COMPILE_OPTION(h, o, name) rb_hash_aset((h), ID2SYM(rb_intern_const(#name)), (FEATURE_SET_P(o->features, FEATURE_BIT(name)) ? Qtrue : Qfalse));
