...
#if defined(HAVE_TRUE_LONG_LONG)
...
#define PACK_LENGTH_ADJUST_SIZE(sz) do { tmp_len = 0; if (len > (long)((send-s)/(sz))) { if (!star) { tmp_len = len-(send-s)/(sz); } len = (send-s)/(sz); } } while (0)
...
#define PACK_ITEM_ADJUST() do { if (tmp_len > 0 && mode == UNPACK_ARRAY) rb_ary_store(ary, RARRAY_LEN(ary)+tmp_len-1, Qnil); } while (0)
...
#define UNPACK_PUSH(item) do {VALUE item_val = (item);if ((mode) == UNPACK_BLOCK) {rb_yield(item_val);}else if ((mode) == UNPACK_ARRAY) {rb_ary_push(ary, item_val);}else {return item_val; }} while (0)
...
case 'm':
{
VALUE buf = rb_str_new(0, (send - s + 3)*3/4); 
char *ptr = RSTRING_PTR(buf);
int a = -1,b = -1,c = 0,d = 0;
