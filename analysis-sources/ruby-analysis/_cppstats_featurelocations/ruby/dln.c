...
#define init_funcname(buf, file) do {const char *base = (file);const size_t flen = init_funcname_len(&base);const size_t plen = sizeof(funcname_prefix);char *const tmp = ALLOCA_N(char, plen+flen+1);if (!tmp) {dln_memerror();}memcpy(tmp, funcname_prefix, plen);memcpy(tmp+plen, base, flen);tmp[plen+flen] = '\0';*(buf) = tmp;} while (0)
...
#if !defined(LIBC_NAME)
#define LIBC_NAME "libc.a"
#endif

#if !defined(DLN_DEFAULT_LIB_PATH)
...
#define DLN_ENOEXEC ENOEXEC 
#define DLN_ECONFL 1201 
#define DLN_ENOINIT 1202 
#define DLN_EUNDEF 1203 
#define DLN_ENOTLIB 1204 
#define DLN_EBADLIB 1205 
#define DLN_EINIT 1206 
...
#define R_PCREL(r) ((r)->r_type >= RELOC_DISP8 && (r)->r_type <= RELOC_WDISP22)
...
#if !defined(NSLINKMODULE_OPTION_BINDNOW)
...
#define format_message(sublang) FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, error, MAKELANGID(LANG_NEUTRAL, (sublang)), message + len, size - len, NULL)
...
#define translit_separator(src) do { char *tmp = ALLOCA_N(char, strlen(src) + 1), *p = tmp, c; do { *p++ = ((c = *file++) == '/') ? DLN_NEEDS_ALT_SEPARATOR : c; } while (c); (src) = tmp; } while (0)
...
#if defined __APPLE__ && defined(MAC_OS_X_VERSION_MIN_REQUIRED) && (MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_11)
