...
static inline struct optional
size_mul_add_overflow(size_t x, size_t y, size_t z) 
{
...
static inline struct optional
size_mul_add_mul_overflow(size_t x, size_t y, size_t z, size_t w) 
{
struct optional t = size_mul_overflow(x, y);
...
#if !defined(GC_HEAP_INIT_SLOTS)
#define GC_HEAP_INIT_SLOTS 10000
#endif
#if !defined(GC_HEAP_FREE_SLOTS)
#define GC_HEAP_FREE_SLOTS 4096
#endif
#if !defined(GC_HEAP_GROWTH_FACTOR)
#define GC_HEAP_GROWTH_FACTOR 1.8
#endif
#if !defined(GC_HEAP_GROWTH_MAX_SLOTS)
#define GC_HEAP_GROWTH_MAX_SLOTS 0 
#endif
#if !defined(GC_HEAP_OLDOBJECT_LIMIT_FACTOR)
#define GC_HEAP_OLDOBJECT_LIMIT_FACTOR 2.0
#endif

#if !defined(GC_HEAP_FREE_SLOTS_MIN_RATIO)
#define GC_HEAP_FREE_SLOTS_MIN_RATIO 0.20
#endif
#if !defined(GC_HEAP_FREE_SLOTS_GOAL_RATIO)
#define GC_HEAP_FREE_SLOTS_GOAL_RATIO 0.40
#endif
#if !defined(GC_HEAP_FREE_SLOTS_MAX_RATIO)
#define GC_HEAP_FREE_SLOTS_MAX_RATIO 0.65
#endif

#if !defined(GC_MALLOC_LIMIT_MIN)
#define GC_MALLOC_LIMIT_MIN (16 * 1024 * 1024 )
#endif
#if !defined(GC_MALLOC_LIMIT_MAX)
#define GC_MALLOC_LIMIT_MAX (32 * 1024 * 1024 )
#endif
#if !defined(GC_MALLOC_LIMIT_GROWTH_FACTOR)
#define GC_MALLOC_LIMIT_GROWTH_FACTOR 1.4
#endif

#if !defined(GC_OLDMALLOC_LIMIT_MIN)
#define GC_OLDMALLOC_LIMIT_MIN (16 * 1024 * 1024 )
#endif
#if !defined(GC_OLDMALLOC_LIMIT_GROWTH_FACTOR)
#define GC_OLDMALLOC_LIMIT_GROWTH_FACTOR 1.2
#endif
#if !defined(GC_OLDMALLOC_LIMIT_MAX)
#define GC_OLDMALLOC_LIMIT_MAX (128 * 1024 * 1024 )
#endif

#if !defined(PRINT_MEASURE_LINE)
#define PRINT_MEASURE_LINE 0
#endif
#if !defined(PRINT_ENTER_EXIT_TICK)
#define PRINT_ENTER_EXIT_TICK 0
#endif
#if !defined(PRINT_ROOT_TICKS)
...
#if !defined(RGENGC_OLD_NEWOBJ_CHECK)
...
#if !defined(RGENGC_ESTIMATE_OLDMALLOC)
...
#if !defined(RGENGC_FORCE_MAJOR_GC)
...
#endif
#if !defined(GC_PROFILE_DETAIL_MEMORY)
#define GC_PROFILE_DETAIL_MEMORY 0
#endif
#if !defined(GC_ENABLE_INCREMENTAL_MARK)
#define GC_ENABLE_INCREMENTAL_MARK USE_RINCGC
#endif
#if !defined(GC_ENABLE_LAZY_SWEEP)
#define GC_ENABLE_LAZY_SWEEP 1
#endif
#if !defined(CALC_EXACT_MALLOC_SIZE)
#define CALC_EXACT_MALLOC_SIZE USE_GC_MALLOC_OBJ_INFO_DETAILS
#endif
#if defined(HAVE_MALLOC_USABLE_SIZE) || CALC_EXACT_MALLOC_SIZE > 0
#if !defined(MALLOC_ALLOCATED_SIZE)
#define MALLOC_ALLOCATED_SIZE 0
#endif
#else
#define MALLOC_ALLOCATED_SIZE 0
#endif
#if !defined(MALLOC_ALLOCATED_SIZE_CHECK)
#define MALLOC_ALLOCATED_SIZE_CHECK 0
#endif

#if !defined(GC_DEBUG_STRESS_TO_CLASS)
#define GC_DEBUG_STRESS_TO_CLASS 0
#endif

#if !defined(RGENGC_OBJ_INFO)
...
#define TYPED_UPDATE_IF_MOVED(_objspace, _type, _thing) do { if (gc_object_moved_p(_objspace, (VALUE)_thing)) { *((_type *)(&_thing)) = (_type)RMOVED((_thing))->destination; } } while (0)
...
#define PUSH_MARK_FUNC_DATA(v) do { struct mark_func_data_struct *prev_mark_func_data = objspace->mark_func_data; objspace->mark_func_data = (v);
...
#define MEASURE_LINE(expr) do { volatile tick_t start_time = tick(); volatile tick_t end_time; expr; end_time = tick(); fprintf(stderr, "0\t%"PRItick"\t%s\n", end_time - start_time, #expr); } while (0)
...
#define FL_CHECK2(name, x, pred) ((RGENGC_CHECK_MODE && SPECIAL_CONST_P(x)) ? (rb_bug(name": SPECIAL_CONST (%p)", (void *)(x)), 0) : (pred))
...
#define gc_event_hook(objspace, event, data) do { if (UNLIKELY(gc_event_hook_needed_p(objspace, event))) { gc_event_hook_body(GET_EC(), (objspace), (event), (data)); } } while (0)
...
#define UNEXPECTED_NODE(func) rb_bug(#func"(): GC does not handle T_NODE 0x%x(%p) 0x%"PRIxVALUE, BUILTIN_TYPE(obj), (void*)(obj), RBASIC(obj)->flags)
...
#endif
if ( !FL_TEST_RAW(obj, RHASH_ST_TABLE_FLAG)) {
...
#define RESTORE_FINALIZER() (ec->cfp = saved.cfp, rb_set_errinfo(saved.errinfo))
...
#else
#define STACK_LENGTH ((STACK_END < STACK_START) ? (size_t)(STACK_START - STACK_END) : (size_t)(STACK_END - STACK_START + 1))
...
#if !defined(PREVENT_STACK_OVERFLOW)
...
#else
#define GET_STACK_BOUNDS(start, end, appendix) ((STACK_END < STACK_START) ? ((start) = STACK_END, (end) = STACK_START) : ((start) = STACK_START, (end) = STACK_END+(appendix)))
...
#if PRINT_ROOT_TICKS
#define MARK_CHECKPOINT_PRINT_TICK(category) do { if (prev_category) { tick_t t = tick(); mark_ticks[tick_count] = t - start_tick; mark_ticks_categories[tick_count] = prev_category; tick_count++; } prev_category = category; start_tick = tick(); } while (0)
...
#define MARK_CHECKPOINT(category) do { if (categoryp) *categoryp = category; MARK_CHECKPOINT_PRINT_TICK(category); } while (0)
...
#if !defined(PROFILE_REMEMBERSET_MARK)
...
#if !defined(MARK_OBJECT_ARY_BUCKET_SIZE)
...
malloc_limit = (size_t)(malloc_limit * 0.98); 
...
if (!heap_allocated_pages) return FALSE; 
if (!(reason & GPR_FLAG_METHOD) && !ready_to_gc(objspace)) return TRUE; 
...
gc_event_hook(objspace, RUBY_INTERNAL_EVENT_GC_START, 0 );
GC_ASSERT(during_gc);
...
gc_event_hook(objspace, RUBY_INTERNAL_EVENT_GC_ENTER, 0); 
}
...
gc_event_hook(objspace, RUBY_INTERNAL_EVENT_GC_EXIT, 0); 
...
#define SET(name, attr) if (key == sym_##name) return (attr); else if (hash != Qnil) rb_hash_aset(hash, sym_##name, (attr));
...
#define SET(name, attr) if (key == gc_stat_symbols[gc_stat_sym_##name]) return attr; else if (hash != Qnil) rb_hash_aset(hash, gc_stat_symbols[gc_stat_sym_##name], SIZET2NUM(attr));
...
static VALUE
gc_stat(rb_execution_context_t *ec, VALUE self, VALUE arg) 
{
if (NIL_P(arg)) {
arg = rb_hash_new();
}
...
#define TRY_WITH_GC(alloc) do { objspace_malloc_gc_stress(objspace); if (!(alloc) && (!garbage_collect_with_gvl(objspace, GPR_FLAG_FULL_MARK | GPR_FLAG_IMMEDIATE_MARK | GPR_FLAG_IMMEDIATE_SWEEP | GPR_FLAG_MALLOC) || !(alloc))) { ruby_memerror(); } } while (0)
...
#if defined(ruby_sized_xrealloc)
...
#if defined(ruby_sized_xrealloc2)
...
void *
rb_xmalloc_mul_add(size_t x, size_t y, size_t z) 
{
...
void *
rb_xrealloc_mul_add(const void *p, size_t x, size_t y, size_t z) 
{
size_t w = size_mul_add_or_raise(x, y, z, rb_eArgError);
...
void *
rb_xmalloc_mul_add_mul(size_t x, size_t y, size_t z, size_t w) 
{
...
void *
rb_xcalloc_mul_add_mul(size_t x, size_t y, size_t z, size_t w) 
{
size_t u = size_mul_add_mul_or_raise(x, y, z, w, rb_eArgError);
...
#define C(x, s) if (reason & GPR_FLAG_MAJOR_BY_##x) { buff[i++] = #x[0]; if (i >= MAJOR_REASON_MAX) rb_bug("gc_profile_dump_major_reason: overflow"); buff[i] = 0; }
...
#define ARY_SHARED_P(ary) (GC_ASSERT(!FL_TEST((ary), ELTS_SHARED) || !FL_TEST((ary), RARRAY_EMBED_FLAG)), FL_TEST((ary),ELTS_SHARED)!=0)


#define ARY_EMBED_P(ary) (GC_ASSERT(!FL_TEST((ary), ELTS_SHARED) || !FL_TEST((ary), RARRAY_EMBED_FLAG)), FL_TEST((ary), RARRAY_EMBED_FLAG)!=0)
...
#if defined(ruby_xmalloc)
#undef ruby_xmalloc
#endif
#if defined(ruby_xmalloc2)
#undef ruby_xmalloc2
#endif
#if defined(ruby_xcalloc)
#undef ruby_xcalloc
#endif
#if defined(ruby_xrealloc)
#undef ruby_xrealloc
#endif
#if defined(ruby_xrealloc2)
