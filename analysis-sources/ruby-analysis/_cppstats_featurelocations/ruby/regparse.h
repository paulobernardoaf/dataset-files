...
#define IS_NODE_TYPE_SIMPLE(type) ((NTYPE2BIT(type) & (BIT_NT_STR | BIT_NT_CCLASS | BIT_NT_CTYPE |BIT_NT_CANY | BIT_NT_BREF)) != 0)



#define NTYPE(node) ((node)->u.base.type)
#define SET_NTYPE(node, ntype) do { int value = ntype; memcpy(&((node)->u.base.type), &value, sizeof(int)); } while (0)
...
#define SCANENV_MEM_NODES(senv) (IS_NOT_NULL((senv)->mem_nodes_dynamic) ? (senv)->mem_nodes_dynamic : (senv)->mem_nodes_static)
