...
#if !defined(USE_TOKEN_THREADED_VM)
#if defined(__GNUC__)
...
#define ONIGENC_IS_MBC_ASCII_WORD(enc,s,end) (rb_enc_asciicompat(enc) ? (ISALNUM(*s) || *s=='_') : onigenc_ascii_is_code_ctype( ONIGENC_MBC_TO_CODE(enc,s,end),ONIGENC_CTYPE_WORD,enc))



#endif 

#if defined(USE_CRNL_AS_LINE_TERMINATOR)
#define ONIGENC_IS_MBC_CRNL(enc,p,end) (ONIGENC_MBC_TO_CODE(enc,p,end) == 13 && ONIGENC_MBC_TO_CODE(enc,(p+enclen(enc,p,end)),end) == 10)
...
#else 
#define ONIGENC_IS_MBC_NEWLINE_EX(enc,p,start,end,option,check_prev) ONIGENC_IS_MBC_NEWLINE((enc), (p), (end))
...
#define STK_ABSENT_POS 0x0b00 
#define STK_ABSENT 0x0c00 
...
#define MATCH_ARG_INIT(msa, arg_option, arg_region, arg_start, arg_gpos) do {(msa).stack_p = (void* )0;(msa).options = (arg_option);(msa).region = (arg_region);(msa).start = (arg_start);(msa).gpos = (arg_gpos);(msa).best_len = ONIG_MISMATCH;} while(0)
...
#else
#define MATCH_ARG_INIT(msa, arg_option, arg_region, arg_start, arg_gpos) do {(msa).stack_p = (void* )0;(msa).options = (arg_option);(msa).region = (arg_region);(msa).start = (arg_start);(msa).gpos = (arg_gpos);} while(0)
...
#define MATCH_ARG_FREE(msa) do {if ((msa).stack_p) xfree((msa).stack_p);if ((msa).state_check_buff_size >= STATE_CHECK_BUFF_MALLOC_THRESHOLD_SIZE) { if ((msa).state_check_buff) xfree((msa).state_check_buff);}} while(0)
...
#define STACK_SAVE do{if (stk_base != stk_alloc) {msa->stack_p = stk_base;msa->stack_n = stk_end - stk_base; };} while(0)
...
#define STACK_ENSURE(n) do {if (stk_end - stk < (n)) {int r = stack_double(&stk_base, &stk_end, &stk, stk_alloc, msa);if (r != 0) {STACK_SAVE;if (xmalloc_base) xfree(xmalloc_base);return r;}}} while(0)
...
#define STACK_PUSH_TYPE(stack_type) do {STACK_ENSURE(1);stk->type = (stack_type);STACK_INC;} while(0)
...
#define STATE_CHECK_VAL(v,snum) do {if (state_check_buff != NULL) {int x = STATE_CHECK_POS(s,snum);(v) = state_check_buff[x/8] & (1<<(x%8));}else (v) = 0;} while(0)
...
#define ELSE_IF_STATE_CHECK_MARK(stk) else if ((stk)->type == STK_STATE_CHECK_MARK) { int x = STATE_CHECK_POS(stk->u.state.pstr, stk->u.state.state_check);state_check_buff[x/8] |= (1<<(x%8)); }





#define STACK_PUSH(stack_type,pat,s,sprev,keep) do {STACK_ENSURE(1);stk->type = (stack_type);stk->u.state.pcode = (pat);stk->u.state.pstr = (s);stk->u.state.pstr_prev = (sprev);stk->u.state.state_check = 0;stk->u.state.pkeep = (keep);STACK_INC;} while(0)
...
#define STACK_PUSH_ENSURED(stack_type,pat) do {stk->type = (stack_type);stk->u.state.pcode = (pat);stk->u.state.state_check = 0;STACK_INC;} while(0)
...
#define STACK_PUSH_ALT_WITH_STATE_CHECK(pat,s,sprev,snum,keep) do {STACK_ENSURE(1);stk->type = STK_ALT;stk->u.state.pcode = (pat);stk->u.state.pstr = (s);stk->u.state.pstr_prev = (sprev);stk->u.state.state_check = ((state_check_buff != NULL) ? (snum) : 0);stk->u.state.pkeep = (keep);STACK_INC;} while(0)
...
#define STACK_PUSH_STATE_CHECK(s,snum) do {if (state_check_buff != NULL) {STACK_ENSURE(1);stk->type = STK_STATE_CHECK_MARK;stk->u.state.pstr = (s);stk->u.state.state_check = (snum);STACK_INC;}} while(0)
...
#define STACK_PUSH(stack_type,pat,s,sprev,keep) do {STACK_ENSURE(1);stk->type = (stack_type);stk->u.state.pcode = (pat);stk->u.state.pstr = (s);stk->u.state.pstr_prev = (sprev);stk->u.state.pkeep = (keep);STACK_INC;} while(0)
...
#define STACK_PUSH_ENSURED(stack_type,pat) do {stk->type = (stack_type);stk->u.state.pcode = (pat);STACK_INC;} while(0)
...
#define STACK_PUSH_REPEAT(id, pat) do {STACK_ENSURE(1);stk->type = STK_REPEAT;stk->u.repeat.num = (id);stk->u.repeat.pcode = (pat);stk->u.repeat.count = 0;STACK_INC;} while(0)
...
#define STACK_PUSH_REPEAT_INC(sindex) do {STACK_ENSURE(1);stk->type = STK_REPEAT_INC;stk->u.repeat_inc.si = (sindex);STACK_INC;} while(0)
...
#define STACK_PUSH_MEM_START(mnum, s) do {STACK_ENSURE(1);stk->type = STK_MEM_START;stk->u.mem.num = (mnum);stk->u.mem.pstr = (s);stk->u.mem.start = mem_start_stk[mnum];stk->u.mem.end = mem_end_stk[mnum];mem_start_stk[mnum] = GET_STACK_INDEX(stk);mem_end_stk[mnum] = INVALID_STACK_INDEX;STACK_INC;} while(0)
...
#define STACK_PUSH_MEM_END(mnum, s) do {STACK_ENSURE(1);stk->type = STK_MEM_END;stk->u.mem.num = (mnum);stk->u.mem.pstr = (s);stk->u.mem.start = mem_start_stk[mnum];stk->u.mem.end = mem_end_stk[mnum];mem_end_stk[mnum] = GET_STACK_INDEX(stk);STACK_INC;} while(0)
...
#define STACK_PUSH_MEM_END_MARK(mnum) do {STACK_ENSURE(1);stk->type = STK_MEM_END_MARK;stk->u.mem.num = (mnum);STACK_INC;} while(0)
...
#define STACK_GET_MEM_START(mnum, k) do {int level = 0;k = stk;while (k > stk_base) {k--;if ((k->type & STK_MASK_MEM_END_OR_MARK) != 0 && k->u.mem.num == (mnum)) {level++;}else if (k->type == STK_MEM_START && k->u.mem.num == (mnum)) {if (level == 0) break;level--;}}} while(0)
...
#define STACK_GET_MEM_RANGE(k, mnum, start, end) do {int level = 0;while (k < stk) {if (k->type == STK_MEM_START && k->u.mem.num == (mnum)) {if (level == 0) (start) = k->u.mem.pstr;level++;}else if (k->type == STK_MEM_END && k->u.mem.num == (mnum)) {level--;if (level == 0) {(end) = k->u.mem.pstr;break;}}k++;}} while(0)
...
#define STACK_PUSH_NULL_CHECK_START(cnum, s) do {STACK_ENSURE(1);stk->type = STK_NULL_CHECK_START;stk->u.null_check.num = (cnum);stk->u.null_check.pstr = (s);STACK_INC;} while(0)
...
#define STACK_PUSH_NULL_CHECK_END(cnum) do {STACK_ENSURE(1);stk->type = STK_NULL_CHECK_END;stk->u.null_check.num = (cnum);STACK_INC;} while(0)
...
#define STACK_PUSH_CALL_FRAME(pat) do {STACK_ENSURE(1);stk->type = STK_CALL_FRAME;stk->u.call_frame.ret_addr = (pat);STACK_INC;} while(0)
...
#define STACK_PUSH_RETURN do {STACK_ENSURE(1);stk->type = STK_RETURN;STACK_INC;} while(0)





#define STACK_PUSH_ABSENT_POS(start, end) do {STACK_ENSURE(1);stk->type = STK_ABSENT_POS;stk->u.absent_pos.abs_pstr = (start);stk->u.absent_pos.end_pstr = (end);STACK_INC;} while(0)
...
#define STACK_BASE_CHECK(p, at) if ((p) < stk_base) {fprintf(stderr, "at %s\n", at);goto stack_error;}
...
#define STACK_POP_TIL_POS_NOT do {while (1) {stk--;STACK_BASE_CHECK(stk, "STACK_POP_TIL_POS_NOT"); if (stk->type == STK_POS_NOT) break;else if (stk->type == STK_MEM_START) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}else if (stk->type == STK_REPEAT_INC) {STACK_AT(stk->u.repeat_inc.si)->u.repeat.count--;}else if (stk->type == STK_MEM_END) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}ELSE_IF_STATE_CHECK_MARK(stk);}} while(0)
...
#define STACK_POP_TIL_LOOK_BEHIND_NOT do {while (1) {stk--;STACK_BASE_CHECK(stk, "STACK_POP_TIL_LOOK_BEHIND_NOT"); if (stk->type == STK_LOOK_BEHIND_NOT) break;else if (stk->type == STK_MEM_START) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}else if (stk->type == STK_REPEAT_INC) {STACK_AT(stk->u.repeat_inc.si)->u.repeat.count--;}else if (stk->type == STK_MEM_END) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}ELSE_IF_STATE_CHECK_MARK(stk);}} while(0)
...
#define STACK_POP_TIL_ABSENT do {while (1) {stk--;STACK_BASE_CHECK(stk, "STACK_POP_TIL_ABSENT"); if (stk->type == STK_ABSENT) break;else if (stk->type == STK_MEM_START) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}else if (stk->type == STK_REPEAT_INC) {STACK_AT(stk->u.repeat_inc.si)->u.repeat.count--;}else if (stk->type == STK_MEM_END) {mem_start_stk[stk->u.mem.num] = stk->u.mem.start;mem_end_stk[stk->u.mem.num] = stk->u.mem.end;}ELSE_IF_STATE_CHECK_MARK(stk);}} while(0)
...
#define STACK_POP_ABSENT_POS(start, end) do {stk--;STACK_BASE_CHECK(stk, "STACK_POP_ABSENT_POS"); (start) = stk->u.absent_pos.abs_pstr;(end) = stk->u.absent_pos.end_pstr;} while(0)
...
#define STACK_POS_END(k) do {k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_POS_END"); if (IS_TO_VOID_TARGET(k)) {k->type = STK_VOID;}else if (k->type == STK_POS) {k->type = STK_VOID;break;}}} while(0)
...
#define STACK_STOP_BT_END do {OnigStackType *k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_STOP_BT_END"); if (IS_TO_VOID_TARGET(k)) {k->type = STK_VOID;}else if (k->type == STK_STOP_BT) {k->type = STK_VOID;break;}}} while(0)
...
#define STACK_NULL_CHECK(isnull,id,s) do {OnigStackType* k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_NULL_CHECK"); if (k->type == STK_NULL_CHECK_START) {if (k->u.null_check.num == (id)) {(isnull) = (k->u.null_check.pstr == (s));break;}}}} while(0)
...
#define STACK_NULL_CHECK_REC(isnull,id,s) do {int level = 0;OnigStackType* k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_NULL_CHECK_REC"); if (k->type == STK_NULL_CHECK_START) {if (k->u.null_check.num == (id)) {if (level == 0) {(isnull) = (k->u.null_check.pstr == (s));break;}else level--;}}else if (k->type == STK_NULL_CHECK_END) {level++;}}} while(0)
...
#define STACK_NULL_CHECK_MEMST(isnull,id,s,reg) do {OnigStackType* k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_NULL_CHECK_MEMST"); if (k->type == STK_NULL_CHECK_START) {if (k->u.null_check.num == (id)) {if (k->u.null_check.pstr != (s)) {(isnull) = 0;break;}else {UChar* endp;(isnull) = 1;while (k < stk) {if (k->type == STK_MEM_START) {if (k->u.mem.end == INVALID_STACK_INDEX) {(isnull) = 0; break;}if (BIT_STATUS_AT(reg->bt_mem_end, k->u.mem.num))endp = STACK_AT(k->u.mem.end)->u.mem.pstr;elseendp = (UChar* )k->u.mem.end;if (STACK_AT(k->u.mem.start)->u.mem.pstr != endp) {(isnull) = 0; break;}else if (endp != s) {(isnull) = -1; }}k++;}break;}}}}} while(0)
...
#define STACK_NULL_CHECK_MEMST_REC(isnull,id,s,reg) do {int level = 0;OnigStackType* k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_NULL_CHECK_MEMST_REC"); if (k->type == STK_NULL_CHECK_START) {if (k->u.null_check.num == (id)) {if (level == 0) {if (k->u.null_check.pstr != (s)) {(isnull) = 0;break;}else {UChar* endp;(isnull) = 1;while (k < stk) {if (k->type == STK_MEM_START) {if (k->u.mem.end == INVALID_STACK_INDEX) {(isnull) = 0; break;}if (BIT_STATUS_AT(reg->bt_mem_end, k->u.mem.num))endp = STACK_AT(k->u.mem.end)->u.mem.pstr;elseendp = (UChar* )k->u.mem.end;if (STACK_AT(k->u.mem.start)->u.mem.pstr != endp) {(isnull) = 0; break;}else if (endp != s) {(isnull) = -1; }}k++;}break;}}else {level--;}}}else if (k->type == STK_NULL_CHECK_END) {if (k->u.null_check.num == (id)) level++;}}} while(0)
...
#define STACK_GET_REPEAT(id, k) do {int level = 0;k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_GET_REPEAT"); if (k->type == STK_REPEAT) {if (level == 0) {if (k->u.repeat.num == (id)) {break;}}}else if (k->type == STK_CALL_FRAME) level--;else if (k->type == STK_RETURN) level++;}} while(0)
...
#define STACK_RETURN(addr) do {int level = 0;OnigStackType* k = stk;while (1) {k--;STACK_BASE_CHECK(k, "STACK_RETURN"); if (k->type == STK_CALL_FRAME) {if (level == 0) {(addr) = k->u.call_frame.ret_addr;break;}else level--;}else if (k->type == STK_RETURN)level++;}} while(0)
...
#define STRING_CMP(s1,s2,len) do {while (len-- > 0) {if (*s1++ != *s2++) goto fail;}} while(0)





#define STRING_CMP_IC(case_fold_flag,s1,ps2,len,text_end) do {if (string_cmp_ic(encode, case_fold_flag, s1, ps2, len, text_end) == 0) goto fail; } while(0)
...
#define STRING_CMP_VALUE(s1,s2,len,is_fail) do {is_fail = 0;while (len-- > 0) {if (*s1++ != *s2++) {is_fail = 1; break;}}} while(0)
...
#define STRING_CMP_VALUE_IC(case_fold_flag,s1,ps2,len,text_end,is_fail) do {if (string_cmp_ic(encode, case_fold_flag, s1, ps2, len, text_end) == 0) is_fail = 1; else is_fail = 0; } while(0)
...
#if defined(USE_MATCH_RANGE_MUST_BE_INSIDE_OF_SPECIFIED_RANGE)
...
#if defined(USE_BACKREF_WITH_LEVEL)
...
#define TIMEDIFF(te,ts) (unsigned long )(((te).QuadPart - (ts).QuadPart) * 1000000 / freq.QuadPart)
...
#define TIMEDIFF(te,ts) (((te).tv_usec - (ts).tv_usec) + (((te).tv_sec - (ts).tv_sec)*1000000))
...
#define MOP_IN(opcode) do {if (opcode == OpPrevTarget) OpPrevCounter[OpCurr]++;OpCurr = opcode;OpCounter[opcode]++;GETTIME(ts);} while(0)
...
#define MOP_OUT do {GETTIME(te);OpTime[OpCurr] += TIMEDIFF(te, ts);} while(0)
...
#define STACK_INC do {stk++;if (stk - stk_base > MaxStackDepth) MaxStackDepth = stk - stk_base;} while(0)
...
#if defined(USE_BACKREF_WITH_LEVEL)
&&L_OP_BACKREF_WITH_LEVEL, 
#else
&&L_DEFAULT,
#endif
&&L_OP_MEMORY_START,
&&L_OP_MEMORY_START_PUSH, 
&&L_OP_MEMORY_END_PUSH, 
#if defined(USE_SUBEXP_CALL)
&&L_OP_MEMORY_END_PUSH_REC, 
#else
&&L_DEFAULT,
#endif
&&L_OP_MEMORY_END,
...
&&L_OP_PUSH,
&&L_OP_POP,
#if defined(USE_OP_PUSH_OR_JUMP_EXACT)
&&L_OP_PUSH_OR_JUMP_EXACT1, 
#else
&&L_DEFAULT,
#endif
&&L_OP_PUSH_IF_PEEK_NEXT, 
&&L_OP_REPEAT, 
&&L_OP_REPEAT_NG, 
&&L_OP_REPEAT_INC,
&&L_OP_REPEAT_INC_NG, 
&&L_OP_REPEAT_INC_SG, 
&&L_OP_REPEAT_INC_NG_SG, 
&&L_OP_NULL_CHECK_START, 
&&L_OP_NULL_CHECK_END, 
#if defined(USE_MONOMANIAC_CHECK_CAPTURES_IN_ENDLESS_REPEAT)
&&L_OP_NULL_CHECK_END_MEMST, 
#else
&&L_DEFAULT,
#endif
#if defined(USE_SUBEXP_CALL)
&&L_OP_NULL_CHECK_END_MEMST_PUSH, 
#else
&&L_DEFAULT,
#endif
...
&&L_OP_POP_STOP_BT, 
&&L_OP_LOOK_BEHIND, 
&&L_OP_PUSH_LOOK_BEHIND_NOT, 
&&L_OP_FAIL_LOOK_BEHIND_NOT, 
...
#if defined(USE_COMBINATION_EXPLOSION_CHECK)
&&L_OP_STATE_CHECK_PUSH, 
&&L_OP_STATE_CHECK_PUSH_OR_JUMP, 
&&L_OP_STATE_CHECK, 
#else
&&L_DEFAULT,
&&L_DEFAULT,
...
#define OP_OFFSET 0
#define VM_LOOP while (1) { OPCODE_EXEC_HOOK; sbegin = s; switch (*p++) {
...
STACK_PUSH_ENSURED(STK_ALT, (UChar* )FinishCode); 
...
if (ON_STR_END(s)) {
#if !defined(USE_NEWLINE_AT_END_OF_STRING_HAS_EMPTY_LINE)
...
STACK_POP_ABSENT_POS(absent, ABSENT_END_POS); 
GET_RELADDR_INC(addr, p);
#if defined(ONIG_DEBUG_MATCH)
...
STACK_PUSH_ALT(p + addr, s, sprev, pkeep); 
n = enclen(encode, s, end);
STACK_PUSH_ABSENT_POS(absent, ABSENT_END_POS); 
...
#if !defined(USE_SUNDAY_QUICK_SEARCH)
...
#if defined(USE_INT_MAP_BACKWARD)
static int
...
#if defined(USE_MATCH_RANGE_MUST_BE_INSIDE_OF_SPECIFIED_RANGE)
#if defined(USE_FIND_LONGEST_SEARCH_ALL_OF_RANGE)
#define MATCH_AND_RETURN_CHECK(upper_range) r = match_at(reg, str, end, (upper_range), s, prev, &msa); if (r != ONIG_MISMATCH) {if (r >= 0) {if (! IS_FIND_LONGEST(reg->options)) {goto match;}}else goto finish; }
...
#else
#define MATCH_AND_RETURN_CHECK(upper_range) r = match_at(reg, str, end, (upper_range), s, prev, &msa); if (r != ONIG_MISMATCH) {if (r >= 0) {goto match;}else goto finish; }
...
#else
#if defined(USE_FIND_LONGEST_SEARCH_ALL_OF_RANGE)
#define MATCH_AND_RETURN_CHECK(none) r = match_at(reg, str, end, s, prev, &msa);if (r != ONIG_MISMATCH) {if (r >= 0) {if (! IS_FIND_LONGEST(reg->options)) {goto match;}}else goto finish; }
...
#else
#define MATCH_AND_RETURN_CHECK(none) r = match_at(reg, str, end, s, prev, &msa);if (r != ONIG_MISMATCH) {if (r >= 0) {goto match;}else goto finish; }
