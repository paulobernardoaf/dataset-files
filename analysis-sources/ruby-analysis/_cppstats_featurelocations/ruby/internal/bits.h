...
#define MUL_OVERFLOW_FIXNUM_P(a, b) __extension__ ({ struct { long fixnum : sizeof(long) * CHAR_BIT - 1; } c; __builtin_mul_overflow_p((a), (b), c.fixnum); })
...
#else
#define bit_length(x) (unsigned int) (sizeof(x) <= sizeof(int32_t) ? 32 - nlz_int32((uint32_t)(x)) : 64 - nlz_int64((uint64_t)(x)))
