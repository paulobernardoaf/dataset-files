...
#if defined(_WIN32)
#define WAIT_FD_IN_WIN32(fptr) (rb_w32_io_cancelable_p((fptr)->fd) ? 0 : rb_thread_wait_fd((fptr)->fd))

#else
#define WAIT_FD_IN_WIN32(fptr)
#endif

#define READ_CHECK(fptr) do {if (!READ_DATA_PENDING(fptr)) {WAIT_FD_IN_WIN32(fptr);rb_io_check_closed(fptr);}} while(0)
...
#define WRITECONV_MASK ( (ECONV_DECORATOR_MASK & ~ECONV_CRLF_NEWLINE_DECORATOR)|ECONV_STATEFUL_DECORATOR_MASK|0)



#define NEED_WRITECONV(fptr) ( ((fptr)->encs.enc != NULL && (fptr)->encs.enc != rb_ascii8bit_encoding()) || ((fptr)->encs.ecflags & WRITECONV_MASK) || 0)



#define SET_BINARY_MODE(fptr) setmode((fptr)->fd, O_BINARY)

#define NEED_NEWLINE_DECORATOR_ON_READ_CHECK(fptr) do {if (NEED_NEWLINE_DECORATOR_ON_READ(fptr)) {if (((fptr)->mode & FMODE_READABLE) &&!((fptr)->encs.ecflags & ECONV_NEWLINE_DECORATOR_MASK)) {setmode((fptr)->fd, O_BINARY);}else {setmode((fptr)->fd, O_TEXT);}}} while(0)
...
#define SET_UNIVERSAL_NEWLINE_DECORATOR_IF_ENC2(enc2, ecflags) do {if ((enc2) && ((ecflags) & ECONV_DEFAULT_NEWLINE_DECORATOR)) {(ecflags) |= ECONV_UNIVERSAL_NEWLINE_DECORATOR;}} while(0)
...
#define NEED_WRITECONV(fptr) ( ((fptr)->encs.enc != NULL && (fptr)->encs.enc != rb_ascii8bit_encoding()) || NEED_NEWLINE_DECORATOR_ON_WRITE(fptr) || ((fptr)->encs.ecflags & (ECONV_DECORATOR_MASK|ECONV_STATEFUL_DECORATOR_MASK)) || 0)
...
#define MODE_BTMODE(a,b,c) ((fmode & FMODE_BINMODE) ? (b) : (fmode & FMODE_TEXTMODE) ? (c) : (a))


#define MODE_BTXMODE(a, b, c, d, e, f) ((fmode & FMODE_EXCL) ? MODE_BTMODE(d, e, f) : MODE_BTMODE(a, b, c))
...
#if defined(TEXTMODE_NEWLINE_DECORATOR_ON_WRITE)
...
#define DO_SPAWN(cmd, args, envp) ((args) ? spawnve(P_NOWAIT, (cmd), (args), (envp)) : spawne(P_NOWAIT, (cmd), (envp)))


#else
#define DO_SPAWN(cmd, args, envp) ((args) ? spawnv(P_NOWAIT, (cmd), (args)) : spawn(P_NOWAIT, (cmd)))
...
#define ARGF_FORWARD(argc, argv) do {if (ARGF_GENERIC_INPUT_P())return argf_forward((argc), (argv), argf);} while (0)



#define NEXT_ARGF_FORWARD(argc, argv) do {if (!next_argv()) return Qnil;ARGF_FORWARD((argc), (argv));} while (0)
...
{
VALUE arg[8];
#if SIZEOF_VOIDP == 8 && defined(HAVE___SYSCALL) && SIZEOF_INT != 8 
...
return 0;
if (lseek(stp->dst_fd, 0, SEEK_CUR) > (off_t)0) 
return 0;
...
#define ARGF_block_call(mid, argc, argv, func, argf) rb_block_call_kw(ARGF.current_file, mid, argc, argv, func, argf, rb_keyword_given_p())
