...
{
#define SET_COMPILE_OPTION(o, h, mem) { VALUE flag = rb_hash_aref((h), ID2SYM(rb_intern(#mem))); if (flag == Qtrue) { (o)->mem = 1; } else if (flag == Qfalse) { (o)->mem = 0; } }
#define SET_COMPILE_OPTION_NUM(o, h, mem) { VALUE num = rb_hash_aref(opt, ID2SYM(rb_intern(#mem))); if (!NIL_P(num)) (o)->mem = NUM2INT(num); }
...
option.peephole_optimization = FALSE; 
...
snprintf(argi, sizeof(argi), "%s%s%s%s%s%s", 
...
static ID insn_syms[VM_INSTRUCTION_SIZE/2]; 
...
rb_ary_push(val, INT2FIX(ISEQ_MAJOR_VERSION)); 
rb_ary_push(val, INT2FIX(ISEQ_MINOR_VERSION)); 
...
#define PARAM(i, type) ( PARAM_TYPE(type), rb_id2str(PARAM_ID(i)) ? rb_ary_push(a, ID2SYM(PARAM_ID(i))) : a)
CONST_ID(req, "req");
CONST_ID(opt, "opt");
if (is_proc) {
...
if (0) rb_funcall(Qnil, rb_intern("puts"), 1, rb_iseq_disasm(iseq)); 
return data.n;
}
static int
...
#if VM_INSN_INFO_TABLE_IMPL == 2
#define IMMEDIATE_TABLE_SIZE 54 
