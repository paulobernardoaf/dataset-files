...
#define PEEK_TOKEN(parser) ((parser->token_available || yaml_parser_fetch_more_tokens(parser)) ? parser->tokens.head : NULL)
#define SKIP_TOKEN(parser) (parser->token_available = 0, parser->tokens_parsed ++, parser->stream_end_produced = (parser->tokens.head->type == YAML_STREAM_END_TOKEN), parser->tokens.head ++)
YAML_DECLARE(int)
