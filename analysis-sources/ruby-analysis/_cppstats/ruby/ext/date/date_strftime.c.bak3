...
#define FLAG_FOUND() do { if (precision > 0 || flags & (BIT_OF(LOCALE_E) | BIT_OF(LOCALE_O) | BIT_OF(COLONS))) goto unknown; } while (0)
...
#define FMT(def_pad, def_prec, fmt, val) do { int l; if (precision <= 0) precision = (def_prec); if (flags & BIT_OF(LEFT)) precision = 1; l = snprintf(s, endp - s, ((padding == '0' || (!padding && (def_pad) == '0')) ? "%0*"fmt : "%*"fmt), precision, (val)); if (l < 0) goto err; s += l; } while (0)
...
#define STRFTIME(fmt) do { i = date_strftime_with_tmx(s, endp - s, (fmt), tmx); if (!i) return 0; if (flags & BIT_OF(UPPER)) upcase(s, i); if (!(flags & BIT_OF(LEFT)) && precision > i) { if (start + maxsize < s + precision) { errno = ERANGE; return 0; } memmove(s + precision - i, s, i); memset(s, padding ? padding : ' ', precision - i); s += precision; } else s += i; } while (0)
