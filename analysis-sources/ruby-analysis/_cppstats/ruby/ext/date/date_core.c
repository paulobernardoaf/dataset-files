...
#define PACK5(m,d,h,min,s) (((m) << MON_SHIFT) | ((d) << MDAY_SHIFT) |((h) << HOUR_SHIFT) | ((min) << MIN_SHIFT) | ((s) << SEC_SHIFT))
...
#define get_d1(x)union DateData *dat;TypedData_Get_Struct(x, union DateData, &d_lite_type, dat);
#define get_d1a(x)union DateData *adat;TypedData_Get_Struct(x, union DateData, &d_lite_type, adat);
#define get_d1b(x)union DateData *bdat;TypedData_Get_Struct(x, union DateData, &d_lite_type, bdat);
#define get_d2(x,y)union DateData *adat, *bdat;TypedData_Get_Struct(x, union DateData, &d_lite_type, adat);TypedData_Get_Struct(y, union DateData, &d_lite_type, bdat);
inline static VALUE
canon(VALUE x)
{
...
#if !defined(USE_PACK)
#define set_to_simple(obj, x, _nth, _jd ,_sg, _year, _mon, _mday, _flags) do {RB_OBJ_WRITE((obj), &(x)->nth, canon(_nth)); (x)->jd = _jd;(x)->sg = (date_sg_t)(_sg);(x)->year = _year;(x)->mon = _mon;(x)->mday = _mday;(x)->flags = (_flags) & ~COMPLEX_DAT;} while (0)
#else
#define set_to_simple(obj, x, _nth, _jd ,_sg, _year, _mon, _mday, _flags) do {RB_OBJ_WRITE((obj), &(x)->nth, canon(_nth)); (x)->jd = _jd;(x)->sg = (date_sg_t)(_sg);(x)->year = _year;(x)->pc = PACK2(_mon, _mday);(x)->flags = (_flags) & ~COMPLEX_DAT;} while (0)
#endif
#if !defined(USE_PACK)
#define set_to_complex(obj, x, _nth, _jd ,_df, _sf, _of, _sg,_year, _mon, _mday, _hour, _min, _sec, _flags) do {RB_OBJ_WRITE((obj), &(x)->nth, canon(_nth));(x)->jd = _jd;(x)->df = _df;RB_OBJ_WRITE((obj), &(x)->sf, canon(_sf));(x)->of = _of;(x)->sg = (date_sg_t)(_sg);(x)->year = _year;(x)->mon = _mon;(x)->mday = _mday;(x)->hour = _hour;(x)->min = _min;(x)->sec = _sec;(x)->flags = (_flags) | COMPLEX_DAT;} while (0)
#else
#define set_to_complex(obj, x, _nth, _jd ,_df, _sf, _of, _sg,_year, _mon, _mday, _hour, _min, _sec, _flags) do {RB_OBJ_WRITE((obj), &(x)->nth, canon(_nth));(x)->jd = _jd;(x)->df = _df;RB_OBJ_WRITE((obj), &(x)->sf, canon(_sf));(x)->of = _of;(x)->sg = (date_sg_t)(_sg);(x)->year = _year;(x)->pc = PACK5(_mon, _mday, _hour, _min, _sec);(x)->flags = (_flags) | COMPLEX_DAT;} while (0)
#endif
#if !defined(USE_PACK)
#define copy_simple_to_complex(obj, x, y) do {RB_OBJ_WRITE((obj), &(x)->nth, (y)->nth);(x)->jd = (y)->jd;(x)->df = 0;(x)->sf = INT2FIX(0);(x)->of = 0;(x)->sg = (date_sg_t)((y)->sg);(x)->year = (y)->year;(x)->mon = (y)->mon;(x)->mday = (y)->mday;(x)->hour = 0;(x)->min = 0;(x)->sec = 0;(x)->flags = (y)->flags;} while (0)
#else
#define copy_simple_to_complex(obj, x, y) do {RB_OBJ_WRITE((obj), &(x)->nth, (y)->nth);(x)->jd = (y)->jd;(x)->df = 0;RB_OBJ_WRITE((obj), &(x)->sf, INT2FIX(0));(x)->of = 0;(x)->sg = (date_sg_t)((y)->sg);(x)->year = (y)->year;(x)->pc = PACK5(EX_MON((y)->pc), EX_MDAY((y)->pc), 0, 0, 0);(x)->flags = (y)->flags;} while (0)
#endif
#if !defined(USE_PACK)
#define copy_complex_to_simple(obj, x, y) do {RB_OBJ_WRITE((obj), &(x)->nth, (y)->nth);(x)->jd = (y)->jd;(x)->sg = (date_sg_t)((y)->sg);(x)->year = (y)->year;(x)->mon = (y)->mon;(x)->mday = (y)->mday;(x)->flags = (y)->flags;} while (0)
#else
#define copy_complex_to_simple(obj, x, y) do {RB_OBJ_WRITE((obj), &(x)->nth, (y)->nth);(x)->jd = (y)->jd;(x)->sg = (date_sg_t)((y)->sg);(x)->year = (y)->year;(x)->pc = PACK2(EX_MON((y)->pc), EX_MDAY((y)->pc));(x)->flags = (y)->flags;} while (0)
#endif
...
#define canonicalize_jd(_nth, _jd) do {if (_jd < 0) {_nth = f_sub(_nth, INT2FIX(1));_jd += CM_PERIOD;}if (_jd >= CM_PERIOD) {_nth = f_add(_nth, INT2FIX(1));_jd -= CM_PERIOD;}} while (0)
inline static void
...
#define decode_offset(of,s,h,m)do {int a;s = (of < 0) ? '-' : '+';a = (of < 0) ? -of : of;h = a / HOUR_IN_SECONDS;m = a % HOUR_IN_SECONDS / MINUTE_IN_SECONDS;} while (0)
static VALUE
of2str(int of)
{
int s, h, m;
...
#define valid_sg(sg) do {if (!c_valid_start_p(sg)) {sg = 0;rb_warning("invalid start is ignored");}} while (0)
static VALUE
...
#define num2num_with_frac(s,n) do {s = s##_trunc(v##s, &fr);if (f_nonzero_p(fr)) {if (argc > n)rb_raise(eDateError, "invalid fraction");fr2 = fr;}} while (0)
#define num2int_with_frac(s,n) do {s = NUM2INT(s##_trunc(v##s, &fr));if (f_nonzero_p(fr)) {if (argc > n)rb_raise(eDateError, "invalid fraction");fr2 = fr;}} while (0)
#define canon24oc() do {if (rh == 24) {rh = 0;fr2 = f_add(fr2, INT2FIX(1));}} while (0)
#define add_frac() do {if (f_nonzero_p(fr2))ret = d_lite_plus(ret, fr2);} while (0)
#define val2sg(vsg,dsg) do {dsg = NUM2DBL(vsg);if (!c_valid_start_p(dsg)) {dsg = DEFAULT_SG;rb_warning("invalid start is ignored");}} while (0)
...
#define val2off(vof,iof) do {if (!offset_to_sec(vof, &iof)) {iof = 0;rb_warning("invalid offset is ignored");}} while (0)
#if 0
static VALUE
...
NUM2INT(n), NUM2INT(k), m_virtual_sg(dat), 
&rjd, &ns);
