...
if (prev_cref != NULL && prev_cref != (void *)1 ) {
...
{
rb_cref_t *cref = vm_cref_new(rb_cObject, METHOD_VISI_PRIVATE , FALSE, NULL, FALSE);
...
#define REWIND_CFP(expr) do { rb_execution_context_t *ec__ = GET_EC(); VALUE *const curr_sp = (ec__->cfp++)->sp; VALUE *const saved_sp = ec__->cfp->sp; ec__->cfp->sp = curr_sp; expr; (ec__->cfp--)->sp = saved_sp; } while (0)
static VALUE
...
#if VM_COLLECT_USAGE_DETAILS
#define define_usage_analysis_hash(name) rb_define_const(rb_cRubyVM, "USAGE_ANALYSIS_" #name, rb_hash_new())
