...
#endif
#endif
#define TO_SIX_BIT(rslt, src) { C_block cvt; cvt.b[0] = (unsigned char)(src); (src) >>= 6; cvt.b[1] = (unsigned char)(src); (src) >>= 6; cvt.b[2] = (unsigned char)(src); (src) >>= 6; cvt.b[3] = (unsigned char)(src); (rslt) = (cvt.b32.i0 & 0x3f3f3f3fL) << 2; }
...
#define PERM6464(d,d0,d1,cpp,p) LOAD((d),(d0),(d1),(p)[(0<<CHUNKBITS)+(cpp)[0]]); OR ((d),(d0),(d1),(p)[(1<<CHUNKBITS)+(cpp)[1]]); OR ((d),(d0),(d1),(p)[(2<<CHUNKBITS)+(cpp)[2]]); OR ((d),(d0),(d1),(p)[(3<<CHUNKBITS)+(cpp)[3]]); OR (d),(d0),(d1),(p)[(4<<CHUNKBITS)+(cpp)[4]]); OR (d),(d0),(d1),(p)[(5<<CHUNKBITS)+(cpp)[5]]); OR (d),(d0),(d1),(p)[(6<<CHUNKBITS)+(cpp)[6]]); OR (d),(d0),(d1),(p)[(7<<CHUNKBITS)+(cpp)[7]]);
#define PERM3264(d,d0,d1,cpp,p) LOAD((d),(d0),(d1),(p)[(0<<CHUNKBITS)+(cpp)[0]]); OR ((d),(d0),(d1),(p)[(1<<CHUNKBITS)+(cpp)[1]]); OR ((d),(d0),(d1),(p)[(2<<CHUNKBITS)+(cpp)[2]]); OR ((d),(d0),(d1),(p)[(3<<CHUNKBITS)+(cpp)[3]]);
#else
...
#define A64TOI1(c) ((c) == '.' ? 0 : (c) == '/' ? 1 : ('0' <= (c) && (c) <= '9') ? (c) - '0' + 2 : ('A' <= (c) && (c) <= 'Z') ? (c) - 'A' + 12 : ('a' <= (c) && (c) <= 'z') ? (c) - 'a' + 38 : 0)
...
#if defined(USE_NONREENTRANT_CRYPT)
...
L1 = R0 | R1; 
STORE(L,L0,L1,B);
PERM3264(L,L0,L1,B.b, IE3264[0]); 
PERM3264(R,R0,R1,B.b+4,IE3264[0]); 
if (num_iter >= 0)
{ 
kp = &KS[0];
ks_inc = +1;
...
#endif
#endif
#define CRUNCH(p0, p1, q0, q1) k = ((q0) ^ (q1)) & SALT; B.b32.i0 = k ^ (q0) ^ kp->b32.i0; B.b32.i1 = k ^ (q1) ^ kp->b32.i1; kp += ks_inc; DOXOR((p0), (p1), 0); DOXOR((p0), (p1), 1); DOXOR((p0), (p1), 2); DOXOR((p0), (p1), 3); DOXOR((p0), (p1), 4); DOXOR((p0), (p1), 5); DOXOR((p0), (p1), 6); DOXOR((p0), (p1), 7);
CRUNCH(L0, L1, R0, R1);
CRUNCH(R0, R1, L0, L1);
} while (--loop_count != 0);
kp -= (ks_inc*KS_SIZE);
