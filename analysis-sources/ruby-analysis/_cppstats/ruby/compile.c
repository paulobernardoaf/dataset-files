...
#define debugp(header, value) (void) (compile_debug_print_indent(1) && ruby_debug_print_value(1, compile_debug, (header), (value)))
#define debugi(header, id) (void) (compile_debug_print_indent(1) && ruby_debug_print_id(1, compile_debug, (header), (id)))
#define debugp_param(header, value) (void) (compile_debug_print_indent(1) && ruby_debug_print_value(1, compile_debug, (header), (value)))
#define debugp_verbose(header, value) (void) (compile_debug_print_indent(2) && ruby_debug_print_value(2, compile_debug, (header), (value)))
#define debugp_verbose_node(header, value) (void) (compile_debug_print_indent(10) && ruby_debug_print_value(10, compile_debug, (header), (value)))
#define debug_node_start(node) ((void) (compile_debug_print_indent(1) && (ruby_debug_print_node(1, CPDEBUG, "", (const NODE *)(node)), gl_node_level)), gl_node_level++)
...
#define ADD_INSN1(seq, line, insn, op1) ADD_ELEM((seq), (LINK_ELEMENT *) new_insn_body(iseq, (line), BIN(insn), 1, (VALUE)(op1)))
#define INSERT_BEFORE_INSN1(next, line, insn, op1) ELEM_INSERT_PREV(&(next)->link, (LINK_ELEMENT *) new_insn_body(iseq, (line), BIN(insn), 1, (VALUE)(op1)))
#define INSERT_AFTER_INSN1(prev, line, insn, op1) ELEM_INSERT_NEXT(&(prev)->link, (LINK_ELEMENT *) new_insn_body(iseq, (line), BIN(insn), 1, (VALUE)(op1)))
#define LABEL_REF(label) ((label)->refcnt++)
#define ADD_INSNL(seq, line, insn, label) (ADD_INSN1(seq, line, insn, label), LABEL_REF(label))
#define ADD_INSN2(seq, line, insn, op1, op2) ADD_ELEM((seq), (LINK_ELEMENT *) new_insn_body(iseq, (line), BIN(insn), 2, (VALUE)(op1), (VALUE)(op2)))
#define ADD_INSN3(seq, line, insn, op1, op2, op3) ADD_ELEM((seq), (LINK_ELEMENT *) new_insn_body(iseq, (line), BIN(insn), 3, (VALUE)(op1), (VALUE)(op2), (VALUE)(op3)))
...
#define DECL_BRANCH_BASE(branches, first_line, first_column, last_line, last_column, type) do { if (ISEQ_COVERAGE(iseq) && ISEQ_BRANCH_COVERAGE(iseq) && (first_line) > 0) { VALUE structure = RARRAY_AREF(ISEQ_BRANCH_COVERAGE(iseq), 0); branches = rb_ary_tmp_new(5); rb_ary_push(structure, branches); rb_ary_push(branches, ID2SYM(rb_intern(type))); rb_ary_push(branches, INT2FIX(first_line)); rb_ary_push(branches, INT2FIX(first_column)); rb_ary_push(branches, INT2FIX(last_line)); rb_ary_push(branches, INT2FIX(last_column)); } } while (0)
#define ADD_TRACE_BRANCH_COVERAGE(seq, first_line, first_column, last_line, last_column, type, branches) do { if (ISEQ_COVERAGE(iseq) && ISEQ_BRANCH_COVERAGE(iseq) && (first_line) > 0) { VALUE counters = RARRAY_AREF(ISEQ_BRANCH_COVERAGE(iseq), 1); long counter_idx = RARRAY_LEN(counters); rb_ary_push(counters, INT2FIX(0)); rb_ary_push(branches, ID2SYM(rb_intern(type))); rb_ary_push(branches, INT2FIX(first_line)); rb_ary_push(branches, INT2FIX(first_column)); rb_ary_push(branches, INT2FIX(last_line)); rb_ary_push(branches, INT2FIX(last_column)); rb_ary_push(branches, INT2FIX(counter_idx)); ADD_TRACE_WITH_DATA(seq, RUBY_EVENT_COVERAGE_BRANCH, counter_idx); ADD_INSN(seq, last_line, nop); } } while (0)
...
#define ADD_CATCH_ENTRY(type, ls, le, iseqv, lc) do { VALUE _e = rb_ary_new3(5, (type), (VALUE)(ls) | 1, (VALUE)(le) | 1, (VALUE)(iseqv), (VALUE)(lc) | 1); LABEL_UNREMOVABLE(ls); LABEL_REF(le); LABEL_REF(lc); if (NIL_P(ISEQ_COMPILE_DATA(iseq)->catch_table_ary)) RB_OBJ_WRITE(iseq, &ISEQ_COMPILE_DATA(iseq)->catch_table_ary, rb_ary_tmp_new(3)); rb_ary_push(ISEQ_COMPILE_DATA(iseq)->catch_table_ary, freeze_hide_obj(_e)); } while (0)
#define COMPILE(anchor, desc, node) (debug_compile("== " desc "\n", iseq_compile_each(iseq, (anchor), (node), 0)))
#define COMPILE_POPPED(anchor, desc, node) (debug_compile("== " desc "\n", iseq_compile_each(iseq, (anchor), (node), 1)))
#define COMPILE_(anchor, desc, node, popped) (debug_compile("== " desc "\n", iseq_compile_each(iseq, (anchor), (node), (popped))))
#define COMPILE_RECV(anchor, desc, node) (private_recv_p(node) ? (ADD_INSN(anchor, nd_line(node), putself), VM_CALL_FCALL) : COMPILE(anchor, desc, node->nd_recv) ? 0 : -1)
...
#define EXPECT_NODE(prefix, node, ndtype, errval) do { const NODE *error_node = (node); enum node_type error_type = nd_type(error_node); if (error_type != (ndtype)) { COMPILE_ERROR(ERROR_ARGS_AT(error_node) prefix ": " #ndtype " is expected, but %s", ruby_node_name(error_type)); return errval; } } while (0)
#define EXPECT_NODE_NONULL(prefix, parent, ndtype, errval) do { COMPILE_ERROR(ERROR_ARGS_AT(parent) prefix ": must be " #ndtype ", but 0"); return errval; } while (0)
#define UNKNOWN_NODE(prefix, node, errval) do { const NODE *error_node = (node); COMPILE_ERROR(ERROR_ARGS_AT(error_node) prefix ": unknown node (%s)", ruby_node_name(nd_type(error_node))); return errval; } while (0)
#define COMPILE_OK 1
#define COMPILE_NG 0
...
break;
default:
NO_CHECK(COMPILE_POPPED(optargs, "kwarg", node)); 
dv = complex_mark;
}
keyword->num = ++di;
...
if (node->nd_next == NULL && keyword_node_p(node->nd_head)) { 
...
int stack_len = 0;
int first_chunk = 1;
#define FLUSH_CHUNK(newarrayinsn) if (stack_len) { ADD_INSN1(ret, line, newarrayinsn, INT2FIX(stack_len)); if (!first_chunk) ADD_INSN(ret, line, concatarray); first_chunk = stack_len = 0; }
while (node) {
int count = 1;
...
DECL_ANCHOR(anchor);
INIT_ANCHOR(anchor);
#define FLUSH_CHUNK() if (stack_len) { if (first_chunk) { APPEND_LIST(ret, anchor); ADD_INSN1(ret, line, newhash, INT2FIX(stack_len)); } else { ADD_INSN1(ret, line, putspecialobject, INT2FIX(VM_SPECIAL_OBJECT_VMCORE)); ADD_INSN(ret, line, swap); APPEND_LIST(ret, anchor); ADD_SEND(ret, line, id_core_hash_merge_ptr, INT2FIX(stack_len + 1)); } INIT_ANCHOR(anchor); first_chunk = stack_len = 0; }
while (node) {
int count = 1;
...
int last_kw = !node->nd_next->nd_next; 
...
#define MEMORY(v) { int i; if (memindex == memsize) return 0; for (i=0; i<memindex; i++) { if (mem[i] == (v)) return 0; } mem[memindex++] = (v); }
...
#if !defined(IBF_ISEQ_DEBUG)
#define IBF_ISEQ_DEBUG 0
#endif
#if !defined(IBF_ISEQ_ENABLE_LOCAL_BUFFER)
...
{
st_table *obj_table = st_init_numtable(); 
st_insert(obj_table, (st_data_t)Qnil, (st_data_t)0); 
return obj_table;
}
static VALUE
...
st_free_table(buffer.obj_table); 
return offset;
#else
return body_offset;
#endif
...
#define IBF_ALIGNED_OFFSET(align, offset) ((((offset) - 1) / (align) + 1) * (align))
#define IBF_OBJBODY(type, offset) (const type *)ibf_load_check_offset(load, IBF_ALIGNED_OFFSET(RUBY_ALIGNOF(type), offset))
static const void *
...
{
dump->global_buffer.obj_table = NULL; 
