...
#if defined(DEFAULT_WARN_FUNCTION)
static OnigWarnFunc onig_warn = (OnigWarnFunc )DEFAULT_WARN_FUNCTION;
#else
static OnigWarnFunc onig_warn = onig_null_warn;
#endif
#if defined(DEFAULT_VERB_WARN_FUNCTION)
...
#define ADD_ALL_MULTI_BYTE_RANGE(enc, mbuf) do {if (! ONIGENC_IS_SINGLEBYTE(enc)) {r = SET_ALL_MULTI_BYTE_RANGE(enc, &(mbuf));if (r) return r;}} while (0)
#define BITSET_SET_BIT_CHKDUP(bs, pos) do { if (BITSET_AT(bs, pos)) CC_DUP_WARN(env, pos, pos); BS_ROOM(bs, pos) |= BS_BIT(pos); } while (0)
#define BITSET_IS_EMPTY(bs,empty) do {int i;empty = 1;for (i = 0; i < BITSET_SIZE; i++) {if ((bs)[i] != 0) {empty = 0; break;}}} while (0)
static void
...
#define PINC do { pfetch_prev = p; p += enclen(enc, p, end); } while (0)
#define PFETCH(c) do { c = ((enc->max_enc_len == 1) ? *p : ONIGENC_MBC_TO_CODE(enc, p, end)); pfetch_prev = p; p += enclen(enc, p, end); } while (0)
#define PINC_S do { p += enclen(enc, p, end); } while (0)
#define PFETCH_S(c) do { c = ((enc->max_enc_len == 1) ? *p : ONIGENC_MBC_TO_CODE(enc, p, end)); p += enclen(enc, p, end); } while (0)
...
#if defined(USE_PAD_TO_SHORT_BYTE_CHAR)
static int
...
#define SWAP_BBUF_NOT(bbuf1, not1, bbuf2, not2) do {BBuf *tbuf; int tnot; tnot = not1; not1 = not2; not2 = tnot; tbuf = bbuf1; bbuf1 = bbuf2; bbuf2 = tbuf; } while (0)
static int
...
#if defined(USE_NO_INVALID_QUANTIFIER)
...
if (cnext == '0') {
PINC;
if (PPEEK_IS(get_name_end_code_point(c))) { 
PINC;
name_end = p;
gnum = 0;
}
...
tok->u.c = c;
tok->escaped = 0;
#if defined(USE_VARIABLE_META_CHARS)
...
#if defined(USE_PERL_SUBEXP_CALL)
c = PPEEK;
...
#endif 
#if defined(USE_CAPITAL_P_NAMED_GROUP)
if (PPEEK_IS('P') &&
...
#if defined(USE_POSIXLINE_OPTION)
case 'p':
#endif
...
#if defined(USE_WARNING_REDUNDANT_NESTED_REPEAT_OPERATOR)
...
#if !defined(CASE_FOLD_IS_APPLIED_INSIDE_NEGATIVE_CCLASS)
static int
...
#if defined(USE_UNICODE_PROPERTIES)
if (ONIGENC_IS_UNICODE(env->enc)) { 
CClassNode* cc;
...
if (ONIGENC_MBC_MINLEN(env->enc) > 1) { 
R_ERR(add_code_range(&(cc->mbuf), env, 0x000A, 0x000A)); 
R_ERR(add_code_range(&(cc->mbuf), env, 0x000D, 0x000D)); 
}
else {
...
R_ERR(quantify_node(XP_list+1, 0, REPEAT_INFINITE)); 
...
env->warnings_flag &= dup_not_warned; 
}
else {
...
#if !defined(NUMBERED_CHAR_IS_NOT_CASE_AMBIG)
