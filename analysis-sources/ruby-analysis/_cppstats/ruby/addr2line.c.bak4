...
#if defined(HAVE_MACH_O_LOADER_H)
...
#define FILL_LINE() do { fill_line(num_traces, traces, addr, file, line, (char *)header.include_directories, (char *)header.filenames, obj, lines, offset); } while (0)
...
die->tag = (int)uleb128(&reader->q); 
die->has_children = *reader->q++; 
if (die->has_children) {
reader->level++;
}
return die;
...
{
#if defined(SUPPORT_COMPRESSED_DEBUG_LINE)
