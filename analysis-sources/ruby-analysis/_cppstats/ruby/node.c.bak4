...
#define A_NODE_HEADER(node, term) rb_str_catf(buf, "@ %s (line: %d, location: (%d,%d)-(%d,%d))%s"term, ruby_node_name(nd_type(node)), nd_line(node), nd_first_lineno(node), nd_first_column(node), nd_last_lineno(node), nd_last_column(node), (node->flags & NODE_FL_NEWLINE ? "*" : ""))
...
#define COMPOUND_FIELD1(name, ann) COMPOUND_FIELD(FIELD_NAME_LEN(name, ann), FIELD_NAME_DESC(name, ann))



#define FIELD_NAME_DESC(name, ann) name " (" ann ")"
#define FIELD_NAME_LEN(name, ann) (int)( comment ? rb_strlen_lit(FIELD_NAME_DESC(name, ann)) : rb_strlen_lit(name))
...
#define FIELD_BLOCK(init, reset) for (init, field_flag = 1; field_flag; reset, field_flag = 0)
