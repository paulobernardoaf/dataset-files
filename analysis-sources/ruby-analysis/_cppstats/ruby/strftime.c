...
#if defined(NO_STRING_LITERAL_CONCATENATION)
...
#define FLAG_FOUND() do { if (precision > 0) goto unknown; } while (0)
#define NEEDS(n) do { if (s >= endp || (n) >= endp - s - 1) { s = resize_buffer(ftime, s, &start, &endp, (n), maxsize); buffer_size_check(s, format_end, format_len, enc); } } while (0)
#define FILL_PADDING(i) do { if (!(flags & BIT_OF(LEFT)) && precision > (i)) { NEEDS(precision); memset(s, padding ? padding : ' ', precision - (i)); s += precision - (i); } else { NEEDS(i); } } while (0);
#define FMT_PADDING(fmt, def_pad) (&"%*"fmt"\0""%0*"fmt[(padding == '0' || (!padding && (def_pad) == '0')) ? rb_strlen_lit("%*"fmt)+1 : 0])
#define FMT_PRECISION(def_prec) ((flags & BIT_OF(LEFT)) ? (1) : (precision <= 0) ? (def_prec) : (precision))
#define FMT(def_pad, def_prec, fmt, val) do { precision = FMT_PRECISION(def_prec); len = s - start; NEEDS(precision); rb_str_set_len(ftime, len); rb_str_catf(ftime, FMT_PADDING(fmt, def_pad), precision, (val)); RSTRING_GETMEM(ftime, s, len); endp = (start = s) + rb_str_capacity(ftime); s += len; } while (0)
#define STRFTIME(fmt) do { len = s - start; rb_str_set_len(ftime, len); if (!rb_strftime_with_timespec(ftime, (fmt), rb_strlen_lit(fmt), enc, time, vtm, timev, ts, gmt, maxsize)) return 0; s = RSTRING_PTR(ftime); i = RSTRING_LEN(ftime) - len; endp = (start = s) + rb_str_capacity(ftime); s += len; if (i > 0) case_conv(s, i, flags); if (precision > i) {NEEDS(precision); memmove(s + precision - i, s, i);memset(s, padding ? padding : ' ', precision - i); s += precision; } else s += i; } while (0)
#define FMTV(def_pad, def_prec, fmt, val) do { VALUE tmp = (val); if (FIXNUM_P(tmp)) { FMT((def_pad), (def_prec), "l"fmt, FIX2LONG(tmp)); } else { const int base = ((fmt[0] == 'x') ? 16 : (fmt[0] == 'o') ? 8 : 10); precision = FMT_PRECISION(def_prec); if (!padding) padding = (def_pad); tmp = format_value(tmp, base); i = RSTRING_LEN(tmp); FILL_PADDING(i); rb_str_set_len(ftime, s-start); rb_str_append(ftime, tmp); RSTRING_GETMEM(ftime, s, len); endp = (start = s) + rb_str_capacity(ftime); s += len; } } while (0)
