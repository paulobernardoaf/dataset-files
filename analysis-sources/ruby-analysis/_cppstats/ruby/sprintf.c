...
#define FPREC 64
#define FPREC0 128
#define CHECK(l) do {int cr = ENC_CODERANGE(result);while ((l) >= bsiz - blen) {bsiz*=2;if (bsiz<0) rb_raise(rb_eArgError, "too big specifier");}rb_str_resize(result, bsiz);ENC_CODERANGE_SET(result, cr);buf = RSTRING_PTR(result);} while (0)
#define PUSH(s, l) do { CHECK(l);PUSH_(s, l);} while (0)
#define PUSH_(s, l) do { memcpy(&buf[blen], (s), (l));blen += (l);} while (0)
#define FILL(c, l) do { if ((l) <= 0) break;CHECK(l);FILL_(c, l);} while (0)
#define FILL_(c, l) do { memset(&buf[blen], (c), (l));blen += (l);} while (0)
#define GETARG() (nextvalue != Qundef ? nextvalue : GETNEXTARG())
#define GETNEXTARG() ( check_next_arg(posarg, nextarg), (posarg = nextarg++, GETNTHARG(posarg)))
#define GETPOSARG(n) ( check_pos_arg(posarg, (n)), (posarg = -1, GETNTHARG(n)))
#define GETNTHARG(nth) (((nth) >= argc) ? (rb_raise(rb_eArgError, "too few arguments"), 0) : argv[(nth)])
#define CHECKNAMEARG(name, len, enc) ( check_name_arg(posarg, name, len, enc), posarg = -2)
#define GETNUM(n, val) (!(p = get_num(p, end, enc, &(n))) ? rb_raise(rb_eArgError, #val " too big") : (void)0)
#define GETASTER(val) do { t = p++; n = 0; GETNUM(n, val); if (*p == '$') { tmp = GETPOSARG(n); } else { tmp = GETNEXTARG(); p = t; } (val) = NUM2INT(tmp); } while (0)
static const char *
...
#define CHECK_FOR_WIDTH(f) if ((f) & FWIDTH) { rb_raise(rb_eArgError, "width given twice"); } if ((f) & FPREC0) { rb_raise(rb_eArgError, "width after precision"); }
#define CHECK_FOR_FLAGS(f) if ((f) & FWIDTH) { rb_raise(rb_eArgError, "flag after width"); } if ((f) & FPREC0) { rb_raise(rb_eArgError, "flag after precision"); }
++argc;
--argv;
StringValue(fmt);
enc = rb_enc_get(fmt);
...
#if defined(RUBY_PRI_VALUE_MARK)
