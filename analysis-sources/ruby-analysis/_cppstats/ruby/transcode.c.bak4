...
#if defined(ENABLE_ECONV_NEWLINE_OPTION)
...
#define TRANSCODING_READBUF(tc) ((tc)->transcoder->max_input <= (int)sizeof((tc)->readbuf.ary) ? (tc)->readbuf.ary : (tc)->readbuf.ptr)



#define TRANSCODING_WRITEBUF(tc) ((tc)->transcoder->max_output <= (int)sizeof((tc)->writebuf.ary) ? (tc)->writebuf.ary : (tc)->writebuf.ptr)



#define TRANSCODING_WRITEBUF_SIZE(tc) ((tc)->transcoder->max_output <= (int)sizeof((tc)->writebuf.ary) ? sizeof((tc)->writebuf.ary) : (size_t)(tc)->transcoder->max_output)



#define TRANSCODING_STATE_EMBED_MAX ((int)sizeof(union rb_transcoding_state_t))
#define TRANSCODING_STATE(tc) ((tc)->transcoder->state_size <= (int)sizeof((tc)->state) ? (tc)->state.ary : (tc)->state.ptr)
...
#define SUSPEND_OBUF(num) do { while (out_stop - out_p < 1) { SUSPEND(econv_destination_buffer_full, num); } } while (0)




#define SUSPEND_AFTER_OUTPUT(num) if ((opt & ECONV_AFTER_OUTPUT) && *out_pos != out_p) { SUSPEND(econv_after_output, num); }
...
str2 = xmalloc(len);
MEMCPY(str2, str, unsigned char, len); 
len2 = len;
encname2 = encname;
}
else {
