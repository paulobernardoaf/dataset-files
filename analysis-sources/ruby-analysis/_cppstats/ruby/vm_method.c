...
method_cref = vm_cref_new_toplevel(GET_EC()); 
}
...
#define CALL_METHOD_HOOK(klass, hook, mid) do { const VALUE arg = ID2SYM(mid); VALUE recv_class = (klass); ID hook_id = (hook); if (FL_TEST((klass), FL_SINGLETON)) { recv_class = rb_ivar_get((klass), attached); hook_id = singleton_##hook; } rb_funcallv(recv_class, hook_id, 1, &arg); } while (0)
static void
