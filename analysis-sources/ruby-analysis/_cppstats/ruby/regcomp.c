...
#define IS_NEED_STR_LEN_OP_EXACT(op) ((op) == OP_EXACTN || (op) == OP_EXACTMB2N ||(op) == OP_EXACTMB3N || (op) == OP_EXACTMBN || (op) == OP_EXACTN_IC)
static int
...
else if (qn->upper == 0 && qn->is_referred != 0) { 
len = SIZE_OP_JUMP + tlen;
}
...
Node* x = node->target;
tlen = compile_length_tree(NCAR(x), reg); 
if (tlen < 0) return tlen;
...
if (NCDR(x) == NULL) return ONIGERR_PARSER_BUG;
x = NCDR(x);
len2 = compile_length_tree(NCAR(x), reg); 
if (len2 < 0) return len2;
if (NCDR(x) != NULL) return ONIGERR_INVALID_CONDITION_PATTERN;
x = node->target;
r = add_rel_addr(reg, len + SIZE_OP_JUMP);
if (r) return r;
r = compile_tree(NCAR(x), reg); 
if (r) return r;
...
break;
#define ALLOWED_TYPE_IN_LB ( BIT_NT_LIST | BIT_NT_ALT | BIT_NT_STR | BIT_NT_CCLASS | BIT_NT_CTYPE | BIT_NT_CANY | BIT_NT_ANCHOR | BIT_NT_ENCLOSE | BIT_NT_QTFR | BIT_NT_CALL )
#define ALLOWED_ENCLOSE_IN_LB ( ENCLOSE_MEMORY | ENCLOSE_OPTION )
#define ALLOWED_ENCLOSE_IN_LB_NOT ENCLOSE_OPTION
#define ALLOWED_ANCHOR_IN_LB ( ANCHOR_LOOK_BEHIND | ANCHOR_LOOK_BEHIND_NOT | ANCHOR_BEGIN_LINE | ANCHOR_END_LINE | ANCHOR_BEGIN_BUF | ANCHOR_BEGIN_POSITION | ANCHOR_KEEP | ANCHOR_WORD_BOUND | ANCHOR_NOT_WORD_BOUND | ANCHOR_WORD_BEGIN | ANCHOR_WORD_END )
#define ALLOWED_ANCHOR_IN_LB_NOT ( ANCHOR_LOOK_BEHIND | ANCHOR_LOOK_BEHIND_NOT | ANCHOR_BEGIN_LINE | ANCHOR_END_LINE | ANCHOR_BEGIN_BUF | ANCHOR_BEGIN_POSITION | ANCHOR_KEEP | ANCHOR_WORD_BOUND | ANCHOR_NOT_WORD_BOUND | ANCHOR_WORD_BEGIN | ANCHOR_WORD_END )
case ANCHOR_LOOK_BEHIND:
{
...
#endif
#define REGEX_TRANSFER(to,from) do {onig_free_body(to);xmemcpy(to, from, sizeof(regex_t));xfree(from);} while (0)
#if 0
extern void
