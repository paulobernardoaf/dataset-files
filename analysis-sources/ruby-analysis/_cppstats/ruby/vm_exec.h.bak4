...
#define DEBUG_ENTER_INSN(insn) rb_vmdebug_debug_print_pre(ec, GET_CFP(), GET_PC());
...
#define DEBUG_END_INSN() rb_vmdebug_debug_print_post(ec, GET_CFP() SC_REGS());
...
#define INSN_ENTRY(insn) static rb_control_frame_t * FUNC_FASTCALL(LABEL(insn))(rb_execution_context_t *ec, rb_control_frame_t *reg_cfp) {
...
#define DISPATCH_ARCH_DEPEND_WAY(addr) __asm__ __volatile__("jmp *%0;\t#-- inserted by vm.h\t[length = 2]" : : "r" (addr))
...
#endif
#define TC_DISPATCH(insn) DISPATCH_ARCH_DEPEND_WAY(insns_address_table[GET_CURRENT_INSN()]); INSN_DISPATCH_SIG(insn); RB_GNUC_EXTENSION_BLOCK(goto *insns_address_table[GET_CURRENT_INSN()]); rb_bug("tc error");
...
#define END_INSN(insn) DEBUG_END_INSN(); break;
...
#define END_INSNS_DISPATCH() default: SDR(); rb_bug("unknown insn: %ld", GET_CURRENT_INSN()); } } 
...
#define THROW_EXCEPTION(exc) do { ec->errinfo = (VALUE)(exc); return 0; } while (0)
...
#define CHECK_VM_STACK_OVERFLOW_FOR_INSN(cfp, margin) WHEN_VM_STACK_OVERFLOWED(cfp, (cfp)->sp, margin) vm_stack_overflow_for_insn()
