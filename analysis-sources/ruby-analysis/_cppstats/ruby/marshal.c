...
#define SINGLETON_DUMP_UNABLE_P(klass) (rb_id_table_size(RCLASS_M_TBL(klass)) > 0 || (RCLASS_IV_TBL(klass) && RCLASS_IV_TBL(klass)->num_entries > 1))
static void
...
#define prohibit_ivar(type, str) do { if (!ivp || !*ivp) break; rb_raise(rb_eTypeError, "can't override instance variable of "type" `%"PRIsVALUE"'", (str)); } while (0)
static VALUE
