...
size_t retlen;
unsigned long n;
#define SET_WHEN(name, var, val) do { if (len == sizeof(name) - 1 && strncmp(str, (name), len) == 0) { (var) = (val); return 1; } } while (0)
#define NAME_MATCH_VALUE(name) ((size_t)len >= sizeof(name)-1 && strncmp(str, (name), sizeof(name)-1) == 0 && ((len == sizeof(name)-1 && !(len = 0)) || (str[sizeof(name)-1] == '=' && (str += sizeof(name), len -= sizeof(name), 1))))
#define SET_UINT(val) do { n = ruby_scan_digits(str, len, 10, &retlen, &ov); if (!ov && retlen) { val = (unsigned int)n; } str += retlen; len -= retlen; } while (0)
#define SET_UINT_LIST(name, vals, num) do { int i; for (i = 0; i < (num); ++i) { SET_UINT((vals)[i]); if (!len || *str != ':') break; ++str; --len; } if (len > 0) { fprintf(stderr, "ignored "name" option: `%.*s'\n", len, str); } } while (0)
