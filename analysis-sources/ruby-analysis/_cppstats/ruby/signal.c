...
#include "vm_core.h"
#if defined(NEED_RUBY_ATOMIC_OPS)
rb_atomic_t
...
#endif
#if !defined(MESSAGE_FAULT_ADDRESS)
...
#define INSTALL_SIGHANDLER(cond, signame, signum) do { static const char failed[] = "failed to install "signame" handler"; if (!(cond)) break; if (reserved_signal_p(signum)) rb_bug(failed); perror(failed); } while (0)
static int
...
#define init_sigchld(signum) INSTALL_SIGHANDLER(init_sigchld(signum), #signum, signum)
#endif
void
