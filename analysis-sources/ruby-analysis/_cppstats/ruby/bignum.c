...
#endif
#define BIGZEROP(x) (BIGNUM_LEN(x) == 0 || (BDIGITS(x)[0] == 0 && (BIGNUM_LEN(x) == 1 || bigzero_p(x))))
#define BIGSIZE(x) (BIGNUM_LEN(x) == 0 ? (size_t)0 : BDIGITS(x)[BIGNUM_LEN(x)-1] ? (size_t)(BIGNUM_LEN(x)*SIZEOF_BDIGIT - nlz(BDIGITS(x)[BIGNUM_LEN(x)-1])/CHAR_BIT) : rb_absint_size(x, NULL))
...
#define BDIGITS_ZERO(ptr, n) do { BDIGIT *bdigitz_zero_ptr = (ptr); size_t bdigitz_zero_n = (n); while (bdigitz_zero_n) { *bdigitz_zero_ptr++ = 0; bdigitz_zero_n--; } } while (0)
#define BARY_TRUNC(ds, n) do { while (0 < (n) && (ds)[(n)-1] == 0) (n)--; } while (0)
...
#define INTEGER_PACK_WORDORDER_MASK (INTEGER_PACK_MSWORD_FIRST | INTEGER_PACK_LSWORD_FIRST)
#define INTEGER_PACK_BYTEORDER_MASK (INTEGER_PACK_MSBYTE_FIRST | INTEGER_PACK_LSBYTE_FIRST | INTEGER_PACK_NATIVE_BYTE_ORDER)
static void
...
#define reinterpret_cast(type, value) (type) __builtin_assume_aligned((value), sizeof(*(type)NULL));
#else
...
#define BIGNUM_SET_LEN(b,l) (BIGNUM_EMBED_P(b) ? (void)(RBASIC(b)->flags = (RBASIC(b)->flags & ~BIGNUM_EMBED_LEN_MASK) | ((l) << BIGNUM_EMBED_LEN_SHIFT)) : (void)(RBIGNUM(b)->as.heap.len = (l)))
static void
...
#define ADV(n) do {if (len > 0 && len <= (n)) goto bad; str += (n); len -= (n); } while (0)
#define ASSERT_LEN() do {assert(len != 0); if (len0 >= 0) assert(s + len0 == str + len); } while (0)
if (!str) {
bad:
...
if (ndigits) *ndigits += num_digits;
if (badcheck) {
if (num_digits == 0) return Qnil; 
while (len < 0 ? *end : end < str + len) {
if (!ISSPACE(*end)) return Qnil; 
end++;
}
}
if (POSFIXABLE(val)) {
...
if (LONG_MAX-1 < len)
rb_raise(rb_eArgError, "too big number");
b2s->result = rb_usascii_str_new(0, (long)(len + 1)); 
...
#endif
#if !defined(BDIGIT_DBL_TO_DOUBLE)
...
long yy;
for (; ! FIXNUM_P(y); y = rb_big_rshift(y, LONG2FIX(1L))) {
