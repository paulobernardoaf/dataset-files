...
#if !defined(UBF_TIMER)
#if defined(HAVE_TIMER_SETTIME) && defined(HAVE_TIMER_CREATE) && defined(CLOCK_MONOTONIC) && defined(USE_UBF_LIST)
...
#if defined(HAVE_PTHREAD_CONDATTR_SETCLOCK) && defined(CLOCK_REALTIME) && defined(CLOCK_MONOTONIC) && defined(HAVE_CLOCK_GETTIME)
...
#if !defined(MAINSTACKADDR_AVAILABLE)
...
{
#define CHECK_ERR(expr) {int err = (expr); if (err) return err;}
...
#endif
#if defined(HAVE_PTHREAD_ATTR_GETGUARDSIZE)
...
enum {stack_check_margin = 0x1000}; 
...
#if defined(HAVE_PTHREAD_ATTR_SETINHERITSCHED)
...
rb_thread_t *th = (rb_thread_t *)ptr;
rb_vm_t *vm = th->vm;
const rb_thread_t *cur = ruby_thread_from_native(); 
...
static void
ubf_timer_arm(rb_pid_t current) 
{
#if UBF_TIMER == UBF_TIMER_POSIX
...
#define CLOSE_INVALIDATE_PAIR(expr) close_invalidate_pair(expr,"close_invalidate: "#expr)
...
return 0;
check_pid:
if (signal_self_pipe.owner_process == getpid()) 
return 1;
return 0;
}
...
#define GVL_UNLOCK_BEGIN_YIELD(th) do { const native_thread_data_t *next; rb_vm_t *vm = th->vm; RB_GC_SAVE_MACHINE_CONTEXT(th); rb_native_mutex_lock(&vm->gvl.lock); next = gvl_release_common(vm); rb_native_mutex_unlock(&vm->gvl.lock); if (!next && vm_living_thread_num(vm) > 1) { native_thread_yield(); }
...
pfd[0].fd = signal_self_pipe.normal[0]; 
