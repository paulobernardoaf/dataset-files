...
#define RUBY_CRITICAL if (0) {} else 
...
#define LK_ERR(f,i) do { if (f) i = 0; else { DWORD err = GetLastError(); if (err == ERROR_LOCK_VIOLATION || err == ERROR_IO_PENDING) errno = EWOULDBLOCK; else if (err == ERROR_NOT_LOCKED) i = 0; else errno = map_errno(err); } } while (0)
...
BOOL f;
#define env wk.val
#define set_env_val(vname) do { typedef char wk_name_offset[(numberof(wk.name) - (numberof(vname) - 1)) * 2 + 1]; WCHAR *const buf = wk.name + sizeof(wk_name_offset) / 2; MEMCPY(buf, vname, WCHAR, numberof(vname) - 1); _wputenv(buf); } while (0)
...
#define FOREACH_CHILD(v) do { struct ChildRecord* v; for (v = ChildRecord; v < ChildRecord + sizeof(ChildRecord) / sizeof(ChildRecord[0]); ++v)
...
int nullfd = -1;
int keep = 0;
#define open_null(fd) (((nullfd < 0) ? (nullfd = open("NUL", O_RDWR)) : 0), ((nullfd == (fd)) ? (keep = 1) : dup2(nullfd, fd)), (fd))
...
ret = (GetLastError() == ERROR_BROKEN_PIPE); 
}
}
...
static int
is_readable_console(SOCKET sock) 
{
int ret = 0;
DWORD n = 0;
INPUT_RECORD ir;
...
rb_fd_init(&pipe_rd);
extract_fd(&pipe_rd, else_rd.fdset, is_pipe); 

rb_fd_init(&cons_rd);
extract_fd(&cons_rd, else_rd.fdset, is_console); 

rb_fd_init(&except);
extract_fd(&except, ex, is_not_socket); 
...
{
struct timeval rest;
const struct timeval wait = {0, 10 * 1000}; 
...
if (ret == WAIT_OBJECT_0 + 1) return -1; 
...
#if !defined(FILE_DEVICE_FILE_SYSTEM)
...
#define COPY_STAT(src, dest, size_cast) do { (dest).st_dev = (src).st_dev; (dest).st_ino = (src).st_ino; (dest).st_mode = (src).st_mode; (dest).st_nlink = (src).st_nlink; (dest).st_uid = (src).st_uid; (dest).st_gid = (src).st_gid; (dest).st_rdev = (src).st_rdev; (dest).st_size = size_cast(src).st_size; (dest).st_atime = (src).st_atime; (dest).st_mtime = (src).st_mtime; (dest).st_ctime = (src).st_ctime; } while (0)
...
#define set_new_std_handle(newfd, handle) do { if ((unsigned)(newfd) > 2) break; SetStdHandle(STD_INPUT_HANDLE+(STD_OUTPUT_HANDLE-STD_INPUT_HANDLE)*(newfd), (handle)); } while (0)
...
#define constat_attr_color_reverse(attr) ((attr) & ~(FOREGROUND_MASK | BACKGROUND_MASK)) | (((attr) & FOREGROUND_MASK) << 4) | (((attr) & BACKGROUND_MASK) >> 4)
