...
#define STR_CASE(type, attr, obj, dest, conv) case type: { dest = conv(((String) { .size = obj->via.attr.size, .data = (obj->via.attr.ptr == NULL || obj->via.attr.size == 0 ? xmemdupz("", 0) : xmemdupz(obj->via.attr.ptr, obj->via.attr.size)), })); break; }
