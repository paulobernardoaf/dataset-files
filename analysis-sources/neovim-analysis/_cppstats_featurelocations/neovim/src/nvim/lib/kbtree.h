...
#define __KB_TREE_T(name,key_t,T) typedef struct kbnode_##name##_s kbnode_##name##_t; struct kbnode_##name##_s { int32_t n; bool is_internal; key_t key[2*T-1]; kbnode_##name##_t *ptr[]; } ; typedef struct { kbnode_##name##_t *root; int n_keys, n_nodes; } kbtree_##name##_t; typedef struct { kbnode_##name##_t *x; int i; } kbpos_##name##_t; typedef struct { kbpos_##name##_t stack[KB_MAX_DEPTH], *p; } kbitr_##name##_t; 
...
#define __KB_GET(name, key_t, kbnode_t) static key_t *kb_getp_##name(kbtree_##name##_t *b, key_t * __restrict k) { if (!b->root) { return 0; } int i, r = 0; kbnode_t *x = b->root; while (x) { i = __kb_getp_aux_##name(x, k, &r); if (i >= 0 && r == 0) return &__KB_KEY(key_t, x)[i]; if (x->is_internal == 0) return 0; x = __KB_PTR(b, x)[i + 1]; } return 0; } static inline key_t *kb_get_##name(kbtree_##name##_t *b, key_t k) { return kb_getp_##name(b, &k); }
...
#define __KB_INTERVAL(name, key_t, kbnode_t) static inline void kb_intervalp_##name(kbtree_##name##_t *b, key_t * __restrict k, key_t **lower, key_t **upper) { if (!b->root) { return; } int i, r = 0; kbnode_t *x = b->root; *lower = *upper = 0; while (x) { i = __kb_getp_aux_##name(x, k, &r); if (i >= 0 && r == 0) { *lower = *upper = &__KB_KEY(key_t, x)[i]; return; } if (i >= 0) *lower = &__KB_KEY(key_t, x)[i]; if (i < x->n - 1) *upper = &__KB_KEY(key_t, x)[i + 1]; if (x->is_internal == 0) return; x = __KB_PTR(b, x)[i + 1]; } } static inline void kb_interval_##name(kbtree_##name##_t *b, key_t k, key_t **lower, key_t **upper) { kb_intervalp_##name(b, &k, lower, upper); }
...
#define KBTREE_INIT_IMPL(name, key_t, kbnode_t, __cmp, T, ILEN) __KB_TREE_T(name, key_t, T) __KB_GET_AUX1(name, key_t, kbnode_t, __cmp) __KB_GET(name, key_t, kbnode_t) __KB_INTERVAL(name, key_t, kbnode_t) __KB_PUT(name, key_t, kbnode_t, __cmp, T, ILEN) __KB_DEL(name, key_t, kbnode_t, T) __KB_ITR(name, key_t, kbnode_t)
