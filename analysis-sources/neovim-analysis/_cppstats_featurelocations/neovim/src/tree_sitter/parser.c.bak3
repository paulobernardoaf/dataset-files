...
#define LOG(...) if (self->lexer.logger.log || self->dot_graph_file) { snprintf(self->lexer.debug_buffer, TREE_SITTER_SERIALIZATION_BUFFER_SIZE, __VA_ARGS__); ts_parser__log(self); }





#define LOG_STACK() if (self->dot_graph_file) { ts_stack_print_dot_graph(self->stack, self->language, self->dot_graph_file); fputs("\n\n", self->dot_graph_file); }





#define LOG_TREE(tree) if (self->dot_graph_file) { ts_subtree_print_dot_graph(tree, self->language, self->dot_graph_file); fputs("\n", self->dot_graph_file); }
