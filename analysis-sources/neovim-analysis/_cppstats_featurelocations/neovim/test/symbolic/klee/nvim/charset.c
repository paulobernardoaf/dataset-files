...
assert(false); 
#define PARSE_NUMBER(base, cond, conv) do { while (!STRING_ENDED(ptr) && (cond)) { if (un < UVARNUMBER_MAX / base) { un = base * un + (uvarnumber_T)(conv); } else { un = UVARNUMBER_MAX; } ptr++; } } while (0)
