...
typedef struct {
PossiblyFreedShadaEntry marks[NLOCALMARKS]; 
PossiblyFreedShadaEntry changes[JUMPLISTSIZE]; 
size_t changes_size; 
...
PossiblyFreedShadaEntry global_marks[NMARKS]; 
PossiblyFreedShadaEntry numbered_marks[EXTRA_MARKS]; 
PossiblyFreedShadaEntry registers[NUM_SAVED_REGISTERS]; 
...
khash_t(strset) dumped_variables; 
khash_t(file_marks) file_marks; 
} WriteMergerState;
struct sd_read_def;
...
#endif
#define DEF_SDE(name, attr, ...) [kSDItem##name] = { .timestamp = 0, .type = kSDItem##name, .data = { .attr = { __VA_ARGS__ } } }
...
#define HMLL_FORALL(hmll, cur_entry, code) for (HMLListEntry *cur_entry = (hmll)->first; cur_entry != NULL; cur_entry = cur_entry->next) { code } 
...
#define HMLL_ITER_BACK(hmll, cur_entry, code) for (cur_entry = (hmll)->last; cur_entry != NULL; cur_entry = cur_entry->prev) { code }
...
#define MERGE_JUMPS(jumps_size, jumps, jumps_type, timestamp_attr, mark_attr, entry, fname_cond, free_func, fin_func, idxadj_func, afterfree_func) do { const int jl_len = (int) jumps_size; int i; for (i = jl_len; i > 0; i--) { const jumps_type jl_entry = jumps[i - 1]; if (jl_entry.timestamp_attr <= entry.timestamp) { if (marks_equal(jl_entry.mark_attr, entry.data.filemark.mark) && fname_cond) { i = -1; } break; } } if (i > 0) { if (jl_len == JUMPLISTSIZE) { free_func(jumps[0]); i--; if (i > 0) { memmove(&jumps[0], &jumps[1], sizeof(jumps[1]) * (size_t) i); } } else if (i != jl_len) { memmove(&jumps[i + 1], &jumps[i], sizeof(jumps[0]) * (size_t) (jl_len - i)); } } else if (i == 0) { if (jl_len == JUMPLISTSIZE) { i = -1; } else if (jl_len > 0) { memmove(&jumps[1], &jumps[0], sizeof(jumps[0]) * (size_t) jl_len); } } if (i != -1) { jumps[i] = fin_func(entry); if (jl_len < JUMPLISTSIZE) { jumps_size++; } idxadj_func(i); } else { shada_free_shada_entry(&entry); afterfree_func(entry); } } while (0)
...
#define ADJUST_IDX(i) if (curwin->w_jumplistidx >= i && curwin->w_jumplistidx + 1 <= curwin->w_jumplistlen) { curwin->w_jumplistidx++; }
...
#define PACK_STATIC_STR(s) do { msgpack_pack_str(spacker, sizeof(s) - 1); msgpack_pack_str_body(spacker, s, sizeof(s) - 1); } while (0)
#define PACK_STRING(s) do { const String s_ = (s); msgpack_pack_str(spacker, s_.size); if (s_.size) { msgpack_pack_str_body(spacker, s_.data, s_.size); } } while (0)
#define PACK_BIN(s) do { const String s_ = (s); msgpack_pack_bin(spacker, s_.size); if (s_.size > 0) { msgpack_pack_bin_body(spacker, s_.data, s_.size); } } while (0)
...
#define DUMP_ADDITIONAL_ELEMENTS(src, what) do { if ((src) != NULL) { TV_LIST_ITER((src), li, { if (encode_vim_to_msgpack(spacker, TV_LIST_ITEM_TV(li), _("additional elements of ShaDa " what)) == FAIL) { goto shada_pack_entry_error; } }); } } while (0)
#define DUMP_ADDITIONAL_DATA(src, what) do { dict_T *const d = (src); if (d != NULL) { size_t todo = d->dv_hashtab.ht_used; for (const hashitem_T *hi= d->dv_hashtab.ht_array; todo; hi++) { if (!HASHITEM_EMPTY(hi)) { todo--; dictitem_T *const di = TV_DICT_HI2DI(hi); const size_t key_len = strlen((const char *)hi->hi_key); msgpack_pack_str(spacker, key_len); msgpack_pack_str_body(spacker, (const char *)hi->hi_key, key_len); if (encode_vim_to_msgpack(spacker, &di->di_tv, _("additional data of ShaDa " what)) == FAIL) { goto shada_pack_entry_error; } } } } } while (0)
...
#define PACK_BOOL(entry, name, attr) do { if (!CHECK_DEFAULT(entry, search_pattern.attr)) { PACK_STATIC_STR(name); if (sd_default_values[entry.type].data.search_pattern.attr) { msgpack_pack_false(spacker); } else { msgpack_pack_true(spacker); } } } while (0)
...
#define COMPARE_WITH_ENTRY(wms_entry_, entry) do { PossiblyFreedShadaEntry *const wms_entry = (wms_entry_); if (wms_entry->data.type != kSDItemMissing) { if (wms_entry->data.timestamp >= (entry).timestamp) { shada_free_shada_entry(&(entry)); break; } if (wms_entry->can_free_entry) { shada_free_shada_entry(&wms_entry->data); } } *wms_entry = pfs_entry; } while (0)
const PossiblyFreedShadaEntry pfs_entry = {
.can_free_entry = true,
.data = entry,
};
...
#define FREE_POSSIBLY_FREED_SHADA_ENTRY(entry) do { if (entry.can_free_entry) { shada_free_shada_entry(&entry.data); } } while (0)
...
#define PACK_WMS_ARRAY(wms_array) do { for (size_t i_ = 0; i_ < ARRAY_SIZE(wms_array); i_++) { if (wms_array[i_].data.type != kSDItemMissing) { if (shada_pack_pfreed_entry(packer, wms_array[i_], max_kbyte) == kSDWriteFailed) { ret = kSDWriteFailed; goto shada_write_exit; } } } } while (0)
...
#define PACK_WMS_ENTRY(wms_entry) do { if (wms_entry.data.type != kSDItemMissing) { if (shada_pack_pfreed_entry(packer, wms_entry, max_kbyte) == kSDWriteFailed) { ret = kSDWriteFailed; goto shada_write_exit; } } } while (0)
...
#define READERR(entry_name, error_desc) RERR "Error while reading ShaDa file: " entry_name " entry at position %" PRIu64 " " error_desc
#define CHECK_KEY(key, expected) ( key.via.str.size == sizeof(expected) - 1 && STRNCMP(key.via.str.ptr, expected, sizeof(expected) - 1) == 0)
#define CLEAR_GA_AND_ERROR_OUT(ga) do { ga_clear(&ga); goto shada_read_next_item_error; } while (0)
#define ID(s) s
...
#define CHECKED_ENTRY(condition, error_desc, entry_name, obj, tgt, attr, proc) do { if (!(condition)) { emsgf(_(READERR(entry_name, error_desc)), initial_fpos); CLEAR_GA_AND_ERROR_OUT(ad_ga); } tgt = proc(obj.via.attr); } while (0)
#define CHECK_KEY_IS_STR(un, entry_name) if (un.data.via.map.ptr[i].key.type != MSGPACK_OBJECT_STR) { emsgf(_(READERR(entry_name, "has key which is not a string")), initial_fpos); CLEAR_GA_AND_ERROR_OUT(ad_ga); } else if (un.data.via.map.ptr[i].key.via.str.size == 0) { emsgf(_(READERR(entry_name, "has empty key")), initial_fpos); CLEAR_GA_AND_ERROR_OUT(ad_ga); }
#define CHECKED_KEY(un, entry_name, name, error_desc, tgt, condition, attr, proc) else if (CHECK_KEY( un.data.via.map.ptr[i].key, name)) { CHECKED_ENTRY( condition, "has " name " key value " error_desc, entry_name, un.data.via.map.ptr[i].val, tgt, attr, proc); }
#define TYPED_KEY(un, entry_name, name, type_name, tgt, objtype, attr, proc) CHECKED_KEY( un, entry_name, name, "which is not " type_name, tgt, un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_##objtype, attr, proc)
...
#define CONVERTED_STRING_KEY(un, entry_name, name, tgt) TYPED_KEY(un, entry_name, name, "a binary", tgt, BIN, bin, BIN_CONVERTED)
#define INT_KEY(un, entry_name, name, tgt, proc) CHECKED_KEY( un, entry_name, name, "which is not an integer", tgt, ((un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_POSITIVE_INTEGER) || (un.data.via.map.ptr[i].val.type == MSGPACK_OBJECT_NEGATIVE_INTEGER)), i64, proc)
...
#define ADDITIONAL_KEY(un) else { ga_grow(&ad_ga, 1); memcpy(((char *)ad_ga.ga_data) + ((size_t)ad_ga.ga_len * sizeof(*un.data.via.map.ptr)), un.data.via.map.ptr + i, sizeof(*un.data.via.map.ptr)); ad_ga.ga_len++; }
#define CONVERTED(str, len) (xmemdupz((str), (len)))
#define BIN_CONVERTED(b) CONVERTED(b.ptr, b.size)
#define SET_ADDITIONAL_DATA(tgt, name) do { if (ad_ga.ga_len) { msgpack_object obj = { .type = MSGPACK_OBJECT_MAP, .via = { .map = { .size = (uint32_t) ad_ga.ga_len, .ptr = ad_ga.ga_data, } } }; typval_T adtv; if (msgpack_to_vim(obj, &adtv) == FAIL || adtv.v_type != VAR_DICT) { emsgf(_(READERR(name, "cannot be converted to a VimL dictionary")), initial_fpos); ga_clear(&ad_ga); tv_clear(&adtv); goto shada_read_next_item_error; } tgt = adtv.vval.v_dict; } ga_clear(&ad_ga); } while (0)
#define SET_ADDITIONAL_ELEMENTS(src, src_maxsize, tgt, name) do { if ((src).size > (size_t) (src_maxsize)) { msgpack_object obj = { .type = MSGPACK_OBJECT_ARRAY, .via = { .array = { .size = ((src).size - (uint32_t) (src_maxsize)), .ptr = (src).ptr + (src_maxsize), } } }; typval_T aetv; if (msgpack_to_vim(obj, &aetv) == FAIL) { emsgf(_(READERR(name, "cannot be converted to a VimL list")), initial_fpos); tv_clear(&aetv); goto shada_read_next_item_error; } assert(aetv.v_type == VAR_LIST); (tgt) = aetv.vval.v_list; } } while (0)
