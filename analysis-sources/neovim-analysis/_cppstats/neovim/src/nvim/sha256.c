...
#endif
#define GET_UINT32(n, b, i) { (n) = ((uint32_t)(b)[(i)] << 24) | ((uint32_t)(b)[(i) + 1] << 16) | ((uint32_t)(b)[(i) + 2] << 8) | ((uint32_t)(b)[(i) + 3]); }
#define PUT_UINT32(n, b, i) { (b)[(i)] = (char_u)((n) >> 24); (b)[(i) + 1] = (char_u)((n) >> 16); (b)[(i) + 2] = (char_u)((n) >> 8); (b)[(i) + 3] = (char_u)((n)); }
...
uint32_t left = ctx->total[0] & (SHA256_BUFFER_SIZE-1); 
...
static char hexit[SHA256_BUFFER_SIZE + 1]; 
...
{
char output[SHA256_BUFFER_SIZE + 1]; 
context_sha256_T ctx;
char_u buf[1000];
char_u sha256sum[SHA256_SUM_SIZE];
const char *hexit;
