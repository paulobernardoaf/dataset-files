...
#define TYPVAL_ENCODE_CONV_STRING(tv, buf, len) do { const char *const buf_ = (const char *) buf; if (buf == NULL) { ga_concat(gap, "''"); } else { const size_t len_ = (len); ga_grow(gap, (int) (2 + len_ + memcnt(buf_, '\'', len_))); ga_append(gap, '\''); for (size_t i_ = 0; i_ < len_; i_++) { if (buf_[i_] == '\'') { ga_append(gap, '\''); } ga_append(gap, buf_[i_]); } ga_append(gap, '\''); } } while (0)
#define TYPVAL_ENCODE_CONV_STR_STRING(tv, buf, len) TYPVAL_ENCODE_CONV_STRING(tv, buf, len)
#define TYPVAL_ENCODE_CONV_EXT_STRING(tv, buf, len, type)
#define TYPVAL_ENCODE_CONV_NUMBER(tv, num) do { char numbuf[NUMBUFLEN]; vim_snprintf(numbuf, ARRAY_SIZE(numbuf), "%" PRId64, (int64_t) (num)); ga_concat(gap, numbuf); } while (0)
#define TYPVAL_ENCODE_CONV_FLOAT(tv, flt) do { const float_T flt_ = (flt); switch (xfpclassify(flt_)) { case FP_NAN: { ga_concat(gap, (char_u *) "str2float('nan')"); break; } case FP_INFINITE: { if (flt_ < 0) { ga_append(gap, '-'); } ga_concat(gap, (char_u *) "str2float('inf')"); break; } default: { char numbuf[NUMBUFLEN]; vim_snprintf(numbuf, ARRAY_SIZE(numbuf), "%g", flt_); ga_concat(gap, (char_u *) numbuf); } } } while (0)
#define TYPVAL_ENCODE_CONV_FUNC_START(tv, fun) do { const char *const fun_ = (const char *)(fun); if (fun_ == NULL) { internal_error("string(): NULL function name"); ga_concat(gap, "function(NULL"); } else { ga_concat(gap, "function("); TYPVAL_ENCODE_CONV_STRING(tv, fun_, strlen(fun_)); }} while (0)
#define TYPVAL_ENCODE_CONV_FUNC_BEFORE_ARGS(tv, len) do { if (len != 0) { ga_concat(gap, ", "); } } while (0)
#define TYPVAL_ENCODE_CONV_FUNC_BEFORE_SELF(tv, len) do { if ((ptrdiff_t)len != -1) { ga_concat(gap, ", "); } } while (0)
...
#define TYPVAL_ENCODE_CONV_RECURSE(val, conv_type) do { char ebuf[NUMBUFLEN + 7]; size_t backref = 0; for (; backref < kv_size(*mpstack); backref++) { const MPConvStackVal mpval = kv_A(*mpstack, backref); if (mpval.type == conv_type) { if (conv_type == kMPConvDict) { if ((void *) mpval.data.d.dict == (void *) val) { break; } } else if (conv_type == kMPConvList) { if ((void *) mpval.data.l.list == (void *) val) { break; } } } } if (conv_type == kMPConvDict) { vim_snprintf(ebuf, ARRAY_SIZE(ebuf), "{...@%zu}", backref); } else { vim_snprintf(ebuf, ARRAY_SIZE(ebuf), "[...@%zu]", backref); } ga_concat(gap, &ebuf[0]); return OK; } while (0)
...
#undef TYPVAL_ENCODE_FIRST_ARG_NAME
#undef TYPVAL_ENCODE_CONV_RECURSE
#define TYPVAL_ENCODE_CONV_RECURSE(val, conv_type) do { if (!did_echo_string_emsg) { did_echo_string_emsg = true; EMSG(_("E724: unable to correctly dump variable " "with self-referencing container")); } } while (0)
...
#define TYPVAL_ENCODE_CONV_STRING(tv, buf, len) do { if (convert_to_json_string(gap, (const char *) (buf), (len)) != OK) { return FAIL; } } while (0)
#undef TYPVAL_ENCODE_CONV_EXT_STRING
#define TYPVAL_ENCODE_CONV_EXT_STRING(tv, buf, len, type) do { xfree(buf); EMSG(_("E474: Unable to convert EXT string to JSON")); return FAIL; } while (0)
#undef TYPVAL_ENCODE_CONV_FUNC_START
#define TYPVAL_ENCODE_CONV_FUNC_START(tv, fun) return conv_error(_("E474: Error while dumping %s, %s: " "attempt to dump function reference"), mpstack, objname)
...
#define TYPVAL_ENCODE_SPECIAL_DICT_KEY_CHECK(label, key) do { if (!encode_check_json_key(&key)) { EMSG(_("E474: Invalid key in special dictionary")); goto label; } } while (0)
#define TYPVAL_ENCODE_SCOPE static
...
#define TYPVAL_ENCODE_CONV_STRING(tv, buf, len) do { if (buf == NULL) { msgpack_pack_bin(packer, 0); } else { const size_t len_ = (len); msgpack_pack_bin(packer, len_); msgpack_pack_bin_body(packer, buf, len_); } } while (0)
#define TYPVAL_ENCODE_CONV_STR_STRING(tv, buf, len) do { if (buf == NULL) { msgpack_pack_str(packer, 0); } else { const size_t len_ = (len); msgpack_pack_str(packer, len_); msgpack_pack_str_body(packer, buf, len_); } } while (0)
#define TYPVAL_ENCODE_CONV_EXT_STRING(tv, buf, len, type) do { if (buf == NULL) { msgpack_pack_ext(packer, 0, (int8_t) type); } else { const size_t len_ = (len); msgpack_pack_ext(packer, len_, (int8_t) type); msgpack_pack_ext_body(packer, buf, len_); } } while (0)
...
#define TYPVAL_ENCODE_CONV_FUNC_START(tv, fun) return conv_error(_("E5004: Error while dumping %s, %s: " "attempt to dump function reference"), mpstack, objname)
...
#define TYPVAL_ENCODE_CONV_BOOL(tv, num) do { if (num) { msgpack_pack_true(packer); } else { msgpack_pack_false(packer); } } while (0)
...
#define TYPVAL_ENCODE_CONV_RECURSE(val, conv_type) return conv_error(_("E5005: Unable to dump %s: " "container references itself in %s"), mpstack, objname)
#define TYPVAL_ENCODE_ALLOW_SPECIALS true
#define TYPVAL_ENCODE_SCOPE
