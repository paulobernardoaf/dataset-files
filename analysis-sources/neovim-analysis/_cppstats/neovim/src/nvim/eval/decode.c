...
#define OBJ(obj_tv, is_sp_string, didcomma_, didcolon_) ((ValuesStackItem) { .is_special_string = (is_sp_string), .val = (obj_tv), .didcomma = (didcomma_), .didcolon = (didcolon_), })
#define POP(obj_tv, is_sp_string) do { if (json_decoder_pop(OBJ(obj_tv, is_sp_string, *didcomma, *didcolon), stack, container_stack, &p, next_map_special, didcomma, didcolon) == FAIL) { goto parse_json_string_fail; } if (*next_map_special) { goto parse_json_string_ret; } } while (0)
...
char *str_end = str;
bool hasnul = false;
#define PUT_FST_IN_PAIR(fst_in_pair, str_end) do { if (fst_in_pair != 0) { str_end += utf_char2bytes(fst_in_pair, (char_u *) str_end); fst_in_pair = 0; } } while (0)
...
#define POP(obj_tv, is_sp_string) do { if (json_decoder_pop(OBJ(obj_tv, is_sp_string, didcomma, didcolon), &stack, &container_stack, &p, &next_map_special, &didcomma, &didcolon) == FAIL) { goto json_decode_string_fail; } if (next_map_special) { goto json_decode_string_cycle_start; } } while (0)
...
#if defined(NVIM_MSGPACK_HAS_FLOAT32)
