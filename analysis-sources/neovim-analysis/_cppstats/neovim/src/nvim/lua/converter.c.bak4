...
#define TYPVAL_ENCODE_CONV_NIL(tv) do { if (typval_conv_special) { lua_pushnil(lstate); } else { nlua_pushref(lstate, nlua_nil_ref); } } while (0)
...
#define TYPVAL_ENCODE_CONV_FUNC_START(tv, fun) do { TYPVAL_ENCODE_CONV_NIL(tv); goto typval_encode_stop_converting_one_item; } while (0)
...
#define TYPVAL_ENCODE_CONV_EMPTY_DICT(tv, dict) do { if (typval_conv_special) { nlua_create_typed_table(lstate, 0, 0, kObjectTypeDictionary); } else { lua_createtable(lstate, 0, 0); nlua_pushref(lstate, nlua_empty_dict_ref); lua_setmetatable(lstate, -2); } } while (0)
...
#define TYPVAL_ENCODE_CONV_LIST_START(tv, len) do { if (!lua_checkstack(lstate, lua_gettop(lstate) + 3)) { emsgf(_("E5102: Lua failed to grow stack to %i"), lua_gettop(lstate) + 3); return false; } lua_createtable(lstate, (int)(len), 0); lua_pushnumber(lstate, 1); } while (0)
...
#define TYPVAL_ENCODE_CONV_LIST_BETWEEN_ITEMS(tv) do { lua_Number idx = lua_tonumber(lstate, -2); lua_rawset(lstate, -3); lua_pushnumber(lstate, idx + 1); } while (0)
...
#define TYPVAL_ENCODE_CONV_DICT_START(tv, dict, len) do { if (!lua_checkstack(lstate, lua_gettop(lstate) + 3)) { emsgf(_("E5102: Lua failed to grow stack to %i"), lua_gettop(lstate) + 3); return false; } lua_createtable(lstate, 0, (int)(len)); } while (0)
...
#define TYPVAL_ENCODE_CONV_RECURSE(val, conv_type) do { for (size_t backref = kv_size(*mpstack); backref; backref--) { const MPConvStackVal mpval = kv_A(*mpstack, backref - 1); if (mpval.type == conv_type) { if (conv_type == kMPConvDict ? (void *)mpval.data.d.dict == (void *)(val) : (void *)mpval.data.l.list == (void *)(val)) { lua_pushvalue(lstate, -((int)((kv_size(*mpstack) - backref + 1) * 2))); break; } } } } while (0)
...
#define GENERATE_INDEX_FUNCTION(type) void nlua_push_##type(lua_State *lstate, const type item, bool special) FUNC_ATTR_NONNULL_ALL { lua_pushnumber(lstate, (lua_Number)(item)); }
...
#define ADD_TYPE(type, data_key) case kObjectType##type: { nlua_push_##type(lstate, obj.data.data_key, special); break; }
...
#undef ADD_TYPE
#define ADD_REMOTE_TYPE(type) case kObjectType##type: { nlua_push_##type(lstate, (type)obj.data.integer, special); break; }
...
#define GENERATE_INDEX_FUNCTION(type) type nlua_pop_##type(lua_State *lstate, Error *err) FUNC_ATTR_NONNULL_ALL FUNC_ATTR_WARN_UNUSED_RESULT { type ret; ret = (type)lua_tonumber(lstate, -1); lua_pop(lstate, 1); return ret; }
