...
TSLua_parser *p = lua_newuserdata(L, sizeof(TSLua_parser)); 
p->parser = parser;
p->tree = NULL;

lua_getfield(L, LUA_REGISTRYINDEX, "treesitter_parser"); 
...
lua_getfield(L, LUA_REGISTRYINDEX, "treesitter_tree"); 
lua_setmetatable(L, -2); 
...
*ud = node;
lua_getfield(L, LUA_REGISTRYINDEX, "treesitter_node"); 
lua_setmetatable(L, -2); 
...
push_node(L, capture.node, lua_upvalueindex(2)); 
...
*ud = query;
lua_getfield(L, LUA_REGISTRYINDEX, "treesitter_query"); 
lua_setmetatable(L, -2); 
return 1;
}
