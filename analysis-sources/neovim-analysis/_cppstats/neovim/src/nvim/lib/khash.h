...
#define kh_foreach_key(h, kvar, code) { khint_t __i; for (__i = kh_begin(h); __i != kh_end(h); __i++) { if (!kh_exist(h, __i)) { continue; } (kvar) = kh_key(h, __i); code; } }
...
#define KHASH_EMPTY_TABLE(name) ((kh_##name##_t) { .n_buckets = 0, .size = 0, .n_occupied = 0, .upper_bound = 0, .flags = NULL, .keys = NULL, .vals = NULL, })
