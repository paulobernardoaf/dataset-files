...
#define _RINGBUF_LENGTH(rb) ((rb)->first == NULL ? 0 : ((rb)->next == (rb)->first) ? (size_t) ((rb)->buf_end - (rb)->buf) + 1 : ((rb)->next > (rb)->first) ? (size_t) ((rb)->next - (rb)->first) : (size_t) ((rb)->next - (rb)->buf + (rb)->buf_end - (rb)->first + 1))
...
#define RINGBUF_FORALL(rb, RBType, varname) size_t varname##_length_fa_ = _RINGBUF_LENGTH(rb); for (RBType *varname = ((rb)->first == NULL ? (rb)->next : (rb)->first); varname##_length_fa_; (varname = _RINGBUF_NEXT(rb, varname)), varname##_length_fa_--)
...
#define RINGBUF_ITER_BACK(rb, RBType, varname) size_t varname##_length_ib_ = _RINGBUF_LENGTH(rb); for (varname = ((rb)->next == (rb)->buf ? (rb)->buf_end : (rb)->next - 1); varname##_length_ib_; (varname = _RINGBUF_PREV(rb, varname)), varname##_length_ib_--)
...
#define RINGBUF_TYPEDEF(TypeName, RBType) typedef struct { RBType *buf; RBType *next; RBType *first; RBType *buf_end; } TypeName##RingBuffer;
...
#define RINGBUF_STATIC(scope, TypeName, RBType, varname, rbsize) static RBType _##varname##_buf[rbsize]; scope TypeName##RingBuffer varname = { .buf = _##varname##_buf, .next = _##varname##_buf, .first = NULL, .buf_end = _##varname##_buf + rbsize - 1, };
