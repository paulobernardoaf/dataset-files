...
#define UI_LOG(funname) do { if (strequal(uilog_last_event, STR(funname))) { uilog_seen++; } else { if (uilog_seen > 0) { logmsg(DEBUG_LOG_LEVEL, "UI: ", NULL, -1, true, "%s (+%zu times...)", uilog_last_event, uilog_seen); } logmsg(DEBUG_LOG_LEVEL, "UI: ", NULL, -1, true, STR(funname)); uilog_seen = 0; xstrlcpy(uilog_last_event, STR(funname), sizeof(uilog_last_event)); } } while (0)
#endif
#define UI_CALL(cond, funname, ...) do { bool any_call = false; for (size_t i = 0; i < ui_count; i++) { UI *ui = uis[i]; if (ui->funname && (cond)) { ui->funname(__VA_ARGS__); any_call = true; } } if (any_call) { UI_LOG(funname); } } while (0)
#if defined(INCLUDE_GENERATED_DECLARATIONS)
