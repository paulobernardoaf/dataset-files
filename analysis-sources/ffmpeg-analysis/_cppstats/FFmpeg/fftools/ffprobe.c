...
#if !HAVE_THREADS
#if defined(pthread_mutex_lock)
#undef pthread_mutex_lock
#endif
#define pthread_mutex_lock(a) do{}while(0)
#if defined(pthread_mutex_unlock)
...
#define DEFINE_WRITER_CLASS(name) static const char *name##_get_name(void *ctx) { return #name ; } static const AVClass name##_class = { .class_name = #name, .item_name = name##_get_name, .option = name##_options }
...
#define CHECK_COMPLIANCE(opt, opt_name) if (opt) { av_log(wctx, AV_LOG_ERROR, "XSD-compliant output selected but option '%s' was selected, XML output may be non-compliant.\n" "You need to disable such option with '-no%s'\n", opt_name, opt_name); return AVERROR(EINVAL); }
...
#define print_fmt(k, f, ...) do { av_bprint_clear(&pbuf); av_bprintf(&pbuf, f, __VA_ARGS__); writer_print_string(w, k, pbuf.str, 0); } while (0)
...
#define print_val(k, v, u) do { struct unit_value uv; uv.val.i = v; uv.unit = u; writer_print_string(w, k, value_string(val_str, sizeof(val_str), uv), 0); } while (0)
#define print_section_header(s) writer_print_section_header(w, s)
#define print_section_footer(s) writer_print_section_footer(w, s)
#define REALLOCZ_ARRAY_STREAM(ptr, cur_n, new_n) { ret = av_reallocp_array(&(ptr), (new_n), sizeof(*(ptr))); if (ret < 0) goto end; memset( (ptr) + (cur_n), 0, ((new_n) - (cur_n)) * sizeof(*(ptr)) ); }
...
par->extradata_size);
#define PRINT_DISPOSITION(flagname, name) do { print_int(name, !!(stream->disposition & AV_DISPOSITION_##flagname)); } while (0)
...
#define PRINT_PIX_FMT_FLAG(flagname, name) do { print_int(name, !!(pixdesc->flags & AV_PIX_FMT_FLAG_##flagname)); } while (0)
...
#define DEFINE_OPT_SHOW_SECTION(section, target_section_id) static int opt_show_##section(void *optctx, const char *opt, const char *arg) { mark_section_show_entries(SECTION_ID_##target_section_id, 1, NULL); return 0; }
...
#define SET_DO_SHOW(id, varname) do { if (check_section_show_entries(SECTION_ID_##id)) do_show_##varname = 1; } while (0)
