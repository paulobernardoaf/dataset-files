...
#define WARN_MULTIPLE_OPT_USAGE(name, type, so, st){char namestr[128] = "";const char *spec = so->specifier && so->specifier[0] ? so->specifier : "";for (i = 0; opt_name_##name[i]; i++)av_strlcatf(namestr, sizeof(namestr), "-%s%s", opt_name_##name[i], opt_name_##name[i+1] ? (opt_name_##name[i+2] ? ", " : " or ") : "");av_log(NULL, AV_LOG_WARNING, "Multiple %s options specified for stream %d, only the last option '-%s%s%s "SPECIFIER_OPT_FMT_##type"' will be used.\n",namestr, st->index, opt_name_##name[0], spec[0] ? ":" : "", spec, so->u.type);}
...
if (!ic)
return 0;
#define METADATA_CHECK_INDEX(index, nb_elems, desc)if ((index) < 0 || (index) >= (nb_elems)) {av_log(NULL, AV_LOG_FATAL, "Invalid %s index %d while processing metadata maps.\n",(desc), (index));exit_program(1);}
#define SET_DICT(type, meta, context, index)switch (type) {case 'g':meta = &context->metadata;break;case 'c':METADATA_CHECK_INDEX(index, context->nb_chapters, "chapter")meta = &context->chapters[index]->metadata;break;case 'p':METADATA_CHECK_INDEX(index, context->nb_programs, "program")meta = &context->programs[index]->metadata;break;case 's':break; default: av_assert0(0);}
