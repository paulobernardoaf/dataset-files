...
#define DEFINE_LAVU_MD(suffix, type, namespace, hsize) static void run_lavu_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { static struct type *h; if (!h && !(h = av_ ##namespace ##_alloc())) fatal_error("out of memory"); av_ ##namespace ##_init(h, hsize); av_ ##namespace ##_update(h, input, size); av_ ##namespace ##_final(h, output); }
...
#define DEFINE_CRYPTO_WRAPPER(suffix, function) static void run_crypto_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { function(input, size, output); }
...
#include <gcrypt.h>
#define DEFINE_GCRYPT_WRAPPER(suffix, algo) static void run_gcrypt_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { gcry_md_hash_buffer(GCRY_MD_ ##algo, output, input, size); }
...
#define DEFINE_GCRYPT_CYPHER_WRAPPER(suffix, cypher, mode, sz) static void run_gcrypt_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { static gcry_cipher_hd_t suffix; if (!suffix) gcry_cipher_open(&suffix, GCRY_CIPHER_ ##cypher, GCRY_CIPHER_MODE_ ##mode, 0); gcry_cipher_setkey(suffix, hardcoded_key, sz); gcry_cipher_encrypt(suffix, output, size, input, size); }
...
#define DEFINE_MBEDCRYPTO_WRAPPER(suffix) static void run_mbedcrypto_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { mbedtls_ ##suffix ##_ret(input, size, output); }
#define DEFINE_MBEDCRYPTO_WRAPPER_SHA2(suffix) static void run_mbedcrypto_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { mbedtls_ ##suffix ##_ret(input, size, output, 0); }
...
#define DEFINE_MBEDCRYPTO_CYPHER_WRAPPER(suffix, cypher, algo) static void run_mbedcrypto_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { mbedtls_ ##cypher ##_context cypher; mbedtls_ ##cypher ##_init(&cypher); mbedtls_ ##cypher ##_setkey_enc(&cypher, hardcoded_key, 128); for (int i = 0; i < size; i += 16) mbedtls_ ##cypher ##_crypt_ecb(&cypher, MBEDTLS_ ##algo ##_ENCRYPT, input + i, output + i); mbedtls_ ##cypher ##_free(&cypher); }
...
#include <tomcrypt.h>
#define DEFINE_TOMCRYPT_WRAPPER(suffix, namespace, algo) static void run_tomcrypt_ ##suffix(uint8_t *output, const uint8_t *input, unsigned size) { hash_state md; namespace ##_init(&md); namespace ##_process(&md, input, size); namespace ##_done(&md, output); }
...
#define IMPL_ALL(...) IMPL(lavu, __VA_ARGS__) IMPL(crypto, __VA_ARGS__) IMPL(gcrypt, __VA_ARGS__) IMPL(mbedcrypto, __VA_ARGS__) IMPL(tomcrypt, __VA_ARGS__)
