...
#if !HAVE_BIGENDIAN
#define GET_LS(a,b,c,s) {ls = a;a = vec_vsx_ld(((b) << 1) + 16, s);}
#define yuv2planeX_8(d1, d2, l1, src, x, perm, filter) do {vector signed short ls;vector signed int vf1, vf2, i1, i2;GET_LS(l1, x, perm, src);i1 = vec_mule(filter, ls);i2 = vec_mulo(filter, ls);vf1 = vec_mergeh(i1, i2);vf2 = vec_mergel(i1, i2);d1 = vec_add(d1, vf1);d2 = vec_add(d2, vf2);} while (0)
#define LOAD_FILTER(vf,f) {vf = vec_vsx_ld(joffset, f);}
#define LOAD_L1(ll1,s,p){ll1 = vec_vsx_ld(xoffset, s);}
#define GET_VF4(a, vf, f) {vf = (vector signed short)vec_vsx_ld(a << 3, f);vf = vec_mergeh(vf, (vector signed short)vzero);}
#define FIRST_LOAD(sv, pos, s, per) {}
#define UPDATE_PTR(s0, d0, s1, d1) {}
#define LOAD_SRCV(pos, a, s, per, v0, v1, vf) {vf = vec_vsx_ld(pos + a, s);}
#define LOAD_SRCV8(pos, a, s, per, v0, v1, vf) LOAD_SRCV(pos, a, s, per, v0, v1, vf)
#define GET_VFD(a, b, f, vf0, vf1, per, vf, off) {vf = vec_vsx_ld((a * 2 * filterSize) + (b * 2) + off, f);}
...
#if !HAVE_BIGENDIAN
#define output_pixel(pos, val) if (big_endian) { AV_WB16(pos, av_clip_uintp2(val >> shift, output_bits)); } else { AV_WL16(pos, av_clip_uintp2(val >> shift, output_bits)); }
...
#undef output_pixel
#define output_pixel(pos, val, bias, signedness) if (big_endian) { AV_WB16(pos, bias + av_clip_ ##signedness ##16(val >> shift)); } else { AV_WL16(pos, bias + av_clip_ ##signedness ##16(val >> shift)); }
...
#endif 
#define yuv2NBPS(bits, BE_LE, is_be, template_size, typeX_t) yuv2NBPS1(bits, BE_LE, is_be, template_size, typeX_t) yuv2NBPSX(bits, BE_LE, is_be, template_size, typeX_t)
#define yuv2NBPS1(bits, BE_LE, is_be, template_size, typeX_t) static void yuv2plane1_ ##bits ##BE_LE ##_vsx(const int16_t *src, uint8_t *dest, int dstW, const uint8_t *dither, int offset) { yuv2plane1_ ##template_size ##_vsx((const typeX_t *) src, (uint16_t *) dest, dstW, is_be, bits); }
#define yuv2NBPSX(bits, BE_LE, is_be, template_size, typeX_t) static void yuv2planeX_ ##bits ##BE_LE ##_vsx(const int16_t *filter, int filterSize, const int16_t **src, uint8_t *dest, int dstW, const uint8_t *dither, int offset){ yuv2planeX_##template_size ##_vsx(filter, filterSize, (const typeX_t **) src, (uint16_t *) dest, dstW, is_be, bits); }
...
#define SETUP(x, buf0, alpha1, buf1, alpha) { x = vec_ld(0, buf0); tmp = vec_mule(x, alpha1); tmp2 = vec_mulo(x, alpha1); tmp3 = vec_mergeh(tmp, tmp2); tmp4 = vec_mergel(tmp, tmp2); x = vec_ld(0, buf1); tmp = vec_mule(x, alpha); tmp2 = vec_mulo(x, alpha); tmp5 = vec_mergeh(tmp, tmp2); tmp6 = vec_mergel(tmp, tmp2); tmp3 = vec_add(tmp3, tmp5); tmp4 = vec_add(tmp4, tmp6); }
static av_always_inline void
...
#define YUV2RGBWRAPPER(name, base, ext, fmt, hasAlpha) static void name ##ext ##_1_vsx(SwsContext *c, const int16_t *buf0, const int16_t *ubuf[2], const int16_t *vbuf[2], const int16_t *abuf0, uint8_t *dest, int dstW, int uvalpha, int y) { name ##base ##_1_vsx_template(c, buf0, ubuf, vbuf, abuf0, dest, dstW, uvalpha, y, fmt, hasAlpha); }
...
#define YUV2PACKEDWRAPPERX(name, base, ext, fmt) static void name ##ext ##_X_vsx(SwsContext *c, const int16_t *lumFilter, const int16_t **lumSrc, int lumFilterSize, const int16_t *chrFilter, const int16_t **chrUSrc, const int16_t **chrVSrc, int chrFilterSize, const int16_t **alpSrc, uint8_t *dest, int dstW, int y) { name ##base ##_X_vsx_template(c, lumFilter, lumSrc, lumFilterSize, chrFilter, chrUSrc, chrVSrc, chrFilterSize, alpSrc, dest, dstW, y, fmt); }
#define YUV2PACKEDWRAPPER2(name, base, ext, fmt) YUV2PACKEDWRAPPERX(name, base, ext, fmt) static void name ##ext ##_2_vsx(SwsContext *c, const int16_t *buf[2], const int16_t *ubuf[2], const int16_t *vbuf[2], const int16_t *abuf[2], uint8_t *dest, int dstW, int yalpha, int uvalpha, int y) { name ##base ##_2_vsx_template(c, buf, ubuf, vbuf, abuf, dest, dstW, yalpha, uvalpha, y, fmt); }
#define YUV2PACKEDWRAPPER(name, base, ext, fmt) YUV2PACKEDWRAPPER2(name, base, ext, fmt) static void name ##ext ##_1_vsx(SwsContext *c, const int16_t *buf0, const int16_t *ubuf[2], const int16_t *vbuf[2], const int16_t *abuf0, uint8_t *dest, int dstW, int uvalpha, int y) { name ##base ##_1_vsx_template(c, buf0, ubuf, vbuf, abuf0, dest, dstW, uvalpha, y, fmt); }
...
#define HCSCALE(in, out) vin = vec_vsx_ld(0, &in[xx]); vin = vec_perm(vin, vin, vperm); vin2 = vec_vsx_ld(1, &in[xx]); vin2 = vec_perm(vin2, vin2, vperm); vtmp = vec_mule(vin, valphaxor); vtmp2 = vec_mulo(vin, valphaxor); vtmp3 = vec_mergeh(vtmp, vtmp2); vtmp4 = vec_mergel(vtmp, vtmp2); vtmp = vec_mule(vin2, valpha); vtmp2 = vec_mulo(vin2, valpha); vd_l = vec_mergeh(vtmp, vtmp2); vd_r = vec_mergel(vtmp, vtmp2); vd_l = vec_add(vd_l, vtmp3); vd_r = vec_add(vd_r, vtmp4); vec_st((vec_s16) vd_l, 0, &out[i]); vec_st((vec_s16) vd_r, 0, &out[i + 8])
