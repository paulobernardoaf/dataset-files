...
#define GET_LS(a,b,c,s) {vector signed short l2 = vec_ld(((b) << 1) + 16, s);ls = vec_perm(a, l2, c);a = l2;}
...
#define LOAD_FILTER(vf,f) {vector unsigned char perm0 = vec_lvsl(joffset, f);vf = vec_ld(joffset, f);vf = vec_perm(vf, vf, perm0);}
#define LOAD_L1(ll1,s,p){p = vec_lvsl(xoffset, s);ll1 = vec_ld(xoffset, s);}
#define GET_VF4(a, vf, f) {vf = vec_ld(a<< 3, f);if ((a << 3) % 16)vf = vec_mergel(vf, (vector signed short)vzero);elsevf = vec_mergeh(vf, (vector signed short)vzero);}
#define FIRST_LOAD(sv, pos, s, per) {sv = vec_ld(pos, s);per = vec_lvsl(pos, s);}
#define UPDATE_PTR(s0, d0, s1, d1) {d0 = s0;d1 = s1;}
#define LOAD_SRCV(pos, a, s, per, v0, v1, vf) {v1 = vec_ld(pos + a + 16, s);vf = vec_perm(v0, v1, per);}
#define LOAD_SRCV8(pos, a, s, per, v0, v1, vf) {if ((((uintptr_t)s + pos) % 16) > 8) {v1 = vec_ld(pos + a + 16, s);}vf = vec_perm(v0, src_v1, per);}
#define GET_VFD(a, b, f, vf0, vf1, per, vf, off) {vf1 = vec_ld((a * 2 * filterSize) + (b * 2) + 16 + off, f);vf = vec_perm(vf0, vf1, per);}
...
#define yuv2plane1_float(template, dest_type, BE_LE) static void yuv2plane1_float ##BE_LE ##_altivec(const int16_t *src, uint8_t *dest, int dstW, const uint8_t *dither, int offset) { template((const int32_t *)src, (dest_type *)dest, dstW); }
#if HAVE_BIGENDIAN
