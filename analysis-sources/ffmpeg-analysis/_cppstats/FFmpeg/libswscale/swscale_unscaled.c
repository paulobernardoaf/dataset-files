...
#define output_pixel(p, v) do { uint16_t *pp = (p); AV_WL16(pp, (v)); } while(0)
#else
...
switch(c->srcFormat) {
#define CASE(pixfmt, prefix) case pixfmt: copy = bayer_##prefix##_to_rgb24_copy; interpolate = bayer_##prefix##_to_rgb24_interpolate; break;
...
switch(c->srcFormat) {
#define CASE(pixfmt, prefix) case pixfmt: copy = bayer_##prefix##_to_yv12_copy; interpolate = bayer_##prefix##_to_yv12_interpolate; break;
CASE(AV_PIX_FMT_BAYER_BGGR8, bggr8)
CASE(AV_PIX_FMT_BAYER_BGGR16LE, bggr16le)
CASE(AV_PIX_FMT_BAYER_BGGR16BE, bggr16be)
CASE(AV_PIX_FMT_BAYER_RGGB8, rggb8)
...
#define isRGBA32(x) ( (x) == AV_PIX_FMT_ARGB || (x) == AV_PIX_FMT_RGBA || (x) == AV_PIX_FMT_BGRA || (x) == AV_PIX_FMT_ABGR )
#define isRGBA64(x) ( (x) == AV_PIX_FMT_RGBA64LE || (x) == AV_PIX_FMT_RGBA64BE || (x) == AV_PIX_FMT_BGRA64LE || (x) == AV_PIX_FMT_BGRA64BE )
#define isRGB48(x) ( (x) == AV_PIX_FMT_RGB48LE || (x) == AV_PIX_FMT_RGB48BE || (x) == AV_PIX_FMT_BGR48LE || (x) == AV_PIX_FMT_BGR48BE )
...
rgbConvFn conv = NULL;
#define IS_NOT_NE(bpp, desc) (((bpp + 7) >> 3) == 2 && (!(desc->flags & AV_PIX_FMT_FLAG_BE) != !HAVE_BIGENDIAN))
...
#define DITHER_COPY(dst, dstStride, src, srcStride, bswap, dbswap)unsigned shift= src_depth-dst_depth, tmp;if (c->dither == SWS_DITHER_NONE) {for (i = 0; i < height; i++) {for (j = 0; j < length-7; j+=8) {dst[j+0] = dbswap(bswap(src[j+0])>>shift);dst[j+1] = dbswap(bswap(src[j+1])>>shift);dst[j+2] = dbswap(bswap(src[j+2])>>shift);dst[j+3] = dbswap(bswap(src[j+3])>>shift);dst[j+4] = dbswap(bswap(src[j+4])>>shift);dst[j+5] = dbswap(bswap(src[j+5])>>shift);dst[j+6] = dbswap(bswap(src[j+6])>>shift);dst[j+7] = dbswap(bswap(src[j+7])>>shift);}for (; j < length; j++) {dst[j] = dbswap(bswap(src[j])>>shift);}dst += dstStride;src += srcStride;}} else if (shiftonly) {for (i = 0; i < height; i++) {const uint8_t *dither= dithers[shift-1][i&7];for (j = 0; j < length-7; j+=8) {tmp = (bswap(src[j+0]) + dither[0])>>shift; dst[j+0] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+1]) + dither[1])>>shift; dst[j+1] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+2]) + dither[2])>>shift; dst[j+2] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+3]) + dither[3])>>shift; dst[j+3] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+4]) + dither[4])>>shift; dst[j+4] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+5]) + dither[5])>>shift; dst[j+5] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+6]) + dither[6])>>shift; dst[j+6] = dbswap(tmp - (tmp>>dst_depth));tmp = (bswap(src[j+7]) + dither[7])>>shift; dst[j+7] = dbswap(tmp - (tmp>>dst_depth));}for (; j < length; j++) {tmp = (bswap(src[j]) + dither[j&7])>>shift; dst[j] = dbswap(tmp - (tmp>>dst_depth));}dst += dstStride;src += srcStride;}} else {for (i = 0; i < height; i++) {const uint8_t *dither= dithers[shift-1][i&7];for (j = 0; j < length-7; j+=8) {tmp = bswap(src[j+0]); dst[j+0] = dbswap((tmp - (tmp>>dst_depth) + dither[0])>>shift);tmp = bswap(src[j+1]); dst[j+1] = dbswap((tmp - (tmp>>dst_depth) + dither[1])>>shift);tmp = bswap(src[j+2]); dst[j+2] = dbswap((tmp - (tmp>>dst_depth) + dither[2])>>shift);tmp = bswap(src[j+3]); dst[j+3] = dbswap((tmp - (tmp>>dst_depth) + dither[3])>>shift);tmp = bswap(src[j+4]); dst[j+4] = dbswap((tmp - (tmp>>dst_depth) + dither[4])>>shift);tmp = bswap(src[j+5]); dst[j+5] = dbswap((tmp - (tmp>>dst_depth) + dither[5])>>shift);tmp = bswap(src[j+6]); dst[j+6] = dbswap((tmp - (tmp>>dst_depth) + dither[6])>>shift);tmp = bswap(src[j+7]); dst[j+7] = dbswap((tmp - (tmp>>dst_depth) + dither[7])>>shift);}for (; j < length; j++) {tmp = bswap(src[j]); dst[j] = dbswap((tmp - (tmp>>dst_depth) + dither[j&7])>>shift);}dst += dstStride;src += srcStride;}}
...
for (i = 0; i < height; i++) {
#define COPY816(w)if (shiftonly) {for (j = 0; j < length; j++)w(&dstPtr2[j], srcPtr[j]<<(dst_depth-8));} else {for (j = 0; j < length; j++)w(&dstPtr2[j], (srcPtr[j]<<(dst_depth-8)) |(srcPtr[j]>>(2*8-dst_depth)));}
if(isBE(c->dstFormat)){
COPY816(AV_WB16)
} else {
COPY816(AV_WL16)
...
#if HAVE_FAST_64BIT
#define FAST_COPY_UP(shift) for (; j < length - 3; j += 4) { uint64_t v = AV_RN64A(srcPtr2 + j); AV_WN64A(dstPtr2 + j, v << shift); }
#else
#define FAST_COPY_UP(shift) for (; j < length - 1; j += 2) { uint32_t v = AV_RN32A(srcPtr2 + j); AV_WN32A(dstPtr2 + j, v << shift); }
#endif
switch (shift)
{
...
#define COPY_UP(r,w) if(shiftonly){for (; j < length; j++){ unsigned int v= r(&srcPtr2[j]);w(&dstPtr2[j], v<<(dst_depth-src_depth));}}else{for (; j < length; j++){ unsigned int v= r(&srcPtr2[j]);w(&dstPtr2[j], (v<<(dst_depth-src_depth)) | (v>>(2*src_depth-dst_depth)));}}
if(isBE(c->srcFormat)){
if(isBE(c->dstFormat)){
...
#define IS_DIFFERENT_ENDIANESS(src_fmt, dst_fmt, pix_fmt) ((src_fmt == pix_fmt ##BE && dst_fmt == pix_fmt ##LE) || (src_fmt == pix_fmt ##LE && dst_fmt == pix_fmt ##BE))
