...
#define MAKE_REORDER_FUNC(NAME, TYPE, CHANNELS, LAYOUT, MAP) static void alsa_reorder_ ##NAME ##_ ##LAYOUT(const void *in_v, void *out_v, int n) { const TYPE *in = in_v; TYPE *out = out_v; while (n-- > 0) { MAP in += CHANNELS; out += CHANNELS; } }
#define MAKE_REORDER_FUNCS(CHANNELS, LAYOUT, MAP) MAKE_REORDER_FUNC(int8, int8_t, CHANNELS, LAYOUT, MAP) MAKE_REORDER_FUNC(int16, int16_t, CHANNELS, LAYOUT, MAP) MAKE_REORDER_FUNC(int32, int32_t, CHANNELS, LAYOUT, MAP) MAKE_REORDER_FUNC(f32, float, CHANNELS, LAYOUT, MAP)
...
#define FORMAT_I32 2
#define FORMAT_F32 3
#define PICK_REORDER(layout)switch(format) {case FORMAT_I8: s->reorder_func = alsa_reorder_int8_out_ ##layout; break;case FORMAT_I16: s->reorder_func = alsa_reorder_int16_out_ ##layout; break;case FORMAT_I32: s->reorder_func = alsa_reorder_int32_out_ ##layout; break;case FORMAT_F32: s->reorder_func = alsa_reorder_f32_out_ ##layout; break;}
