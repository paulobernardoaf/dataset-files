...
#define DEFINE_LIST_DITHER(thing, thing_str) static void list_dither_##thing(CACAContext *c) { const char *const *thing = caca_get_dither_##thing ##_list(c->dither); int i; av_log(c->ctx, AV_LOG_INFO, "Available %s:\n", thing_str); for (i = 0; thing[i]; i += 2) av_log(c->ctx, AV_LOG_INFO, "%s: %s\n", thing[i], thing[i + 1]); }
...
#define CHECK_DITHER_OPT(opt) do { if (caca_set_dither_##opt(c->dither, c->opt) < 0) { ret = AVERROR(errno); av_log(s, AV_LOG_ERROR, "Failed to set value '%s' for option '%s'\n", c->opt, #opt); goto fail; } } while (0)
