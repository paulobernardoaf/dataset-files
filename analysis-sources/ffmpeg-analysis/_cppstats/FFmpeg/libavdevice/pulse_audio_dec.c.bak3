...
#define CHECK_SUCCESS_GOTO(rerror, expression, label) do { if (!(expression)) { rerror = AVERROR_EXTERNAL; goto label; } } while (0)
...
#define CHECK_DEAD_GOTO(p, rerror, label) do { if (!(p)->context || !PA_CONTEXT_IS_GOOD(pa_context_get_state((p)->context)) || !(p)->stream || !PA_STREAM_IS_GOOD(pa_stream_get_state((p)->stream))) { rerror = AVERROR_EXTERNAL; goto label; } } while (0)
