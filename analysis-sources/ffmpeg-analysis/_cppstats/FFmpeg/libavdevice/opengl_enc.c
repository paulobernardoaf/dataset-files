...
#define OPENGL_ERROR_CHECK(ctx) {GLenum err_code; if ((err_code = glGetError()) != GL_NO_ERROR) { av_log(ctx, AV_LOG_ERROR, "OpenGL error occurred in '%s', line %d: %d\n", __FUNCTION__, __LINE__, err_code); goto fail; } }
...
#define LOAD_OPENGL_FUN(name, type) procs->name = (type)SDL_GL_GetProcAddress(#name); if (!procs->name) { av_log(opengl, AV_LOG_ERROR, "Cannot load OpenGL function: '%s'\n", #name); return AVERROR(ENOSYS); }
...
#endif
#define LOAD_OPENGL_FUN(name, type) procs->name = (type)SelectedGetProcAddress(#name); if (!procs->name) { av_log(opengl, AV_LOG_ERROR, "Cannot load OpenGL function: '%s'\n", #name); return AVERROR(ENOSYS); }
#if CONFIG_SDL2
if (!opengl->no_window)
return opengl_sdl_load_procedures(opengl);
#endif
...
return;
#define FILL_COMPONENT(i) { shift = (desc->comp[i].depth - 1) >> 3; opengl->color_map[(i << 2) + (desc->comp[i].offset >> shift)] = 1.0; }
