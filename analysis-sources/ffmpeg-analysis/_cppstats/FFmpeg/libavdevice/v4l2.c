...
int err;
int flags = O_RDWR;
#define SET_WRAPPERS(prefix) do { s->open_f = prefix ##open; s->close_f = prefix ##close; s->dup_f = prefix ##dup; s->ioctl_f = prefix ##ioctl; s->read_f = prefix ##read; s->mmap_f = prefix ##mmap; s->munmap_f = prefix ##munmap; } while (0)
if (s->use_libv4l2) {
#if CONFIG_LIBV4L2
SET_WRAPPERS(v4l2_);
#else
...
#if defined(V4L2_FMT_FLAG_EMULATED)
