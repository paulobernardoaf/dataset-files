...
#define MIX_FUNC_GENERIC(fmt, cfmt, stype, ctype, sumtype, expr) static void MIX_FUNC_NAME(fmt, cfmt)(stype **samples, ctype **matrix, int len, int out_ch, int in_ch) { int i, in, out; stype temp[AVRESAMPLE_MAX_CHANNELS]; for (i = 0; i < len; i++) { for (out = 0; out < out_ch; out++) { sumtype sum = 0; for (in = 0; in < in_ch; in++) sum += samples[in][i] * matrix[out][in]; temp[out] = expr; } for (out = 0; out < out_ch; out++) samples[out][i] = temp[out]; } }
...
#define GET_MATRIX_CONVERT(suffix, scale) if (!am->matrix_ ##suffix[0]) { av_log(am->avr, AV_LOG_ERROR, "matrix is not set\n"); return AVERROR(EINVAL); } for (o = 0, o0 = 0; o < am->out_channels; o++) { for (i = 0, i0 = 0; i < am->in_channels; i++) { if (am->input_skip[i] || am->output_zero[o]) matrix[o * stride + i] = 0.0; else matrix[o * stride + i] = am->matrix_ ##suffix[o0][i0] * (scale); if (!am->input_skip[i]) i0++; } if (!am->output_zero[o]) o0++; }
...
#define CONVERT_MATRIX(type, expr) am->matrix_##type[0] = av_mallocz(am->out_matrix_channels * am->in_matrix_channels * sizeof(*am->matrix_##type[0])); if (!am->matrix_##type[0]) return AVERROR(ENOMEM); for (o = 0, o0 = 0; o < am->out_channels; o++) { if (am->output_zero[o] || am->output_skip[o]) continue; if (o0 > 0) am->matrix_##type[o0] = am->matrix_##type[o0 - 1] + am->in_matrix_channels; for (i = 0, i0 = 0; i < am->in_channels; i++) { double v; if (am->input_skip[i] || am->output_zero[i]) continue; v = matrix[o * stride + i]; am->matrix_##type[o0][i0] = expr; i0++; } o0++; } am->matrix = (void **)am->matrix_##type;
