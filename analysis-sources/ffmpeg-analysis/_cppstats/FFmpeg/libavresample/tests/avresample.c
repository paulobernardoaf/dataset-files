...
#define PUT_FUNC(name, fmt, type, expr) static void put_sample_ ##name(void **data, enum AVSampleFormat sample_fmt,int channels, int sample, int ch, double v_dbl) { type v = expr; type **out = (type **)data; if (av_sample_fmt_is_planar(sample_fmt)) out[ch][sample] = v; else out[0][sample * channels + ch] = v; }
