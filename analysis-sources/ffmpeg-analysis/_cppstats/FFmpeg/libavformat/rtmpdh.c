...
#endif
#define P1024 "FFFFFFFFFFFFFFFFC90FDAA22168C234C4C6628B80DC1CD1" "29024E088A67CC74020BBEA63B139B22514A08798E3404DD" "EF9519B3CD3A431B302B0A6DF25F14374FE1356D6D51C245" "E485B576625E7EC6F44C42E9A637ED6B0BFF5CB6F406B7ED" "EE386BFB5A899FA5AE9F24117C4B1FE649286651ECE65381" "FFFFFFFFFFFFFFFF"
#define Q1024 "7FFFFFFFFFFFFFFFE487ED5110B4611A62633145C06E0E68" "948127044533E63A0105DF531D89CD9128A5043CC71A026E" "F7CA8CD9E69D218D98158536F92F8A1BA7F09AB6B6A8E122" "F242DABB312F3F637A262174D31BF6B585FFAE5B7A035BF6" "F71C35FDAD44CFD2D74F9208BE258FF324943328F67329C0" "FFFFFFFFFFFFFFFF"
#if CONFIG_GMP
#define bn_new(bn) do { bn = av_malloc(sizeof(*bn)); if (bn) mpz_init2(bn, 1); } while (0)
#define bn_free(bn) do { mpz_clear(bn); av_free(bn); } while (0)
...
#define bn_bn2bin(bn, buf, len) do { memset(buf, 0, len); if (bn_num_bytes(bn) <= len) mpz_export(buf, NULL, 1, 1, 0, 0, bn); } while (0)
#define bn_bin2bn(bn, buf, len) do { bn_new(bn); if (bn) mpz_import(bn, len, 1, 1, 0, 0, buf); } while (0)
#define bn_hex2bn(bn, buf, ret) do { bn_new(bn); if (bn) ret = (mpz_set_str(bn, buf, 16) == 0); else ret = 1; } while (0)
#define bn_random(bn, num_bits) do { int bits = num_bits; mpz_set_ui(bn, 0); for (bits = num_bits; bits > 0; bits -= 32) { mpz_mul_2exp(bn, bn, 32); mpz_add_ui(bn, bn, av_get_random_seed()); } mpz_fdiv_r_2exp(bn, bn, num_bits); } while (0)
...
#elif CONFIG_GCRYPT
#define bn_new(bn) do { if (!gcry_control(GCRYCTL_INITIALIZATION_FINISHED_P)) { if (!gcry_check_version("1.5.4")) return AVERROR(EINVAL); gcry_control(GCRYCTL_DISABLE_SECMEM, 0); gcry_control(GCRYCTL_INITIALIZATION_FINISHED, 0); } bn = gcry_mpi_new(1); } while (0)
...
#define bn_free(bn) do { mbedtls_mpi_free(bn); av_free(bn); } while (0)
...
#define bn_bin2bn(bn, buf, len) do { bn_new(bn); if (bn) mbedtls_mpi_read_binary(bn, buf, len); } while (0)
#define bn_hex2bn(bn, buf, ret) do { bn_new(bn); if (bn) ret = (mbedtls_mpi_read_string(bn, 16, buf) == 0); else ret = 1; } while (0)
#define bn_random(bn, num_bits) do { mbedtls_entropy_context entropy_ctx; mbedtls_ctr_drbg_context ctr_drbg_ctx; mbedtls_entropy_init(&entropy_ctx); mbedtls_ctr_drbg_init(&ctr_drbg_ctx); mbedtls_ctr_drbg_seed(&ctr_drbg_ctx, mbedtls_entropy_func, &entropy_ctx, NULL, 0); mbedtls_mpi_fill_random(bn, (num_bits + 7) / 8, mbedtls_ctr_drbg_random, &ctr_drbg_ctx); mbedtls_ctr_drbg_free(&ctr_drbg_ctx); mbedtls_entropy_free(&entropy_ctx); } while (0)
