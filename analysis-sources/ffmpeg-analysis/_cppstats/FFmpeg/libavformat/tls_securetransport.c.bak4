...
#define CHECK_ERROR(func, ...) do { OSStatus status = func(__VA_ARGS__); if (status != noErr) { ret = AVERROR_UNKNOWN; av_log(h, AV_LOG_ERROR, #func ": Error %i\n", (int)status); goto fail; } } while (0)
