...
#define MOV_CLASS(flavor)static const AVClass flavor ##_muxer_class = {.class_name = #flavor " muxer",.item_name = av_default_item_name,.option = options,.version = LIBAVUTIL_VERSION_INT,};
...
#define READ_BLOCK do { read_size[read_buf_id] = avio_read(read_pb, read_buf[read_buf_id], moov_size); read_buf_id ^= 1; } while (0)
READ_BLOCK;
do {
int n;
READ_BLOCK;
