...
#define SET_OPT_IF_REQUESTED(libopt, opt, flag) do { if (modplug->opt) { settings.libopt = modplug->opt; settings.mFlags |= flag; } } while (0)
...
#define PRINT_INFO(line, name, idvalue) do { snprintf(intbuf, sizeof(intbuf), "%.0f", var_values[idvalue]); write_text(pkt->data, name ":", modplug->linesize, 0+1, line+1); write_text(pkt->data, intbuf, modplug->linesize, 10+1, line+1); } while (0)
