...
#define COMMON_OPTIONS { "framerate", "set the video framerate", OFFSET(framerate), AV_OPT_TYPE_VIDEO_RATE, {.str = "25"}, 0, INT_MAX, DEC }, { "pixel_format", "set video pixel format", OFFSET(pixel_format), AV_OPT_TYPE_STRING, {.str = NULL}, 0, 0, DEC }, { "video_size", "set video size", OFFSET(width), AV_OPT_TYPE_IMAGE_SIZE, {.str = NULL}, 0, 0, DEC }, { "loop", "force loop over input file sequence", OFFSET(loop), AV_OPT_TYPE_BOOL, {.i64 = 0 }, 0, 1, DEC }, { NULL },
...
|| AV_RB32(b + 4) != XWD_VERSION 
|| AV_RB32(b + 8) != XWD_Z_PIXMAP 
|| AV_RB32(b + 12) > 32 || !AV_RB32(b + 12) 
|| AV_RB32(b + 16) == 0 
|| AV_RB32(b + 20) == 0 
|| AV_RB32(b + 28) > 1 
|| AV_RB32(b + 32) & ~56 || av_popcount(AV_RB32(b + 32)) != 1 
|| AV_RB32(b + 36) > 1 
|| AV_RB32(b + 40) & ~56 || av_popcount(AV_RB32(b + 40)) != 1 
|| AV_RB32(b + 44) > 32 || !AV_RB32(b + 44) 
|| AV_RB32(b + 68) > 256) 
return 0;
width = AV_RB32(b + 16);
bpad = AV_RB32(b + 40);
...
#define IMAGEAUTO_DEMUXER(imgname, codecid)static const AVClass imgname ##_class = {.class_name = AV_STRINGIFY(imgname) " demuxer",.item_name = av_default_item_name,.option = ff_img2pipe_options,.version = LIBAVUTIL_VERSION_INT,};AVInputFormat ff_image_ ##imgname ##_pipe_demuxer = {.name = AV_STRINGIFY(imgname) "_pipe",.long_name = NULL_IF_CONFIG_SMALL("piped " AV_STRINGIFY(imgname) " sequence"),.priv_data_size = sizeof(VideoDemuxData),.read_probe = imgname ##_probe,.read_header = ff_img_read_header,.read_packet = ff_img_read_packet,.priv_class = & imgname ##_class,.flags = AVFMT_GENERIC_INDEX, .raw_codec_id = codecid,};
