#include "avformat.h"
#include "rawenc.h"
#define PCMDEF(name_, long_name_, ext, codec) AVOutputFormat ff_pcm_ ##name_ ##_muxer = { .name = #name_, .long_name = NULL_IF_CONFIG_SMALL(long_name_), .extensions = ext, .audio_codec = codec, .video_codec = AV_CODEC_ID_NONE, .write_packet = ff_raw_write_packet, .flags = AVFMT_NOTIMESTAMPS, };
