...
#define MOD_UNLIKELY(modulus, dividend, divisor, prev_dividend) do { if ((prev_dividend) == 0 || (dividend) - (prev_dividend) != (divisor)) (modulus) = (dividend) % (divisor); (prev_dividend) = (dividend); } while (0)
...
{ 0x11, AVMEDIA_TYPE_AUDIO, AV_CODEC_ID_AAC_LATM }, 
#endif
...
if (!descr)
return AVERROR_INVALIDDATA;
#define R8_CHECK_CLIP_MAX(dst, maxv) do { descr->sl.dst = avio_r8(&d->pb); if (descr->sl.dst > maxv) { descr->sl.dst = maxv; return AVERROR_INVALIDDATA; } } while (0)
