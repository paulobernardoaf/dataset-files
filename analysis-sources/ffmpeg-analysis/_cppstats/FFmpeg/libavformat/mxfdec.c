...
#define MXF_FIELD_DOMINANCE_FF 1 
#define MXF_FIELD_DOMINANCE_FL 2 
int field_dominance;
int channels;
int bits_per_sample;
int64_t duration; 
...
#define READ_STR16(type, big_endian) static int mxf_read_utf16 ##type ##_string(AVIOContext *pb, int size, char** str) { return mxf_read_utf16_string(pb, size, str, big_endian); }
READ_STR16(be, 1)
READ_STR16(le, 0)
#undef READ_STR16
...
#define SET_STR_METADATA(pb, name, str) do { if ((ret = mxf_read_utf16be_string(pb, size, &str)) < 0) return ret; av_dict_set(&s->metadata, name, str, AV_DICT_DONT_STRDUP_VAL); } while (0)
#define SET_UID_METADATA(pb, name, var, str) do { avio_read(pb, var, 16); if ((ret = mxf_uid_to_str(var, &str)) < 0) return ret; av_dict_set(&s->metadata, name, str, AV_DICT_DONT_STRDUP_VAL); } while (0)
#define SET_TS_METADATA(pb, name, var, str) do { var = avio_rb64(pb); if (var && (ret = avpriv_dict_set_timestamp(&s->metadata, name, mxf_timestamp_to_int64(var))) < 0) return ret; } while (0)
...
{ { 0x06,0x0e,0x2b,0x34,0x02,0x53,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x5b,0x00 }, mxf_read_generic_descriptor, sizeof(MXFDescriptor), Descriptor }, 
...
{ { 0x06,0x0e,0x2b,0x34,0x02,0x53,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x64,0x00 }, mxf_read_generic_descriptor, sizeof(MXFDescriptor), Descriptor }, 
