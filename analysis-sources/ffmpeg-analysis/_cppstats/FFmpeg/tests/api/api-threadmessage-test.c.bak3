...
#define SPAWN_THREADS(type) do { for (i = 0; i < nb_##type##s; i++) { struct type##_data *td = &type##s[i]; td->id = i; td->queue = queue; td->workload = get_workload(type##_min_load, type##_max_load); ret = pthread_create(&td->tid, NULL, type##_thread, td); if (ret) { const int err = AVERROR(ret); av_log(NULL, AV_LOG_ERROR, "Unable to start " AV_STRINGIFY(type) " thread: %s\n", av_err2str(err)); goto end; } } } while (0)
...
#define WAIT_THREADS(type) do { for (i = 0; i < nb_##type##s; i++) { struct type##_data *td = &type##s[i]; ret = pthread_join(td->tid, NULL); if (ret) { const int err = AVERROR(ret); av_log(NULL, AV_LOG_ERROR, "Unable to join " AV_STRINGIFY(type) " thread: %s\n", av_err2str(err)); goto end; } } } while (0)
