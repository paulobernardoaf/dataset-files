...
#define randomize_buffers() do { int x, y; uint32_t mask = pixel_mask[bit_depth - 8]; for (y = 0; y < sz; y++) { for (x = 0; x < PIXEL_STRIDE; x += 4) { AV_WN32A(src + y * PIXEL_STRIDE + x, rnd() & mask); AV_WN32A(dst + y * PIXEL_STRIDE + x, rnd() & mask); } for (x = 0; x < sz; x++) { if (bit_depth == 8) { coef[y * sz + x] = src[y * PIXEL_STRIDE + x] - dst[y * PIXEL_STRIDE + x]; } else { ((int32_t *)coef)[y * sz + x] = ((uint16_t *)src)[y * (PIXEL_STRIDE/2) + x] - ((uint16_t *)dst)[y * (PIXEL_STRIDE/2) + x]; } } } } while (0)
#define dct4x4_impl(size, dctcoef) static void dct4x4_##size(dctcoef *coef) { int i, y, x; dctcoef tmp[16]; for (i = 0; i < 4; i++) { const int z0 = coef[i*4 + 0] + coef[i*4 + 3]; const int z1 = coef[i*4 + 1] + coef[i*4 + 2]; const int z2 = coef[i*4 + 0] - coef[i*4 + 3]; const int z3 = coef[i*4 + 1] - coef[i*4 + 2]; tmp[i + 4*0] = z0 + z1; tmp[i + 4*1] = 2*z2 + z3; tmp[i + 4*2] = z0 - z1; tmp[i + 4*3] = z2 - 2*z3; } for (i = 0; i < 4; i++) { const int z0 = tmp[i*4 + 0] + tmp[i*4 + 3]; const int z1 = tmp[i*4 + 1] + tmp[i*4 + 2]; const int z2 = tmp[i*4 + 0] - tmp[i*4 + 3]; const int z3 = tmp[i*4 + 1] - tmp[i*4 + 2]; coef[i*4 + 0] = z0 + z1; coef[i*4 + 1] = 2*z2 + z3; coef[i*4 + 2] = z0 - z1; coef[i*4 + 3] = z2 - 2*z3; } for (y = 0; y < 4; y++) { for (x = 0; x < 4; x++) { static const int scale[] = { 13107 * 10, 8066 * 13, 5243 * 16 }; const int idx = (y & 1) + (x & 1); coef[y*4 + x] = (coef[y*4 + x] * scale[idx] + (1 << 14)) >> 15; } } }
#define DCT8_1D(src, srcstride, dst, dststride) do { const int a0 = (src)[srcstride * 0] + (src)[srcstride * 7]; const int a1 = (src)[srcstride * 0] - (src)[srcstride * 7]; const int a2 = (src)[srcstride * 1] + (src)[srcstride * 6]; const int a3 = (src)[srcstride * 1] - (src)[srcstride * 6]; const int a4 = (src)[srcstride * 2] + (src)[srcstride * 5]; const int a5 = (src)[srcstride * 2] - (src)[srcstride * 5]; const int a6 = (src)[srcstride * 3] + (src)[srcstride * 4]; const int a7 = (src)[srcstride * 3] - (src)[srcstride * 4]; const int b0 = a0 + a6; const int b1 = a2 + a4; const int b2 = a0 - a6; const int b3 = a2 - a4; const int b4 = a3 + a5 + (a1 + (a1 >> 1)); const int b5 = a1 - a7 - (a5 + (a5 >> 1)); const int b6 = a1 + a7 - (a3 + (a3 >> 1)); const int b7 = a3 - a5 + (a7 + (a7 >> 1)); (dst)[dststride * 0] = b0 + b1; (dst)[dststride * 1] = b4 + (b7 >> 2); (dst)[dststride * 2] = b2 + (b3 >> 1); (dst)[dststride * 3] = b5 + (b6 >> 2); (dst)[dststride * 4] = b0 - b1; (dst)[dststride * 5] = b6 - (b5 >> 2); (dst)[dststride * 6] = (b2 >> 1) - b3; (dst)[dststride * 7] = (b4 >> 2) - b7; } while (0)
#define dct8x8_impl(size, dctcoef) static void dct8x8_##size(dctcoef *coef) { int i, x, y; dctcoef tmp[64]; for (i = 0; i < 8; i++) DCT8_1D(coef + i, 8, tmp + i, 8); for (i = 0; i < 8; i++) DCT8_1D(tmp + 8*i, 1, coef + i, 8); for (y = 0; y < 8; y++) { for (x = 0; x < 8; x++) { static const int scale[] = { 13107 * 20, 11428 * 18, 20972 * 32, 12222 * 19, 16777 * 25, 15481 * 24, }; static const int idxmap[] = { 0, 3, 4, 3, 3, 1, 5, 1, 4, 5, 2, 5, 3, 1, 5, 1, }; const int idx = idxmap[(y & 3) * 4 + (x & 3)]; coef[y*8 + x] = ((int64_t)coef[y*8 + x] * scale[idx] + (1 << 17)) >> 18; } } }
