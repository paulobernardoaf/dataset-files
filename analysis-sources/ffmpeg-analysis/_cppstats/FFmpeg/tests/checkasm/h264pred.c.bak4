...
#define check_pred_func(func, name, mode_name) (mode_name && ((codec_ids[codec] == AV_CODEC_ID_H264) ? check_func(func, "pred%s_%s_%d", name, mode_name, bit_depth) : check_func(func, "pred%s_%s", name, mode_name)))




#define randomize_buffers() do { uint32_t mask = pixel_mask[bit_depth - 8]; int i; for (i = 0; i < BUF_SIZE; i += 4) { uint32_t r = rnd() & mask; AV_WN32A(buf0 + i, r); AV_WN32A(buf1 + i, r); } } while (0)
