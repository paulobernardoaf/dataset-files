...
#define randomize_buffers(src, dst, stride, coef) do { int x, y; for (y = 0; y < 4; y++) { AV_WN32A((src) + y * (stride), rnd()); AV_WN32A((dst) + y * (stride), rnd()); for (x = 0; x < 4; x++) (coef)[y * 4 + x] = (src)[y * (stride) + x] - (dst)[y * (stride) + x]; } } while (0)
...
#define randomize_buffers() do { int k; for (k = 0; k < SRC_BUF_SIZE; k += 4) { AV_WN32A(buf + k, rnd()); } } while (0)
