...
#define randomize_buffers() do { int i, j; for (i = 0; i < BUF_SIZE; i += 4) { for (j = 0; j < channels; j++) { uint32_t r = rnd() & (1 << (bits - 2)) - 1; AV_WN32A(ref_src[j] + i, r); AV_WN32A(new_src[j] + i, r); } } } while (0)
