...
#define randomize_buffer(buf) do { int i; double bmg[2], stddev = 10.0, mean = 0.0; for (i = 0; i < LEN; i += 2) { av_bmg_get(&checkasm_lfg, bmg); buf[i] = bmg[0] * stddev + mean; buf[i + 1] = bmg[1] * stddev + mean; } } while(0);
