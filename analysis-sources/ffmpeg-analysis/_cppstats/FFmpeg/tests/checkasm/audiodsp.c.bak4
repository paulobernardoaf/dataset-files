...
#define randomize_float(buf, len) do { int i; for (i = 0; i < len; i++) { float f = (float)rnd() / (UINT_MAX >> 5) - 16.0f; buf[i] = f; } } while (0)
...
#define randomize_int(buf, len, size, bits) do { int i; for (i = 0; i < len; i++) { uint ##size ##_t r = rnd() & ((1LL << bits) - 1); AV_WN ##size ##A(buf + i, -(1LL << (bits - 1)) + r); } } while (0)
