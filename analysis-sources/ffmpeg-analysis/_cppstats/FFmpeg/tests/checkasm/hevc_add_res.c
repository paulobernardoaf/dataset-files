...
#include "checkasm.h"
#define randomize_buffers(buf, size) do { int j; for (j = 0; j < size; j++) { int16_t r = rnd(); AV_WN16A(buf + j, r >> 3); } } while (0)
#define randomize_buffers2(buf, size) do { int j; for (j = 0; j < size; j++) AV_WN16A(buf + j * 2, rnd() & 0x3FF); } while (0)
