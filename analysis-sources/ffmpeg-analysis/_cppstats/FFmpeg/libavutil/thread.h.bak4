...
#define ASSERT_PTHREAD_ABORT(func, ret) do { char errbuf[AV_ERROR_MAX_STRING_SIZE] = ""; av_log(NULL, AV_LOG_FATAL, AV_STRINGIFY(func) " failed with error: %s\n", av_make_error_string(errbuf, AV_ERROR_MAX_STRING_SIZE, AVERROR(ret))); abort(); } while (0)
...
#define ASSERT_PTHREAD_NORET(func, ...) do { int ret = func(__VA_ARGS__); if (ret) ASSERT_PTHREAD_ABORT(func, ret); } while (0)





#define ASSERT_PTHREAD(func, ...) do { ASSERT_PTHREAD_NORET(func, __VA_ARGS__); return 0; } while (0)
