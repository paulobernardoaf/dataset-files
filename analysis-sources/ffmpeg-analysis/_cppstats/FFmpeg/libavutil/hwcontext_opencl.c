...
#define CL_FUNC(name, desc) do { if (fail) break; priv->name = clGetExtensionFunctionAddressForPlatform( priv->platform_id, #name); if (!priv->name) { av_log(hwdev, AV_LOG_VERBOSE, desc " function not found (%s).\n", #name); fail = 1; } else { av_log(hwdev, AV_LOG_VERBOSE, desc " function found (%s).\n", #name); } } while (0)
#if HAVE_OPENCL_DRM_BEIGNET
{
int fail = 0;
