...
#define INIT_FF_COS_TABS_FUNC(index, size) static av_cold void init_cos_tabs_ ##size (void) { init_cos_tabs_idx(index); }
...
#define BUTTERFLIES(a0,a1,a2,a3) {BF(t3, t5, t5, t1);BF(a2.re, a0.re, a0.re, t5);BF(a3.im, a1.im, a1.im, t3);BF(t4, t6, t2, t6);BF(a3.re, a1.re, a1.re, t4);BF(a2.im, a0.im, a0.im, t6);}
...
#define BUTTERFLIES_BIG(a0,a1,a2,a3) {FFTSample r0=a0.re, i0=a0.im, r1=a1.re, i1=a1.im;BF(t3, t5, t5, t1);BF(a2.re, a0.re, r0, t5);BF(a3.im, a1.im, i1, t3);BF(t4, t6, t2, t6);BF(a3.re, a1.re, r1, t4);BF(a2.im, a0.im, i0, t6);}
...
#define TRANSFORM(a0,a1,a2,a3,wre,wim) {CMUL(t1, t2, a2.re, a2.im, wre, -wim);CMUL(t5, t6, a3.re, a3.im, wre, wim);BUTTERFLIES(a0,a1,a2,a3)}





#define TRANSFORM_ZERO(a0,a1,a2,a3) {t1 = a2.re;t2 = a2.im;t5 = a3.re;t6 = a3.im;BUTTERFLIES(a0,a1,a2,a3)}
...
#define DECL_FFT(n,n2,n4)static void fft##n(FFTComplex *z){fft##n2(z);fft##n4(z+n4*2);fft##n4(z+n4*3);pass(z,TX_NAME(ff_cos_##n),n4/2);}
...
#define DECL_COMP_FFT(N) static void compound_fft_##N##xM(AVTXContext *s, void *_out, void *_in, ptrdiff_t stride) { const int m = s->m, *in_map = s->pfatab, *out_map = in_map + N*m; FFTComplex *in = _in; FFTComplex *out = _out; FFTComplex fft##N##in[N]; void (*fftp)(FFTComplex *z) = fft_dispatch[av_log2(m)]; for (int i = 0; i < m; i++) { for (int j = 0; j < N; j++) fft##N##in[j] = in[in_map[i*N + j]]; fft##N(s->tmp + s->revtab[i], fft##N##in, m); } for (int i = 0; i < N; i++) fftp(s->tmp + m*i); for (int i = 0; i < N*m; i++) out[i] = s->tmp[out_map[i]]; }
...
#define CHECK_FACTOR(DST, FACTOR, SRC) if (DST == 1 && !(SRC % FACTOR)) { DST = FACTOR; SRC /= FACTOR; }
