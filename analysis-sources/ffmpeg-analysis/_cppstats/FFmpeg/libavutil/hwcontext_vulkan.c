...
#define VK_LOAD_PFN(inst, name) PFN_##name pfn_##name = (PFN_##name) vkGetInstanceProcAddr(inst, #name)
#define DEFAULT_USAGE_FLAGS (VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT)
#define ADD_VAL_TO_LIST(list, count, val) do { list = av_realloc_array(list, sizeof(*list), ++count); if (!list) { err = AVERROR(ENOMEM); goto end; } list[count - 1] = val; } while(0)
static const struct {
...
EXT_EXTERNAL_DMABUF_MEMORY = 1ULL << 0, 
EXT_DRM_MODIFIER_FLAGS = 1ULL << 1, 
EXT_EXTERNAL_FD_MEMORY = 1ULL << 2, 
EXT_EXTERNAL_FD_SEM = 1ULL << 3, 
...
#define SEARCH_FLAGS(expr, out) for (int i = 0; i < num; i++) { const VkQueueFlagBits flags = qs[i].queueFlags; if (expr) { out = i; break; } }
...
#undef SEARCH_FLAGS
#define QF_FLAGS(flags) ((flags) & VK_QUEUE_GRAPHICS_BIT ) ? "(graphics) " : "", ((flags) & VK_QUEUE_COMPUTE_BIT ) ? "(compute) " : "", ((flags) & VK_QUEUE_TRANSFER_BIT ) ? "(transfer) " : "", ((flags) & VK_QUEUE_SPARSE_BINDING_BIT) ? "(sparse) " : ""
...
#define CHECK_QUEUE(type, n) if (n >= queue_num) { av_log(ctx, AV_LOG_ERROR, "Invalid %s queue index %i (device has %i queues)!\n", type, n, queue_num); return AVERROR(EINVAL); }
