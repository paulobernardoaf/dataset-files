...
#define STORE_BLOCK(type) scantable += zeros_left + total_coeff - 1; if(n >= LUMA_DC_BLOCK_INDEX){ ((type*)block)[*scantable] = level[0]; for(i=1;i<total_coeff && zeros_left > 0;i++) { if(zeros_left < 7) run_before= get_vlc2(gb, run_vlc[zeros_left].table, RUN_VLC_BITS, 1); else run_before= get_vlc2(gb, run7_vlc.table, RUN7_VLC_BITS, 2); zeros_left -= run_before; scantable -= 1 + run_before; ((type*)block)[*scantable]= level[i]; } for(;i<total_coeff;i++) { scantable--; ((type*)block)[*scantable]= level[i]; } }else{ ((type*)block)[*scantable] = ((int)(level[0] * qmul[*scantable] + 32))>>6; for(i=1;i<total_coeff && zeros_left > 0;i++) { if(zeros_left < 7) run_before= get_vlc2(gb, run_vlc[zeros_left].table, RUN_VLC_BITS, 1); else run_before= get_vlc2(gb, run7_vlc.table, RUN7_VLC_BITS, 2); zeros_left -= run_before; scantable -= 1 + run_before; ((type*)block)[*scantable]= ((int)(level[i] * qmul[*scantable] + 32))>>6; } for(;i<total_coeff;i++) { scantable--; ((type*)block)[*scantable]= ((int)(level[i] * qmul[*scantable] + 32))>>6; } }
if (h->pixel_shift) {
STORE_BLOCK(int32_t)
} else {
STORE_BLOCK(int16_t)
