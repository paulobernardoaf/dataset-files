...
#define memset_bpp(c, i1, v, i2, num) do { if (bytesperpixel == 1) { memset(&(c)[(i1)], (v)[(i2)], (num)); } else { int n, val = AV_RN16A(&(v)[(i2) * 2]); for (n = 0; n < (num); n++) { AV_WN16A(&(c)[((i1) + n) * 2], val); } } } while (0)
...
#define memset_val(c, val, num) do { if (bytesperpixel == 1) { memset((c), (val), (num)); } else { int n; for (n = 0; n < (num); n++) { AV_WN16A(&(c)[n * 2], (val)); } } } while (0)
...
#define assign_bpp(c, i1, v, i2) do { if (bytesperpixel == 1) { (c)[(i1)] = (v)[(i2)]; } else { AV_COPY16(&(c)[(i1) * 2], &(v)[(i2) * 2]); } } while (0)
...
#define assign_val(c, i, v) do { if (bytesperpixel == 1) { (c)[(i)] = (v); } else { AV_WN16A(&(c)[(i) * 2], (v)); } } while (0)
...
#define mc_luma_dir(td, mc, dst, dst_ls, src, src_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, i) mc_luma_unscaled(td, s->dsp.mc, dst, dst_ls, src, src_ls, tref, row, col, mv, bw, bh, w, h, bytesperpixel)



#define mc_chroma_dir(td, mc, dstu, dstv, dst_ls, srcu, srcu_ls, srcv, srcv_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, i) mc_chroma_unscaled(td, s->dsp.mc, dstu, dstv, dst_ls, srcu, srcu_ls, srcv, srcv_ls, tref, row, col, mv, bw, bh, w, h, bytesperpixel)
...
#define mc_luma_dir(td, mc, dst, dst_ls, src, src_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, i) mc_luma_scaled(td, s->dsp.s##mc, s->dsp.mc, dst, dst_ls, src, src_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, bytesperpixel, s->mvscale[b->ref[i]], s->mvstep[b->ref[i]])




#define mc_chroma_dir(td, mc, dstu, dstv, dst_ls, srcu, srcu_ls, srcv, srcv_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, i) mc_chroma_scaled(td, s->dsp.s##mc, s->dsp.mc, dstu, dstv, dst_ls, srcu, srcu_ls, srcv, srcv_ls, tref, row, col, mv, px, py, pw, ph, bw, bh, w, h, bytesperpixel, s->mvscale[b->ref[i]], s->mvstep[b->ref[i]])
