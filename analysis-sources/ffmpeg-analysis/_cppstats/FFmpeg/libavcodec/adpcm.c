...
#define DK3_GET_NEXT_NIBBLE() if (decode_top_nibble_next) { nibble = last_byte >> 4; decode_top_nibble_next = 0; } else { last_byte = bytestream2_get_byteu(&gb); nibble = last_byte & 0x0F; decode_top_nibble_next = 1; }
...
int table[14][16];
int ch;
#define THP_GET16(g) sign_extend( avctx->codec->id == AV_CODEC_ID_ADPCM_THP_LE ? bytestream2_get_le16u(&(g)) : bytestream2_get_be16u(&(g)), 16)
if (avctx->extradata) {
GetByteContext tb;
