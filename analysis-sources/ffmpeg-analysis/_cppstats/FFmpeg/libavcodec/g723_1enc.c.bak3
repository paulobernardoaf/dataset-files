...
#define get_index(num, offset, size) { int error, max = -1; int16_t temp[4]; int i, j; for (i = 0; i < LSP_CB_SIZE; i++) { for (j = 0; j < size; j++){ temp[j] = (weight[j + (offset)] * lsp_band##num[i][j] + (1 << 14)) >> 15; } error = ff_g723_1_dot_product(lsp + (offset), temp, size) << 1; error -= ff_g723_1_dot_product(lsp_band##num[i], temp, size); if (error > max) { max = error; lsp_index[num] = i; } } }
