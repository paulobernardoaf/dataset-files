...
#define IVI_SLANT_BFLY(s1, s2, o1, o2, t) t = (s1) - (s2);o1 = (s1) + (s2);o2 = (t);
#define IVI_IREFLECT(s1, s2, o1, o2, t) t = (((s1) + (s2)*2 + 2) >> 2) + (s1);o2 = (((s1)*2 - (s2) + 2) >> 2) - (s2);o1 = (t);
#define IVI_SLANT_PART4(s1, s2, o1, o2, t) t = (s2) + (((s1)*4 - (s2) + 4) >> 3);o2 = (s1) + ((-(s1) - (s2)*4 + 4) >> 3);o1 = (t);
#define IVI_INV_SLANT8(s1, s4, s8, s5, s2, s6, s3, s7,d1, d2, d3, d4, d5, d6, d7, d8,t0, t1, t2, t3, t4, t5, t6, t7, t8) {IVI_SLANT_PART4(s4, s5, t4, t5, t0);IVI_SLANT_BFLY(s1, t5, t1, t5, t0); IVI_SLANT_BFLY(s2, s6, t2, t6, t0);IVI_SLANT_BFLY(s7, s3, t7, t3, t0); IVI_SLANT_BFLY(t4, s8, t4, t8, t0);IVI_SLANT_BFLY(t1, t2, t1, t2, t0); IVI_IREFLECT (t4, t3, t4, t3, t0);IVI_SLANT_BFLY(t5, t6, t5, t6, t0); IVI_IREFLECT (t8, t7, t8, t7, t0);IVI_SLANT_BFLY(t1, t4, t1, t4, t0); IVI_SLANT_BFLY(t2, t3, t2, t3, t0);IVI_SLANT_BFLY(t5, t8, t5, t8, t0); IVI_SLANT_BFLY(t6, t7, t6, t7, t0);d1 = COMPENSATE(t1);d2 = COMPENSATE(t2);d3 = COMPENSATE(t3);d4 = COMPENSATE(t4);d5 = COMPENSATE(t5);d6 = COMPENSATE(t6);d7 = COMPENSATE(t7);d8 = COMPENSATE(t8);}
#define IVI_INV_SLANT4(s1, s4, s2, s3, d1, d2, d3, d4, t0, t1, t2, t3, t4) {IVI_SLANT_BFLY(s1, s2, t1, t2, t0); IVI_IREFLECT (s4, s3, t4, t3, t0);IVI_SLANT_BFLY(t1, t4, t1, t4, t0); IVI_SLANT_BFLY(t2, t3, t2, t3, t0);d1 = COMPENSATE(t1);d2 = COMPENSATE(t2);d3 = COMPENSATE(t3);d4 = COMPENSATE(t4);}
...
#define IVI_MC_TEMPLATE(size, suffix, OP) static void ivi_mc_ ##size ##x##size ##suffix(int16_t *buf, ptrdiff_t dpitch, const int16_t *ref_buf, ptrdiff_t pitch, int mc_type) { int i, j; const int16_t *wptr; switch (mc_type) { case 0: for (i = 0; i < size; i++, buf += dpitch, ref_buf += pitch) { for (j = 0; j < size; j++) {OP(buf[j], ref_buf[j]); } } break; case 1: for (i = 0; i < size; i++, buf += dpitch, ref_buf += pitch) for (j = 0; j < size; j++) OP(buf[j], (ref_buf[j] + ref_buf[j+1]) >> 1); break; case 2: wptr = ref_buf + pitch; for (i = 0; i < size; i++, buf += dpitch, wptr += pitch, ref_buf += pitch) for (j = 0; j < size; j++) OP(buf[j], (ref_buf[j] + wptr[j]) >> 1); break; case 3: wptr = ref_buf + pitch; for (i = 0; i < size; i++, buf += dpitch, wptr += pitch, ref_buf += pitch) for (j = 0; j < size; j++) OP(buf[j], (ref_buf[j] + ref_buf[j+1] + wptr[j] + wptr[j+1]) >> 2); break; } } void ff_ivi_mc_ ##size ##x##size ##suffix(int16_t *buf, const int16_t *ref_buf, ptrdiff_t pitch, int mc_type) { ivi_mc_ ##size ##x##size ##suffix(buf, pitch, ref_buf, pitch, mc_type); } 
#define IVI_MC_AVG_TEMPLATE(size, suffix, OP) void ff_ivi_mc_avg_ ##size ##x##size ##suffix(int16_t *buf, const int16_t *ref_buf, const int16_t *ref_buf2, ptrdiff_t pitch, int mc_type, int mc_type2) { int16_t tmp[size * size]; int i, j; ivi_mc_ ##size ##x##size ##_no_delta(tmp, size, ref_buf, pitch, mc_type); ivi_mc_ ##size ##x##size ##_delta(tmp, size, ref_buf2, pitch, mc_type2); for (i = 0; i < size; i++, buf += pitch) { for (j = 0; j < size; j++) {OP(buf[j], tmp[i * size + j] >> 1); } } } 
