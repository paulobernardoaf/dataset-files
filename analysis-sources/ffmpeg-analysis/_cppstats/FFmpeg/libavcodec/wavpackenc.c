...
#define APPLY_WEIGHT_F(weight, sample) ((((((sample) & 0xffff) * (weight)) >> 9) + ((((sample) & ~0xffff) >> 9) * (weight)) + 1) >> 1)
...
#define update_weight_d2(weight, delta, source, result) if (source && result) weight -= (((source ^ result) >> 29) & 4) - 2;
#define update_weight_clip_d2(weight, delta, source, result) if (source && result) { const int32_t s = (source ^ result) >> 31; if ((weight = (weight ^ s) + (2 - s)) > 1024) weight = 1024; weight = (weight ^ s) - s; }
...
if (s->num_terms & 1)
bytestream2_put_byte(&pb, 0);
#define WRITE_DECWEIGHT(type) do { temp = store_weight(type); bytestream2_put_byte(&pb, temp); type = restore_weight(temp); } while (0)
...
if ((end - start) & 1)
bytestream2_put_byte(&pb, 0);
#define WRITE_DECSAMPLE(type) do { temp = log2s(type); type = wp_exp2(temp); bytestream2_put_le16(&pb, temp); } while (0)
...
#define WRITE_CHAN_ENTROPY(chan) do { for (i = 0; i < 3; i++) { temp = wp_log2(s->w.c[chan].median[i]); bytestream2_put_le16(&pb, temp); s->w.c[chan].median[i] = wp_exp2(temp); } } while (0)
...
{
int i;
#define COPY_SAMPLES(type, offset, shift) do { const type *sptr = (const type *)src; for (i = 0; i < nb_samples; i++) dst[i] = (sptr[i] - offset) >> shift; } while (0)
