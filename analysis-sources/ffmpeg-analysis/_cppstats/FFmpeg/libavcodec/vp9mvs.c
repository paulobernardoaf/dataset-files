...
int i;
#define RETURN_DIRECT_MV(mv) do { uint32_t m = AV_RN32A(&mv); if (!idx) { AV_WN32A(pmv, m); return; } else if (mem == INVALID_MV) { mem = m; } else if (m != mem) { AV_WN32A(pmv, m); return; } } while (0)
if (sb >= 0) {
if (sb == 2 || sb == 1) {
...
#define RETURN_MV(mv) do { if (sb > 0) { VP56mv tmp; uint32_t m; av_assert2(idx == 1); av_assert2(mem != INVALID_MV); if (mem_sub8x8 == INVALID_MV) { clamp_mv(&tmp, &mv, td); m = AV_RN32A(&tmp); if (m != mem) { AV_WN32A(pmv, m); return; } mem_sub8x8 = AV_RN32A(&mv); } else if (mem_sub8x8 != AV_RN32A(&mv)) { clamp_mv(&tmp, &mv, td); m = AV_RN32A(&tmp); if (m != mem) { AV_WN32A(pmv, m); } else { AV_WN32A(pmv, 0); } return; } } else { uint32_t m = AV_RN32A(&mv); if (!idx) { clamp_mv(pmv, &mv, td); return; } else if (mem == INVALID_MV) { mem = m; } else if (m != mem) { clamp_mv(pmv, &mv, td); return; } } } while (0)
if (row > 0) {
...
#define RETURN_SCALE_MV(mv, scale) do { if (scale) { VP56mv mv_temp = { -mv.x, -mv.y }; RETURN_MV(mv_temp); } else { RETURN_MV(mv); } } while (0)
for (i = 0; i < 8; i++) {
int c = p[i][0] + col, r = p[i][1] + row;
