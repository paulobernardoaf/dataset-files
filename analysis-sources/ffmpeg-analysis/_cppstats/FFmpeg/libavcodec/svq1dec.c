...
#define SVQ1_PROCESS_VECTOR() for (; level > 0; i++) { if (i == m) { m = n; if (--level == 0) break; } if (!get_bits1(bitbuf)) break; list[n++] = list[i]; list[n++] = list[i] + (((level & 1) ? pitch : 1) << ((level >> 1) + 1));}
#define SVQ1_ADD_CODEBOOK() for (j = 0; j < stages; j++) { n3 = codebook[entries[j]] ^ 0x80808080; n1 += (n3 & 0xFF00FF00) >> 8; n2 += n3 & 0x00FF00FF; } if (n1 & 0xFF00FF00) { n3 = (n1 >> 15 & 0x00010001 | 0x01000100) - 0x00010001; n1 += 0x7F007F00; n1 |= (~n1 >> 15 & 0x00010001 | 0x01000100) - 0x00010001; n1 &= n3 & 0x00FF00FF; } if (n2 & 0xFF00FF00) { n3 = (n2 >> 15 & 0x00010001 | 0x01000100) - 0x00010001; n2 += 0x7F007F00; n2 |= (~n2 >> 15 & 0x00010001 | 0x01000100) - 0x00010001; n2 &= n3 & 0x00FF00FF; }
#define SVQ1_CALC_CODEBOOK_ENTRIES(cbook) codebook = (const uint32_t *)cbook[level]; if (stages > 0) bit_cache = get_bits(bitbuf, 4 * stages); for (j = 0; j < stages; j++) { entries[j] = (((bit_cache >> (4 * (stages - j - 1))) & 0xF) + 16 * j) << (level + 1); } mean -= stages * 128; n4 = (mean << 16) + mean;
