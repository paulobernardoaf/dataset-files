...
#define mca(dx,dy,b_w)static void mc_block_hpel ##dx ##dy ##b_w(uint8_t *dst, const uint8_t *src, ptrdiff_t stride, int h){av_assert2(h==b_w);mc_block(NULL, dst, src-(HTAPS_MAX/2-1)-(HTAPS_MAX/2-1)*stride, stride, b_w, b_w, dx, dy);}
mca( 0, 0,16)
mca( 8, 0,16)
mca( 0, 8,16)
mca( 8, 8,16)
...
#define mcf(dx,dy)s->qdsp.put_qpel_pixels_tab [0][dy+dx/4]=s->qdsp.put_no_rnd_qpel_pixels_tab[0][dy+dx/4]=s->h264qpel.put_h264_qpel_pixels_tab[0][dy+dx/4];s->qdsp.put_qpel_pixels_tab [1][dy+dx/4]=s->qdsp.put_no_rnd_qpel_pixels_tab[1][dy+dx/4]=s->h264qpel.put_h264_qpel_pixels_tab[1][dy+dx/4];
mcf( 0, 0)
mcf( 4, 0)
mcf( 8, 0)
mcf(12, 0)
...
mcf( 8,12)
mcf(12,12)
#define mcfh(dx,dy)s->hdsp.put_pixels_tab [0][dy/4+dx/8]=s->hdsp.put_no_rnd_pixels_tab[0][dy/4+dx/8]=mc_block_hpel ##dx ##dy ##16;s->hdsp.put_pixels_tab [1][dy/4+dx/8]=s->hdsp.put_no_rnd_pixels_tab[1][dy/4+dx/8]=mc_block_hpel ##dx ##dy ##8;
mcfh(0, 0)
mcfh(8, 0)
mcfh(0, 8)
mcfh(8, 8)
...
FF_ALLOCZ_ARRAY_OR_GOTO(avctx, s->spatial_dwt_buffer, width, height * sizeof(DWTELEM), fail); 
