...
#define xf(width, name, var, subs, ...) do { uint32_t value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, 0, (1 << width) - 1)); var = value; } while (0)
#define xs(width, name, var, subs, ...) do { int32_t value; CHECK(cbs_vp9_read_s(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value)); var = value; } while (0)
#define increment(name, min, max) do { uint32_t value; CHECK(cbs_vp9_read_increment(ctx, rw, min, max, #name, &value)); current->name = value; } while (0)
#define fle(width, name, subs, ...) do { CHECK(cbs_vp9_read_le(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &current->name)); } while (0)
#define delta_q(name) do { uint8_t delta_coded; int8_t delta_q; xf(1, name.delta_coded, delta_coded, 0); if (delta_coded) xs(4, name.delta_q, delta_q, 0); else delta_q = 0; current->name = delta_q; } while (0)
#define prob(name, subs, ...) do { uint8_t prob_coded; uint8_t prob; xf(1, name.prob_coded, prob_coded, subs, __VA_ARGS__); if (prob_coded) xf(8, name.prob, prob, subs, __VA_ARGS__); else prob = 255; current->name = prob; } while (0)
#define fixed(width, name, value) do { av_unused uint32_t fixed_value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, 0, &fixed_value, value, value)); } while (0)
...
#define xf(width, name, var, subs, ...) do { CHECK(ff_cbs_write_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), var, 0, (1 << width) - 1)); } while (0)
#define xs(width, name, var, subs, ...) do { CHECK(cbs_vp9_write_s(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), var)); } while (0)
#define increment(name, min, max) do { CHECK(cbs_vp9_write_increment(ctx, rw, min, max, #name, current->name)); } while (0)
#define fle(width, name, subs, ...) do { CHECK(cbs_vp9_write_le(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), current->name)); } while (0)
#define delta_q(name) do { xf(1, name.delta_coded, !!current->name, 0); if (current->name) xs(4, name.delta_q, current->name, 0); } while (0)
#define prob(name, subs, ...) do { xf(1, name.prob_coded, current->name != 255, subs, __VA_ARGS__); if (current->name != 255) xf(8, name.prob, current->name, subs, __VA_ARGS__); } while (0)
#define fixed(width, name, value) do { CHECK(ff_cbs_write_unsigned(ctx, rw, width, #name, 0, value, value, value)); } while (0)
