...
s->spatial_decomposition_type= s->pred; 
...
s->m.mb_num = (avctx->width * avctx->height + 255) / 256; 
s->m.me.temp =
...
IDWTELEM *dst= (IDWTELEM*)s->m.sc.obmc_scratchpad + plane_index*block_size*block_size*4; 
...
*b= backup;
return av_clip_uint8( ROUNDED_DIV(ab<<LOG2_OBMC_MAX, aa) ); 
}
...
x, y, block_w, block_h, w, h, 0, ref_stride, obmc_stride, mb_x2, mb_y2, 1, 1, plane_index);
for(y2= y; y2<0; y2++)
