...
#if !defined(AVCODEC_TABLEPRINT_H)
...
#define WRITE_1D_FUNC_ARGV(type, linebrk, fmtstr, ...)void write_##type##_array(const type *data, int len){int i;printf(" ");for (i = 0; i < len - 1; i++) {printf(" "fmtstr",", __VA_ARGS__);if ((i & linebrk) == linebrk) printf("\n ");}printf(" "fmtstr"\n", __VA_ARGS__);}
...
#define WRITE_2D_FUNC(type)void write_##type##_2d_array(const void *arg, int len, int len2){const type *data = arg;int i;printf(" {\n");for (i = 0; i < len; i++) {write_##type##_array(data + i * len2, len2);printf(i == len - 1 ? " }\n" : " }, {\n");}}
...
#define WRITE_ARRAY_ALIGNED(prefix, align, type, name) do { const size_t array_size = FF_ARRAY_ELEMS(name); printf(prefix" DECLARE_ALIGNED("#align", " #type", "#name")[%"FMT"] = {\n", array_size); write_##type##_array(name, array_size); printf("};\n"); } while(0)
...
#define WRITE_ARRAY(prefix, type, name) do { const size_t array_size = FF_ARRAY_ELEMS(name); printf(prefix" "#type" "#name"[%"FMT"] = {\n", array_size); write_##type##_array(name, array_size); printf("};\n"); } while(0)
...
#define WRITE_2D_ARRAY(prefix, type, name) do { const size_t array_size1 = FF_ARRAY_ELEMS(name); const size_t array_size2 = FF_ARRAY_ELEMS(name[0]); printf(prefix" "#type" "#name"[%"FMT"][%"FMT"] = {\n", array_size1, array_size2 ); write_##type##_2d_array(name, array_size1, array_size2); printf("};\n"); } while(0)
