...
#define PROCESS_VALS do { val <<= lut.val0_bits; val |= lut.val0; dst[0] = (val - 1) * lut.sign; dst[1] = lut.val1; dst[2] = lut.val2; dst[3] = lut.val3; dst[4] = lut.val4; dst[5] = 0; dst[6] = 0; dst[7] = 0; if (lut.num) val = lut.val; dst += lut.num; if (dst >= last) return coeffs; lut = ff_dirac_golomb_lut[lut.state + *buf++]; } while (0)
