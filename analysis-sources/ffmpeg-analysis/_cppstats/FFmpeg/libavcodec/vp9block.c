...
#if HAVE_FAST_64BIT
#define SPLAT_CTX(var, val, n) switch (n) { case 1: var = val; break; case 2: AV_WN16A(&var, val * 0x0101); break; case 4: AV_WN32A(&var, val * 0x01010101); break; case 8: AV_WN64A(&var, val * 0x0101010101010101ULL); break; case 16: { uint64_t v64 = val * 0x0101010101010101ULL; AV_WN64A( &var, v64); AV_WN64A(&((uint8_t *) &var)[8], v64); break; } }
#else
#define SPLAT_CTX(var, val, n) switch (n) { case 1: var = val; break; case 2: AV_WN16A(&var, val * 0x0101); break; case 4: AV_WN32A(&var, val * 0x01010101); break; case 8: { uint32_t v32 = val * 0x01010101; AV_WN32A( &var, v32); AV_WN32A(&((uint8_t *) &var)[4], v32); break; } case 16: { uint32_t v32 = val * 0x01010101; AV_WN32A( &var, v32); AV_WN32A(&((uint8_t *) &var)[4], v32); AV_WN32A(&((uint8_t *) &var)[8], v32); AV_WN32A(&((uint8_t *) &var)[12], v32); break; } }
#endif
switch (ff_vp9_bwh_tab[1][b->bs][0]) {
#define SET_CTXS(perf, dir, off, n) do { SPLAT_CTX(perf->dir##_skip_ctx[off], b->skip, n); SPLAT_CTX(perf->dir##_txfm_ctx[off], b->tx, n); SPLAT_CTX(perf->dir##_partition_ctx[off], dir##_ctx[b->bs], n); if (!s->s.h.keyframe && !s->s.h.intraonly) { SPLAT_CTX(perf->dir##_intra_ctx[off], b->intra, n); SPLAT_CTX(perf->dir##_comp_ctx[off], b->comp, n); SPLAT_CTX(perf->dir##_mode_ctx[off], b->mode[3], n); if (!b->intra) { SPLAT_CTX(perf->dir##_ref_ctx[off], vref, n); if (s->s.h.filtermode == FILTER_SWITCHABLE) { SPLAT_CTX(perf->dir##_filter_ctx[off], filter_id, n); } } } } while (0)
...
#define STORE_COEF(c, i, v) do { if (is8bitsperpixel) { c[i] = v; } else { AV_WN32A(&c[i * 2], v); } } while (0)
if (!--band_left)
band_left = band_counts[++band];
if (is_tx32x32)
...
unsigned (*c)[6][3] = td->counts.coef[b->tx][0 ][!b->intra];
unsigned (*e)[6][2] = td->counts.eob[b->tx][0 ][!b->intra];
...
int total_coeff = 0;
#define MERGE(la, end, step, rd) for (n = 0; n < end; n += step) la[n] = !!rd(&la[n])
#define MERGE_CTX(step, rd) do { MERGE(l, end_y, step, rd); MERGE(a, end_x, step, rd); } while (0)
#define DECODE_Y_COEF_LOOP(step, mode_index, v) for (n = 0, y = 0; y < end_y; y += step) { for (x = 0; x < end_x; x += step, n += step * step) { enum TxfmType txtp = ff_vp9_intra_txfm_type[b->mode[mode_index]]; ret = (is8bitsperpixel ? decode_coeffs_b##v##_8bpp : decode_coeffs_b##v##_16bpp) (td, td->block + 16 * n * bytesperpixel, 16 * step * step, c, e, p, a[x] + l[y], yscans[txtp], ynbs[txtp], y_band_counts, qmul[0]); a[x] = l[y] = !!ret; total_coeff |= !!ret; if (step >= 4) { AV_WN16A(&td->eob[n], ret); } else { td->eob[n] = ret; } } }
#define SPLAT(la, end, step, cond) if (step == 2) { for (n = 1; n < end; n += step) la[n] = la[n - 1]; } else if (step == 4) { if (cond) { for (n = 0; n < end; n += step) AV_WN32A(&la[n], la[n] * 0x01010101); } else { for (n = 0; n < end; n += step) memset(&la[n + 1], la[n], FFMIN(end - n - 1, 3)); } } else { if (cond) { if (HAVE_FAST_64BIT) { for (n = 0; n < end; n += step) AV_WN64A(&la[n], la[n] * 0x0101010101010101ULL); } else { for (n = 0; n < end; n += step) { uint32_t v32 = la[n] * 0x01010101; AV_WN32A(&la[n], v32); AV_WN32A(&la[n + 4], v32); } } } else { for (n = 0; n < end; n += step) memset(&la[n + 1], la[n], FFMIN(end - n - 1, 7)); } }
#define SPLAT_CTX(step) do { SPLAT(a, end_x, step, end_x == w4); SPLAT(l, end_y, step, end_y == h4); } while (0)
switch (b->tx) {
case TX_4X4:
...
int row7 = td->row7;
#define SPLAT_ZERO_CTX(v, n) switch (n) { case 1: v = 0; break; case 2: AV_ZERO16(&v); break; case 4: AV_ZERO32(&v); break; case 8: AV_ZERO64(&v); break; case 16: AV_ZERO128(&v); break; }
#define SPLAT_ZERO_YUV(dir, var, off, n, dir2) do { SPLAT_ZERO_CTX(dir##_y_##var[off * 2], n * 2); if (s->ss_##dir2) { SPLAT_ZERO_CTX(dir##_uv_##var[0][off], n); SPLAT_ZERO_CTX(dir##_uv_##var[1][off], n); } else { SPLAT_ZERO_CTX(dir##_uv_##var[0][off * 2], n * 2); SPLAT_ZERO_CTX(dir##_uv_##var[1][off * 2], n * 2); } } while (0)
switch (w4) {
