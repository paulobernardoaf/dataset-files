...
#define get_char(c, lc, gb) { c = (c << 8) | bytestream2_get_byte(gb); lc += 8; }





#define get_code(po, rlc, c, lc, gb, out, oe, outb) { if (po == rlc) { if (lc < 8) get_char(c, lc, gb); lc -= 8; cs = c >> lc; if (out + cs > oe || out == outb) return AVERROR_INVALIDDATA; s = out[-1]; while (cs-- > 0) *out++ = s; } else if (out < oe) { *out++ = po; } else { return AVERROR_INVALIDDATA; } }
