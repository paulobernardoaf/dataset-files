...
qpix_op[luma_xy](dest_y, src_y, sl->mb_linesize); 
if (!square)
...
qpix_op[luma_xy](dest_cb, src_cb, sl->mb_linesize); 
if (!square)
...
qpix_op[luma_xy](dest_cr, src_cr, sl->mb_linesize); 
if (!square)
qpix_op[luma_xy](dest_cr + delta, src_cr + delta, sl->mb_linesize);
return;
}
ysh = 3 - (chroma_idc == 2 );
if (chroma_idc == 1 && MB_FIELD(sl)) {
...
height >> (chroma_idc == 1 ),
mx & 7, ((unsigned)my << (chroma_idc == 2 )) & 7);
if (emu) {
...
chroma_op(dest_cr, src_cr, sl->mb_uvlinesize, height >> (chroma_idc == 1 ),
mx & 7, ((unsigned)my << (chroma_idc == 2 )) & 7);
}
...
#define XCHG(a, b, xchg) if (pixel_shift) { if (xchg) { AV_SWAP64(b + 0, a + 0); AV_SWAP64(b + 8, a + 8); } else { AV_COPY128(b, a); } } else if (xchg) AV_SWAP64(b, a); else AV_COPY64(b, a);
if (deblock_top) {
if (deblock_topleft) {
