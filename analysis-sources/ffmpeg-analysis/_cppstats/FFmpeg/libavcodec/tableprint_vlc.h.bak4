...
#if !defined(AVCODEC_TABLEPRINT_VLC_H)
...
#define WRITE_VLC_TYPE(prefix, name, base_table) do { printf(prefix" VLC "#name" = {\n"); write_vlc_type(&name, base_table, #base_table); printf("};\n"); } while(0)
...
#define WRITE_VLC_ARRAY(prefix, name, base_table) do { int i; const size_t array_size = FF_ARRAY_ELEMS(name); printf(prefix" VLC "#name"[%"FMT"] = {{\n", array_size); for (i = 0; i < array_size; i++) { write_vlc_type(name + i, base_table, #base_table); if (i != array_size - 1) printf("}, {\n"); } printf("}};\n"); } while(0)
