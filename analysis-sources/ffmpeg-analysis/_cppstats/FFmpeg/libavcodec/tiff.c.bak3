...
#define RET_GEOKEY(TYPE, array, element)if (key >= TIFF_##TYPE##_KEY_ID_OFFSET &&key - TIFF_##TYPE##_KEY_ID_OFFSET < FF_ARRAY_ELEMS(ff_tiff_##array##_name_type_map))return ff_tiff_##array##_name_type_map[key - TIFF_##TYPE##_KEY_ID_OFFSET].element;
...
#define RET_GEOKEY_VAL(TYPE, array)if (val >= TIFF_##TYPE##_OFFSET &&val - TIFF_##TYPE##_OFFSET < FF_ARRAY_ELEMS(ff_tiff_##array##_codes))return av_strdup(ff_tiff_##array##_codes[val - TIFF_##TYPE##_OFFSET]);
...
if (has_width_leftover && tile_x == tile_count_x - 1) 
tile_width = s->width % s->tile_width;
else
tile_width = s->tile_width;

if (has_height_leftover && tile_y == tile_count_y - 1) 
...
unsigned tag, type, count, off, value = 0, value2 = 1; 
int i, start;
int pos;
int ret;
double *dp;
