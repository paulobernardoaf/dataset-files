...
#define GET_SYM(symbol, defaultVal) do{ CFStringRef* handle = (CFStringRef*)dlsym(RTLD_DEFAULT, #symbol); if(!handle) compat_keys.symbol = CFSTR(defaultVal); else compat_keys.symbol = *handle; }while(0)
