...
#define EXTEND(ptr, val, len) do { pixel4 pix = PIXEL_SPLAT_X4(val); for (i = 0; i < (len); i += 4) AV_WN4P(ptr + i, pix); } while (0)
...
#define EXTEND_RIGHT_CIP(ptr, start, length) for (i = start; i < (start) + (length); i += 4) if (!IS_INTRA(i, -1)) AV_WN4P(&ptr[i], a); else a = PIXEL_SPLAT_X4(ptr[i+3])
...
#define EXTEND_DOWN_CIP(ptr, start, length) for (i = start; i < (start) + (length); i += 4) if (!IS_INTRA(-1, i)) AV_WN4P(&ptr[i], a); else a = PIXEL_SPLAT_X4(ptr[i + 3])
...
#define INTRA_PRED(size) static void FUNC(intra_pred_ ##size)(HEVCContext *s, int x0, int y0, int c_idx) { FUNC(intra_pred)(s, x0, y0, size, c_idx); }
...
#define PRED_PLANAR(size)static void FUNC(pred_planar_ ##size)(uint8_t *src, const uint8_t *top, const uint8_t *left, ptrdiff_t stride) { FUNC(pred_planar)(src, top, left, stride, size + 2); }
