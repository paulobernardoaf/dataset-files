...
DECLARE_ALIGNED(16, INTFLOAT, sb_hybrid)[SBLIMIT * 18]; 
} GranuleDef;
...
#define SPLIT(dst,sf,n) if (n == 3) { int m = (sf * 171) >> 9; dst = sf - 3 * m; sf = m; } else if (n == 4) { dst = sf & 3; sf >>= 2; } else if (n == 5) { int m = (sf * 205) >> 10; dst = sf - 5 * m; sf = m; } else if (n == 6) { int m = (sf * 171) >> 10; dst = sf - 6 * m; sf = m; } else { dst = 0; }
...
#if USE_FLOATS
#define READ_FLIP_SIGN(dst,src) v = AV_RN32A(src) ^ (get_bits1(&s->gb) << 31); AV_WN32A(dst, v);


#else
#define READ_FLIP_SIGN(dst,src) v = -get_bits1(&s->gb); *(dst) = (*(src) ^ v) - v;
...
#if !defined(compute_antialias)
#if USE_FLOATS
#define AA(j) do { float tmp0 = ptr[-1-j]; float tmp1 = ptr[ j]; ptr[-1-j] = tmp0 * csa_table[j][0] - tmp1 * csa_table[j][1]; ptr[ j] = tmp0 * csa_table[j][1] + tmp1 * csa_table[j][0]; } while (0)
...
#else
#define AA(j) do { SUINT tmp0 = ptr[-1-j]; SUINT tmp1 = ptr[ j]; SUINT tmp2 = MULH(tmp0 + tmp1, csa_table[j][0]); ptr[-1-j] = 4 * (tmp2 - MULH(tmp1, csa_table[j][2])); ptr[ j] = 4 * (tmp2 + MULH(tmp0, csa_table[j][3])); } while (0)
