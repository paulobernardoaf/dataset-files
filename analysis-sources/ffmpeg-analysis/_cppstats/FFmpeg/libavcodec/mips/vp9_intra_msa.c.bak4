...
#define IPRED_SUBS_UH2_UH(in0, in1, out0, out1) { out0 = __msa_subs_u_h(out0, in0); out1 = __msa_subs_u_h(out1, in1); }
...
#define INTRA_PREDICT_VALDC_16X16_MSA(val) void ff_dc_##val##_16x16_msa(uint8_t *dst, ptrdiff_t dst_stride, const uint8_t *left, const uint8_t *top) { v16u8 out = (v16u8) __msa_ldi_b(val); ST_UB8(out, out, out, out, out, out, out, out, dst, dst_stride); dst += (8 * dst_stride); ST_UB8(out, out, out, out, out, out, out, out, dst, dst_stride); }
...
#define INTRA_PREDICT_VALDC_32X32_MSA(val) void ff_dc_##val##_32x32_msa(uint8_t *dst, ptrdiff_t dst_stride, const uint8_t *left, const uint8_t *top) { uint32_t row; v16u8 out = (v16u8) __msa_ldi_b(val); for (row = 16; row--;) { ST_UB2(out, out, dst, 16); dst += dst_stride; ST_UB2(out, out, dst, 16); dst += dst_stride; } }
