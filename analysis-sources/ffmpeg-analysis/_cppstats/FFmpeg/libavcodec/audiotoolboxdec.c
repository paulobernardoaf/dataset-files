...
#define COPY_SAMPLES(type) type *in_ptr = (type*)at->decoded_data; type *end_ptr = in_ptr + frame->nb_samples * avctx->channels; type *out_ptr = (type*)frame->data[0]; for (; in_ptr < end_ptr; in_ptr += avctx->channels, out_ptr += avctx->channels) { int c; for (c = 0; c < avctx->channels; c++) out_ptr[c] = in_ptr[at->channel_map[c]]; }
