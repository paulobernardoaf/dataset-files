...
#define fixed(width, name, value) do { av_unused uint32_t fixed_value = value; xu(width, name, fixed_value, value, value, 0); } while (0)
#define READ
#define READWRITE read
#define RWContext GetBitContext
#define xu(width, name, var, range_min, range_max, subs, ...) do { uint32_t value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
#define xue(name, var, range_min, range_max, subs, ...) do { uint32_t value; CHECK(cbs_read_ue_golomb(ctx, rw, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
#define xi(width, name, var, range_min, range_max, subs, ...) do { int32_t value; CHECK(ff_cbs_read_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
#define xse(name, var, range_min, range_max, subs, ...) do { int32_t value; CHECK(cbs_read_se_golomb(ctx, rw, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
...
#define allocate(name, size) do { name ##_ref = av_buffer_allocz(size + AV_INPUT_BUFFER_PADDING_SIZE); if (!name ##_ref) return AVERROR(ENOMEM); name = name ##_ref->data; } while (0)
...
#define xu(width, name, var, range_min, range_max, subs, ...) do { uint32_t value = var; CHECK(ff_cbs_write_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), value, range_min, range_max)); } while (0)
#define xue(name, var, range_min, range_max, subs, ...) do { uint32_t value = var; CHECK(cbs_write_ue_golomb(ctx, rw, #name, SUBSCRIPTS(subs, __VA_ARGS__), value, range_min, range_max)); } while (0)
#define xi(width, name, var, range_min, range_max, subs, ...) do { int32_t value = var; CHECK(ff_cbs_write_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), value, range_min, range_max)); } while (0)
#define xse(name, var, range_min, range_max, subs, ...) do { int32_t value = var; CHECK(cbs_write_se_golomb(ctx, rw, #name, SUBSCRIPTS(subs, __VA_ARGS__), value, range_min, range_max)); } while (0)
...
#define allocate(name, size) do { if (!name) { av_log(ctx->log_ctx, AV_LOG_ERROR, "%s must be set " "for writing.\n", #name); return AVERROR_INVALIDDATA; } } while (0)
#define FUNC(name) FUNC_H264(READWRITE, name)
#include "cbs_h264_syntax_template.c"
#undef FUNC
#define FUNC(name) FUNC_H265(READWRITE, name)
