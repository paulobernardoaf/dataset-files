...
#define STORE_NODE(NAME, STEP_INDEX)int d;uint32_t ssd;int pos;TrellisNode *u;uint8_t *h;dec_sample = av_clip_int16(dec_sample);d = sample - dec_sample;ssd = nodes[j]->ssd + d*(unsigned)d;if (ssd < nodes[j]->ssd)goto next_##NAME;
\
...
#define ADPCM_ENCODER(id_, name_, sample_fmts_, long_name_) AVCodec ff_ ##name_ ##_encoder = { .name = #name_, .long_name = NULL_IF_CONFIG_SMALL(long_name_), .type = AVMEDIA_TYPE_AUDIO, .id = id_, .priv_data_size = sizeof(ADPCMEncodeContext), .init = adpcm_encode_init, .encode2 = adpcm_encode_frame, .close = adpcm_encode_close, .sample_fmts = sample_fmts_, }
