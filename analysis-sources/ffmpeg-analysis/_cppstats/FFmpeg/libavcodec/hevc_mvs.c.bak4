...
#define CHECK_MVSET(l) check_mvset(mvLXCol, temp_col.mv + l, colPic, s->poc, refPicList, X, refIdxLx, refPicList_col, L ##l, temp_col.ref_idx[l])
...
#define DERIVE_TEMPORAL_COLOCATED_MVS derive_temporal_colocated_mvs(s, temp_col, refIdxLx, mvLXCol, X, colPic, ff_hevc_get_ref_list(s, ref, x, y))
...
#define MP_MX(v, pred, mx) mv_mp_mode_mx(s, (x ##v) >> s->ps.sps->log2_min_pu_size, (y ##v) >> s->ps.sps->log2_min_pu_size, pred, &mx, ref_idx_curr, ref_idx)





#define MP_MX_LT(v, pred, mx) mv_mp_mode_mx_lt(s, (x ##v) >> s->ps.sps->log2_min_pu_size, (y ##v) >> s->ps.sps->log2_min_pu_size, pred, &mx, ref_idx_curr, ref_idx)
