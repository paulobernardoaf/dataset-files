...
#define SET_OR_INFER(field, value, present_flag, infer) do { if (value >= 0) { field = value; need_vui = 1; } else if (!present_flag) field = infer; } while (0)
...
#define CROP(border, unit) do { if (ctx->crop_ ##border >= 0) { if (ctx->crop_ ##border % unit != 0) { av_log(bsf, AV_LOG_ERROR, "Invalid value for crop_%s: " "must be a multiple of %d.\n", #border, unit); return AVERROR(EINVAL); } sps->conf_win_ ##border ##_offset = ctx->crop_ ##border / unit; sps->conformance_window_flag = 1; } } while (0)
...
#define LEVEL(name, value) name, NULL, 0, AV_OPT_TYPE_CONST, { .i64 = value }, .flags = FLAGS, .unit = "level"
