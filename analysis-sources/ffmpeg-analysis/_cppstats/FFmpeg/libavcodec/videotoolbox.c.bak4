...
#if !defined(kVTVideoDecoderSpecification_RequireHardwareAcceleratedVideoDecoder)
#define kVTVideoDecoderSpecification_RequireHardwareAcceleratedVideoDecoder CFSTR("RequireHardwareAcceleratedVideoDecoder")
#endif
#if !defined(kVTVideoDecoderSpecification_EnableHardwareAcceleratedVideoDecoder)
...
#define COUNT_SIZE_PS(T, t) for (i = 0; i < HEVC_MAX_##T##PS_COUNT; i++) { if (h->ps.t##ps_list[i]) { const HEVC##T##PS *lps = (const HEVC##T##PS *)h->ps.t##ps_list[i]->data; vt_extradata_size += 2 + lps->data_size; num_##t##ps++; } }
...
#define APPEND_PS(T, t) AV_W8(p, 1 << 7 | HEVC_NAL_##T##PS & 0x3f); AV_WB16(p + 1, num_##t##ps); p += 3; for (i = 0; i < HEVC_MAX_##T##PS_COUNT; i++) { if (h->ps.t##ps_list[i]) { const HEVC##T##PS *lps = (const HEVC##T##PS *)h->ps.t##ps_list[i]->data; AV_WB16(p, lps->data_size); memcpy(p + 2, lps->data, lps->data_size); p += 2 + lps->data_size; } }
...
bytestream2_put_byteu(&pb, 0x06); 
bytestream2_put_byteu(&pb, 0x01); 
