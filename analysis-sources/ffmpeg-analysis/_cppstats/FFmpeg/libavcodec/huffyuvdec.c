...
#endif
#define VLC_INTERN(dst, table, gb, name, bits, max_depth) code = table[index][0]; n = table[index][1]; if (max_depth > 1 && n < 0) { LAST_SKIP_BITS(name, gb, bits); UPDATE_CACHE(name, gb); nb_bits = -n; index = SHOW_UBITS(name, gb, nb_bits) + code; code = table[index][0]; n = table[index][1]; if (max_depth > 2 && n < 0) { LAST_SKIP_BITS(name, gb, nb_bits); UPDATE_CACHE(name, gb); nb_bits = -n; index = SHOW_UBITS(name, gb, nb_bits) + code; code = table[index][0]; n = table[index][1]; } } dst = code; LAST_SKIP_BITS(name, gb, n)
#define GET_VLC_DUAL(dst0, dst1, name, gb, dtable, table1, table2, bits, max_depth, OP) do { unsigned int index = SHOW_UBITS(name, gb, bits); int code, n = dtable[index][1]; if (n<=0) { int nb_bits; VLC_INTERN(dst0, table1, gb, name, bits, max_depth); UPDATE_CACHE(re, gb); index = SHOW_UBITS(name, gb, bits); VLC_INTERN(dst1, table2, gb, name, bits, max_depth); } else { code = dtable[index][0]; OP(dst0, dst1, code); LAST_SKIP_BITS(name, gb, n); } } while (0)
#define OP8bits(dst0, dst1, code) dst0 = code>>8; dst1 = code
#define READ_2PIX(dst0, dst1, plane1) UPDATE_CACHE(re, &s->gb); GET_VLC_DUAL(dst0, dst1, re, &s->gb, s->vlc[4+plane1].table, s->vlc[0].table, s->vlc[plane1].table, VLC_BITS, 3, OP8bits)
...
#define READ_2PIX_PLANE(dst0, dst1, plane, OP) UPDATE_CACHE(re, &s->gb); GET_VLC_DUAL(dst0, dst1, re, &s->gb, s->vlc[4+plane].table, s->vlc[plane].table, s->vlc[plane].table, VLC_BITS, 3, OP)
#define OP14bits(dst0, dst1, code) dst0 = code>>8; dst1 = sign_extend(code, 8)
#define READ_2PIX_PLANE16(dst0, dst1, plane){dst0 = get_vlc2(&s->gb, s->vlc[plane].table, VLC_BITS, 3)<<2;dst0 += get_bits(&s->gb, 2);dst1 = get_vlc2(&s->gb, s->vlc[plane].table, VLC_BITS, 3)<<2;dst1 += get_bits(&s->gb, 2);}
