#include "avcodec.h"
#include "diracdsp.h"
#define FILTER(src, stride) ((21*((src)[ 0*stride] + (src)[1*stride]) -7*((src)[-1*stride] + (src)[2*stride]) +3*((src)[-2*stride] + (src)[3*stride]) -1*((src)[-3*stride] + (src)[4*stride]) + 16) >> 5)
...
#define DIRAC_WEIGHT(W) static void weight_dirac_pixels ##W ##_c(uint8_t *block, int stride, int log2_denom, int weight, int h) { int x; while (h--) { for (x = 0; x < W; x++) { op_scale1(x); op_scale1(x+1); } block += stride; } } static void biweight_dirac_pixels ##W ##_c(uint8_t *dst, const uint8_t *src, int stride, int log2_denom, int weightd, int weights, int h) { int x; while (h--) { for (x = 0; x < W; x++) { op_scale2(x); op_scale2(x+1); } dst += stride; src += stride; } }
DIRAC_WEIGHT(8)
DIRAC_WEIGHT(16)
DIRAC_WEIGHT(32)
#define ADD_OBMC(xblen) static void add_obmc ##xblen ##_c(uint16_t *dst, const uint8_t *src, int stride, const uint8_t *obmc_weight, int yblen) { int x; while (yblen--) { for (x = 0; x < xblen; x += 2) { dst[x ] += src[x ] * obmc_weight[x ]; dst[x+1] += src[x+1] * obmc_weight[x+1]; } dst += stride; src += stride; obmc_weight += 32; } }
ADD_OBMC(8)
ADD_OBMC(16)
ADD_OBMC(32)
...
#define DEQUANT_SUBBAND(PX) static void dequant_subband_ ##PX ##_c(uint8_t *src, uint8_t *dst, ptrdiff_t stride, const int qf, const int qs, int tot_v, int tot_h) { int i, y; for (y = 0; y < tot_v; y++) { PX c, sign, *src_r = (PX *)src, *dst_r = (PX *)dst; for (i = 0; i < tot_h; i++) { c = *src_r++; sign = FFSIGN(c)*(!!c); c = (FFABS(c)*(unsigned)qf + qs) >> 2; *dst_r++ = c*sign; } src += tot_h << (sizeof(PX) >> 1); dst += stride; } }
DEQUANT_SUBBAND(int16_t)
DEQUANT_SUBBAND(int32_t)
#define PIXFUNC(PFX, WIDTH) c->PFX ##_dirac_pixels_tab[WIDTH>>4][0] = ff_ ##PFX ##_dirac_pixels ##WIDTH ##_c; c->PFX ##_dirac_pixels_tab[WIDTH>>4][1] = ff_ ##PFX ##_dirac_pixels ##WIDTH ##_l2_c; c->PFX ##_dirac_pixels_tab[WIDTH>>4][2] = ff_ ##PFX ##_dirac_pixels ##WIDTH ##_l4_c; c->PFX ##_dirac_pixels_tab[WIDTH>>4][3] = ff_ ##PFX ##_dirac_pixels ##WIDTH ##_bilinear_c
