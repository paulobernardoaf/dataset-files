...
#define HEADER(name) do { ff_cbs_trace_header(ctx, name); } while (0)
#define CHECK(call) do { err = (call); if (err < 0) return err; } while (0)
...
#define fixed(width, name, value) do { av_unused uint32_t fixed_value = value; xf(width, name, fixed_value, value, value, 0); } while (0)
#define READ
#define READWRITE read
#define RWContext GetBitContext
#define xf(width, name, var, range_min, range_max, subs, ...) do { uint32_t value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
#define xsu(width, name, var, subs, ...) do { int32_t value; CHECK(ff_cbs_read_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, MIN_INT_BITS(width), MAX_INT_BITS(width))); var = value; } while (0)
#define uvlc(name, range_min, range_max) do { uint32_t value; CHECK(cbs_av1_read_uvlc(ctx, rw, #name, &value, range_min, range_max)); current->name = value; } while (0)
#define ns(max_value, name, subs, ...) do { uint32_t value; CHECK(cbs_av1_read_ns(ctx, rw, max_value, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value)); current->name = value; } while (0)
#define increment(name, min, max) do { uint32_t value; CHECK(cbs_av1_read_increment(ctx, rw, min, max, #name, &value)); current->name = value; } while (0)
#define subexp(name, max, subs, ...) do { uint32_t value; CHECK(cbs_av1_read_subexp(ctx, rw, max, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value)); current->name = value; } while (0)
#define delta_q(name) do { uint8_t delta_coded; int8_t delta_q; xf(1, name.delta_coded, delta_coded, 0, 1, 0); if (delta_coded) xsu(1 + 6, name.delta_q, delta_q, 0); else delta_q = 0; current->name = delta_q; } while (0)
#define leb128(name) do { uint64_t value; CHECK(cbs_av1_read_leb128(ctx, rw, #name, &value)); current->name = value; } while (0)
#define infer(name, value) do { current->name = value; } while (0)
...
#define xf(width, name, var, range_min, range_max, subs, ...) do { CHECK(ff_cbs_write_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), var, range_min, range_max)); } while (0)
#define xsu(width, name, var, subs, ...) do { CHECK(ff_cbs_write_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), var, MIN_INT_BITS(width), MAX_INT_BITS(width))); } while (0)
#define uvlc(name, range_min, range_max) do { CHECK(cbs_av1_write_uvlc(ctx, rw, #name, current->name, range_min, range_max)); } while (0)
#define ns(max_value, name, subs, ...) do { CHECK(cbs_av1_write_ns(ctx, rw, max_value, #name, SUBSCRIPTS(subs, __VA_ARGS__), current->name)); } while (0)
#define increment(name, min, max) do { CHECK(cbs_av1_write_increment(ctx, rw, min, max, #name, current->name)); } while (0)
#define subexp(name, max, subs, ...) do { CHECK(cbs_av1_write_subexp(ctx, rw, max, #name, SUBSCRIPTS(subs, __VA_ARGS__), current->name)); } while (0)
#define delta_q(name) do { xf(1, name.delta_coded, current->name != 0, 0, 1, 0); if (current->name) xsu(1 + 6, name.delta_q, current->name, 0); } while (0)
#define leb128(name) do { CHECK(cbs_av1_write_leb128(ctx, rw, #name, current->name)); } while (0)
#define infer(name, value) do { if (current->name != (value)) { av_log(ctx->log_ctx, AV_LOG_WARNING, "Warning: " "%s does not match inferred value: " "%"PRId64", but should be %"PRId64".\n", #name, (int64_t)current->name, (int64_t)(value)); } } while (0)
