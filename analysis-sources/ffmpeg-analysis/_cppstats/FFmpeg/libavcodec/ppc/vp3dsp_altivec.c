...
IDCT_1D(ADD8, SHIFT4)
#define PUT(a)t = vec_packsu(a, a);vec_ste((vec_u32)t, 0, (unsigned int *)dst);vec_ste((vec_u32)t, 4, (unsigned int *)dst);
PUT(b0) dst += stride;
PUT(b1) dst += stride;
PUT(b2) dst += stride;
PUT(b3) dst += stride;
...
IDCT_1D(ADD8, SHIFT4)
#if HAVE_BIGENDIAN
#define GET_VDST16vdst = vec_ld(0, dst);vdst_16 = (vec_s16)vec_perm(vdst, zero_u8v, vdst_mask);
#else
#define GET_VDST16vdst = vec_vsx_ld(0,dst);vdst_16 = (vec_s16)vec_mergeh(vdst, zero_u8v);
#endif
#define ADD(a)GET_VDST16;vdst_16 = vec_adds(a, vdst_16);t = vec_packsu(vdst_16, vdst_16);vec_ste((vec_u32)t, 0, (unsigned int *)dst);vec_ste((vec_u32)t, 4, (unsigned int *)dst);
ADD(b0) dst += stride;
ADD(b1) dst += stride;
ADD(b2) dst += stride;
ADD(b3) dst += stride;
