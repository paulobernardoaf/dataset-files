...
#define LOAD_H_SUBPEL_FILTER(i) vec_s8 filter_inner = h_subpel_filters_inner[i]; vec_s8 filter_outerh = h_subpel_filters_outer[(i)>>1]; vec_s8 filter_outerl = vec_sld(filter_outerh, filter_outerh, 2)
#if HAVE_BIGENDIAN
#define GET_PIXHL(offset) a = vec_ld((offset)-is6tap-1, src); b = vec_ld((offset)-is6tap-1+15, src); pixh = vec_perm(a, b, permh##offset); pixl = vec_perm(a, b, perml##offset)
#define GET_OUTER(offset) outer = vec_perm(a, b, perm_6tap##offset)
#else
#define GET_PIXHL(offset) a = vec_vsx_ld((offset)-is6tap-1, src); pixh = vec_perm(a, a, perm_inner); pixl = vec_perm(a, a, vec_add(perm_inner, vec_splat_u8(4)))
#define GET_OUTER(offset) outer = vec_perm(a, a, perm_outer)
#endif
#define FILTER_H(dstv, off) GET_PIXHL(off); filth = vec_msum(filter_inner, pixh, c64); filtl = vec_msum(filter_inner, pixl, c64); if (is6tap) { GET_OUTER(off); filth = vec_msum(filter_outerh, outer, filth); filtl = vec_msum(filter_outerl, outer, filtl); } if (w == 4) filtl = filth; dstv = vec_packs(filth, filtl); dstv = vec_sra(dstv, c7)
...
#define LOAD_V_SUBPEL_FILTER(i) vec_u8 subpel_filter = v_subpel_filters[i]; vec_u8 f0 = vec_splat(subpel_filter, 0); vec_u8 f1 = vec_splat(subpel_filter, 1); vec_u8 f2 = vec_splat(subpel_filter, 2); vec_u8 f3 = vec_splat(subpel_filter, 3); vec_u8 f4 = vec_splat(subpel_filter, 4); vec_u8 f5 = vec_splat(subpel_filter, 5)
#define FILTER_V(dstv, vec_mul) s1f = (vec_s16)vec_mul(s1, f1); s2f = (vec_s16)vec_mul(s2, f2); s3f = (vec_s16)vec_mul(s3, f3); s4f = (vec_s16)vec_mul(s4, f4); s2f = vec_subs(s2f, s1f); s3f = vec_subs(s3f, s4f); if (is6tap) { s0f = (vec_s16)vec_mul(s0, f0); s5f = (vec_s16)vec_mul(s5, f5); s2f = vec_adds(s2f, s0f); s3f = vec_adds(s3f, s5f); } dstv = vec_adds(s2f, s3f); dstv = vec_adds(dstv, c64); dstv = vec_sra(dstv, c7)
#if HAVE_BIGENDIAN
