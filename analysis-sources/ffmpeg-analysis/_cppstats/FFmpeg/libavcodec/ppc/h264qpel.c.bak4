...
#if HAVE_BIGENDIAN
#define put_unligned_store(s, dest) { tmp1 = vec_ld(0, dest); mask = vec_lvsl(0, dest); tmp2 = vec_ld(15, dest); edges = vec_perm(tmp2, tmp1, mask); align = vec_lvsr(0, dest); tmp2 = vec_perm(s, edges, align); tmp1 = vec_perm(edges, s, align); vec_st(tmp2, 15, dest); vec_st(tmp1, 0 , dest); }
...
#if HAVE_BIGENDIAN
#define avg_unligned_store(s, dest){ tmp1 = vec_ld(0, dest); mask = vec_lvsl(0, dest); tmp2 = vec_ld(15, dest); a = vec_avg(vec_perm(tmp1, tmp2, mask), s); edges = vec_perm(tmp2, tmp1, mask); align = vec_lvsr(0, dest); tmp2 = vec_perm(a, edges, align); tmp1 = vec_perm(edges, a, align); vec_st(tmp2, 15, dest); vec_st(tmp1, 0 , dest); }
...
#else
#define avg_unligned_store(s, dest){ a = vec_avg(vec_vsx_ld(0, dst), s); vec_vsx_st(a, 0, dst); }
