...
#if HAVE_BIGENDIAN
#define GET_VSRC1(vs0, off, b, perm0, s){ vec_u8 vsrcCuc, vsrcDuc; vsrcCuc = vec_ld(off, s); if (loadSecond){ vsrcDuc = vec_ld(off + b, s); } else vsrcDuc = vsrcCuc; vs0 = vec_perm(vsrcCuc, vsrcDuc, perm0); }
...
#define GET_VSRC(vs0, vs1, off, b, perm0, perm1, s){ vec_u8 vsrcCuc, vsrcDuc; vsrcCuc = vec_ld(off, s); if (loadSecond){ vsrcDuc = vec_ld(off + b, s); } else vsrcDuc = vsrcCuc; vs0 = vec_perm(vsrcCuc, vsrcDuc, perm0); if (reallyBadAlign){ vs1 = vsrcDuc; } else vs1 = vec_perm(vsrcCuc, vsrcDuc, perm1); }
...
#define GET_VSRC1(vs0, off, b, perm0, s){ vs0 = vec_vsx_ld(off, s); }


#define GET_VSRC(vs0, vs1, off, b, perm0, perm1, s){ vs0 = vec_vsx_ld(off, s); vs1 = vec_vsx_ld(off + 1, s); }



#endif 

#if defined(PREFIX_h264_chroma_mc8_altivec)
...
#if defined(PREFIX_no_rnd_vc1_chroma_mc8_altivec)
