...
#define COPY(dest, src) tmp = vec_packsu(src, src); vec_ste((vec_u32) tmp, 0, (unsigned int *) dest); vec_ste((vec_u32) tmp, 4, (unsigned int *) dest)
...
#if HAVE_BIGENDIAN
#define GET_TMP2(dest, prm) tmp = vec_ld(0, dest); tmp2 = (vec_s16) vec_perm(tmp, (vec_u8) zero, prm);


#else
#define GET_TMP2(dest, prm) tmp = vec_vsx_ld(0, dest); tmp2 = (vec_s16) vec_mergeh(tmp, (vec_u8) zero)


#endif

#define ADD(dest, src, perm) GET_TMP2(dest, perm); tmp3 = vec_adds(tmp2, src); tmp = vec_packsu(tmp3, tmp3); vec_ste((vec_u32) tmp, 0, (unsigned int *) dest); vec_ste((vec_u32) tmp, 4, (unsigned int *) dest)
