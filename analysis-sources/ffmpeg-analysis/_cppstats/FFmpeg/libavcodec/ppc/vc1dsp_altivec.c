...
#if HAVE_ALTIVEC
#define STEP8(s0, s1, s2, s3, s4, s5, s6, s7, vec_rnd) do { t0 = vec_sl(vec_add(s0, s4), vec_2); t0 = vec_add(vec_sl(t0, vec_1), t0); t0 = vec_add(t0, vec_rnd); t1 = vec_sl(vec_sub(s0, s4), vec_2); t1 = vec_add(vec_sl(t1, vec_1), t1); t1 = vec_add(t1, vec_rnd); t2 = vec_add(vec_sl(s6, vec_2), vec_sl(s6, vec_1)); t2 = vec_add(t2, vec_sl(s2, vec_4)); t3 = vec_add(vec_sl(s2, vec_2), vec_sl(s2, vec_1)); t3 = vec_sub(t3, vec_sl(s6, vec_4)); t4 = vec_add(t0, t2); t5 = vec_add(t1, t3); t6 = vec_sub(t1, t3); t7 = vec_sub(t0, t2); t0 = vec_sl(vec_add(s1, s3), vec_4); t0 = vec_add(t0, vec_sl(s5, vec_3)); t0 = vec_add(t0, vec_sl(s7, vec_2)); t0 = vec_add(t0, vec_sub(s5, s3)); t1 = vec_sl(vec_sub(s1, s5), vec_4); t1 = vec_sub(t1, vec_sl(s7, vec_3)); t1 = vec_sub(t1, vec_sl(s3, vec_2)); t1 = vec_sub(t1, vec_add(s1, s7)); t2 = vec_sl(vec_sub(s7, s3), vec_4); t2 = vec_add(t2, vec_sl(s1, vec_3)); t2 = vec_add(t2, vec_sl(s5, vec_2)); t2 = vec_add(t2, vec_sub(s1, s7)); t3 = vec_sl(vec_sub(s5, s7), vec_4); t3 = vec_sub(t3, vec_sl(s3, vec_3)); t3 = vec_add(t3, vec_sl(s1, vec_2)); t3 = vec_sub(t3, vec_add(s3, s5)); s0 = vec_add(t4, t0); s1 = vec_add(t5, t1); s2 = vec_add(t6, t2); s3 = vec_add(t7, t3); s4 = vec_sub(t7, t3); s5 = vec_sub(t6, t2); s6 = vec_sub(t5, t1); s7 = vec_sub(t4, t0); }while(0)
#define SHIFT_HOR8(s0, s1, s2, s3, s4, s5, s6, s7) do { s0 = vec_sra(s0, vec_3); s1 = vec_sra(s1, vec_3); s2 = vec_sra(s2, vec_3); s3 = vec_sra(s3, vec_3); s4 = vec_sra(s4, vec_3); s5 = vec_sra(s5, vec_3); s6 = vec_sra(s6, vec_3); s7 = vec_sra(s7, vec_3); }while(0)
#define SHIFT_VERT8(s0, s1, s2, s3, s4, s5, s6, s7) do { s0 = vec_sra(s0, vec_7); s1 = vec_sra(s1, vec_7); s2 = vec_sra(s2, vec_7); s3 = vec_sra(s3, vec_7); s4 = vec_sra(vec_add(s4, vec_1s), vec_7); s5 = vec_sra(vec_add(s5, vec_1s), vec_7); s6 = vec_sra(vec_add(s6, vec_1s), vec_7); s7 = vec_sra(vec_add(s7, vec_1s), vec_7); }while(0)
...
#define SHIFT_HOR4(s0, s1, s2, s3) s0 = vec_sra(s0, vec_3); s1 = vec_sra(s1, vec_3); s2 = vec_sra(s2, vec_3); s3 = vec_sra(s3, vec_3);
#define SHIFT_VERT4(s0, s1, s2, s3) s0 = vec_sra(s0, vec_7); s1 = vec_sra(s1, vec_7); s2 = vec_sra(s2, vec_7); s3 = vec_sra(s3, vec_7);
...
#define GET_TMP2(dst, p) tmp = vec_ld (0, dest); tmp2 = (vector signed short)vec_perm (tmp, vec_splat_u8(0), p);
#else
#define GET_TMP2(dst,p) tmp = vec_vsx_ld (0, dst); tmp2 = (vector signed short)vec_mergeh (tmp, vec_splat_u8(0));
#endif
#define ADD(dest,src,perm) GET_TMP2(dest, perm); tmp3 = vec_adds (tmp2, src); tmp = vec_packsu (tmp3, tmp3); vec_ste ((vector unsigned int)tmp, 0, (unsigned int *)dest); vec_ste ((vector unsigned int)tmp, 4, (unsigned int *)dest);
