...
#define DECODE_CABAC_MB_MVD(sl, list, n ){int amvd0 = sl->mvd_cache[list][scan8[n] - 1][0] +sl->mvd_cache[list][scan8[n] - 8][0];int amvd1 = sl->mvd_cache[list][scan8[n] - 1][1] +sl->mvd_cache[list][scan8[n] - 8][1];int mxd = decode_cabac_mb_mvd(sl, 40, amvd0, &mpx);int myd = decode_cabac_mb_mvd(sl, 47, amvd1, &mpy);if (mxd == INT_MIN || myd == INT_MIN) return AVERROR_INVALIDDATA; mx += mxd;my += myd;}
...
#define STORE_BLOCK(type) do { uint8_t *ctx = coeff_abs_level1_ctx[node_ctx] + abs_level_m1_ctx_base; int j= scantable[index[--coeff_count]]; if( get_cabac( CC, ctx ) == 0 ) { node_ctx = coeff_abs_level_transition[0][node_ctx]; if( is_dc ) { ((type*)block)[j] = get_cabac_bypass_sign( CC, -1); }else{ ((type*)block)[j] = (get_cabac_bypass_sign( CC, -qmul[j]) + 32) >> 6; } } else { unsigned coeff_abs = 2; ctx = coeff_abs_levelgt1_ctx[is_dc && chroma422][node_ctx] + abs_level_m1_ctx_base; node_ctx = coeff_abs_level_transition[1][node_ctx]; while( coeff_abs < 15 && get_cabac( CC, ctx ) ) { coeff_abs++; } if( coeff_abs >= 15 ) { int j = 0; while (get_cabac_bypass(CC) && j < 16+7) { j++; } coeff_abs=1; while( j-- ) { coeff_abs += coeff_abs + get_cabac_bypass( CC ); } coeff_abs+= 14U; } if( is_dc ) { ((type*)block)[j] = get_cabac_bypass_sign( CC, -coeff_abs ); }else{ ((type*)block)[j] = ((int)(get_cabac_bypass_sign( CC, -coeff_abs ) * qmul[j] + 32)) >> 6; } } } while ( coeff_count );
if (h->pixel_shift) {
STORE_BLOCK(int32_t)
} else {
STORE_BLOCK(int16_t)
