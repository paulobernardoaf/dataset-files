...
#define CHECK_KEYWORD(key) if (strcmp(keyword, key)) { av_log(avcl, AV_LOG_ERROR, "expected %s keyword, found %s = %s\n", key, keyword, value); return AVERROR_INVALIDDATA; }
#define CHECK_VALUE(key, val) if (sscanf(value, "%d", &header->val) != 1) { av_log(avcl, AV_LOG_ERROR, "invalid value of %s keyword, %s = %s\n", key, keyword, value); return AVERROR_INVALIDDATA; }
