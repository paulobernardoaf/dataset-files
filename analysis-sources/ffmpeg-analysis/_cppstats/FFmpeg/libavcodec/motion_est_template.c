...
#include "mpegvideo.h"
#define LOAD_COMMONuint32_t av_unused * const score_map= c->score_map;const int av_unused xmin= c->xmin;const int av_unused ymin= c->ymin;const int av_unused xmax= c->xmax;const int av_unused ymax= c->ymax;uint8_t *mv_penalty= c->current_mv_penalty;const int pred_x= c->pred_x;const int pred_y= c->pred_y;
#define CHECK_HALF_MV(dx, dy, x, y){const int hx= 2*(x)+(dx);const int hy= 2*(y)+(dy);d= cmp_hpel(s, x, y, dx, dy, size, h, ref_index, src_index, cmp_sub, chroma_cmp_sub, flags);d += (mv_penalty[hx - pred_x] + mv_penalty[hy - pred_y])*penalty_factor;COPY3_IF_LT(dmin, d, bx, hx, by, hy)}
...
#define CHECK_QUARTER_MV(dx, dy, x, y){const int hx= 4*(x)+(dx);const int hy= 4*(y)+(dy);d= cmp_qpel(s, x, y, dx, dy, size, h, ref_index, src_index, cmpf, chroma_cmpf, flags);d += (mv_penalty[hx - pred_x] + mv_penalty[hy - pred_y])*penalty_factor;COPY3_IF_LT(dmin, d, bx, hx, by, hy)}
...
#define CHECK_MV(x,y){const unsigned key = ((unsigned)(y)<<ME_MAP_MV_BITS) + (x) + map_generation;const int index= (((unsigned)(y)<<ME_MAP_SHIFT) + (x))&(ME_MAP_SIZE-1);av_assert2((x) >= xmin);av_assert2((x) <= xmax);av_assert2((y) >= ymin);av_assert2((y) <= ymax);if(map[index]!=key){d= cmp(s, x, y, 0, 0, size, h, ref_index, src_index, cmpf, chroma_cmpf, flags);map[index]= key;score_map[index]= d;d += (mv_penalty[((x)*(1<<shift))-pred_x] + mv_penalty[((y)*(1<<shift))-pred_y])*penalty_factor;COPY3_IF_LT(dmin, d, best[0], x, best[1], y)}}
#define CHECK_CLIPPED_MV(ax,ay){const int Lx= ax;const int Ly= ay;const int Lx2= FFMAX(xmin, FFMIN(Lx, xmax));const int Ly2= FFMAX(ymin, FFMIN(Ly, ymax));CHECK_MV(Lx2, Ly2)}
#define CHECK_MV_DIR(x,y,new_dir){const unsigned key = ((unsigned)(y)<<ME_MAP_MV_BITS) + (x) + map_generation;const int index= (((unsigned)(y)<<ME_MAP_SHIFT) + (x))&(ME_MAP_SIZE-1);if(map[index]!=key){d= cmp(s, x, y, 0, 0, size, h, ref_index, src_index, cmpf, chroma_cmpf, flags);map[index]= key;score_map[index]= d;d += (mv_penalty[(int)((unsigned)(x)<<shift)-pred_x] + mv_penalty[(int)((unsigned)(y)<<shift)-pred_y])*penalty_factor;if(d<dmin){best[0]=x;best[1]=y;dmin=d;next_dir= new_dir;}}}
#define check(x,y,S,v)if( (x)<(xmin<<(S)) ) av_log(NULL, AV_LOG_ERROR, "%d %d %d %d %d xmin" #v, xmin, (x), (y), s->mb_x, s->mb_y);if( (x)>(xmax<<(S)) ) av_log(NULL, AV_LOG_ERROR, "%d %d %d %d %d xmax" #v, xmax, (x), (y), s->mb_x, s->mb_y);if( (y)<(ymin<<(S)) ) av_log(NULL, AV_LOG_ERROR, "%d %d %d %d %d ymin" #v, ymin, (x), (y), s->mb_x, s->mb_y);if( (y)>(ymax<<(S)) ) av_log(NULL, AV_LOG_ERROR, "%d %d %d %d %d ymax" #v, ymax, (x), (y), s->mb_x, s->mb_y);
#define LOAD_COMMON2uint32_t *map= c->map;const int qpel= flags&FLAG_QPEL;const int shift= 1+qpel;
