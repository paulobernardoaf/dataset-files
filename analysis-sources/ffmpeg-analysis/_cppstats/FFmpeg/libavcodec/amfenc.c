...
res = ctx->context->pVtbl->CreateSurfaceFromDX11Native(ctx->context, texture, &surface, NULL); 
...
{
IDirect3DSurface9 *texture = (IDirect3DSurface9 *)frame->data[3]; 
res = ctx->context->pVtbl->CreateSurfaceFromDX9Native(ctx->context, texture, &surface, NULL); 
