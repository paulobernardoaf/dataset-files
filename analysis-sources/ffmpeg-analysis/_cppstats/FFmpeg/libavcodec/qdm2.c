...
#define QDM2_LIST_ADD(list, size, packet) do { if (size > 0) { list[size - 1].next = &list[size]; } list[size].packet = packet; list[size].next = NULL; size++; } while(0)
#define QDM2_SB_USED(sub_sampling) (((sub_sampling) >= 2) ? 30 : 8 << (sub_sampling))
#define FIX_NOISE_IDX(noise_idx) if ((noise_idx) >= 3840) (noise_idx) -= 3840; 
