...
#define REFINE_BIT(j) { UPDATE_CACHE(re, &s->gb); sign = block[j] >> 15; block[j] += SHOW_UBITS(re, &s->gb, 1) * ((quant_matrix[i] ^ sign) - sign) << Al; LAST_SKIP_BITS(re, &s->gb, 1); }
#define ZERO_RUN for (; ; i++) { if (i > last) { i += run; if (i > se) { av_log(s->avctx, AV_LOG_ERROR, "error count: %d\n", i); return -1; } break; } j = s->scantable.permutated[i]; if (block[j]) REFINE_BIT(j) else if (run-- == 0) break; }
...
if (s->bayer)
width = s->mb_width / nb_components; 
else
width = s->mb_width;
...
#define copy_data_segment(skip) do { ptrdiff_t length = (ptr - src) - (skip); if (length > 0) { memcpy(dst, src, length); dst += length; src = ptr; } } while (0)
