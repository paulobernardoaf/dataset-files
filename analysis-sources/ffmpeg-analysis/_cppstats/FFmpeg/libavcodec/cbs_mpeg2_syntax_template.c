...
#if defined(READ)
#define READ_AND_PATCH(name) do { ui(8, name); if (current->name == 0) { current->name = 2; av_log(ctx->log_ctx, AV_LOG_WARNING, "%s in a sequence display " "extension had the invalid value 0. Setting it to 2 " "(meaning unknown) instead.\n", #name); } } while (0)
...
if (current->picture_structure == 1 || 
current->picture_structure == 2) { 
mpeg2->number_of_frame_centre_offsets = 1;
} else {
if (current->repeat_first_field)
mpeg2->number_of_frame_centre_offsets = 3;
