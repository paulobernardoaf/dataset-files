...
#define xu(width, name, range_min, range_max, subs, ...) do { uint32_t value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); current->name = value; } while (0)
...
#define xu(width, name, range_min, range_max, subs, ...) do { uint32_t value = current->name; CHECK(ff_cbs_write_unsigned(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), value, range_min, range_max)); } while (0)
#include "cbs_jpeg_syntax_template.c"
#undef WRITE
#undef READWRITE
#undef RWContext
...
switch (unit->type) {
#define SEGMENT(marker, type, func, free) case JPEG_MARKER_ ##marker: { err = ff_cbs_alloc_unit_content(ctx, unit, sizeof(type), free); if (err < 0) return err; err = cbs_jpeg_read_ ##func(ctx, &gbc, unit->content); if (err < 0) return err; } break
...
switch (unit->type) {
#define SEGMENT(marker, func) case JPEG_MARKER_ ##marker: err = cbs_jpeg_write_ ##func(ctx, pbc, unit->content); break;
SEGMENT(DQT, dqt);
SEGMENT(DHT, dht);
SEGMENT(COM, comment);
default:
