...
#define GET_HUFF(gb, tree) (tree).syms[get_vlc2(gb, bink_trees[(tree).vlc_num].table,bink_trees[(tree).vlc_num].bits, 1)]
...
#define CHECK_READ_VAL(gb, b, t) if (!b->cur_dec || (b->cur_dec > b->cur_ptr)) return 0; t = get_bits(gb, b->len); if (!t) { b->cur_dec = NULL; return 0; } 
