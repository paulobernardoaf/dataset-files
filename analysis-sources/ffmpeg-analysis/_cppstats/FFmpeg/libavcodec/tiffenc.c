...
#define ADD_ENTRY(s, tag, type, count, ptr_val) do { ret = add_entry(s, tag, type, count, ptr_val); if (ret < 0) goto fail; } while (0)
#define ADD_ENTRY1(s, tag, type, val) do { ret = add_entry1(s, tag, type, val); if (ret < 0) goto fail; } while (0)
