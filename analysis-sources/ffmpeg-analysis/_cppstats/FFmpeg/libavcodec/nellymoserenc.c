...
DECLARE_ALIGNED(32, float, buf)[3 * NELLY_BUF_LEN]; 
float (*opt )[OPT_SIZE];
...
#define find_best(val, table, LUT, LUT_add, LUT_size) best_idx = LUT[av_clip ((lrintf(val) >> 8) + LUT_add, 0, LUT_size - 1)]; if (fabs(val - table[best_idx]) > fabs(val - table[best_idx + 1])) best_idx++;
