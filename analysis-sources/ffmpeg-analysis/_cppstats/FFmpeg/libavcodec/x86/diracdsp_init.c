...
#if HAVE_X86ASM
#define HPEL_FILTER(MMSIZE, EXT) void ff_dirac_hpel_filter_v_ ##EXT(uint8_t *, const uint8_t *, int, int); void ff_dirac_hpel_filter_h_ ##EXT(uint8_t *, const uint8_t *, int); static void dirac_hpel_filter_ ##EXT(uint8_t *dsth, uint8_t *dstv, uint8_t *dstc, const uint8_t *src, int stride, int width, int height) { while( height-- ) { ff_dirac_hpel_filter_v_ ##EXT(dstv-MMSIZE, src-MMSIZE, stride, width+MMSIZE+5); ff_dirac_hpel_filter_h_ ##EXT(dsth, src, width); ff_dirac_hpel_filter_h_ ##EXT(dstc, dstv, width); dsth += stride; dstv += stride; dstc += stride; src += stride; } }
#define PIXFUNC(PFX, IDX, EXT) c->PFX ##_dirac_pixels_tab[1][IDX] = ff_ ##PFX ##_dirac_pixels16_ ##EXT; c->PFX ##_dirac_pixels_tab[2][IDX] = ff_ ##PFX ##_dirac_pixels32_ ##EXT
#define DIRAC_PIXOP(OPNAME2, OPNAME, EXT)void ff_ ##OPNAME2 ##_dirac_pixels8_ ##EXT(uint8_t *dst, const uint8_t *src[5], int stride, int h){if (h&3)ff_ ##OPNAME2 ##_dirac_pixels8_c(dst, src, stride, h);elseOPNAME ##_pixels8_ ##EXT(dst, src[0], stride, h);}void ff_ ##OPNAME2 ##_dirac_pixels16_ ##EXT(uint8_t *dst, const uint8_t *src[5], int stride, int h){if (h&3)ff_ ##OPNAME2 ##_dirac_pixels16_c(dst, src, stride, h);elseOPNAME ##_pixels16_ ##EXT(dst, src[0], stride, h);}void ff_ ##OPNAME2 ##_dirac_pixels32_ ##EXT(uint8_t *dst, const uint8_t *src[5], int stride, int h){if (h&3) {ff_ ##OPNAME2 ##_dirac_pixels32_c(dst, src, stride, h);} else {OPNAME ##_pixels16_ ##EXT(dst , src[0] , stride, h);OPNAME ##_pixels16_ ##EXT(dst+16, src[0]+16, stride, h);}}
...
#else 
#define HPEL_FILTER(MMSIZE, EXT) void dirac_hpel_filter_ ##EXT(uint8_t *dsth, uint8_t *dstv, uint8_t *dstc, const uint8_t *src, int stride, int width, int height);
