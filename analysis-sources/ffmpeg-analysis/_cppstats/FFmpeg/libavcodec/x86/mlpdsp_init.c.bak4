...
#define REMATRIX_CHANNEL_FUNC(opt) void ff_mlp_rematrix_channel_##opt(int32_t *samples, const int32_t *coeffs, const uint8_t *bypassed_lsbs, const int8_t *noise_buffer, int index, unsigned int dest_ch, uint16_t blockpos, unsigned int maxchan, int matrix_noise_shift, int access_unit_size_pow2, int32_t mask);
...
#define MLPMUL(label, offset, offs, offc) LABEL_MANGLE(label)": \n\t" "movslq "offset"+"offs"(%0), %%rax\n\t" "movslq "offset"+"offc"(%1), %%rdx\n\t" "imul %%rdx, %%rax\n\t" "add %%rax, %%rsi\n\t"
...
#define CLEAR_ACCUM "xor %%esi, %%esi\n\t" "xor %%ecx, %%ecx\n\t"
