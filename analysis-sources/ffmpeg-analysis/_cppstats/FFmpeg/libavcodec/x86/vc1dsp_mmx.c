...
#define NORMALIZE_MMX(SHIFT) "paddw %%mm7, %%mm3 \n\t" "paddw %%mm7, %%mm4 \n\t" "psraw "SHIFT", %%mm3 \n\t" "psraw "SHIFT", %%mm4 \n\t"
...
#define LOAD_ROUNDER_MMX(ROUND) "movd "ROUND", %%mm7 \n\t" "punpcklwd %%mm7, %%mm7 \n\t" "punpckldq %%mm7, %%mm7 \n\t"
#define VC1_SHIFT2(OP, OPNAME)static void OPNAME ##vc1_shift2_mmx(uint8_t *dst, const uint8_t *src,x86_reg stride, int rnd, x86_reg offset){rnd = 8-rnd;__asm__ volatile("mov $8, %%"FF_REG_c" \n\t"LOAD_ROUNDER_MMX("%5")"movq "MANGLE(ff_pw_9)", %%mm6\n\t""1: \n\t""movd 0(%0 ), %%mm3 \n\t""movd 4(%0 ), %%mm4 \n\t""movd 0(%0,%2), %%mm1 \n\t""movd 4(%0,%2), %%mm2 \n\t""add %2, %0 \n\t""punpcklbw %%mm0, %%mm3 \n\t""punpcklbw %%mm0, %%mm4 \n\t""punpcklbw %%mm0, %%mm1 \n\t""punpcklbw %%mm0, %%mm2 \n\t""paddw %%mm1, %%mm3 \n\t""paddw %%mm2, %%mm4 \n\t""movd 0(%0,%3), %%mm1 \n\t""movd 4(%0,%3), %%mm2 \n\t""pmullw %%mm6, %%mm3 \n\t" /* 0,9,9,0*/"pmullw %%mm6, %%mm4 \n\t" /* 0,9,9,0*/"punpcklbw %%mm0, %%mm1 \n\t""punpcklbw %%mm0, %%mm2 \n\t""psubw %%mm1, %%mm3 \n\t" /*-1,9,9,0*/"psubw %%mm2, %%mm4 \n\t" /*-1,9,9,0*/"movd 0(%0,%2), %%mm1 \n\t""movd 4(%0,%2), %%mm2 \n\t""punpcklbw %%mm0, %%mm1 \n\t""punpcklbw %%mm0, %%mm2 \n\t""psubw %%mm1, %%mm3 \n\t" /*-1,9,9,-1*/"psubw %%mm2, %%mm4 \n\t" /*-1,9,9,-1*/NORMALIZE_MMX("$4")"packuswb %%mm4, %%mm3 \n\t"OP((%1), %%mm3)"movq %%mm3, (%1) \n\t""add %6, %0 \n\t""add %4, %1 \n\t""dec %%"FF_REG_c" \n\t""jnz 1b \n\t": "+r"(src), "+r"(dst): "r"(offset), "r"(-2*offset), "g"(stride), "m"(rnd),"g"(stride-offset)NAMED_CONSTRAINTS_ADD(ff_pw_9): "%"FF_REG_c, "memory");}
VC1_SHIFT2(OP_PUT, put_)
VC1_SHIFT2(OP_AVG, avg_)
#define MSPEL_FILTER13_CORE(UNPACK, MOVQ, A1, A2, A3, A4) MOVQ "*0+"A1", %%mm1 \n\t" MOVQ "*4+"A1", %%mm2 \n\t" UNPACK("%%mm1") UNPACK("%%mm2") "pmullw "MANGLE(ff_pw_3)", %%mm1\n\t" "pmullw "MANGLE(ff_pw_3)", %%mm2\n\t" MOVQ "*0+"A2", %%mm3 \n\t" MOVQ "*4+"A2", %%mm4 \n\t" UNPACK("%%mm3") UNPACK("%%mm4") "pmullw %%mm6, %%mm3 \n\t" "pmullw %%mm6, %%mm4 \n\t" "psubw %%mm1, %%mm3 \n\t" "psubw %%mm2, %%mm4 \n\t" MOVQ "*0+"A4", %%mm1 \n\t" MOVQ "*4+"A4", %%mm2 \n\t" UNPACK("%%mm1") UNPACK("%%mm2") "psllw $2, %%mm1 \n\t" "psllw $2, %%mm2 \n\t" "psubw %%mm1, %%mm3 \n\t" "psubw %%mm2, %%mm4 \n\t" MOVQ "*0+"A3", %%mm1 \n\t" MOVQ "*4+"A3", %%mm2 \n\t" UNPACK("%%mm1") UNPACK("%%mm2") "pmullw %%mm5, %%mm1 \n\t" "pmullw %%mm5, %%mm2 \n\t" "paddw %%mm1, %%mm3 \n\t" "paddw %%mm2, %%mm4 \n\t" 
#define MSPEL_FILTER13_VER_16B(NAME, A1, A2, A3, A4) static void vc1_put_ver_16b_ ##NAME ##_mmx(int16_t *dst, const uint8_t *src, x86_reg src_stride, int rnd, int64_t shift) { int h = 8; src -= src_stride; __asm__ volatile( LOAD_ROUNDER_MMX("%5") "movq "MANGLE(ff_pw_53)", %%mm5\n\t" "movq "MANGLE(ff_pw_18)", %%mm6\n\t" ".p2align 3 \n\t" "1: \n\t" MSPEL_FILTER13_CORE(DO_UNPACK, "movd 1", A1, A2, A3, A4) NORMALIZE_MMX("%6") TRANSFER_DONT_PACK(OP_PUT) "movd 8+"A1", %%mm1 \n\t" DO_UNPACK("%%mm1") "movq %%mm1, %%mm3 \n\t" "paddw %%mm1, %%mm1 \n\t" "paddw %%mm3, %%mm1 \n\t" "movd 8+"A2", %%mm3 \n\t" DO_UNPACK("%%mm3") "pmullw %%mm6, %%mm3 \n\t" "psubw %%mm1, %%mm3 \n\t" "movd 8+"A3", %%mm1 \n\t" DO_UNPACK("%%mm1") "pmullw %%mm5, %%mm1 \n\t" "paddw %%mm1, %%mm3 \n\t" "movd 8+"A4", %%mm1 \n\t" DO_UNPACK("%%mm1") "psllw $2, %%mm1 \n\t" "psubw %%mm1, %%mm3 \n\t" "paddw %%mm7, %%mm3 \n\t" "psraw %6, %%mm3 \n\t" "movq %%mm3, 16(%2) \n\t" "add %3, %1 \n\t" "add $24, %2 \n\t" "decl %0 \n\t" "jnz 1b \n\t" : "+r"(h), "+r" (src), "+r" (dst) : "r"(src_stride), "r"(3*src_stride), "m"(rnd), "m"(shift) NAMED_CONSTRAINTS_ADD(ff_pw_3,ff_pw_53,ff_pw_18) : "memory" ); }
#define MSPEL_FILTER13_HOR_16B(NAME, A1, A2, A3, A4, OP, OPNAME) static void OPNAME ##vc1_hor_16b_ ##NAME ##_mmx(uint8_t *dst, x86_reg stride, const int16_t *src, int rnd) { int h = 8; src -= 1; rnd -= (-4+58+13-3)*256; __asm__ volatile( LOAD_ROUNDER_MMX("%4") "movq "MANGLE(ff_pw_18)", %%mm6 \n\t" "movq "MANGLE(ff_pw_53)", %%mm5 \n\t" ".p2align 3 \n\t" "1: \n\t" MSPEL_FILTER13_CORE(DONT_UNPACK, "movq 2", A1, A2, A3, A4) NORMALIZE_MMX("$7") "paddw "MANGLE(ff_pw_128)", %%mm3 \n\t" "paddw "MANGLE(ff_pw_128)", %%mm4 \n\t" TRANSFER_DO_PACK(OP) "add $24, %1 \n\t" "add %3, %2 \n\t" "decl %0 \n\t" "jnz 1b \n\t" : "+r"(h), "+r" (src), "+r" (dst) : "r"(stride), "m"(rnd) NAMED_CONSTRAINTS_ADD(ff_pw_3,ff_pw_18,ff_pw_53,ff_pw_128) : "memory" ); }
#define MSPEL_FILTER13_8B(NAME, A1, A2, A3, A4, OP, OPNAME) static void OPNAME ##vc1_##NAME ##_mmx(uint8_t *dst, const uint8_t *src, x86_reg stride, int rnd, x86_reg offset) { int h = 8; src -= offset; rnd = 32-rnd; __asm__ volatile ( LOAD_ROUNDER_MMX("%6") "movq "MANGLE(ff_pw_53)", %%mm5 \n\t" "movq "MANGLE(ff_pw_18)", %%mm6 \n\t" ".p2align 3 \n\t" "1: \n\t" MSPEL_FILTER13_CORE(DO_UNPACK, "movd 1", A1, A2, A3, A4) NORMALIZE_MMX("$6") TRANSFER_DO_PACK(OP) "add %5, %1 \n\t" "add %5, %2 \n\t" "decl %0 \n\t" "jnz 1b \n\t" : "+r"(h), "+r" (src), "+r" (dst) : "r"(offset), "r"(3*offset), "g"(stride), "m"(rnd) NAMED_CONSTRAINTS_ADD(ff_pw_53,ff_pw_18,ff_pw_3) : "memory" ); }
...
#define FN_ASSIGN(OP, X, Y, INSN) dsp->OP##vc1_mspel_pixels_tab[1][X+4*Y] = OP##vc1_mspel_mc##X##Y##INSN; dsp->OP##vc1_mspel_pixels_tab[0][X+4*Y] = OP##vc1_mspel_mc##X##Y##_16##INSN
