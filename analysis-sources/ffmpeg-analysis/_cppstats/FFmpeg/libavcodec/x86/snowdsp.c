...
#if HAVE_7REGS
#define snow_vertical_compose_sse2_load_add(op,r,t0,t1,t2,t3)""op" ("r",%%"FF_REG_d"), %%"t0" \n\t"""op" 16("r",%%"FF_REG_d"), %%"t1" \n\t"""op" 32("r",%%"FF_REG_d"), %%"t2" \n\t"""op" 48("r",%%"FF_REG_d"), %%"t3" \n\t"
...
#define snow_vertical_compose_mmx_load_add(op,r,t0,t1,t2,t3)""op" ("r",%%"FF_REG_d"), %%"t0" \n\t"""op" 8("r",%%"FF_REG_d"), %%"t1" \n\t"""op" 16("r",%%"FF_REG_d"), %%"t2" \n\t"""op" 24("r",%%"FF_REG_d"), %%"t3" \n\t"
...
#define snow_vertical_compose_mmx_store(w,s0,s1,s2,s3)"movq %%"s0", ("w",%%"FF_REG_d") \n\t""movq %%"s1", 8("w",%%"FF_REG_d") \n\t""movq %%"s2", 16("w",%%"FF_REG_d") \n\t""movq %%"s3", 24("w",%%"FF_REG_d") \n\t"
...
#endif 
#if HAVE_6REGS
#define snow_inner_add_yblock_sse2_header IDWTELEM * * dst_array = sb->line + src_y;x86_reg tmp;__asm__ volatile("mov %7, %%"FF_REG_c" \n\t""mov %6, %2 \n\t""mov %4, %%"FF_REG_S" \n\t""pxor %%xmm7, %%xmm7 \n\t" /* 0 */"pcmpeqd %%xmm3, %%xmm3 \n\t""psllw $15, %%xmm3 \n\t""psrlw $12, %%xmm3 \n\t" /* FRAC_BITS >> 1 */"1: \n\t""mov %1, %%"FF_REG_D" \n\t""mov (%%"FF_REG_D"), %%"FF_REG_D" \n\t""add %3, %%"FF_REG_D" \n\t"
...
#define snow_inner_add_yblock_sse2_end_common2"jnz 1b \n\t":"+m"(dst8),"+m"(dst_array),"=&r"(tmp):"rm"((x86_reg)(src_x<<1)),"m"(obmc),"a"(block),"m"(b_h),"m"(src_stride):XMM_CLOBBERS("%xmm0", "%xmm1", "%xmm2", "%xmm3", "%xmm4", "%xmm5", "%xmm6", "%xmm7", )"%"FF_REG_c"","%"FF_REG_S"","%"FF_REG_D"","%"FF_REG_d"");
#define snow_inner_add_yblock_sse2_end_8"sal $1, %%"FF_REG_c" \n\t""add"FF_OPSIZE" $"FF_PTR_SIZE"*2, %1 \n\t"snow_inner_add_yblock_sse2_end_common1"sar $1, %%"FF_REG_c" \n\t""sub $2, %2 \n\t"snow_inner_add_yblock_sse2_end_common2
#define snow_inner_add_yblock_sse2_end_16"add"FF_OPSIZE" $"FF_PTR_SIZE"*1, %1 \n\t"snow_inner_add_yblock_sse2_end_common1"dec %2 \n\t"snow_inner_add_yblock_sse2_end_common2
...
#define snow_inner_add_yblock_mmx_header IDWTELEM * * dst_array = sb->line + src_y;x86_reg tmp;__asm__ volatile("mov %7, %%"FF_REG_c" \n\t""mov %6, %2 \n\t""mov %4, %%"FF_REG_S" \n\t""pxor %%mm7, %%mm7 \n\t" /* 0 */"pcmpeqd %%mm3, %%mm3 \n\t""psllw $15, %%mm3 \n\t""psrlw $12, %%mm3 \n\t" /* FRAC_BITS >> 1 */"1: \n\t""mov %1, %%"FF_REG_D" \n\t""mov (%%"FF_REG_D"), %%"FF_REG_D" \n\t""add %3, %%"FF_REG_D" \n\t"
...
#define snow_inner_add_yblock_mmx_end(s_step)"add $"s_step", %%"FF_REG_S" \n\t""add %%"FF_REG_c", "FF_PTR_SIZE"*3(%%"FF_REG_a"); \n\t""add %%"FF_REG_c", "FF_PTR_SIZE"*2(%%"FF_REG_a"); \n\t""add %%"FF_REG_c", "FF_PTR_SIZE"*1(%%"FF_REG_a"); \n\t""add %%"FF_REG_c", (%%"FF_REG_a") \n\t""add"FF_OPSIZE " $"FF_PTR_SIZE"*1, %1 \n\t""add %%"FF_REG_c", %0 \n\t""dec %2 \n\t""jnz 1b \n\t":"+m"(dst8),"+m"(dst_array),"=&r"(tmp):"rm"((x86_reg)(src_x<<1)),"m"(obmc),"a"(block),"m"(b_h),"m"(src_stride):"%"FF_REG_c"","%"FF_REG_S"","%"FF_REG_D"","%"FF_REG_d"");
