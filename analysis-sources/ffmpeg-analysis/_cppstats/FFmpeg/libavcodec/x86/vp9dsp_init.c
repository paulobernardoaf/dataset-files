...
#endif
#define itxfm_func(typea, typeb, size, opt) void ff_vp9_##typea##_##typeb##_##size##x##size##_add_##opt(uint8_t *dst, ptrdiff_t stride, int16_t *block, int eob)
#define itxfm_funcs(size, opt) itxfm_func(idct, idct, size, opt); itxfm_func(iadst, idct, size, opt); itxfm_func(idct, iadst, size, opt); itxfm_func(iadst, iadst, size, opt)
...
#undef itxfm_func
#undef itxfm_funcs
#define lpf_funcs(size1, size2, opt) void ff_vp9_loop_filter_v_##size1##_##size2##_##opt(uint8_t *dst, ptrdiff_t stride, int E, int I, int H); void ff_vp9_loop_filter_h_##size1##_##size2##_##opt(uint8_t *dst, ptrdiff_t stride, int E, int I, int H)
...
lpf_funcs(88, 16, avx);
#undef lpf_funcs
#define ipred_func(size, type, opt) void ff_vp9_ipred_##type##_##size##x##size##_##opt(uint8_t *dst, ptrdiff_t stride, const uint8_t *l, const uint8_t *a)
ipred_func(8, v, mmx);
#define ipred_dc_funcs(size, opt) ipred_func(size, dc, opt); ipred_func(size, dc_left, opt); ipred_func(size, dc_top, opt)
ipred_dc_funcs(4, mmxext);
ipred_dc_funcs(8, mmxext);
#define ipred_dir_tm_funcs(size, opt) ipred_func(size, tm, opt); ipred_func(size, dl, opt); ipred_func(size, dr, opt); ipred_func(size, hd, opt); ipred_func(size, hu, opt); ipred_func(size, vl, opt); ipred_func(size, vr, opt)
ipred_dir_tm_funcs(4, mmxext);
ipred_func(16, v, sse);
ipred_func(32, v, sse);
ipred_dc_funcs(16, sse2);
ipred_dc_funcs(32, sse2);
#define ipred_dir_tm_h_funcs(size, opt) ipred_dir_tm_funcs(size, opt); ipred_func(size, h, opt)
ipred_dir_tm_h_funcs(8, sse2);
ipred_dir_tm_h_funcs(16, sse2);
ipred_dir_tm_h_funcs(32, sse2);
ipred_func(4, h, sse2);
#define ipred_all_funcs(size, opt) ipred_dc_funcs(size, opt); ipred_dir_tm_h_funcs(size, opt)
...
#define init_dir_tm_ipred(sz, opt) do { init_ipred(sz, opt, dl, DIAG_DOWN_LEFT); init_ipred(sz, opt, dr, DIAG_DOWN_RIGHT); init_ipred(sz, opt, hd, HOR_DOWN); init_ipred(sz, opt, vl, VERT_LEFT); init_ipred(sz, opt, hu, HOR_UP); init_ipred(sz, opt, tm, TM_VP8); init_ipred(sz, opt, vr, VERT_RIGHT); } while (0)
#define init_dir_tm_h_ipred(sz, opt) do { init_dir_tm_ipred(sz, opt); init_ipred(sz, opt, h, HOR); } while (0)
#define init_dc_ipred(sz, opt) do { init_ipred(sz, opt, dc, DC); init_ipred(sz, opt, dc_left, LEFT_DC); init_ipred(sz, opt, dc_top, TOP_DC); } while (0)
#define init_all_ipred(sz, opt) do { init_dc_ipred(sz, opt); init_dir_tm_h_ipred(sz, opt); } while (0)
