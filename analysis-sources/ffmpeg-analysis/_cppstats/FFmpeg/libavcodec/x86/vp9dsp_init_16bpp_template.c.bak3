...
#define decl_lpf_func(dir, wd, bpp, opt) void ff_vp9_loop_filter_##dir##_##wd##_##bpp##_##opt(uint8_t *dst, ptrdiff_t stride, int E, int I, int H)



#define decl_lpf_funcs(dir, wd, bpp) decl_lpf_func(dir, wd, bpp, sse2); decl_lpf_func(dir, wd, bpp, ssse3); decl_lpf_func(dir, wd, bpp, avx)




#define decl_lpf_funcs_wd(dir) decl_lpf_funcs(dir, 4, BPC); decl_lpf_funcs(dir, 8, BPC); decl_lpf_funcs(dir, 16, BPC)
...
#define lpf_16_wrapper(dir, off, bpp, opt) static void loop_filter_##dir##_16_##bpp##_##opt(uint8_t *dst, ptrdiff_t stride, int E, int I, int H) { ff_vp9_loop_filter_##dir##_16_##bpp##_##opt(dst, stride, E, I, H); ff_vp9_loop_filter_##dir##_16_##bpp##_##opt(dst + off, stride, E, I, H); }
...
#define lpf_16_wrappers(bpp, opt) lpf_16_wrapper(h, 8 * stride, bpp, opt) lpf_16_wrapper(v, 16, bpp, opt)
...
#define lpf_mix2_wrapper(dir, off, wd1, wd2, bpp, opt) static void loop_filter_##dir##_##wd1##wd2##_##bpp##_##opt(uint8_t *dst, ptrdiff_t stride, int E, int I, int H) { ff_vp9_loop_filter_##dir##_##wd1##_##bpp##_##opt(dst, stride, E & 0xff, I & 0xff, H & 0xff); ff_vp9_loop_filter_##dir##_##wd2##_##bpp##_##opt(dst + off, stride, E >> 8, I >> 8, H >> 8); }
...
#define lpf_mix2_wrappers(wd1, wd2, bpp, opt) lpf_mix2_wrapper(h, 8 * stride, wd1, wd2, bpp, opt) lpf_mix2_wrapper(v, 16, wd1, wd2, bpp, opt)



#define lpf_mix2_wrappers_set(bpp, opt) lpf_mix2_wrappers(4, 4, bpp, opt) lpf_mix2_wrappers(4, 8, bpp, opt) lpf_mix2_wrappers(8, 4, bpp, opt) lpf_mix2_wrappers(8, 8, bpp, opt) 
...
#define init_lpf_funcs(bpp, opt) init_lpf_8_func(0, 0, h, 4, bpp, opt); init_lpf_8_func(0, 1, v, 4, bpp, opt); init_lpf_8_func(1, 0, h, 8, bpp, opt); init_lpf_8_func(1, 1, v, 8, bpp, opt); init_lpf_8_func(2, 0, h, 16, bpp, opt); init_lpf_8_func(2, 1, v, 16, bpp, opt); init_lpf_16_func(0, h, bpp, opt); init_lpf_16_func(1, v, bpp, opt); init_lpf_mix2_func(0, 0, 0, h, 4, 4, bpp, opt); init_lpf_mix2_func(0, 1, 0, h, 4, 8, bpp, opt); init_lpf_mix2_func(1, 0, 0, h, 8, 4, bpp, opt); init_lpf_mix2_func(1, 1, 0, h, 8, 8, bpp, opt); init_lpf_mix2_func(0, 0, 1, v, 4, 4, bpp, opt); init_lpf_mix2_func(0, 1, 1, v, 4, 8, bpp, opt); init_lpf_mix2_func(1, 0, 1, v, 8, 4, bpp, opt); init_lpf_mix2_func(1, 1, 1, v, 8, 8, bpp, opt)
...
#define init_itx_func(idxa, idxb, typea, typeb, size, bpp, opt) dsp->itxfm_add[idxa][idxb] = cat(ff_vp9_##typea##_##typeb##_##size##x##size##_add_, bpp, _##opt);


#define init_itx_func_one(idx, typea, typeb, size, bpp, opt) init_itx_func(idx, DCT_DCT, typea, typeb, size, bpp, opt); init_itx_func(idx, ADST_DCT, typea, typeb, size, bpp, opt); init_itx_func(idx, DCT_ADST, typea, typeb, size, bpp, opt); init_itx_func(idx, ADST_ADST, typea, typeb, size, bpp, opt)




#define init_itx_funcs(idx, size, bpp, opt) init_itx_func(idx, DCT_DCT, idct, idct, size, bpp, opt); init_itx_func(idx, ADST_DCT, idct, iadst, size, bpp, opt); init_itx_func(idx, DCT_ADST, iadst, idct, size, bpp, opt); init_itx_func(idx, ADST_ADST, iadst, iadst, size, bpp, opt); 
