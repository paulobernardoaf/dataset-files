...
#define ALLOC_BUF(data, size) do { av_buffer_realloc(&pkt->buf, size); data = pkt->buf ? pkt->buf->data : NULL; } while (0)
#define DUP_DATA(dst, src, size, padding, ALLOC) do { void *data; if (padding) { if ((unsigned)(size) > (unsigned)(size) + AV_INPUT_BUFFER_PADDING_SIZE) goto failed_alloc; ALLOC(data, size + AV_INPUT_BUFFER_PADDING_SIZE); } else { ALLOC(data, size); } if (!data) goto failed_alloc; memcpy(data, src, size); if (padding) memset((uint8_t *)data + size, 0, AV_INPUT_BUFFER_PADDING_SIZE); dst = data; } while (0)
