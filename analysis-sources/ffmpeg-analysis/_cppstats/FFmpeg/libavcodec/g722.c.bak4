...
#define ACCUM(k, x, d) do { int tmp = x; band->zero_mem[k] = ((band->zero_mem[k] * 255) >> 8) + d*((band->diff_mem[k]^cur_diff) < 0 ? -128 : 128); band->diff_mem[k] = tmp; s_zero += (tmp * band->zero_mem[k]) >> 15; } while (0)
