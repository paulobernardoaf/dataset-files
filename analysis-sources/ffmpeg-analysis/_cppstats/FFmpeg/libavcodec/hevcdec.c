...
{
#define HWACCEL_MAX (CONFIG_HEVC_DXVA2_HWACCEL + CONFIG_HEVC_D3D11VA_HWACCEL * 2 + CONFIG_HEVC_NVDEC_HWACCEL + CONFIG_HEVC_VAAPI_HWACCEL + CONFIG_HEVC_VIDEOTOOLBOX_HWACCEL + CONFIG_HEVC_VDPAU_HWACCEL)
...
#define SET_SAO(elem, value) do { if (!sao_merge_up_flag && !sao_merge_left_flag) sao->elem = value; else if (sao_merge_left_flag) sao->elem = CTB(s->sao, rx-1, ry).elem; else if (sao_merge_up_flag) sao->elem = CTB(s->sao, rx, ry-1).elem; else sao->elem = 0; } while (0)
...
#define SUBDIVIDE(x, y, idx) do { ret = hls_transform_tree(s, x, y, x0, y0, cb_xBase, cb_yBase, log2_cb_size, log2_trafo_size - 1, trafo_depth + 1, idx, cbf_cb, cbf_cr); if (ret < 0) return ret; } while (0)
SUBDIVIDE(x0, y0, 0);
SUBDIVIDE(x1, y0, 1);
SUBDIVIDE(x0, y1, 2);
SUBDIVIDE(x1, y1, 3);
...
{
#define POS(c_idx, x, y) &s->frame->data[c_idx][((y) >> s->ps.sps->vshift[c_idx]) * s->frame->linesize[c_idx] + (((x) >> s->ps.sps->hshift[c_idx]) << s->ps.sps->pixel_shift)]
HEVCLocalContext *lc = s->HEVClc;
int merge_idx = 0;
