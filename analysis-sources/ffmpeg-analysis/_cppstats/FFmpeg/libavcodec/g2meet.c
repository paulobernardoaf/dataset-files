...
#define LOAD_NEIGHBOURS(x) W = curr_row[(x) - 1]; N = above_row[(x)]; WW = curr_row[(x) - 2]; NW = above_row[(x) - 1]; NE = above_row[(x) + 1]; NN = above2_row[(x)]; NNW = above2_row[(x) - 1]; NWW = above_row[(x) - 2]; NNE = above2_row[(x) + 1]
#define UPDATE_NEIGHBOURS(x) NNW = NN; NN = NNE; NWW = NW; NW = N; N = NE; NE = above_row[(x) + 1]; NNE = above2_row[(x) + 1]
#define R_shift 16
#define G_shift 8
#define B_shift 0
...
els_dsize = prefix & ((0x80 >> extrabytes) - 1); 
while (extrabytes-- > 0) {
