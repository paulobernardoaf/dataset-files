...
#define COPY_SAMPLES(type) do { for (ch = 0; ch < channels; ch++) { int32_t *bptr = s->sample_buf[ch]; const type *sptr = (const type *)samples[ch]; for (i = 0; i < s->frame_size; i++) bptr[i] = sptr[i] >> shift; } } while (0)
