...
#if !defined(AVCODEC_H264_MVPRED_H)
...
#define FIX_MV_MBAFF(type, refn, mvn, idx) if (FRAME_MBAFF(h)) { if (MB_FIELD(sl)) { if (!IS_INTERLACED(type)) { refn <<= 1; AV_COPY32(mvbuf[idx], mvn); mvbuf[idx][1] /= 2; mvn = mvbuf[idx]; } } else { if (IS_INTERLACED(type)) { refn >>= 1; AV_COPY32(mvbuf[idx], mvn); mvbuf[idx][1] *= 2; mvn = mvbuf[idx]; } } }
...
#define MAP_F2F(idx, mb_type) if (!IS_INTERLACED(mb_type) && sl->ref_cache[list][idx] >= 0) { sl->ref_cache[list][idx] *= 2; sl->mv_cache[list][idx][1] /= 2; sl->mvd_cache[list][idx][1] >>= 1; }
...
#undef MAP_F2F
#define MAP_F2F(idx, mb_type) if (IS_INTERLACED(mb_type) && sl->ref_cache[list][idx] >= 0) { sl->ref_cache[list][idx] >>= 1; sl->mv_cache[list][idx][1] *= 2; sl->mvd_cache[list][idx][1] <<= 1; }
