...
put_bits(&s->pb, 4, 1); 
put_bits(&s->pb, 1, s->avctx->profile == 0); 
put_bits(&s->pb, 3, s->avctx->profile); 
put_bits(&s->pb, 4, s->avctx->level); 
...
put_bits(&s->pb, 2, s->mpeg2_frame_rate_ext.num-1); 
put_bits(&s->pb, 5, s->mpeg2_frame_rate_ext.den-1); 
...
put_header(s, EXT_START_CODE);
put_bits(&s->pb, 4, 2); 
put_bits(&s->pb, 3, s->video_format); 
put_bits(&s->pb, 1, 1); 
put_bits(&s->pb, 8, s->avctx->color_primaries); 
put_bits(&s->pb, 8, s->avctx->color_trc); 
put_bits(&s->pb, 8, s->avctx->colorspace); 
put_bits(&s->pb, 14, width); 
put_bits(&s->pb, 1, 1); 
put_bits(&s->pb, 14, height); 
put_bits(&s->pb, 3, 0); 
}
}
put_header(s, GOP_START_CODE);
put_bits(&s->pb, 1, s->drop_frame_timecode); 
...
#define mpeg12_class(x) static const AVClass mpeg ##x ##_class = { .class_name = "mpeg" #x "video encoder", .item_name = av_default_item_name, .option = mpeg ##x ##_options, .version = LIBAVUTIL_VERSION_INT, };
mpeg12_class(1)
mpeg12_class(2)
