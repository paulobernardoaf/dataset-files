...
#define UPDATE_PICTURE(pic)do {ff_mpeg_unref_picture(s->avctx, &s->pic);if (s1->pic.f && s1->pic.f->buf[0])ret = ff_mpeg_ref_picture(s->avctx, &s->pic, &s1->pic);elseret = ff_update_picture_tables(&s->pic, &s1->pic);if (ret < 0)return ret;} while (0)
UPDATE_PICTURE(current_picture);
UPDATE_PICTURE(last_picture);
UPDATE_PICTURE(next_picture);
#define REBASE_PICTURE(pic, new_ctx, old_ctx) ((pic && pic >= old_ctx->picture && pic < old_ctx->picture + MAX_PICTURE_COUNT) ? &new_ctx->picture[pic - old_ctx->picture] : NULL)
