...
#define CMUL0(dst, id, s0, s1, s2, s3, t0, t1, t2, t3, is, it) dst[id] = s0[is] * t0[it] + s1[is] * t1[it] + s2[is] * t2[it] + s3[is] * t3[it]; dst[id + 1] = s0[is] * t0[it + 1] + s1[is] * t1[it + 1] + s2[is] * t2[it + 1] + s3[is] * t3[it + 1];
#define CMUL1(dst, s0, s1, s2, s3, t0, t1, t2, t3, is, it) *dst++ = CMUL1_R(s0, t0, is, it) + CMUL1_R(s1, t1, is, it) + CMUL1_R(s2, t2, is, it) + CMUL1_R(s3, t3, is, it); *dst++ = CMUL1_I(s0, t0, is, it) + CMUL1_I(s1, t1, is, it) + CMUL1_I(s2, t2, is, it) + CMUL1_I(s3, t3, is, it);
#define CMUL2(dst, s0, s1, s2, s3, t0, t1, t2, t3, is, it) *dst++ = CMUL2_R(s0, t0, is, it) + CMUL2_R(s1, t1, is, it) + CMUL2_R(s2, t2, is, it) + CMUL2_R(s3, t3, is, it); *dst++ = CMUL2_I(s0, t0, is, it) + CMUL2_I(s1, t1, is, it) + CMUL2_I(s2, t2, is, it) + CMUL2_I(s3, t3, is, it);
