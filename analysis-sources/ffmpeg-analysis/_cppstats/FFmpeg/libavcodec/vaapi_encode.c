...
#define TRY_RC_MODE(mode, fail) do { rc_mode = &vaapi_encode_rc_modes[mode]; if (!(rc_mode->va_mode & supported_va_rc_modes)) { if (fail) { av_log(avctx, AV_LOG_ERROR, "Driver does not support %s " "RC mode (supported modes: %s).\n", rc_mode->name, supported_rc_modes_string); return AVERROR(EINVAL); } av_log(avctx, AV_LOG_DEBUG, "Driver does not support %s " "RC mode.\n", rc_mode->name); rc_mode = NULL; } else { goto rc_mode_found; } } while (0)
