...
#define JNI_GET_ENV_OR_RETURN(env, log_ctx, ret) do { (env) = ff_jni_get_env(log_ctx); if (!(env)) { return ret; } } while (0)
...
#define JNI_GET_ENV_OR_RETURN_VOID(env, log_ctx) do { (env) = ff_jni_get_env(log_ctx); if (!(env)) { return; } } while (0)
...
#define DECLARE_FF_AMEDIACODEC_CREATE_FUNC(name, method) FFAMediaCodec *ff_AMediaCodec_##name(const char *arg) { return codec_create(method, arg); } 
