...
#define CASE_N(a, t, rd) case a: for (i = 0; i < header->naxisn[1]; i++) { for (j = 0; j < header->naxisn[0]; j++) { t = rd; if (!header->blank_found || t != header->blank) { if (t > header->data_max) header->data_max = t; if (t < header->data_min) header->data_min = t; } ptr8 += abs(a) >> 3; } } break
...
if (header.rgb) {
switch(header.bitpix) {
#define CASE_RGB(cas, dst, type, dref) case cas: for (k = 0; k < header.naxisn[2]; k++) { for (i = 0; i < avctx->height; i++) { dst = (type *) (p->data[map[k]] + (avctx->height - i - 1) * p->linesize[map[k]]); for (j = 0; j < avctx->width; j++) { t32 = dref(ptr8); if (!header.blank_found || t32 != header.blank) { t = t32 * header.bscale + header.bzero; } else { t = fitsctx->blank_val; } *dst++ = (type) t; ptr8 += cas >> 3; } } } break
...
switch (header.bitpix) {
#define CASE_GRAY(cas, dst, type, t, rd) case cas: for (i = 0; i < avctx->height; i++) { dst = (type *) (p->data[0] + (avctx->height-i-1)* p->linesize[0]); for (j = 0; j < avctx->width; j++) { t = rd; if (!header.blank_found || t != header.blank) { *dst++ = lrint(((t - header.data_min) * ((1 << (sizeof(type) * 8)) - 1)) * scale); } else { *dst++ = fitsctx->blank_val; } ptr8 += abs(cas) >> 3; } } break
