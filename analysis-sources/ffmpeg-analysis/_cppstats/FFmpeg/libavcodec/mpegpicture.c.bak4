...
{
int ret, i;
#define MAKE_WRITABLE(table) do {if (pic->table &&(ret = av_buffer_make_writable(&pic->table)) < 0)return ret;} while (0)
...
#define UPDATE_TABLE(table) do { if (src->table && (!dst->table || dst->table->buffer != src->table->buffer)) { av_buffer_unref(&dst->table); dst->table = av_buffer_ref(src->table); if (!dst->table) { ff_free_picture_tables(dst); return AVERROR(ENOMEM); } } } while (0)
