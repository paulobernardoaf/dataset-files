...
#define bit(string, value) do { av_unused uint32_t bit = value; xuia(1, string, bit, value, value, 0); } while (0)
#define READ
#define READWRITE read
#define RWContext GetBitContext
#define xuia(width, string, var, range_min, range_max, subs, ...) do { uint32_t value; CHECK(ff_cbs_read_unsigned(ctx, rw, width, string, SUBSCRIPTS(subs, __VA_ARGS__), &value, range_min, range_max)); var = value; } while (0)
#define xsi(width, name, var, subs, ...) do { int32_t value; CHECK(ff_cbs_read_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), &value, MIN_INT_BITS(width), MAX_INT_BITS(width))); var = value; } while (0)
#define nextbits(width, compare, var) (get_bits_left(rw) >= width && (var = show_bits(rw, width)) == (compare))
...
#define xuia(width, string, var, range_min, range_max, subs, ...) do { CHECK(ff_cbs_write_unsigned(ctx, rw, width, string, SUBSCRIPTS(subs, __VA_ARGS__), var, range_min, range_max)); } while (0)
#define xsi(width, name, var, subs, ...) do { CHECK(ff_cbs_write_signed(ctx, rw, width, #name, SUBSCRIPTS(subs, __VA_ARGS__), var, MIN_INT_BITS(width), MAX_INT_BITS(width))); } while (0)
...
switch (unit->type) {
#define START(start_code, type, read_func, free_func) case start_code: { type *header; err = ff_cbs_alloc_unit_content(ctx, unit, sizeof(*header), free_func); if (err < 0) return err; header = unit->content; err = cbs_mpeg2_read_ ##read_func(ctx, &gbc, header); if (err < 0) return err; } break;
...
int err;
switch (unit->type) {
#define START(start_code, type, func) case start_code: err = cbs_mpeg2_write_ ##func(ctx, pbc, unit->content); break;
