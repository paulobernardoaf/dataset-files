...
#define DECL_PUT(X, Y) void ff_put_vc1_mspel_mc##X##Y##_neon(uint8_t *dst, const uint8_t *src, ptrdiff_t stride, int rnd); static void ff_put_vc1_mspel_mc##X##Y##_16_neon(uint8_t *dst, const uint8_t *src, ptrdiff_t stride, int rnd) { ff_put_vc1_mspel_mc##X##Y##_neon(dst+0, src+0, stride, rnd); ff_put_vc1_mspel_mc##X##Y##_neon(dst+8, src+8, stride, rnd); dst += 8*stride; src += 8*stride; ff_put_vc1_mspel_mc##X##Y##_neon(dst+0, src+0, stride, rnd); ff_put_vc1_mspel_mc##X##Y##_neon(dst+8, src+8, stride, rnd); }
DECL_PUT(1, 0)
DECL_PUT(2, 0)
DECL_PUT(3, 0)
DECL_PUT(0, 1)
...
int h, int x, int y);
#define FN_ASSIGN(X, Y) dsp->put_vc1_mspel_pixels_tab[0][X+4*Y] = ff_put_vc1_mspel_mc##X##Y##_16_neon; dsp->put_vc1_mspel_pixels_tab[1][X+4*Y] = ff_put_vc1_mspel_mc##X##Y##_neon
