...
#define FFAT_ENC_CLASS(NAME) static const AVClass ffat_##NAME##_enc_class = { .class_name = "at_" #NAME "_enc", .item_name = av_default_item_name, .option = options, .version = LIBAVUTIL_VERSION_INT, };
...
#define FFAT_ENC(NAME, ID, PROFILES, ...) FFAT_ENC_CLASS(NAME) AVCodec ff_##NAME##_at_encoder = { .name = #NAME "_at", .long_name = NULL_IF_CONFIG_SMALL(#NAME " (AudioToolbox)"), .type = AVMEDIA_TYPE_AUDIO, .id = ID, .priv_data_size = sizeof(ATDecodeContext), .init = ffat_init_encoder, .close = ffat_close_encoder, .encode2 = ffat_encode, .flush = ffat_encode_flush, .priv_class = &ffat_##NAME##_enc_class, .capabilities = AV_CODEC_CAP_DR1 | AV_CODEC_CAP_DELAY __VA_ARGS__, .sample_fmts = (const enum AVSampleFormat[]) { AV_SAMPLE_FMT_S16, AV_SAMPLE_FMT_U8, AV_SAMPLE_FMT_NONE }, .caps_internal = FF_CODEC_CAP_INIT_THREADSAFE, .profiles = PROFILES, .wrapper_name = "at", };
