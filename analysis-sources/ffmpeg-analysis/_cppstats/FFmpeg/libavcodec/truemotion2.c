...
#define TM2_INIT_POINTERS_2() unsigned *Yo, *Uo, *Vo;int oYstride, oUstride, oVstride;TM2_INIT_POINTERS();oYstride = Ystride;oVstride = Vstride;oUstride = Ustride;Yo = (ctx->cur?ctx->Y1:ctx->Y2) + by * 4 * oYstride + bx * 4;Vo = (ctx->cur?ctx->V1:ctx->V2) + by * 2 * oVstride + bx * 2;Uo = (ctx->cur?ctx->U1:ctx->U2) + by * 2 * oUstride + bx * 2;
#define TM2_RECALC_BLOCK(CHR, stride, last, CD) {CD[0] = (unsigned)CHR[ 1] - (unsigned)last[1];CD[1] = (unsigned)CHR[stride + 1] - (unsigned) CHR[1];last[0] = (int)CHR[stride + 0];last[1] = (int)CHR[stride + 1];}
