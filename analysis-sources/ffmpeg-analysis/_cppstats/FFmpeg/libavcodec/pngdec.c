...
#define UNROLL1(bpp, op) { r = dst[0]; if (bpp >= 2) g = dst[1]; if (bpp >= 3) b = dst[2]; if (bpp >= 4) a = dst[3]; for (; i <= size - bpp; i += bpp) { dst[i + 0] = r = op(r, src[i + 0], last[i + 0]); if (bpp == 1) continue; dst[i + 1] = g = op(g, src[i + 1], last[i + 1]); if (bpp == 2) continue; dst[i + 2] = b = op(b, src[i + 2], last[i + 2]); if (bpp == 3) continue; dst[i + 3] = a = op(a, src[i + 3], last[i + 3]); } }
#define UNROLL_FILTER(op) if (bpp == 1) { UNROLL1(1, op) } else if (bpp == 2) { UNROLL1(2, op) } else if (bpp == 3) { UNROLL1(3, op) } else if (bpp == 4) { UNROLL1(4, op) } for (; i < size; i++) { dst[i] = op(dst[i - bpp], src[i], last[i]); }
...
#define YUV2RGB(NAME, TYPE) static void deloco_ ##NAME(TYPE *dst, int size, int alpha) { int i; for (i = 0; i < size; i += 3 + alpha) { int g = dst [i + 1]; dst[i + 0] += g; dst[i + 2] += g; } }
