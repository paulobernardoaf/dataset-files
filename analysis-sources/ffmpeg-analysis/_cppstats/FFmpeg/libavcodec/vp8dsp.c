...
#include "mathops.h"
#include "vp8dsp.h"
#define MK_IDCT_DC_ADD4_C(name) static void name ##_idct_dc_add4uv_c(uint8_t *dst, int16_t block[4][16], ptrdiff_t stride) { name ##_idct_dc_add_c(dst + stride * 0 + 0, block[0], stride); name ##_idct_dc_add_c(dst + stride * 0 + 4, block[1], stride); name ##_idct_dc_add_c(dst + stride * 4 + 0, block[2], stride); name ##_idct_dc_add_c(dst + stride * 4 + 4, block[3], stride); } static void name ##_idct_dc_add4y_c(uint8_t *dst, int16_t block[4][16], ptrdiff_t stride) { name ##_idct_dc_add_c(dst + 0, block[0], stride); name ##_idct_dc_add_c(dst + 4, block[1], stride); name ##_idct_dc_add_c(dst + 8, block[2], stride); name ##_idct_dc_add_c(dst + 12, block[3], stride); }
...
#endif 
#define LOAD_PIXELS int av_unused p3 = p[-4 * stride]; int av_unused p2 = p[-3 * stride]; int av_unused p1 = p[-2 * stride]; int av_unused p0 = p[-1 * stride]; int av_unused q0 = p[ 0 * stride]; int av_unused q1 = p[ 1 * stride]; int av_unused q2 = p[ 2 * stride]; int av_unused q3 = p[ 3 * stride];
...
#define NORMAL_LIMIT(vpn) static av_always_inline int vp ##vpn ##_normal_limit(uint8_t *p, ptrdiff_t stride, int E, int I) { LOAD_PIXELS return vp ##vpn ##_simple_limit(p, stride, E) && FFABS(p3 - p2) <= I && FFABS(p2 - p1) <= I && FFABS(p1 - p0) <= I && FFABS(q3 - q2) <= I && FFABS(q2 - q1) <= I && FFABS(q1 - q0) <= I; }
NORMAL_LIMIT(7)
NORMAL_LIMIT(8)
...
#define LOOP_FILTER(vpn, dir, size, stridea, strideb, maybe_inline) static maybe_inline void vpn ##_ ##dir ##_loop_filter ##size ##_c(uint8_t *dst, ptrdiff_t stride, int flim_E, int flim_I, int hev_thresh) { int i; for (i = 0; i < size; i++) if (vpn ##_normal_limit(dst + i * stridea, strideb, flim_E, flim_I)) { if (hev(dst + i * stridea, strideb, hev_thresh)) vpn ##_filter_common(dst + i * stridea, strideb, 1); else filter_mbedge(dst + i * stridea, strideb); } } static maybe_inline void vpn ##_ ##dir ##_loop_filter ##size ##_inner_c(uint8_t *dst, ptrdiff_t stride, int flim_E, int flim_I, int hev_thresh) { int i; for (i = 0; i < size; i++) if (vpn ##_normal_limit(dst + i * stridea, strideb, flim_E, flim_I)) { int hv = hev(dst + i * stridea, strideb, hev_thresh); if (hv) vpn ##_filter_common(dst + i * stridea, strideb, 1); else vpn ##_filter_common(dst + i * stridea, strideb, 0); } }
#define UV_LOOP_FILTER(vpn, dir, stridea, strideb) LOOP_FILTER(vpn, dir, 8, stridea, strideb, av_always_inline) static void vpn ##_ ##dir ##_loop_filter8uv_c(uint8_t *dstU, uint8_t *dstV, ptrdiff_t stride, int fE, int fI, int hev_thresh) { vpn ##_ ##dir ##_loop_filter8_c(dstU, stride, fE, fI, hev_thresh); vpn ##_ ##dir ##_loop_filter8_c(dstV, stride, fE, fI, hev_thresh); } static void vpn ##_ ##dir ##_loop_filter8uv_inner_c(uint8_t *dstU, uint8_t *dstV, ptrdiff_t stride, int fE, int fI, int hev_thresh) { vpn ##_ ##dir ##_loop_filter8_inner_c(dstU, stride, fE, fI, hev_thresh); vpn ##_ ##dir ##_loop_filter8_inner_c(dstV, stride, fE, fI, hev_thresh); }
#define LOOP_FILTER_SIMPLE(vpn) static void vpn ##_v_loop_filter_simple_c(uint8_t *dst, ptrdiff_t stride, int flim) { int i; for (i = 0; i < 16; i++) if (vpn ##_simple_limit(dst + i, stride, flim)) vpn ##_filter_common(dst + i, stride, 1); } static void vpn ##_h_loop_filter_simple_c(uint8_t *dst, ptrdiff_t stride, int flim) { int i; for (i = 0; i < 16; i++) if (vpn ##_simple_limit(dst + i * stride, 1, flim)) vpn ##_filter_common(dst + i * stride, 1, 1); }
#define LOOP_FILTERS(vpn) LOOP_FILTER(vpn, v, 16, 1, stride, ) LOOP_FILTER(vpn, h, 16, stride, 1, ) UV_LOOP_FILTER(vpn, v, 1, stride) UV_LOOP_FILTER(vpn, h, stride, 1) LOOP_FILTER_SIMPLE(vpn) 
...
#define PUT_PIXELS(WIDTH) static void put_vp8_pixels ##WIDTH ##_c(uint8_t *dst, ptrdiff_t dststride, uint8_t *src, ptrdiff_t srcstride, int h, int x, int y) { int i; for (i = 0; i < h; i++, dst += dststride, src += srcstride) memcpy(dst, src, WIDTH); }
PUT_PIXELS(16)
PUT_PIXELS(8)
PUT_PIXELS(4)
