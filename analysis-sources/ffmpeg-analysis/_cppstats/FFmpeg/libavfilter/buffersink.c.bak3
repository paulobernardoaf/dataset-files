...
#define MAKE_AVFILTERLINK_ACCESSOR(type, field) type av_buffersink_get_##field(const AVFilterContext *ctx) { av_assert0(ctx->filter->activate == activate); return ctx->inputs[0]->field; }
...
#define CHECK_LIST_SIZE(field) if (buf->field ##_size % sizeof(*buf->field)) { av_log(ctx, AV_LOG_ERROR, "Invalid size for " #field ": %d, " "should be multiple of %d\n", buf->field ##_size, (int)sizeof(*buf->field)); return AVERROR(EINVAL); }
