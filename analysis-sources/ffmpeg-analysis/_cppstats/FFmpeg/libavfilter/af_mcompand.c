...
#define _ out_low += p->coefs[j] * p->previous[ch][p->pos + j].in - p->coefs[2*N+2 + j] * p->previous[ch][p->pos + j].out_low, j++;
{
int j = 1;
...
#undef _
#define _ out_high += p->coefs[j+N+1] * p->previous[ch][p->pos + j].in - p->coefs[2*N+2 + j] * p->previous[ch][p->pos + j].out_high, j++;
{
int j = 1;
