...
#define DECLARE_CMP_FUNC(name, pos) static int cmp_##name(const void *pa, const void *pb) { const struct color_ref * const *a = pa; const struct color_ref * const *b = pb; return ((*a)->color >> (8 * (2 - (pos))) & 0xff) - ((*b)->color >> (8 * (2 - (pos))) & 0xff); }
