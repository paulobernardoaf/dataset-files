...
{
int y, x;
#define STORE(pos) do { temp = ((src[x + y*src_linesize + pos] << log2_scale) + d[pos]) >> 6; if (temp & 0x100) temp = ~(temp >> 31); dst[x + y*dst_linesize + pos] = temp; } while (0)
for (y = 0; y < height; y++) {
...
#define STORE16(pos) do { temp = ((src[x + y*src_linesize + pos] << log2_scale) + (d[pos]>>1)) >> 5; if (temp & mask ) temp = ~(temp >> 31); dst[x + y*dst_linesize + pos] = temp; } while (0)
for (y = 0; y < height; y++) {
const uint8_t *d = dither[y];
for (x = 0; x < width; x += 8) {
int temp;
