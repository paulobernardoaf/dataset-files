...
#define ADD_PRED(preds, px, py)do {preds.mvs[preds.nb][0] = px;preds.mvs[preds.nb][1] = py;preds.nb++;} while(0)
...
#define ADD_PIXELS(b_weight, mv_x, mv_y)do {if (!b_weight || pixel_refs->nb + 1 >= NB_PIXEL_MVS)continue;pixel_refs->refs[pixel_refs->nb] = 1;pixel_weights->weights[pixel_refs->nb] = b_weight * (ALPHA_MAX - alpha);pixel_mvs->mvs[pixel_refs->nb][0] = av_clip((mv_x * alpha) / ALPHA_MAX, x_min, x_max);pixel_mvs->mvs[pixel_refs->nb][1] = av_clip((mv_y * alpha) / ALPHA_MAX, y_min, y_max);pixel_refs->nb++;pixel_refs->refs[pixel_refs->nb] = 2;pixel_weights->weights[pixel_refs->nb] = b_weight * alpha;pixel_mvs->mvs[pixel_refs->nb][0] = av_clip(-mv_x * (ALPHA_MAX - alpha) / ALPHA_MAX, x_min, x_max);pixel_mvs->mvs[pixel_refs->nb][1] = av_clip(-mv_y * (ALPHA_MAX - alpha) / ALPHA_MAX, y_min, y_max);pixel_refs->nb++;} while(0)
