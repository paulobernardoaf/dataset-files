...
#define FONTCOLOR "st(0, (midi(f)-59.5)/12);" "st(1, if(between(ld(0),0,1), 0.5-0.5*cos(2*PI*ld(0)), 0));" "r(1-ld(1)) + b(ld(1))"
...
#define DRAW_BAR_WITH_CHROMA(x) do { if (h[x] <= ht) { *lpy++ = 16; *lpu++ = 128; *lpv++ = 128; } else { mul = (h[x] - ht) * rcp_h[x]; mul = (mul < bar_t) ? (mul * rcp_bar_t) : 1.0f; *lpy++ = lrintf(mul * c[x].yuv.y + 16.0f); *lpu++ = lrintf(mul * c[x].yuv.u + 128.0f); *lpv++ = lrintf(mul * c[x].yuv.v + 128.0f); } } while (0)
#define DRAW_BAR_WITHOUT_CHROMA(x) do { if (h[x] <= ht) { *lpy++ = 16; } else { mul = (h[x] - ht) * rcp_h[x]; mul = (mul < bar_t) ? (mul * rcp_bar_t) : 1.0f; *lpy++ = lrintf(mul * c[x].yuv.y + 16.0f); } } while (0)
...
#define UPDATE_TIME(t) cur_time = av_gettime(); t += cur_time - last_time; last_time = cur_time
