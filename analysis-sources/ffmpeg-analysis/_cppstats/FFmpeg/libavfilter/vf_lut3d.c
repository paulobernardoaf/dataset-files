...
#define NEXT_LINE(loop_cond) do { if (!fgets(line, sizeof(line), f)) { av_log(ctx, AV_LOG_ERROR, "Unexpected EOF\n"); return AVERROR_INVALIDDATA; } } while (loop_cond)
...
const char *p = line + 6;
#define SET_COLOR(id) do { while (av_isspace(*p)) p++; switch (*p) { case 'r': rgb_map[id] = 0; break; case 'g': rgb_map[id] = 1; break; case 'b': rgb_map[id] = 2; break; } while (*p && !av_isspace(*p)) p++; } while (0)
SET_COLOR(0);
SET_COLOR(1);
SET_COLOR(2);
break;
...
#define SET_FUNC(name) do { if (planar) { switch (depth) { case 8: lut3d->interp = interp_8_##name##_p8; break; case 9: lut3d->interp = interp_16_##name##_p9; break; case 10: lut3d->interp = interp_16_##name##_p10; break; case 12: lut3d->interp = interp_16_##name##_p12; break; case 14: lut3d->interp = interp_16_##name##_p14; break; case 16: lut3d->interp = interp_16_##name##_p16; break; } } else if (is16bit) { lut3d->interp = interp_16_##name; } else { lut3d->interp = interp_8_##name; } } while (0)
...
#define SET_FUNC_1D(name) do { if (planar) { switch (depth) { case 8: lut1d->interp = interp_1d_8_##name##_p8; break; case 9: lut1d->interp = interp_1d_16_##name##_p9; break; case 10: lut1d->interp = interp_1d_16_##name##_p10; break; case 12: lut1d->interp = interp_1d_16_##name##_p12; break; case 14: lut1d->interp = interp_1d_16_##name##_p14; break; case 16: lut1d->interp = interp_1d_16_##name##_p16; break; } } else if (is16bit) { lut1d->interp = interp_1d_16_##name; } else { lut1d->interp = interp_1d_8_##name; } } while (0)
