...
#define DERIVATIVE(name, type) static void aderivative_##name ##p(void **d, void **p, const void **s, int nb_samples, int channels) { int n, c; for (c = 0; c < channels; c++) { const type *src = s[c]; type *dst = d[c]; type *prv = p[c]; for (n = 0; n < nb_samples; n++) { const type current = src[n]; dst[n] = current - prv[0]; prv[0] = current; } } }
...
#define INTEGRAL(name, type) static void aintegral_##name ##p(void **d, void **p, const void **s, int nb_samples, int channels) { int n, c; for (c = 0; c < channels; c++) { const type *src = s[c]; type *dst = d[c]; type *prv = p[c]; for (n = 0; n < nb_samples; n++) { const type current = src[n]; dst[n] = current + prv[0]; prv[0] = dst[n]; } } }
