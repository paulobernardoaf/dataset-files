...
#define LOAD_CLUT(nbits) do { uint##nbits##_t *dst = ((uint##nbits##_t *)(data + y*linesize)) + x*step; dst[rgba_map[0]] = av_clip_uint##nbits(i * scale); dst[rgba_map[1]] = av_clip_uint##nbits(j * scale); dst[rgba_map[2]] = av_clip_uint##nbits(k * scale); if (step == 4) dst[rgba_map[3]] = alpha; } while (0)
level *= level;
for (k = 0; k < level; k++) {
for (j = 0; j < level; j++) {
for (i = 0; i < level; i++) {
