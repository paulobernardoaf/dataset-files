...
#define LOAD_PACKED_COMMONLutContext *s = ctx->priv;const struct thread_data *td = arg;int i, j;const int w = td->w;const int h = td->h;AVFrame *in = td->in;AVFrame *out = td->out;const uint16_t (*tab)[256*256] = (const uint16_t (*)[256*256])s->lut;const int step = s->step;const int slice_start = (h * jobnr ) / nb_jobs;const int slice_end = (h * (jobnr+1)) / nb_jobs;
...
#define LOAD_PLANAR_COMMONLutContext *s = ctx->priv;const struct thread_data *td = arg;int i, j, plane;AVFrame *in = td->in;AVFrame *out = td->out;
#define PLANAR_COMMONint vsub = plane == 1 || plane == 2 ? s->vsub : 0;int hsub = plane == 1 || plane == 2 ? s->hsub : 0;int h = AV_CEIL_RSHIFT(td->h, vsub);int w = AV_CEIL_RSHIFT(td->w, hsub);const uint16_t *tab = s->lut[plane];const int slice_start = (h * jobnr ) / nb_jobs;const int slice_end = (h * (jobnr+1)) / nb_jobs;
...
#define PACKED_THREAD_DATAstruct thread_data td = {.in = in,.out = out,.w = inlink->w,.h = in->height,};
#define PLANAR_THREAD_DATAstruct thread_data td = {.in = in,.out = out,.w = inlink->w,.h = inlink->h,};
