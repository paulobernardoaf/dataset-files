...
#define FILTER(xm2, xm1, xp1, xp2) abs( 4 * srcp[x] -3 * (srcp[x + (xm1)*src_linesize] + srcp[x + (xp1)*src_linesize]) + (srcp[x + (xm2)*src_linesize] + srcp[x + (xp2)*src_linesize])) > cthresh6
for (x = 0; x < width; x++) {
...
for (x = 1; x < width - 1; x++) {
#define HAS_FF_AROUND(p, lz) (p[(x)-1 - (lz)] == 0xff || p[(x) - (lz)] == 0xff || p[(x)+1 - (lz)] == 0xff || p[(x)-1 ] == 0xff || p[(x)+1 ] == 0xff || p[(x)-1 + (lz)] == 0xff || p[(x) + (lz)] == 0xff || p[(x)+1 + (lz)] == 0xff)
...
#define C_ARRAY_ADD(v) do { const int box1 = (x / blockx) * 4; const int box2 = ((x + xhalf) / blockx) * 4; c_array[temp1 + box1 ] += v; c_array[temp1 + box2 + 1] += v; c_array[temp2 + box1 + 2] += v; c_array[temp2 + box2 + 3] += v; } while (0)
#define VERTICAL_HALF(y_start, y_end) do { for (y = y_start; y < y_end; y++) { const int temp1 = (y / blocky) * xblocks4; const int temp2 = ((y + yhalf) / blocky) * xblocks4; for (x = 0; x < width; x++) if (cmkp[x - cmk_linesize] == 0xff && cmkp[x ] == 0xff && cmkp[x + cmk_linesize] == 0xff) C_ARRAY_ADD(1); cmkp += cmk_linesize; } } while (0)
...
#define LOAD_COMB(mid) do { if (combs[mid] < 0) { if (!gen_frames[mid]) gen_frames[mid] = create_weave_frame(ctx, mid, field, fm->prv, fm->src, fm->nxt); combs[mid] = calc_combed_score(fm, gen_frames[mid]); } } while (0)
LOAD_COMB(m1);
LOAD_COMB(m2);
...
AVFrame *dst;
#define SLIDING_FRAME_WINDOW(prv, src, nxt) do { if (prv != src) av_frame_free(&prv); prv = src; src = nxt; if (in) nxt = in; if (!prv) prv = src; if (!prv) return 0; av_assert0(prv && src && nxt); } while (0)
