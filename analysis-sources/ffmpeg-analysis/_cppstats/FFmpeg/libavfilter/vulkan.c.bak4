...
#define FN_CREATING(ctx, type, shortname, array, num) static av_always_inline type *create_ ##shortname(ctx *dctx) { type **array, *sctx = av_mallocz(sizeof(*sctx)); if (!sctx) return NULL; array = av_realloc_array(dctx->array, sizeof(*dctx->array), dctx->num + 1);if (!array) { av_free(sctx); return NULL; } dctx->array = array; dctx->array[dctx->num++] = sctx; return sctx; }
