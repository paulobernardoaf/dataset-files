...
#define denoise(...) do { int ret = AVERROR_BUG; switch (s->depth) { case 8: ret = denoise_depth(__VA_ARGS__, 8); break; case 9: ret = denoise_depth(__VA_ARGS__, 9); break; case 10: ret = denoise_depth(__VA_ARGS__, 10); break; case 12: ret = denoise_depth(__VA_ARGS__, 12); break; case 14: ret = denoise_depth(__VA_ARGS__, 14); break; case 16: ret = denoise_depth(__VA_ARGS__, 16); break; } if (ret < 0) { av_frame_free(&out); if (!direct) av_frame_free(&in); return ret; } } while (0)
