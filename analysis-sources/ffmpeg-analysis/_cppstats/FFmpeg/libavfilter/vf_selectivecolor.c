...
#define DECLARE_RANGE_SCALE_FUNCS(nbits) static int get_neutrals_scale##nbits(int r, int g, int b, int min_val, int max_val) { return (((1<<nbits)-1)*2 - ( abs((max_val<<1) - ((1<<nbits)-1)) + abs((min_val<<1) - ((1<<nbits)-1))) + 1) >> 1; } static int get_whites_scale##nbits(int r, int g, int b, int min_val, int max_val) { return (min_val<<1) - ((1<<nbits)-1); } static int get_blacks_scale##nbits(int r, int g, int b, int min_val, int max_val) { return ((1<<nbits)-1) - (max_val<<1); } 
...
if (ret < 0)
return ret;
#define READ16(dst) do { if (size < 2) { ret = AVERROR_INVALIDDATA; goto end; } dst = AV_RB16(buf); buf += 2; size -= 2; } while (0)
READ16(version);
if (version != 1)
...
#define DEF_SELECTIVE_COLOR_FUNC(name, direct, correction_method, nbits) static int selective_color_##name##_##nbits(AVFilterContext *ctx, void *arg, int jobnr, int nb_jobs) { return selective_color_##nbits(ctx, arg, jobnr, nb_jobs, direct, correction_method); }
