...
int ydest = y / hblocky;
int xdest = 0;
#define CALC_DIFF(nbits) do { for (x = 0; x < width; x += hblockx) { int64_t acc = 0; int m = FFMIN(width, x + hblockx); for (xl = x; xl < m; xl++) acc += abs(((const uint##nbits##_t *)f1p)[xl] - ((const uint##nbits##_t *)f2p)[xl]); bdiffs[ydest * dm->nxblocks + xdest] += acc; xdest++; } } while (0)
