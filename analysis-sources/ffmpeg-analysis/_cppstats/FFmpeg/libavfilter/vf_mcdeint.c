...
#define GET_SCORE_EDGE(j)FFABS(srcp[-srcs+DELTA(-1+(j))] - srcp[+srcs+DELTA(-1-(j))])+FFABS(srcp[-srcs+DELTA(j) ] - srcp[+srcs+DELTA( -(j))])+FFABS(srcp[-srcs+DELTA(1+(j)) ] - srcp[+srcs+DELTA( 1-(j))])
#define GET_SCORE(j)FFABS(srcp[-srcs-1+(j)] - srcp[+srcs-1-(j)])+FFABS(srcp[-srcs +(j)] - srcp[+srcs -(j)])+FFABS(srcp[-srcs+1+(j)] - srcp[+srcs+1-(j)])
#define CHECK_EDGE(j){ int score = GET_SCORE_EDGE(j);if (score < spatial_score){spatial_score = score;diff0 = filp[-fils+DELTA(j)] - srcp[-srcs+DELTA(j)];diff1 = filp[+fils+DELTA(-(j))] - srcp[+srcs+DELTA(-(j))];
#define CHECK(j){ int score = GET_SCORE(j);if (score < spatial_score){spatial_score= score;diff0 = filp[-fils+(j)] - srcp[-srcs+(j)];diff1 = filp[+fils-(j)] - srcp[+srcs-(j)];
if (is_edge) {
