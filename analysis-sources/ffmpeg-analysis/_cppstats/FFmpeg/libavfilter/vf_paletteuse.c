...
#define COLORMAP_NEAREST(search, palette, root, target, trans_thresh) search == COLOR_SEARCH_NNS_ITERATIVE ? colormap_nearest_iterative(root, target, trans_thresh) : search == COLOR_SEARCH_NNS_RECURSIVE ? colormap_nearest_recursive(root, target, trans_thresh) : colormap_nearest_bruteforce(palette, target, trans_thresh)
...
#define DECLARE_CMP_FUNC(name, pos) static int cmp_##name(const void *pa, const void *pb) { const struct color *a = pa; const struct color *b = pb; return (a->value >> (8 * (3 - (pos))) & 0xff) - (b->value >> (8 * (3 - (pos))) & 0xff); }
...
#define DEFINE_SET_FRAME(color_search, name, value) static int set_frame_##name(PaletteUseContext *s, AVFrame *out, AVFrame *in, int x_start, int y_start, int w, int h) { return set_frame(s, out, in, x_start, y_start, w, h, value, color_search); }
...
#define DITHERING_ENTRIES(color_search) { set_frame_##color_search##_none, set_frame_##color_search##_bayer, set_frame_##color_search##_heckbert, set_frame_##color_search##_floyd_steinberg, set_frame_##color_search##_sierra2, set_frame_##color_search##_sierra2_4a, }
