...
#define CHECK_ERROR(condition, errorcode, goto_point) if ((condition)) { errcode = (errorcode); goto goto_point; }
...
#define FF_EBUR128_ADD_FRAMES_PLANAR(type) void ff_ebur128_add_frames_planar_##type(FFEBUR128State* st, const type** srcs, size_t frames, int stride) { size_t src_index = 0; while (frames > 0) { if (frames >= st->d->needed_frames) { ebur128_filter_##type(st, srcs, src_index, st->d->needed_frames, stride); src_index += st->d->needed_frames * stride; frames -= st->d->needed_frames; st->d->audio_data_index += st->d->needed_frames * st->channels; if ((st->mode & FF_EBUR128_MODE_I) == FF_EBUR128_MODE_I) { ebur128_calc_gating_block(st, st->d->samples_in_100ms * 4, NULL); } if ((st->mode & FF_EBUR128_MODE_LRA) == FF_EBUR128_MODE_LRA) { st->d->short_term_frame_counter += st->d->needed_frames; if (st->d->short_term_frame_counter == st->d->samples_in_100ms * 30) { double st_energy; ebur128_energy_shortterm(st, &st_energy); if (st_energy >= histogram_energy_boundaries[0]) { ++st->d->short_term_block_energy_histogram[ find_histogram_index(st_energy)]; } st->d->short_term_frame_counter = st->d->samples_in_100ms * 20; } } st->d->needed_frames = st->d->samples_in_100ms; if (st->d->audio_data_index == st->d->audio_data_frames * st->channels) { st->d->audio_data_index = 0; } } else { ebur128_filter_##type(st, srcs, src_index, frames, stride); st->d->audio_data_index += frames * st->channels; if ((st->mode & FF_EBUR128_MODE_LRA) == FF_EBUR128_MODE_LRA) { st->d->short_term_frame_counter += frames; } st->d->needed_frames -= frames; frames = 0; } } }
...
#define FF_EBUR128_ADD_FRAMES(type) void ff_ebur128_add_frames_##type(FFEBUR128State* st, const type* src, size_t frames) { int i; const type **buf = (const type**)st->d->data_ptrs; for (i = 0; i < st->channels; i++) buf[i] = src + i; ff_ebur128_add_frames_planar_##type(st, buf, frames, st->channels); }
