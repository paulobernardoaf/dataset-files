...
#define MERGE_DISPATCH(field, statement) if (!(link->in_ ##field && link->out_ ##field)) { count_delayed++; } else if (link->in_ ##field == link->out_ ##field) { count_already_merged++; } else if (!convert_needed) { count_merged++; statement }
...
#define REDUCE_FORMATS(fmt_type, list_type, list, var, nb, add_format, unref_format) do { for (i = 0; i < filter->nb_inputs; i++) { AVFilterLink *link = filter->inputs[i]; fmt_type fmt; if (!link->out_ ##list || link->out_ ##list->nb != 1) continue; fmt = link->out_ ##list->var[0]; for (j = 0; j < filter->nb_outputs; j++) { AVFilterLink *out_link = filter->outputs[j]; list_type *fmts; if (link->type != out_link->type || out_link->in_ ##list->nb == 1) continue; fmts = out_link->in_ ##list; if (!out_link->in_ ##list->nb) { if ((ret = add_format(&out_link->in_ ##list, fmt)) < 0)return ret; ret = 1; break; } for (k = 0; k < out_link->in_ ##list->nb; k++) if (fmts->var[k] == fmt) { fmts->var[0] = fmt; fmts->nb = 1; ret = 1; break; } } } } while (0)
