...
#define DECLARE_INTERPOLATE_FUNC(nbits) static int interpolate##nbits(void *log_ctx, uint16_t *y, const struct keypoint *points) { return interpolate(log_ctx, y, points, nbits); }
...
#define SET_COMP_IF_NOT_SET(n, name) do { if (!pts[n] && curves_presets[curves->preset].name) { pts[n] = av_strdup(curves_presets[curves->preset].name); if (!pts[n]) return AVERROR(ENOMEM); } } while (0)
