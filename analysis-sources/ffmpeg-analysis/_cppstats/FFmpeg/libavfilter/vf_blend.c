...
#define COPY(src, depth) static void blend_copy ##src##_##depth(const uint8_t *top, ptrdiff_t top_linesize, const uint8_t *bottom, ptrdiff_t bottom_linesize,uint8_t *dst, ptrdiff_t dst_linesize, ptrdiff_t width, ptrdiff_t height, FilterParams *param, double *values, int starty) { av_image_copy_plane(dst, dst_linesize, src, src ##_linesize, width * depth / 8, height); }
COPY(top, 8)
COPY(bottom, 8)
COPY(top, 16)
COPY(bottom, 16)
...
#define DEFINE_BLEND8(name, expr) static void blend_##name##_8bit(const uint8_t *top, ptrdiff_t top_linesize, const uint8_t *bottom, ptrdiff_t bottom_linesize, uint8_t *dst, ptrdiff_t dst_linesize, ptrdiff_t width, ptrdiff_t height, FilterParams *param, double *values, int starty) { double opacity = param->opacity; int i, j; for (i = 0; i < height; i++) { for (j = 0; j < width; j++) { dst[j] = top[j] + ((expr) - top[j]) * opacity; } dst += dst_linesize; top += top_linesize; bottom += bottom_linesize; } }
#define DEFINE_BLEND16(name, expr, depth) static void blend_##name##_##depth##bit(const uint8_t *_top, ptrdiff_t top_linesize,const uint8_t *_bottom, ptrdiff_t bottom_linesize, uint8_t *_dst, ptrdiff_t dst_linesize, ptrdiff_t width, ptrdiff_t height, FilterParams *param, double *values, int starty) { const uint16_t *top = (const uint16_t*)_top; const uint16_t *bottom = (const uint16_t*)_bottom; uint16_t *dst = (uint16_t*)_dst; double opacity = param->opacity; int i, j; dst_linesize /= 2; top_linesize /= 2; bottom_linesize /= 2; for (i = 0; i < height; i++) { for (j = 0; j < width; j++) { dst[j] = top[j] + ((expr) - top[j]) * opacity; } dst += dst_linesize; top += top_linesize; bottom += bottom_linesize; } }
#define A top[j]
#define B bottom[j]
...
#define DEFINE_BLEND_EXPR(type, name, div) static void blend_expr_##name(const uint8_t *_top, ptrdiff_t top_linesize, const uint8_t *_bottom, ptrdiff_t bottom_linesize, uint8_t *_dst, ptrdiff_t dst_linesize, ptrdiff_t width, ptrdiff_t height, FilterParams *param, double *values, int starty) { const type *top = (type*)_top; const type *bottom = (type*)_bottom; type *dst = (type*)_dst; AVExpr *e = param->e; int y, x; dst_linesize /= div; top_linesize /= div; bottom_linesize /= div; for (y = 0; y < height; y++) { values[VAR_Y] = y + starty; for (x = 0; x < width; x++) { values[VAR_X] = x; values[VAR_TOP] = values[VAR_A] = top[x]; values[VAR_BOTTOM] = values[VAR_B] = bottom[x]; dst[x] = av_expr_eval(e, values, NULL); } dst += dst_linesize; top += top_linesize; bottom += bottom_linesize; } }
