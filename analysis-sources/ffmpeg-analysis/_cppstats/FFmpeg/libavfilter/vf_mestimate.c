...
#define SEARCH_MV(method)do {for (mb_y = 0; mb_y < s->b_height; mb_y++)for (mb_x = 0; mb_x < s->b_width; mb_x++) {const int x_mb = mb_x << s->log2_mb_size;const int y_mb = mb_y << s->log2_mb_size;int mv[2] = {x_mb, y_mb};ff_me_search_##method(me_ctx, x_mb, y_mb, mv);add_mv_data(((AVMotionVector *) sd->data) + mv_count++, me_ctx->mb_size, x_mb, y_mb, mv[0], mv[1], dir);}} while (0)
