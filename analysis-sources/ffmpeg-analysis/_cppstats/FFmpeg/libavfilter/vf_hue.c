...
#define SET_EXPR(expr, option) if (hue->expr##_expr) do { ret = set_expr(&hue->expr##_pexpr, &hue->expr##_expr, hue->expr##_expr, option, ctx); if (ret < 0) return ret; } while (0)
...
HueContext *hue = ctx->priv;
int ret;
#define SET_EXPR(expr, option) do { ret = set_expr(&hue->expr##_pexpr, &hue->expr##_expr, args, option, ctx); if (ret < 0) return ret; } while (0)
if (!strcmp(cmd, "h")) {
SET_EXPR(hue_deg, "h");
