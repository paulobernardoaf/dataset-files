...
if (y - 1 < 0 || y + 1 >= h)
continue;
#define FILTER(i, j) filter_tout_outlier(p[(y-j) * lw + x + i], p[ y * lw + x + i], p[(y+j) * lw + x + i])
...
av_frame_free(&s->frame_prev);
s->frame_prev = av_frame_clone(in);
#define SET_META(key, fmt, val) do { snprintf(metabuf, sizeof(metabuf), fmt, val); av_dict_set(&out->metadata, "lavfi.signalstats." key, metabuf, 0); } while (0)
