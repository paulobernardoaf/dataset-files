...
#define COST_MV(x, y)do {cost = me_ctx->get_cost(me_ctx, x_mb, y_mb, x, y);if (cost < cost_min) {cost_min = cost;mv[0] = x;mv[1] = y;}} while(0)
#define COST_P_MV(x, y)if (x >= x_min && x <= x_max && y >= y_min && y <= y_max)COST_MV(x, y);
