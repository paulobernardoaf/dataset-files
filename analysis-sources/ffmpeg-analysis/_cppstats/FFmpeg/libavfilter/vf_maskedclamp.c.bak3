...
#define MASKEDCLAMP(type, name) static void maskedclamp##name(const uint8_t *bbsrc, uint8_t *ddst, const uint8_t *ddarksrc, const uint8_t *bbrightsrc, int w, int undershoot, int overshoot) { const type *bsrc = (const type *)bbsrc; const type *darksrc = (const type *)ddarksrc; const type *brightsrc = (const type *)bbrightsrc; type *dst = (type *)ddst; for (int x = 0; x < w; x++) { dst[x] = FFMAX(bsrc[x], darksrc[x] - undershoot); dst[x] = FFMIN(dst[x], brightsrc[x] + overshoot); } }
