...
{ 1, 7}, { 9, 7}, { 1,15}, { 9,15}, { 5, 3}, {13, 3}, { 5,11}, {13,11}, 
...
{
int y, x;
#define STORE(pos) do { temp = ((src[x + y * src_stride + pos] << log2_scale) + d[pos]) >> 8; if (temp & 0x100) temp = ~(temp >> 31); dst[x + y * dst_stride + pos] = temp; } while (0)
for (y = 0; y < height; y++) {
