...
#define SILENCE_DETECT(name, type) static void silencedetect_##name(SilenceDetectContext *s, AVFrame *insamples, int nb_samples, int64_t nb_samples_notify, AVRational time_base) { const type *p = (const type *)insamples->data[0]; const type noise = s->noise; int i; for (i = 0; i < nb_samples; i++, p++) update(s, insamples, *p < noise && *p > -noise, i, nb_samples_notify, time_base); }
