...
#define ADD_FORMAT(f, fmt, unref_fn, type, list, nb) do { type *fmts; void *oldf = *f; if (!(*f) && !(*f = av_mallocz(sizeof(**f)))) { return AVERROR(ENOMEM); } fmts = av_realloc_array((*f)->list, (*f)->nb + 1, sizeof(*(*f)->list)); if (!fmts) { unref_fn(f); if (!oldf) av_freep(f); return AVERROR(ENOMEM); } (*f)->list = fmts; (*f)->list[(*f)->nb++] = fmt; } while (0)
...
#define FORMATS_REF(f, ref, unref_fn) void *tmp; if (!f || !ref) return AVERROR(ENOMEM); tmp = av_realloc_array(f->refs, sizeof(*f->refs), f->refcount + 1); if (!tmp) { unref_fn(&f); return AVERROR(ENOMEM); } f->refs = tmp; f->refs[f->refcount++] = ref; *ref = f; return 0
...
#define FIND_REF_INDEX(ref, idx) do { int i; for (i = 0; i < (*ref)->refcount; i ++) if((*ref)->refs[i] == ref) { idx = i; break; } } while (0)
...
#define FORMATS_UNREF(ref, list) do { int idx = -1; if (!ref || !*ref || !(*ref)->refs) return; FIND_REF_INDEX(ref, idx); if (idx >= 0) memmove((*ref)->refs + idx, (*ref)->refs + idx + 1, sizeof(*(*ref)->refs) * ((*ref)->refcount - idx - 1)); if(!--(*ref)->refcount) { av_free((*ref)->list); av_free((*ref)->refs); av_free(*ref); } *ref = NULL; } while (0)
...
#define FORMATS_CHANGEREF(oldref, newref) do { int idx = -1; FIND_REF_INDEX(oldref, idx); if (idx >= 0) { (*oldref)->refs[idx] = newref; *newref = *oldref; *oldref = NULL; } } while (0)
