...
#define Z_Z2_C(outr,outi,inr,ini)outr= inr*inr - ini*ini + cr;outi= 2*inr*ini + ci;



#define Z_Z2_C_ZYKLUS(outr,outi,inr,ini, Z)Z_Z2_C(outr,outi,inr,ini)if(use_zyklus){if(Z && fabs(s->zyklus[i>>1][0]-outr)+fabs(s->zyklus[i>>1][1]-outi) <= epsilon)break;}s->zyklus[i][0]= outr;s->zyklus[i][1]= outi;
