...
#define UPDATE_FACTOR do { int factor; factor = f.color_diff_coeff[COLOR_DIFF_COEFF_SIZE/2 + pre_val - f.pre_filter_buf[ix + iy*f.pre_filter_linesize]] * f.dist_coeff[dx + dy*f.dist_linesize]; sum += src[ix + iy*src_linesize] * factor; div += factor; } while (0)
