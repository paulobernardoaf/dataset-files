...
#define FREE_DENSE(name) do { if (model->name) { av_free((void *) model->name->input_weights); av_free((void *) model->name->bias); av_free((void *) model->name); } } while (0)
...
#define FREE_GRU(name) do { if (model->name) { av_free((void *) model->name->input_weights); av_free((void *) model->name->recurrent_weights); av_free((void *) model->name->bias); av_free((void *) model->name); } } while (0)
...
#define ALLOC_LAYER(type, name) name = av_calloc(1, sizeof(type)); if (!name) { rnnoise_model_free(ret); return NULL; } ret->name = name
...
#define INPUT_VAL(name) do { if (fscanf(f, "%d", &in) != 1 || in < 0 || in > 128) { rnnoise_model_free(ret); return NULL; } name = in; } while (0)
...
#define INPUT_ACTIVATION(name) do { int activation; INPUT_VAL(activation); switch (activation) { case F_ACTIVATION_SIGMOID: name = ACTIVATION_SIGMOID; break; case F_ACTIVATION_RELU: name = ACTIVATION_RELU; break; default: name = ACTIVATION_TANH; } } while (0)
...
#define INPUT_ARRAY(name, len) do { float *values = av_calloc((len), sizeof(float)); if (!values) { rnnoise_model_free(ret); return NULL; } name = values; for (int i = 0; i < (len); i++) { if (fscanf(f, "%d", &in) != 1) { rnnoise_model_free(ret); return NULL; } values[i] = in; } } while (0)
...
#define INPUT_ARRAY3(name, len0, len1, len2) do { float *values = av_calloc(FFALIGN((len0), 4) * FFALIGN((len1), 4) * (len2), sizeof(float)); if (!values) { rnnoise_model_free(ret); return NULL; } name = values; for (int k = 0; k < (len0); k++) { for (int i = 0; i < (len2); i++) { for (int j = 0; j < (len1); j++) { if (fscanf(f, "%d", &in) != 1) { rnnoise_model_free(ret); return NULL; } values[j * (len2) * FFALIGN((len0), 4) + i * FFALIGN((len0), 4) + k] = in; } } } } while (0)
...
#define INPUT_DENSE(name) do { INPUT_VAL(name->nb_inputs); INPUT_VAL(name->nb_neurons); ret->name ##_size = name->nb_neurons; INPUT_ACTIVATION(name->activation); INPUT_ARRAY(name->input_weights, name->nb_inputs * name->nb_neurons); INPUT_ARRAY(name->bias, name->nb_neurons); } while (0)
...
#define INPUT_GRU(name) do { INPUT_VAL(name->nb_inputs); INPUT_VAL(name->nb_neurons); ret->name ##_size = name->nb_neurons; INPUT_ACTIVATION(name->activation); INPUT_ARRAY3(name->input_weights, name->nb_inputs, name->nb_neurons, 3); INPUT_ARRAY3(name->recurrent_weights, name->nb_neurons, name->nb_neurons, 3); INPUT_ARRAY(name->bias, name->nb_neurons * 3); } while (0)
