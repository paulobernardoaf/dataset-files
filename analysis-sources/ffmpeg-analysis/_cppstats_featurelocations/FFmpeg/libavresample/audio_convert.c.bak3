...
#define CONV_LOOP(otype, expr) do { *(otype *)po = expr; pi += is; po += os; } while (po < end); 
...
#define CONV_FUNC_FLAT(ofmt, otype, ifmt, itype, expr) static void CONV_FUNC_NAME(ofmt, ifmt)(uint8_t *out, const uint8_t *in, int len) { int is = sizeof(itype); int os = sizeof(otype); const uint8_t *pi = in; uint8_t *po = out; uint8_t *end = out + os * len; CONV_LOOP(otype, expr) }
...
#define CONV_FUNC_INTERLEAVE(ofmt, otype, ifmt, itype, expr) static void CONV_FUNC_NAME(ofmt, ifmt)(uint8_t *out, const uint8_t **in, int len, int channels) { int ch; int out_bps = sizeof(otype); int is = sizeof(itype); int os = channels * out_bps; for (ch = 0; ch < channels; ch++) { const uint8_t *pi = in[ch]; uint8_t *po = out + ch * out_bps; uint8_t *end = po + os * len; CONV_LOOP(otype, expr) } }
...
#define CONV_FUNC_GROUP(ofmt, otype, ifmt, itype, expr) CONV_FUNC_FLAT( ofmt, otype, ifmt, itype, expr) CONV_FUNC_INTERLEAVE( ofmt, otype, ifmt ##P, itype, expr) CONV_FUNC_DEINTERLEAVE(ofmt ##P, otype, ifmt, itype, expr)
...
#define SET_CONV_FUNC_GROUP(ofmt, ifmt) ff_audio_convert_set_func(ac, ofmt, ifmt, 0, 1, 1, "C", CONV_FUNC_NAME(ofmt, ifmt)); ff_audio_convert_set_func(ac, ofmt ##P, ifmt, 0, 1, 1, "C", CONV_FUNC_NAME(ofmt ##P, ifmt)); ff_audio_convert_set_func(ac, ofmt, ifmt ##P, 0, 1, 1, "C", CONV_FUNC_NAME(ofmt, ifmt ##P));
