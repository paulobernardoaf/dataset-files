...
#define yuv2plane1_float(template, dest_type, BE_LE) static void yuv2plane1_float ##BE_LE ##_c(const int16_t *src, uint8_t *dest, int dstW, const uint8_t *dither, int offset) { template((const int32_t *)src, (dest_type *)dest, dstW); }
...
#define yuv2planeX_float(template, dest_type, BE_LE) static void yuv2planeX_float ##BE_LE ##_c(const int16_t *filter, int filterSize, const int16_t **src, uint8_t *dest, int dstW, const uint8_t *dither, int offset) { template(filter, filterSize, (const int32_t **)src, (dest_type *)dest, dstW); }
...
#define output_pixel(pos, val) if (big_endian) { AV_WB16(pos, av_clip_uintp2(val >> shift, 10) << 6); } else { AV_WL16(pos, av_clip_uintp2(val >> shift, 10) << 6); }
...
#define accumulate_bit(acc, val) acc <<= 1; acc |= (val) >= 234


#define output_pixel(pos, acc) if (target == AV_PIX_FMT_MONOBLACK) { pos = acc; } else { pos = ~acc; }
...
#define output_pixels(pos, Y1, U, Y2, V) if (target == AV_PIX_FMT_YUYV422) { dest[pos + 0] = Y1; dest[pos + 1] = U; dest[pos + 2] = Y2; dest[pos + 3] = V; } else if (target == AV_PIX_FMT_YVYU422) { dest[pos + 0] = Y1; dest[pos + 1] = V; dest[pos + 2] = Y2; dest[pos + 3] = U; } else { dest[pos + 0] = U; dest[pos + 1] = Y1; dest[pos + 2] = V; dest[pos + 3] = Y2; }
...
#define output_pixel(pos, val) if (isBE(target)) { AV_WB16(pos, val); } else { AV_WL16(pos, val); }
