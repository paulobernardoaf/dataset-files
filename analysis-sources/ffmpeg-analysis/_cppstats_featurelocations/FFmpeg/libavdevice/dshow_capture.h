...
#if !defined(AVDEVICE_DSHOW_CAPTURE_H)
...
#define DECLARE_ADDREF(class) unsigned long WINAPI class##_AddRef(class *this) { dshowdebug(AV_STRINGIFY(class)"_AddRef(%p)\t%ld\n", this, this->ref+1); return InterlockedIncrement(&this->ref); }
...
#define DECLARE_RELEASE(class) unsigned long WINAPI class##_Release(class *this) { long ref = InterlockedDecrement(&this->ref); dshowdebug(AV_STRINGIFY(class)"_Release(%p)\t%ld\n", this, ref); if (!ref) class##_Destroy(this); return ref; }
...
#define DECLARE_DESTROY(class, func) void class##_Destroy(class *this) { dshowdebug(AV_STRINGIFY(class)"_Destroy(%p)\n", this); func(this); if (this) { if (this->vtbl) CoTaskMemFree(this->vtbl); CoTaskMemFree(this); } }
