...
#define LOAD4int y0 = y[2 * i];int y1 = y[2 * i + 1];int u0 = u[i];int v0 = v[i];
...
#define LOADENDint y0 = s->temp[0][width-1];

#define LOADEND_14int y0 = s->temp16[0][width-1] & mask;

#define LOADEND_16int y0 = s->temp16[0][width-1];

#define STATENDs->stats[plane][y0]++;

#define STATEND_16s->stats[plane][y0>>2]++;

#define WRITEENDput_bits(&s->pb, s->len[plane][y0], s->bits[plane][y0]);

#define WRITEEND_16put_bits(&s->pb, s->len[plane][y0>>2], s->bits[plane][y0>>2]);put_bits(&s->pb, 2, y0&3);



#define LOAD2int y0 = s->temp[0][2 * i];int y1 = s->temp[0][2 * i + 1];


#define LOAD2_14int y0 = s->temp16[0][2 * i] & mask;int y1 = s->temp16[0][2 * i + 1] & mask;


#define LOAD2_16int y0 = s->temp16[0][2 * i];int y1 = s->temp16[0][2 * i + 1];


#define STAT2s->stats[plane][y0]++;s->stats[plane][y1]++;


#define STAT2_16s->stats[plane][y0>>2]++;s->stats[plane][y1>>2]++;


#define WRITE2put_bits(&s->pb, s->len[plane][y0], s->bits[plane][y0]);put_bits(&s->pb, s->len[plane][y1], s->bits[plane][y1]);


#define WRITE2_16put_bits(&s->pb, s->len[plane][y0>>2], s->bits[plane][y0>>2]);put_bits(&s->pb, 2, y0&3);put_bits(&s->pb, s->len[plane][y1>>2], s->bits[plane][y1>>2]);put_bits(&s->pb, 2, y1&3);
...
#define STAT2s->stats[0][y0]++;s->stats[0][y1]++;


#define WRITE2put_bits(&s->pb, s->len[0][y0], s->bits[0][y0]);put_bits(&s->pb, s->len[0][y1], s->bits[0][y1]);
...
#define LOAD_GBRA int g = s->temp[0][planes == 3 ? 3 * i + 1 : 4 * i + G]; int b =(s->temp[0][planes == 3 ? 3 * i + 2 : 4 * i + B] - g) & 0xFF;int r =(s->temp[0][planes == 3 ? 3 * i + 0 : 4 * i + R] - g) & 0xFF;int a = s->temp[0][planes * i + A];





#define STAT_BGRA s->stats[0][b]++; s->stats[1][g]++; s->stats[2][r]++; if (planes == 4) s->stats[2][a]++;
...
#define WRITE_GBRA put_bits(&s->pb, s->len[1][g], s->bits[1][g]); put_bits(&s->pb, s->len[0][b], s->bits[0][b]); put_bits(&s->pb, s->len[2][r], s->bits[2][r]); if (planes == 4) put_bits(&s->pb, s->len[2][a], s->bits[2][a]);
...
#define COMMON_OPTIONS { "non_deterministic", "Allow multithreading for e.g. context=1 at the expense of determinism", OFFSET(non_determ), AV_OPT_TYPE_BOOL, { .i64 = 1 }, 0, 1, VE }, { "pred", "Prediction method", OFFSET(predictor), AV_OPT_TYPE_INT, { .i64 = LEFT }, LEFT, MEDIAN, VE, "pred" }, { "left", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = LEFT }, INT_MIN, INT_MAX, VE, "pred" }, { "plane", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = PLANE }, INT_MIN, INT_MAX, VE, "pred" }, { "median", NULL, 0, AV_OPT_TYPE_CONST, { .i64 = MEDIAN }, INT_MIN, INT_MAX, VE, "pred" }, 
