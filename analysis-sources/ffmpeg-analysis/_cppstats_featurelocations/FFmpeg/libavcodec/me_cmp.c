...
#define BUTTERFLY2(o1, o2, i1, i2) o1 = (i1) + (i2); o2 = (i1) - (i2);



#define BUTTERFLY1(x, y) { int a, b; a = x; b = y; x = a + b; y = a - b; }
...
ff_simple_idct_int16_8bit(temp); 
...
#define VSAD_INTRA(size) static int vsad_intra ##size ##_c(MpegEncContext *c, uint8_t *s, uint8_t *dummy, ptrdiff_t stride, int h) { int score = 0, x, y; for (y = 1; y < h; y++) { for (x = 0; x < size; x += 4) { score += FFABS(s[x] - s[x + stride]) + FFABS(s[x + 1] - s[x + stride + 1]) + FFABS(s[x + 2] - s[x + 2 + stride]) + FFABS(s[x + 3] - s[x + 3 + stride]); } s += stride; } return score; }
...
#define VSAD(size) static int vsad ##size ##_c(MpegEncContext *c, uint8_t *s1, uint8_t *s2, ptrdiff_t stride, int h) { int score = 0, x, y; for (y = 1; y < h; y++) { for (x = 0; x < size; x++) score += FFABS(s1[x] - s2[x] - s1[x + stride] + s2[x + stride]); s1 += stride; s2 += stride; } return score; }
...
#define SQ(a) ((a) * (a))
#define VSSE_INTRA(size) static int vsse_intra ##size ##_c(MpegEncContext *c, uint8_t *s, uint8_t *dummy, ptrdiff_t stride, int h) { int score = 0, x, y; for (y = 1; y < h; y++) { for (x = 0; x < size; x += 4) { score += SQ(s[x] - s[x + stride]) + SQ(s[x + 1] - s[x + stride + 1]) + SQ(s[x + 2] - s[x + stride + 2]) + SQ(s[x + 3] - s[x + stride + 3]); } s += stride; } return score; }
...
#define VSSE(size) static int vsse ##size ##_c(MpegEncContext *c, uint8_t *s1, uint8_t *s2, ptrdiff_t stride, int h) { int score = 0, x, y; for (y = 1; y < h; y++) { for (x = 0; x < size; x++) score += SQ(s1[x] - s2[x] - s1[x + stride] + s2[x + stride]); s1 += stride; s2 += stride; } return score; }
...
#define WRAPPER8_16_SQ(name8, name16) static int name16(MpegEncContext *s, uint8_t *dst, uint8_t *src, ptrdiff_t stride, int h) { int score = 0; score += name8(s, dst, src, stride, 8); score += name8(s, dst + 8, src + 8, stride, 8); if (h == 16) { dst += 8 * stride; src += 8 * stride; score += name8(s, dst, src, stride, 8); score += name8(s, dst + 8, src + 8, stride, 8); } return score; }
