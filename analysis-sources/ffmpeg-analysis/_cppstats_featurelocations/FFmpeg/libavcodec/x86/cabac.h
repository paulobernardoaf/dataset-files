...
#if !defined(AVCODEC_X86_CABAC_H)
...
#if (defined(__i386) && defined(__clang__) && (__clang_major__<2 || (__clang_major__==2 && __clang_minor__<10)))|| ( !defined(__clang__) && defined(__llvm__) && __GNUC__==4 && __GNUC_MINOR__==2 && __GNUC_PATCHLEVEL__<=1)|| (defined(__INTEL_COMPILER) && defined(_MSC_VER))
...
#else
#define END_CHECK(end) "cmp "end" , %%"FF_REG_c" \n\t""jge 1f \n\t"
...
#if HAVE_FAST_CMOV
#define BRANCHLESS_GET_CABAC_UPDATE(ret, retq, low, range, tmp) "cmp "low" , "tmp" \n\t""cmova %%ecx , "range" \n\t""sbb %%rcx , %%rcx \n\t""and %%ecx , "tmp" \n\t""xor %%rcx , "retq" \n\t""sub "tmp" , "low" \n\t"
...
#else 
#define BRANCHLESS_GET_CABAC_UPDATE(ret, retq, low, range, tmp) "sub "low" , "tmp" \n\t""sar $31 , "tmp" \n\t""sub %%ecx , "range" \n\t""and "tmp" , "range" \n\t""add %%ecx , "range" \n\t""shl $17 , %%ecx \n\t""and "tmp" , %%ecx \n\t""sub %%ecx , "low" \n\t""xor "tmp" , "ret" \n\t""movslq "ret" , "retq" \n\t"
...
#if HAVE_FAST_CMOV
#define BRANCHLESS_GET_CABAC_UPDATE(ret, low, range, tmp)"mov "tmp" , %%ecx \n\t""shl $17 , "tmp" \n\t""cmp "low" , "tmp" \n\t""cmova %%ecx , "range" \n\t""sbb %%ecx , %%ecx \n\t""and %%ecx , "tmp" \n\t""xor %%ecx , "ret" \n\t""sub "tmp" , "low" \n\t"
