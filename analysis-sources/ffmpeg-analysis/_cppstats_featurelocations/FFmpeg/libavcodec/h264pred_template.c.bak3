...
#define LOAD_TOP_RIGHT_EDGEconst unsigned av_unused t4 = topright[0];const unsigned av_unused t5 = topright[1];const unsigned av_unused t6 = topright[2];const unsigned av_unused t7 = topright[3];





#define LOAD_DOWN_LEFT_EDGEconst unsigned av_unused l4 = src[-1+4*stride];const unsigned av_unused l5 = src[-1+5*stride];const unsigned av_unused l6 = src[-1+6*stride];const unsigned av_unused l7 = src[-1+7*stride];





#define LOAD_LEFT_EDGEconst unsigned av_unused l0 = src[-1+0*stride];const unsigned av_unused l1 = src[-1+1*stride];const unsigned av_unused l2 = src[-1+2*stride];const unsigned av_unused l3 = src[-1+3*stride];





#define LOAD_TOP_EDGEconst unsigned av_unused t0 = src[ 0-1*stride];const unsigned av_unused t1 = src[ 1-1*stride];const unsigned av_unused t2 = src[ 2-1*stride];const unsigned av_unused t3 = src[ 3-1*stride];
...
#define PREDICT_16x16_DC(v)for(i=0; i<16; i++){AV_WN4PA(src+ 0, v);AV_WN4PA(src+ 4, v);AV_WN4PA(src+ 8, v);AV_WN4PA(src+12, v);src += stride;}
...
#define PRED16x16_X(n, v) static void FUNCC(pred16x16_##n##_dc)(uint8_t *_src, ptrdiff_t stride){int i;pixel *src = (pixel*)_src;stride >>= sizeof(pixel)-1;PREDICT_16x16_DC(PIXEL_SPLAT_X4(v));}
...
#define PRED8x8_X(n, v)static void FUNCC(pred8x8_##n##_dc)(uint8_t *_src, ptrdiff_t stride){int i;const pixel4 a = PIXEL_SPLAT_X4(v);pixel *src = (pixel*)_src;stride >>= sizeof(pixel)-1;for(i=0; i<8; i++){AV_WN4PA(((pixel4*)(src+i*stride))+0, a);AV_WN4PA(((pixel4*)(src+i*stride))+1, a);}}
...
#define PREDICT_8x8_LOAD_LEFT const int l0 = ((has_topleft ? SRC(-1,-1) : SRC(-1,0)) + 2*SRC(-1,0) + SRC(-1,1) + 2) >> 2; PL(1) PL(2) PL(3) PL(4) PL(5) PL(6) const int l7 av_unused = (SRC(-1,6) + 3*SRC(-1,7) + 2) >> 2
...
#define PREDICT_8x8_LOAD_TOP const int t0 = ((has_topleft ? SRC(-1,-1) : SRC(0,-1)) + 2*SRC(0,-1) + SRC(1,-1) + 2) >> 2; PT(1) PT(2) PT(3) PT(4) PT(5) PT(6) const int t7 av_unused = ((has_topright ? SRC(8,-1) : SRC(7,-1)) + 2*SRC(7,-1) + SRC(6,-1) + 2) >> 2
...
#define PREDICT_8x8_LOAD_TOPRIGHT int t8, t9, t10, t11, t12, t13, t14, t15; if(has_topright) { PTR(8) PTR(9) PTR(10) PTR(11) PTR(12) PTR(13) PTR(14) t15 = (SRC(14,-1) + 3*SRC(15,-1) + 2) >> 2; } else t8=t9=t10=t11=t12=t13=t14=t15= SRC(7,-1);
...
#define PREDICT_8x8_DC(v) int y; for( y = 0; y < 8; y++ ) { AV_WN4PA(((pixel4*)src)+0, v); AV_WN4PA(((pixel4*)src)+1, v); src += stride; }
...
PREDICT_8x8_LOAD_LEFT;
#define ROW(y) a = PIXEL_SPLAT_X4(l##y); AV_WN4PA(src+y*stride, a); AV_WN4PA(src+y*stride+4, a);
