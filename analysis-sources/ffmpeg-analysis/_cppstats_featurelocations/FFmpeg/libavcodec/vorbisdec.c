...
#define VALIDATE_INDEX(idx, limit) if (idx >= limit) {av_log(vc->avctx, AV_LOG_ERROR,idx_err_str,(int)(idx), (int)(limit - 1), #idx, __FILE__, __LINE__);return AVERROR_INVALIDDATA;}
...
#define GET_VALIDATED_INDEX(idx, bits, limit) {idx = get_bits(gb, bits);VALIDATE_INDEX(idx, limit)}
...
av_malloc_array(n + 1, sizeof(int32_t)); 
...
memset(floor_ptr[i], 0, vlen * sizeof(floor_ptr[0][0])); 
