...
#define BUFFER_PRECHECK if (*data_ptr >= last_ptr) return IV3_OUT_OF_DATA; 



#define RLE_BLOCK_COPY if (cell->mv_ptr || !skip_flag) copy_block4(dst, ref, row_offset, row_offset, 4 << v_zoom)



#define RLE_BLOCK_COPY_8 pix64 = AV_RN64(ref);if (is_first_row) {pix64 = replicate64(pix64);fill_64(dst + row_offset, pix64, 7, row_offset);AVG_64(dst, ref, dst + row_offset);} else fill_64(dst, pix64, 8, row_offset)
...
#define RLE_LINES_COPY_M10 pix64 = AV_RN64(ref);if (is_top_of_cell) {pix64 = replicate64(pix64);fill_64(dst + row_offset, pix64, (num_lines << 1) - 1, row_offset);AVG_64(dst, ref, dst + row_offset);} else fill_64(dst, pix64, num_lines << 1, row_offset)
...
#define APPLY_DELTA_4 AV_WN16A(dst + line_offset ,(AV_RN16(ref ) + delta_tab->deltas[dyad1]) & 0x7F7F);AV_WN16A(dst + line_offset + 2,(AV_RN16(ref + 2) + delta_tab->deltas[dyad2]) & 0x7F7F);if (mode >= 3) {if (is_top_of_cell && !cell->ypos) {AV_COPY32U(dst, dst + row_offset);} else {AVG_32(dst, ref, dst + row_offset);}}
...
#define UPDATE_BITPOS(n) ctx->skip_bits += (n); ctx->need_resync = 1



#define RESYNC_BITSTREAM if (ctx->need_resync && !(get_bits_count(&ctx->gb) & 7)) { skip_bits_long(&ctx->gb, ctx->skip_bits); ctx->skip_bits = 0; ctx->need_resync = 0; }
...
#define CHECK_CELL if (curr_cell.xpos + curr_cell.width > (plane->width >> 2) || curr_cell.ypos + curr_cell.height > (plane->height >> 2)) { av_log(avctx, AV_LOG_ERROR, "Invalid cell: x=%d, y=%d, w=%d, h=%d\n", curr_cell.xpos, curr_cell.ypos, curr_cell.width, curr_cell.height); return AVERROR_INVALIDDATA; }
