...
#define GET_CODE(val, type, add_bits) do { if (type) val = get_ue_golomb(gb); else val = get_unary(gb, 1, 31); if (add_bits) val = (val << add_bits) + get_bits(gb, add_bits); } while (0)
