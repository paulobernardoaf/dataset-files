...
#define PR_WRAP(INSN) static void ff_prores_idct_put_10_##INSN##_wrap(int16_t *dst){ LOCAL_ALIGNED(16, int16_t, qmat, [64]); LOCAL_ALIGNED(16, int16_t, tmp, [64]); int i; for(i=0; i<64; i++){ qmat[i]=4; tmp[i]= dst[i]; } ff_prores_idct_put_10_##INSN (dst, 16, tmp, qmat); for(i=0; i<64; i++) { dst[i] -= 512; } }
