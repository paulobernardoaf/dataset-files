...
#define RV40_WEIGHT_FUNC(size) static void rv40_weight_func_rnd_ ##size (uint8_t *dst, uint8_t *src1, uint8_t *src2, int w1, int w2, ptrdiff_t stride){int i, j;for (j = 0; j < size; j++) {for (i = 0; i < size; i++)dst[i] = (((w2 * src1[i]) >> 9) + ((w1 * src2[i]) >> 9) + 0x10) >> 5;src1 += stride;src2 += stride;dst += stride;}}static void rv40_weight_func_nornd_ ##size (uint8_t *dst, uint8_t *src1, uint8_t *src2, int w1, int w2, ptrdiff_t stride){int i, j;for (j = 0; j < size; j++) {for (i = 0; i < size; i++)dst[i] = (w2 * src1[i] + w1 * src2[i] + 0x10) >> 5;src1 += stride;src2 += stride;dst += stride;}}
