...
#define FDCTCOL(b0, b1, b2, b3, b4, b5, b6, b7) x0 = vec_add(b0, b7); x7 = vec_sub(b0, b7); x1 = vec_add(b1, b6); x6 = vec_sub(b1, b6); x2 = vec_add(b2, b5); x5 = vec_sub(b2, b5); x3 = vec_add(b3, b4); x4 = vec_sub(b3, b4); b7 = vec_add(x0, x3); b1 = vec_add(x1, x2); b0 = vec_add(b7, b1); b4 = vec_sub(b7, b1); b2 = vec_sub(x0, x3); b6 = vec_sub(x1, x2); b5 = vec_add(b6, b2); cnst = LD_W2; b5 = vec_madd(cnst, b5, mzero); cnst = LD_W1; b2 = vec_madd(cnst, b2, b5); cnst = LD_W0; b6 = vec_madd(cnst, b6, b5); x0 = vec_add(x4, x7); x1 = vec_add(x5, x6); x2 = vec_add(x4, x6); x3 = vec_add(x5, x7); x8 = vec_add(x2, x3); cnst = LD_W3; x8 = vec_madd(cnst, x8, mzero); cnst = LD_W8; x0 = vec_madd(cnst, x0, mzero); cnst = LD_W9; x1 = vec_madd(cnst, x1, mzero); cnst = LD_WA; x2 = vec_madd(cnst, x2, x8); cnst = LD_WB; x3 = vec_madd(cnst, x3, x8); cnst = LD_W4; b7 = vec_madd(cnst, x4, x0); cnst = LD_W5; b5 = vec_madd(cnst, x5, x1); cnst = LD_W6; b3 = vec_madd(cnst, x6, x1); cnst = LD_W7; b1 = vec_madd(cnst, x7, x0); b7 = vec_add(b7, x2); b5 = vec_add(b5, x3); b3 = vec_add(b3, x2); b1 = vec_add(b1, x3) 
...
#define CTF0(n) b ##n ##1 = ((vector float) vec_unpackl(vs16(b ##n ##0))); b ##n ##0 = ((vector float) vec_unpackh(vs16(b ##n ##0))); b ##n ##1 = vec_ctf(vs32(b ##n ##1), 0); b ##n ##0 = vec_ctf(vs32(b ##n ##0), 0)
...
#define CTFX(x, b) b ##0 = ((vector float) vec_unpackh(vs16(x))); b ##1 = ((vector float) vec_unpackl(vs16(x))); b ##0 = vec_ctf(vs32(b ##0), 0); b ##1 = vec_ctf(vs32(b ##1), 0)
...
#define CTS(n) b ##n ##0 = vec_round(b ##n ##0); b ##n ##1 = vec_round(b ##n ##1); b ##n ##0 = ((vector float) vec_cts(b ##n ##0, 0)); b ##n ##1 = ((vector float) vec_cts(b ##n ##1, 0)); b ##n ##0 = ((vector float) vec_pack(vs32(b ##n ##0), vs32(b ##n ##1))); vec_st(vs16(b ##n ##0), 0, bp)
