...
#define LUT32(plane) { 0, 0, 0, 0, 0, 0, 0, 1U << plane, 0, 0, 1U << plane, 0, 0, 0, 1U << plane, 1U << plane, 0, 1U << plane, 0, 0, 0, 1U << plane, 0, 1U << plane, 0, 1U << plane, 1U << plane, 0, 0, 1U << plane, 1U << plane, 1U << plane, 1U << plane, 0, 0, 0, 1U << plane, 0, 0, 1U << plane, 1U << plane, 0, 1U << plane, 0, 1U << plane, 0, 1U << plane, 1U << plane, 1U << plane, 1U << plane, 0, 0, 1U << plane, 1U << plane, 0, 1U << plane, 1U << plane, 1U << plane, 1U << plane, 0, 1U << plane, 1U << plane, 1U << plane, 1U << plane, }
...
return err;
s->planesize = FFALIGN(avctx->width, 16) >> 3; 
...
#define DECODE_HAM_PLANE32(x) first = buf[x] << 1; second = buf[(x)+1] << 1; delta &= pal[first++]; delta |= pal[first]; dst[x] = delta; delta &= pal[second++]; delta |= pal[second]; dst[(x)+1] = delta
...
#define DECODE_RGBX_COMMON(type) if (!length) { length = bytestream2_get_byte(gb); if (!length) { length = bytestream2_get_be16(gb); if (!length) return; } } for (i = 0; i < length; i++) { *(type *)(dst + y*linesize + x * sizeof(type)) = pixel; x += 1; if (x >= width) { y += 1; if (y >= height) return; x = 0; } }
...
} else if (avctx->bits_per_coded_sample <= 8) { 
for (y = 0; y < avctx->height; y++) {
uint8_t *row = &frame->data[0][y * frame->linesize[0]];
