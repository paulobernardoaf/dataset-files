...
#define ENCODE(type, endian, src, dst, n, shift, offset) samples_ ##type = (const type *) src; for (; n > 0; n--) { register type v = (*samples_ ##type++ >> shift) + offset; bytestream_put_ ##endian(&dst, v); }
...
#define ENCODE_PLANAR(type, endian, dst, n, shift, offset) n /= avctx->channels; for (c = 0; c < avctx->channels; c++) { int i; samples_ ##type = (const type *) frame->extended_data[c]; for (i = n; i > 0; i--) { register type v = (*samples_ ##type++ >> shift) + offset; bytestream_put_ ##endian(&dst, v); } }
...
#define DECODE(size, endian, src, dst, n, shift, offset) for (; n > 0; n--) { uint ##size ##_t v = bytestream_get_ ##endian(&src); AV_WN ##size ##A(dst, (uint ##size ##_t)(v - offset) << shift); dst += size / 8; }
...
#define DECODE_PLANAR(size, endian, src, dst, n, shift, offset) n /= avctx->channels; for (c = 0; c < avctx->channels; c++) { int i; dst = frame->extended_data[c]; for (i = n; i > 0; i--) { uint ##size ##_t v = bytestream_get_ ##endian(&src); AV_WN ##size ##A(dst, (uint ##size ##_t)(v - offset) << shift); dst += size / 8; } }
...
#define PCM_ENCODER_1(id_, sample_fmt_, name_, long_name_) AVCodec ff_ ##name_ ##_encoder = { .name = #name_, .long_name = NULL_IF_CONFIG_SMALL(long_name_), .type = AVMEDIA_TYPE_AUDIO, .id = AV_CODEC_ID_ ##id_, .init = pcm_encode_init, .encode2 = pcm_encode_frame, .capabilities = AV_CODEC_CAP_VARIABLE_FRAME_SIZE, .sample_fmts = (const enum AVSampleFormat[]){ sample_fmt_, AV_SAMPLE_FMT_NONE }, }
