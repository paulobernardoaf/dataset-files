...
#if HAVE_BIGENDIAN
#define LE_CENTRIC_MUL(dst, src, scale, dc) { unsigned tmpval = AV_RN32(src); tmpval = (tmpval << 16) | (tmpval >> 16); tmpval = tmpval * (scale) + (dc); tmpval = (tmpval << 16) | (tmpval >> 16); AV_WN32A(dst, tmpval); }
...
#else
#define LE_CENTRIC_MUL(dst, src, scale, dc) { unsigned tmpval = AV_RN32(src) * (scale) + (dc); AV_WN32A(dst, tmpval); }
