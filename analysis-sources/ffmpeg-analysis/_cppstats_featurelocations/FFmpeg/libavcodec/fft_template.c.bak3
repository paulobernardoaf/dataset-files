...
#define PROCESS_FFT_PERM_SWAP_LSBS(num) do {for(i = 0; i < n; i++) {int k;j = i;j = (j & ~3) | ((j >> 1) & 1) | ((j << 1) & 2);k = -split_radix_permutation(i, n, s->inverse) & (n - 1);s->revtab##num[k] = j;} } while(0);
...
#define PROCESS_FFT_PERM_DEFAULT(num) do {for(i = 0; i < n; i++) {int k;j = i;k = -split_radix_permutation(i, n, s->inverse) & (n - 1);s->revtab##num[k] = j;} } while(0);
...
#define SPLIT_RADIX_PERMUTATION(num) do { if (s->fft_permutation == FF_FFT_PERM_SWAP_LSBS) {PROCESS_FFT_PERM_SWAP_LSBS(num) } else {PROCESS_FFT_PERM_DEFAULT(num) }} while(0);
...
#define DECL_FFT(n,n2,n4)static void fft##n(FFTComplex *z){fft##n2(z);fft##n4(z+n4*2);fft##n4(z+n4*3);pass(z,FFT_NAME(ff_cos_##n),n4/2);}
