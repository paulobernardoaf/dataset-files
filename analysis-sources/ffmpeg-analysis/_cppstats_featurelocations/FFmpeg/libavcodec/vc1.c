...
#define INIT_LUT(lumscale, lumshift, luty, lutuv, chain) do { int scale, shift, i; if (!lumscale) { scale = -64; shift = (255 - lumshift * 2) * 64; if (lumshift > 31) shift += 128 << 6; } else { scale = lumscale + 32; if (lumshift > 31) shift = (lumshift - 64) * 64; else shift = lumshift << 6; } for (i = 0; i < 256; i++) { int iy = chain ? luty[i] : i; int iu = chain ? lutuv[i] : i; luty[i] = av_clip_uint8((scale * iy + shift + 32) >> 6); lutuv[i] = av_clip_uint8((scale * (iu - 128) + 128*64 + 32) >> 6);} } while(0)
...
{
#define ROTATE(DEF, L, N, C, A) do { if (v->s.pict_type == AV_PICTURE_TYPE_BI || v->s.pict_type == AV_PICTURE_TYPE_B) { C = A; } else { DEF; memcpy(&tmp, L , sizeof(tmp)); memcpy(L , N , sizeof(tmp)); memcpy(N , &tmp, sizeof(tmp)); C = N; } } while(0)
