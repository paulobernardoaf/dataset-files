...
#define WINDOW_FUNC(type) static void apply_ ##type ##_window(AVFloatDSPContext *fdsp, SingleChannelElement *sce, const float *audio)
...
for (k = 0; k < 1024; k++) {
if (!(fabs(cpe->ch[ch].coeffs[k]) < 1E16)) { 
...
avctx->bit_rate += s->chan_map[i] == TYPE_CPE ? 128000 : 
s->chan_map[i] == TYPE_LFE ? 16000 : 
69000 ; 
}
}
