...
#define body(n) { skip_bits(gb, 2 + n); v += (1 << n) + get_bits(gb, n); }
...
#define SET_CHROMA_MODES if (frag[s->fragment_start[1]].coding_method != MODE_COPY) frag[s->fragment_start[1]].coding_method = coding_mode; if (frag[s->fragment_start[2]].coding_method != MODE_COPY) frag[s->fragment_start[2]].coding_method = coding_mode;
...
#define safe_loop_filter(name, ptr, stride, bounding_values) if ((uintptr_t)(ptr) & 7) s->vp3dsp.name##_unaligned(ptr, stride, bounding_values); else s->vp3dsp.name(ptr, stride, bounding_values);
