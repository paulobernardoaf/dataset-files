...
{
#define NEEDS_ESCAPE(ch) ((ch) <= ' ' || (ch) >= '\x7f' || (ch) == '"' || (ch) == '%' || (ch) == '<' || (ch) == '>' || (ch) == '\\' || (ch) == '^' || (ch) == '`' || (ch) == '{' || (ch) == '}' || (ch) == '|')
...
http_get_line(s, line, sizeof(line)); 
s->chunkend = 1;
return 0;
}
else if (!s->chunksize) {
...
#define HTTP_CLASS(flavor) static const AVClass flavor ##_context_class = { .class_name = #flavor, .item_name = av_default_item_name, .option = options, .version = LIBAVUTIL_VERSION_INT, }
