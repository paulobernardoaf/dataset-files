...
#define RTMP_PROTOCOL(flavor) static const AVClass flavor##_class = { .class_name = #flavor, .item_name = av_default_item_name, .option = rtmp_options, .version = LIBAVUTIL_VERSION_INT, }; const URLProtocol ff_##flavor##_protocol = { .name = #flavor, .url_open2 = rtmp_open, .url_read = rtmp_read, .url_read_seek = rtmp_seek, .url_read_pause = rtmp_pause, .url_write = rtmp_write, .url_close = rtmp_close, .priv_data_size = sizeof(RTMPContext), .flags = URL_PROTOCOL_FLAG_NETWORK, .priv_data_class= &flavor##_class, };
