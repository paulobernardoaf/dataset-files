...
#define SET_SIDE_DATA(name, type) do { if (name##_len) { uint8_t *buf = av_packet_new_side_data(sub, type, name##_len); if (!buf) { res = AVERROR(ENOMEM); goto end; } memcpy(buf, name, name##_len); } } while (0)
