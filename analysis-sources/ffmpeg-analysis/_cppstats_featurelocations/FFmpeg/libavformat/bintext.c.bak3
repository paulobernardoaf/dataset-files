...
#define GET_EFI2_META(name,size) len = avio_r8(pb); if (len < 1 || len > size) return -1; if (avio_read(pb, buf, size) == size && *buf) { buf[len] = 0; av_dict_set(&avctx->metadata, name, buf, 0); }
...
#define CLASS(name) (const AVClass[1]){{ .class_name = name, .item_name = av_default_item_name, .option = options, .version = LIBAVUTIL_VERSION_INT, }}
