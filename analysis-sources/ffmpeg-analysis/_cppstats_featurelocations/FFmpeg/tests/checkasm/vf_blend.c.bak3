...
#define check_blend_func(depth) do { int i, w; declare_func(void, const uint8_t *top, ptrdiff_t top_linesize, const uint8_t *bottom, ptrdiff_t bottom_linesize, uint8_t *dst, ptrdiff_t dst_linesize, ptrdiff_t width, ptrdiff_t height, struct FilterParams *param, double *values); w = WIDTH / depth; for (i = 0; i < BUF_UNITS - 1; i++) { int src_offset = i * SIZE_PER_UNIT + (BUF_UNITS - 1 - i) * depth; int dst_offset = i * SIZE_PER_UNIT; randomize_buffers(); call_ref(top1 + src_offset, w, bot1 + src_offset, w, dst1 + dst_offset, w, w, HEIGHT, &param, NULL); call_new(top2 + src_offset, w, bot2 + src_offset, w, dst2 + dst_offset, w, w, HEIGHT, &param, NULL); if (memcmp(top1, top2, BUF_SIZE) || memcmp(bot1, bot2, BUF_SIZE) || memcmp(dst1, dst2, BUF_SIZE)) fail(); } bench_new(top2, w / 4, bot2, w / 4, dst2, w / 4, w / 4, HEIGHT / 4, &param, NULL); } while (0)
...
#define check_and_report(name, val, depth) param.mode = val; ff_blend_init(&param, depth - 1); if (check_func(param.blend, #name)) check_blend_func(depth);
