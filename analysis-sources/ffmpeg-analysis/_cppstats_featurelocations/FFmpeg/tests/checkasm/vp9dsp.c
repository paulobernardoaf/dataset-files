...
#define randomize_buffers() do { uint32_t mask = pixel_mask[(bit_depth - 8) >> 1]; int k; for (k = -4; k < SIZEOF_PIXEL * FFMAX(8, size); k += 4) { uint32_t r = rnd() & mask; AV_WN32A(a + k, r); } for (k = 0; k < size * SIZEOF_PIXEL; k += 4) { uint32_t r = rnd() & mask; AV_WN32A(l + k, r); } } while (0)
...
#define randomize_buffers() do { uint32_t mask = pixel_mask[(bit_depth - 8) >> 1]; for (y = 0; y < sz; y++) { for (x = 0; x < sz * SIZEOF_PIXEL; x += 4) { uint32_t r = rnd() & mask; AV_WN32A(dst + y * sz * SIZEOF_PIXEL + x, r); AV_WN32A(src + y * sz * SIZEOF_PIXEL + x, rnd() & mask); } for (x = 0; x < sz; x++) { if (bit_depth == 8) { coef[y * sz + x] = src[y * sz + x] - dst[y * sz + x]; } else { ((int32_t *) coef)[y * sz + x] = ((uint16_t *) src)[y * sz + x] - ((uint16_t *) dst)[y * sz + x]; } } } } while(0)
...
for (tx = TX_4X4; tx <= N_TXFM_SIZES ; tx++) {
int sz = 4 << (tx & 3);
...
#define setpx(a,b,c) do { if (SIZEOF_PIXEL == 1) { buf0[(a) + (b) * jstride] = av_clip_uint8(c); } else { ((uint16_t *)buf0)[(a) + (b) * jstride] = av_clip_uintp2(c, bit_depth); } } while (0)
...
#define randomize_buffers(bidx, lineoff, str) randomize_loopfilter_buffers(bidx, lineoff, str, bit_depth, dir, E, F, H, I, buf0, buf1)
...
#define randomize_buffers() do { uint32_t mask = pixel_mask[(bit_depth - 8) >> 1]; int k; for (k = 0; k < SRC_BUF_SIZE; k += 4) { uint32_t r = rnd() & mask; AV_WN32A(buf + k, r); } if (op == 1) { for (k = 0; k < DST_BUF_SIZE; k += 4) { uint32_t r = rnd() & mask; AV_WN32A(dst0 + k, r); AV_WN32A(dst1 + k, r); } } } while (0)
