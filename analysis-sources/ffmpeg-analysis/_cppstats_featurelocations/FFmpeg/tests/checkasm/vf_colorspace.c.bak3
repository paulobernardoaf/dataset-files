...
#define randomize_buffers() do { unsigned mask = bpp_mask[idepth]; int n, m; int bpp = 1 + (!!idepth); int buf_size = W * H * bpp; for (m = 0; m < 3; m++) { int ss = m ? ss_w + ss_h : 0; int plane_sz = buf_size >> ss; for (n = 0; n < plane_sz; n += 4) { unsigned r = rnd() & mask; AV_WN32A(&src[m][n], r); } } } while (0)
...
#define randomize_buffers() do { int y, x, p; for (p = 0; p < 3; p++) { for (y = 0; y < H; y++) { for (x = 0; x < W; x++) { int r = rnd() & 0x7fff; r -= (32768 - 28672) >> 1; src[p][y * W + x] = r; } } } } while (0)
