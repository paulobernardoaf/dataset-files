...
#define randomize_buffers() do { int i; for (i = 0; i < BUF_SIZE*MAX_CHANNELS; i++) { int32_t r = sign_extend(rnd(), 24); ref_buf[i] = r; new_buf[i] = r; } } while (0)
...
#define randomize_buffers() do { int i, j; for (i = 0; i < BUF_SIZE; i++) { for (j = 0; j < ch; j++) { int32_t r = sign_extend(rnd(), 24); ref[j][i] = r; new[j][i] = r; r = rnd() & 0xFF; ref_ebb[j][i] = r; new_ebb[j][i] = r; } } } while (0)
