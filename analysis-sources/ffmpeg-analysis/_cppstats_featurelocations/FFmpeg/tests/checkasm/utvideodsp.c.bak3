...
#define WIDTH 240
#define HEIGHT 120
#define WIDTH_PADDED (WIDTH + 16) 
#define BUFFER_SIZE (WIDTH_PADDED * HEIGHT)


#define randomize_plane(buf, type) do { int w, h; type * tmp = buf; for (h = 0; h < HEIGHT; h++) { for (w = 0; w < WIDTH; w++) tmp[w] = rnd() & 0xFF; tmp += WIDTH_PADDED; } } while (0)
...
#define cmp_plane(buf0, buf1, s) do { int h; for (h = 0; h < HEIGHT; h++) { if (memcmp(buf0 + h*WIDTH_PADDED, buf1 + h*WIDTH_PADDED, WIDTH *s)) fail();} } while (0)
