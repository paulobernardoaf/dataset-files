...
#if !defined(COMPAT_ATOMICS_GCC_STDATOMIC_H)
...
#define atomic_store(object, desired) do { *(object) = (desired); __sync_synchronize(); } while (0)
...
#define atomic_exchange(object, desired) ({ __typeof__(object) _obj = (object); __typeof__(*object) _old; do _old = atomic_load(_obj); while (!__sync_bool_compare_and_swap(_obj, _old, (desired))); _old; })
...
#define atomic_compare_exchange_strong(object, expected, desired) ({ __typeof__(object) _exp = (expected); __typeof__(*object) _old = *_exp; *_exp = __sync_val_compare_and_swap((object), _old, (desired)); *_exp == _old; })
