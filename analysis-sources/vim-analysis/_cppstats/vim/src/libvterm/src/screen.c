...
int padding = 0;
int row, col;
#define PUT(c) if(utf8) { size_t thislen = utf8_seqlen(c); if(buffer && outpos + thislen <= len) outpos += fill_utf8((c), (char *)buffer + outpos); else outpos += thislen; } else { if(buffer && outpos + 1 <= len) ((uint32_t*)buffer)[outpos++] = (c); else outpos++; }
