...
#define RET_WIN_BUF_CHARTABSIZE(wp, buf, p, col) if (*(p) == TAB && (!(wp)->w_p_list || lcs_tab1)) { return tabstop_padding(col, (buf)->b_p_ts, (buf)->b_p_vts_array); } else return ptr2cells(p);
#else
