...
#define luaV_getudata(L, v) lua_pushlightuserdata((L), (void *) (v)); lua_rawget((L), lua_upvalueindex(1))
#define luaV_setudata(L, v) lua_pushlightuserdata((L), (void *) (v)); lua_pushvalue((L), -2); lua_rawset((L), lua_upvalueindex(1))
#define luaV_getfield(L, s) lua_pushlightuserdata((L), (void *)(s)); lua_rawget((L), LUA_REGISTRYINDEX)
#define luaV_checksandbox(L) if (sandbox) luaL_error((L), "not allowed in sandbox")
#define luaV_msg(L) luaV_msgfunc((L), (msgfunc_T) msg)
#define luaV_emsg(L) luaV_msgfunc((L), (msgfunc_T) emsg)
#define luaV_checktypval(L, a, v, msg) do { if (luaV_totypval(L, a, v) == FAIL) luaL_error(L, msg ": cannot convert value"); } while (0)
...
#define luaV_newtype(typ,tname,luatyp,luatname) static luatyp * luaV_new##tname(lua_State *L, typ *obj) { luatyp *o = (luatyp *) lua_newuserdata(L, sizeof(luatyp)); *o = obj; luaV_setudata(L, obj); luaV_getfield(L, luatname); lua_setmetatable(L, -2); return o; }
#define luaV_pushtype(typ,tname,luatyp) static luatyp * luaV_push##tname(lua_State *L, typ *obj) { luatyp *o = NULL; if (obj == NULL) lua_pushnil(L); else { luaV_getudata(L, obj); if (lua_isnil(L, -1)) { lua_pop(L, 1); o = luaV_new##tname(L, obj); } else o = (luatyp *) lua_touserdata(L, -1); } return o; }
#define luaV_type_tostring(tname,luatname) static int luaV_##tname##_tostring(lua_State *L) { lua_pushfstring(L, "%s: %p", luatname, lua_touserdata(L, 1)); return 1; }
static luaV_List *
...
luaV_checktypval(L, -2, &v, "vim.dict"); 
di = dictitem_alloc(key);
...
buf = (lua_toboolean(L, 1)) ? firstbuf : curbuf; 
...
win = (lua_toboolean(L, 1)) ? firstwin : curwin; 
...
if (luaL_loadbuffer(L, str, l, LUAVIM_EVALNAME)) 
{
luaV_emsg(L);
return 0;
}
...
if (lua_rawequal(L, -1, 2)) 
{
list_T *l = (list_T *)lua_touserdata(L, 5); 
abort = set_ref_in_list(l, copyID);
}
else if (lua_rawequal(L, -1, 3)) 
{
dict_T *d = (dict_T *)lua_touserdata(L, 5); 
abort = set_ref_in_dict(d, copyID);
}
else if (lua_rawequal(L, -1, 4)) 
{
luaV_Funcref *f = (luaV_Funcref *)lua_touserdata(L, 5); 
...
luaV_setrange(L, eap->line1, eap->line2);
luaL_buffinit(L, &b);
luaL_addlstring(&b, "return function(line, linenr) ", 30); 
...
#define luaV_freetype(typ,tname) void lua_##tname##_free(typ *o) { if (!lua_isopen()) return; luaV_getfield(L, LUAVIM_FREE); lua_pushlightuserdata(L, (void *) o); lua_call(L, 1, 0); }
