...
#define Py_TYPE_NAME(obj) (obj->ob_type->tp_name == NULL ? "(NULL)" : obj->ob_type->tp_name)



#define RAISE_NO_EMPTY_KEYS PyErr_SET_STRING(PyExc_ValueError, N_("empty keys are not allowed"))
...
#define RAISE_KEY_ADD_FAIL(key) PyErr_VIM_FORMAT(N_("failed to add key '%s' to dictionary"), key)

#define RAISE_INVALID_INDEX_TYPE(idx) PyErr_FORMAT(PyExc_TypeError, N_("index must be int or slice, not %s"), Py_TYPE_NAME(idx));
...
#define NEW_FUNCTION(name, argc, argv, self, pt_auto) FunctionNew(&FunctionType, (name), (argc), (argv), (self), (pt_auto))
...
#define PYTYPE_READY(type) if (PyType_Ready(&type)) return -1;
...
#define ADD_OBJECT(m, name, obj) if (PyModule_AddObject(m, name, obj)) return -1;



#define ADD_CHECKED_OBJECT(m, name, obj) { PyObject *valObject = obj; if (!valObject) return -1; ADD_OBJECT(m, name, valObject); }
