...
#define for_each_builtin(j, tgt_j) for (j = 0, tgt_j = tr2_tgt_builtins[j]; tgt_j; j++, tgt_j = tr2_tgt_builtins[j])
#define for_each_wanted_builtin(j, tgt_j) for_each_builtin(j, tgt_j) if (tr2_dst_trace_want(tgt_j->pdst))
