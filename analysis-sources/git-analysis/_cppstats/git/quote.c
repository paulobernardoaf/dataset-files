...
{
#undef EMIT
#define EMIT(c) do { if (sb) strbuf_addch(sb, (c)); if (fp) fputc((c), fp); count++; } while (0)
#define EMITBUF(s, l) do { if (sb) strbuf_add(sb, (s), (l)); if (fp) fwrite((s), (l), 1, fp); count += (l); } while (0)
size_t len, count = 0;
const char *p = name;
for (;;) {
int ch;
