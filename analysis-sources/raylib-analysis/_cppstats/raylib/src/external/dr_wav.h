...
#if !defined(DRWAV_MAX_SMPL_LOOPS)
...
#if !defined(DR_WAV_NO_CONVERSION_API)
...
#include <stdio.h>
#endif
#if !defined(DRWAV_ASSERT)
#include <assert.h>
#define DRWAV_ASSERT(expression) assert(expression)
#endif
#if !defined(DRWAV_MALLOC)
#define DRWAV_MALLOC(sz) malloc((sz))
#endif
#if !defined(DRWAV_REALLOC)
#define DRWAV_REALLOC(p, sz) realloc((p), (sz))
#endif
#if !defined(DRWAV_FREE)
#define DRWAV_FREE(p) free((p))
#endif
#if !defined(DRWAV_COPY_MEMORY)
#define DRWAV_COPY_MEMORY(dst, src, sz) memcpy((dst), (src), (sz))
#endif
#if !defined(DRWAV_ZERO_MEMORY)
...
#if defined(DR_WAV_LIBSNDFILE_COMPAT)
if (pWav->translatedFormatTag == DR_WAVE_FORMAT_ADPCM) {
drwav_uint64 blockCount = dataChunkSize / fmt.blockAlign;
pWav->totalPCMFrameCount = (((blockCount * (fmt.blockAlign - (6*pWav->channels))) * 2)) / fmt.channels; 
}
...
pSampleData = (drwav_int16*)DRWAV_MALLOC((size_t)sampleDataSize); 
if (pSampleData == NULL) {
drwav_uninit(pWav);
return NULL; 
}
...
pSampleData = (float*)DRWAV_MALLOC((size_t)sampleDataSize); 
if (pSampleData == NULL) {
drwav_uninit(pWav);
return NULL; 
}
...
pSampleData = (drwav_int32*)DRWAV_MALLOC((size_t)sampleDataSize); 
if (pSampleData == NULL) {
drwav_uninit(pWav);
return NULL; 
}
