...
{
#define ADD_TRANSLATION(key_name, var_name) if (t.var_name) obs_set_key_translation(key_name, t.var_name);
...
#define ADD_NUMPAD_NUM(idx) dstr_copy_dstr(&button, &numpad); dstr_replace(&button, "%1", #idx); obs_set_key_translation(OBS_KEY_NUM##idx, button.array)
...
#define ADD_MOUSE_NUM(idx) dstr_copy_dstr(&button, &mouse); dstr_replace(&button, "%1", #idx); obs_set_key_translation(OBS_KEY_MOUSE##idx, button.array)
...
const char *hide)
{
#define SET_T(n) bfree(obs->hotkeys.sceneitem_##n); obs->hotkeys.sceneitem_##n = bstrdup(n)
