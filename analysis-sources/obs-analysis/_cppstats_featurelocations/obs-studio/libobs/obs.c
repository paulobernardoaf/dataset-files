...
#define FREE_OBS_LINKED_LIST(type) do { int unfreed = 0; while (data->first_##type) { obs_##type##_destroy(data->first_##type); unfreed++; } if (unfreed) blog(LOG_INFO, "\t%d " #type "(s) were remaining", unfreed); } while (false)
...
#define FREE_REGISTERED_TYPES(structure, list) do { for (size_t i = 0; i < list.num; i++) { struct structure *item = &list.array[i]; if (item->type_data && item->free_type_data) item->free_type_data(item->type_data); } da_free(list); } while (false)
