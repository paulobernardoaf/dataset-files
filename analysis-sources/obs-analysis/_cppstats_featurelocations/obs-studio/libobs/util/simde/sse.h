...
#define simde_mm_shuffle_pi16(a, imm8) ({ const simde__m64 simde__tmp_a_ = a; (simde__m64){.i16 = SIMDE__SHUFFLE_VECTOR( 16, 8, (simde__tmp_a_).i16, (simde__tmp_a_).i16, (((imm8)) & 3), (((imm8) >> 2) & 3), (((imm8) >> 4) & 3), (((imm8) >> 6) & 3))}; })
...
#elif defined(SIMDE__SHUFFLE_VECTOR)
#define simde_mm_shuffle_ps(a, b, imm8) ({ (simde__m128){.f32 = SIMDE__SHUFFLE_VECTOR( 32, 16, (a).f32, (b).f32, (((imm8)) & 3), (((imm8) >> 2) & 3), (((imm8) >> 4) & 3) + 4, (((imm8) >> 6) & 3) + 4)}; })
...
#define SIMDE_MM_TRANSPOSE4_PS(row0, row1, row2, row3) do { simde__m128 tmp3, tmp2, tmp1, tmp0; tmp0 = simde_mm_unpacklo_ps((row0), (row1)); tmp2 = simde_mm_unpacklo_ps((row2), (row3)); tmp1 = simde_mm_unpackhi_ps((row0), (row1)); tmp3 = simde_mm_unpackhi_ps((row2), (row3)); row0 = simde_mm_movelh_ps(tmp0, tmp2); row1 = simde_mm_movehl_ps(tmp2, tmp0); row2 = simde_mm_movelh_ps(tmp1, tmp3); row3 = simde_mm_movehl_ps(tmp3, tmp1); } while (0)
