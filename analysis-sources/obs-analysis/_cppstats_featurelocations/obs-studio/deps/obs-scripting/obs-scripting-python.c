...
#define libobs_to_py(type, obs_obj, ownership, py_obj) libobs_to_py_(#type " *", obs_obj, ownership, py_obj, NULL, __func__, __LINE__)


#define py_to_libobs(type, py_obj, libobs_out) py_to_libobs_(#type " *", py_obj, libobs_out, NULL, __func__, __LINE__)


#define lock_callback(cb) lock_python(); struct obs_python_script *__last_script = cur_python_script; struct python_obs_callback *__last_cb = cur_python_cb; cur_python_script = (struct obs_python_script *)cb->base.script; cur_python_cb = cb





#define unlock_callback() cur_python_cb = __last_cb; cur_python_script = __last_script; unlock_python()
...
#if defined(DEBUG_PYTHON_STARTUP)
