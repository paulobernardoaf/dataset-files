...
#define get_programdata_path(path, subpath) do { SHGetFolderPathW(NULL, CSIDL_COMMON_APPDATA, NULL, SHGFP_TYPE_CURRENT, path); StringCbCatW(path, sizeof(path), L"\\"); StringCbCatW(path, sizeof(path), subpath); } while (false)
...
#define make_filename(str, name, ext) do { StringCbCatW(str, sizeof(str), name); StringCbCatW(str, sizeof(str), b64 ? L"64" : L"32"); StringCbCatW(str, sizeof(str), ext); } while (false)
