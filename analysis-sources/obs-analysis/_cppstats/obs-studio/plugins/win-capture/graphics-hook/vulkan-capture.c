...
#define GETADDR(x) do { funcs->x = (void *)gpa(inst, "vk" #x); if (!funcs->x) { flog("could not get instance " "address for %s", #x); funcs_not_found = true; } } while (false)
...
data->device = device;
bool funcs_not_found = false;
#define GETADDR(x) do { dfuncs->x = (void *)gdpa(device, "vk" #x); if (!dfuncs->x) { flog("could not get device " "address for %s", #x); funcs_not_found = true; } } while (false)
#define GETADDR_OPTIONAL(x) do { dfuncs->x = (void *)gdpa(device, "vk" #x); } while (false)
...
#define GETPROCADDR(func) if (!strcmp(name, "vk" #func)) return (VkFunc)&OBS_##func;
