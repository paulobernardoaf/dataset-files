...
#include "nt-stuff.h"
#define do_log(level, format, ...) blog(level, "[game-capture: '%s'] " format, obs_source_get_name(gc->source), ##__VA_ARGS__)
...
#define STOP_BEING_BAD " This is most likely due to security software. Please make sure " "that the OBS installation folder is excluded/ignored in the " "settings of the security software you are using."
...
#define NUM_DEFAULT_SCALE_VALS (sizeof(default_scale_vals) / sizeof(default_scale_vals[0]))
