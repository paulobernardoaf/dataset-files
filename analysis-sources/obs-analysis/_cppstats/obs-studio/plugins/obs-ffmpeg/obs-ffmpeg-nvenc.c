...
#define do_log(level, format, ...) blog(level, "[NVENC encoder: '%s'] " format, obs_encoder_get_name(enc->encoder), ##__VA_ARGS__)
...
OBS_COMBO_TYPE_LIST,
OBS_COMBO_FORMAT_STRING);
#define add_preset(val) obs_property_list_add_string(p, obs_module_text("NVENC.Preset." val), val)
add_preset("mq");
add_preset("hq");
add_preset("default");
add_preset("hp");
