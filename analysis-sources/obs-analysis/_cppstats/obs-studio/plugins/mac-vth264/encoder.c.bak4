...
#define VT_LOG_ENCODER(encoder, level, format, ...) blog(level, "[VideoToolbox %s: 'h264']: " format, obs_encoder_get_name(encoder), ##__VA_ARGS__)


#define VT_BLOG(level, format, ...) VT_LOG_ENCODER(enc->encoder, level, format, ##__VA_ARGS__)
...
#define STATUS_CHECK(c) code = c; if (code) { log_osstatus(LOG_ERROR, enc, #c, code); goto fail; }
...
#define ENABLE_HW_ACCEL kVTVideoEncoderSpecification_EnableHardwareAcceleratedVideoEncoder

#define REQUIRE_HW_ACCEL kVTVideoEncoderSpecification_RequireHardwareAcceleratedVideoEncoder
...
#define VT_DICTSTR(key, name) CFStringRef name##_ref = CFDictionaryGetValue(encoder_dict, key); CFIndex name##_len = CFStringGetLength(name##_ref); char *name = bzalloc(name##_len + 1); CFStringGetFileSystemRepresentation(name##_ref, name, name##_len);
