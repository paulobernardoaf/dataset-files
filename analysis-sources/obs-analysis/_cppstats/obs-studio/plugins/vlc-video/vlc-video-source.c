...
#include <util/dstr.h>
#define do_log(level, format, ...) blog(level, "[vlc_source: '%s'] " format, obs_source_get_name(ss->source), ##__VA_ARGS__)
...
{
*full = false;
#define CHROMA_TEST(val, ret) if (chroma_is(chroma, val)) return ret
#define CHROMA_CONV(val, new_val, ret) do { if (chroma_is(chroma, val)) { *(uint32_t *)chroma = *(uint32_t *)new_val; return ret; } } while (false)
#define CHROMA_CONV_FULL(val, new_val, ret) do { *full = true; CHROMA_CONV(val, new_val, ret); } while (false)
...
{
#define AUDIO_TEST(val, ret) if (chroma_is(format, val)) return ret
#define AUDIO_CONV(val, new_val, ret) do { if (chroma_is(format, val)) { *(uint32_t *)format = *(uint32_t *)new_val; return ret; } } while (false)
