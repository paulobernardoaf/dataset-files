...
#define REGISTER_OBS_DEF(size_var, structure, dest, info) do { struct structure data = {0}; if (!size_var) { blog(LOG_ERROR, "Tried to register " #structure " outside of obs_module_load"); return; } if (size_var > sizeof(data)) { blog(LOG_ERROR, "Tried to register " #structure " with size %llu which is more " "than libobs currently supports " "(%llu)", (long long unsigned)size_var, (long long unsigned)sizeof(data)); goto error; } memcpy(&data, info, size_var); da_push_back(dest, &data); } while (false)
...
#define CHECK_REQUIRED_VAL(type, info, val, func) do { if ((offsetof(type, val) + sizeof(info->val) > size) || !info->val) { blog(LOG_ERROR, "Required value '" #val "' for " "'%s' not found. " #func " failed.", info->id); goto error; } } while (false)
...
#define HANDLE_ERROR(size_var, structure, info) do { struct structure data = {0}; if (!size_var) return; memcpy(&data, info, sizeof(data) < size_var ? sizeof(data) : size_var); if (info->type_data && info->free_type_data) info->free_type_data(info->type_data); } while (false)
...
{
#define CHECK_REQUIRED_VAL_(info, val, func) CHECK_REQUIRED_VAL(struct obs_modal_ui, info, val, func)
...
{
#define CHECK_REQUIRED_VAL_(info, val, func) CHECK_REQUIRED_VAL(struct obs_modeless_ui, info, val, func)
