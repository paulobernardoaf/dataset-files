...
#define do_update_transform(item) do { if (!item->parent || item->parent->is_group) os_atomic_set_bool(&item->update_transform, true); else update_item_transform(item, false); } while (false)
...
while (item) {
#define get_min_max(x_val, y_val) do { struct vec3 v; vec3_set(&v, x_val, y_val, 0.0f); vec3_transform(&v, &v, &item->box_transform); if (v.x < minv->x) minv->x = v.x; if (v.y < minv->y) minv->y = v.y; if (v.x > maxv->x) maxv->x = v.x; if (v.y > maxv->y) maxv->y = v.y; } while (false)
