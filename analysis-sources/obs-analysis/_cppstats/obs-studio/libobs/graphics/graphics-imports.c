...
#define GRAPHICS_IMPORT(func) do { exports->func = os_dlsym(module, #func); if (!exports->func) { success = false; blog(LOG_ERROR, "Could not load function '%s' from " "module '%s'", #func, module_name); } } while (false)
#define GRAPHICS_IMPORT_OPTIONAL(func) do { exports->func = os_dlsym(module, #func); } while (false)
