...
#define pack_shift(lum_plane, lum_pos0, lum_pos1, line1, line2, mask, sh) do { __m128i pack_val = _mm_packs_epi32( _mm_srli_si128(_mm_and_si128(line1, mask), sh), _mm_srli_si128(_mm_and_si128(line2, mask), sh)); pack_val = _mm_packus_epi16(pack_val, pack_val); *(uint32_t *)(lum_plane + lum_pos0) = get_m128_32_0(pack_val); *(uint32_t *)(lum_plane + lum_pos1) = get_m128_32_1(pack_val); } while (false)
#define pack_val(lum_plane, lum_pos0, lum_pos1, line1, line2, mask) do { __m128i pack_val = _mm_packs_epi32(_mm_and_si128(line1, mask), _mm_and_si128(line2, mask)); pack_val = _mm_packus_epi16(pack_val, pack_val); *(uint32_t *)(lum_plane + lum_pos0) = get_m128_32_0(pack_val); *(uint32_t *)(lum_plane + lum_pos1) = get_m128_32_1(pack_val); } while (false)
#define pack_ch_1plane(uv_plane, chroma_pos, line1, line2, uv_mask) do { __m128i add_val = _mm_add_epi64(_mm_and_si128(line1, uv_mask), _mm_and_si128(line2, uv_mask)); __m128i avg_val = _mm_add_epi64( add_val, _mm_shuffle_epi32(add_val, _MM_SHUFFLE(2, 3, 0, 1))); avg_val = _mm_srai_epi16(avg_val, 2); avg_val = _mm_shuffle_epi32(avg_val, _MM_SHUFFLE(3, 1, 2, 0)); avg_val = _mm_packus_epi16(avg_val, avg_val); *(uint32_t *)(uv_plane + chroma_pos) = get_m128_32_0(avg_val); } while (false)
#define pack_ch_2plane(u_plane, v_plane, chroma_pos, line1, line2, uv_mask) do { uint32_t packed_vals; __m128i add_val = _mm_add_epi64(_mm_and_si128(line1, uv_mask), _mm_and_si128(line2, uv_mask)); __m128i avg_val = _mm_add_epi64( add_val, _mm_shuffle_epi32(add_val, _MM_SHUFFLE(2, 3, 0, 1))); avg_val = _mm_srai_epi16(avg_val, 2); avg_val = _mm_shuffle_epi32(avg_val, _MM_SHUFFLE(3, 1, 2, 0)); avg_val = _mm_shufflelo_epi16(avg_val, _MM_SHUFFLE(3, 1, 2, 0)); avg_val = _mm_packus_epi16(avg_val, avg_val); packed_vals = get_m128_32_0(avg_val); *(uint16_t *)(u_plane + chroma_pos) = (uint16_t)(packed_vals); *(uint16_t *)(v_plane + chroma_pos) = (uint16_t)(packed_vals >> 16); } while (false)
