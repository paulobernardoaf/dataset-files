...
#define free_circlebuf(x) do { while (x.size) { struct obs_tex_frame frame; circlebuf_pop_front(&x, &frame, sizeof(frame)); gs_texture_destroy(frame.tex); gs_texture_destroy(frame.tex_uv); } circlebuf_free(&x); } while (false)
