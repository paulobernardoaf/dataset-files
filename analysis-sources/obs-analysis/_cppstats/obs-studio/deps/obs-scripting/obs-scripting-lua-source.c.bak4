...
#define get_callback_from_table(script, idx, name, p_reg_idx) get_callback_from_table_(script, idx, name, p_reg_idx, __FUNCTION__)
...
#define ls_get_libobs_obj(type, lua_index, obs_obj) ls_get_libobs_obj_(ls->script, #type " *", lua_index, obs_obj, ls->id, __FUNCTION__, __LINE__)
...
#define ls_push_libobs_obj(type, obs_obj, ownership) ls_push_libobs_obj_(ls->script, #type " *", obs_obj, ownership, ls->id, __FUNCTION__, __LINE__)
...
#define lock_script() struct obs_lua_script *__data = ls->data; struct obs_lua_script *__prev_script = current_lua_script; current_lua_script = __data; pthread_mutex_lock(&__data->mutex);




#define unlock_script() pthread_mutex_unlock(&__data->mutex); current_lua_script = __prev_script;
...
#define DEFINE_VOID_DATA_CALLBACK(name) static void obs_lua_source_##name(void *data) { struct obs_lua_data *ld = data; struct obs_lua_source *ls = ld->ls; if (!have_func(name)) return; lock_script(); ls_push_data(); call_func(name, 1, 0); unlock_script(); }
...
{
#define unref(name) luaL_unref(ls->script, LUA_REGISTRYINDEX, name); name = LUA_REFNIL
...
#define get_callback(val) do { get_callback_from_table(script, -1, #val, &v->func_##val); info.val = obs_lua_source_##val; } while (false)
