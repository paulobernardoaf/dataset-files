...
#define G(r, i, a, b, c, d) do { a = a + b + m[blake2b_sigma[r][2 * i + 0]]; d = ROR64(d ^ a, 32); c = c + d; b = ROR64(b ^ c, 24); a = a + b + m[blake2b_sigma[r][2 * i + 1]]; d = ROR64(d ^ a, 16); c = c + d; b = ROR64(b ^ c, 63); } while (0)
...
if (inlen > fill) {
md->blake2b.curlen = 0;
XMEMCPY(md->blake2b.buf + (left % sizeof(md->blake2b.buf)), in, fill); 
...
XMEMSET(md->blake2b.buf + md->blake2b.curlen, 0, BLAKE2B_BLOCKBYTES - md->blake2b.curlen); 
blake2b_compress(md, md->blake2b.buf);
for (i = 0; i < 8; ++i) { 
