...
#define RULE_A tmp = g_func(w1, &kp, skey->skipjack.key); w1 = tmp ^ w4 ^ x; w4 = w3; w3 = w2; w2 = tmp;
#define RULE_B tmp = g_func(w1, &kp, skey->skipjack.key); tmp1 = w4; w4 = w3; w3 = w1 ^ w2 ^ x; w1 = tmp1; w2 = tmp;
#define RULE_A1 tmp = w1 ^ w2 ^ x; w1 = ig_func(w2, &kp, skey->skipjack.key); w2 = w3; w3 = w4; w4 = tmp;
#define RULE_B1 tmp = ig_func(w2, &kp, skey->skipjack.key); w2 = tmp ^ w3 ^ x; w3 = w4; w4 = w1; w1 = tmp;
