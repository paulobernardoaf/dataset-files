...
#define DECODE_V(y, max) do {y = _char_to_int(buf[x])*10 + _char_to_int(buf[x+1]); if (y >= max) return CRYPT_INVALID_PACKET; x += 2; } while(0)
#define DECODE_V4(y, max) do {y = _char_to_int(buf[x])*1000 + _char_to_int(buf[x+1])*100 + _char_to_int(buf[x+2])*10 + _char_to_int(buf[x+3]); if (y >= max) return CRYPT_INVALID_PACKET; x += 4; } while(0)
