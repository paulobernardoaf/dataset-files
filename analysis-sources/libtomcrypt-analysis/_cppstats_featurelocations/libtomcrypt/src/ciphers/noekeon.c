...
#define kTHETA(a, b, c, d) temp = a^c; temp = temp ^ ROLc(temp, 8) ^ RORc(temp, 8); b ^= temp; d ^= temp; temp = b^d; temp = temp ^ ROLc(temp, 8) ^ RORc(temp, 8); a ^= temp; c ^= temp;





#define THETA(k, a, b, c, d) temp = a^c; temp = temp ^ ROLc(temp, 8) ^ RORc(temp, 8); b ^= temp ^ k[1]; d ^= temp ^ k[3]; temp = b^d; temp = temp ^ ROLc(temp, 8) ^ RORc(temp, 8); a ^= temp ^ k[0]; c ^= temp ^ k[2];





#define GAMMA(a, b, c, d) b ^= ~(d|c); a ^= c&b; temp = d; d = a; a = temp;c ^= a ^ b ^ d; b ^= ~(d|c); a ^= c&b;
...
#define ROUND(i) a ^= RC[i]; THETA(skey->noekeon.K, a,b,c,d); PI1(a,b,c,d); GAMMA(a,b,c,d); PI2(a,b,c,d);
...
#define ROUND(i) THETA(skey->noekeon.dK, a,b,c,d); a ^= RC[i]; PI1(a,b,c,d); GAMMA(a,b,c,d); PI2(a,b,c,d);
