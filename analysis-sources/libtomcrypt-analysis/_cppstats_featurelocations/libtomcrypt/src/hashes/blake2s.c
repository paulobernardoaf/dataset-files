...
#define G(r, i, a, b, c, d) do { a = a + b + m[blake2s_sigma[r][2 * i + 0]]; d = ROR(d ^ a, 16); c = c + d; b = ROR(b ^ c, 12); a = a + b + m[blake2s_sigma[r][2 * i + 1]]; d = ROR(d ^ a, 8); c = c + d; b = ROR(b ^ c, 7); } while (0)
...
if (inlen > fill) {
md->blake2s.curlen = 0;
XMEMCPY(md->blake2s.buf + (left % sizeof(md->blake2s.buf)), in, fill); 
...
XMEMSET(md->blake2s.buf + md->blake2s.curlen, 0, BLAKE2S_BLOCKBYTES - md->blake2s.curlen); 
blake2s_compress(md, md->blake2s.buf);
