...
lv_tabview_set_style(new_tabview, LV_TABVIEW_STYLE_BTN_BG, &lv_style_pretty);
...
char * name_dm;
name_dm = lv_mem_alloc(strlen(name) + 1); 
...
while(pages != NULL) {
if(lv_obj_get_signal_cb(pages) == tabpage_signal) { 
switch(ext->btns_pos) {
case LV_TABVIEW_BTNS_POS_TOP:
case LV_TABVIEW_BTNS_POS_BOTTOM:
